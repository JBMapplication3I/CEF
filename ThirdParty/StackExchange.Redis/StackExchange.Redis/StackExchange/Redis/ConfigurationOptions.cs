namespace StackExchange.Redis
{
...
#endif
    {
        ...
        private static class OptionKeys
        {
            ...
            {
                if (!Format.TryParseInt32(value, out var tmp))
                {
            ...
            {
                if (!Format.TryParseBoolean(value, out var tmp))
                {
            ...
            {
                if (!System.Version.TryParse(value, out var tmp))
                {
            ...
            {
                if (!Enum.TryParse(value, true, out Proxy tmp))
                {
                    throw new ArgumentOutOfRangeException("Keyword '" + key + "' requires a proxy value");
                }
            ...
            internal static SslProtocols ParseSslProtocols(string key, string value)
            {
                ...
                value = value?.Replace("|", ",");
                if (!Enum.TryParse(value, true, out SslProtocols tmp))
                {
                    throw new ArgumentOutOfRangeException("Keyword '" + key + "' requires an SslProtocol value (multiple values separated by '|').");
                }
                return tmp;
            }
            ...
            {
                if (value != null && normalizedOptions.TryGetValue(value, out var tmp))
                {
                    return tmp ?? "";
                }
                return value ?? "";
        ...
        public int ConnectTimeout
        {
            get
            {
                ...
                {
                    return connectTimeout.GetValueOrDefault();
                }
            ...
            for (int i = 0; i < EndPoints.Count; i++)
            {
                if (EndPoints[i] is DnsEndPoint dns)
                {
                    try
                    {
                        ...
                        else if (cache.TryGetValue(dns.Host, out var ip))
                        { // use cache
