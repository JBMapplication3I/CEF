namespace StackExchange.Redis
{
    ...
    public sealed class LuaScript
    {
        ...
        static readonly ConcurrentDictionary<string, WeakReference> Cache = new();
        ...
        readonly Hashtable ParameterMappers;
        ...
        ~LuaScript()
        {
            try
            {
                Cache.TryRemove(OriginalScript, out var ignored);
            }
            catch { }
        }
        ...
        public static LuaScript Prepare(string script)
        {
            ...
            if (!Cache.TryGetValue(script, out var weakRef) || (ret = (LuaScript)weakRef.Target) == null)
            {
            ...
            if (HasArguments)
            {
                ...
                if (ps != null && mapper == null)
                {
                    lock (ParameterMappers)
                    {
                        ...
                        if (mapper == null)
                        {
                            if (!ScriptParameterMapper.IsValidParameterHash(psType, this, out var missingMember, out var badMemberType))
                            {
