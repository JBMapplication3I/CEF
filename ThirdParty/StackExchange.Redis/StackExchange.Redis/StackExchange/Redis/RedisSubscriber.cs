namespace StackExchange.Redis
{
    ...
    {
        private readonly Dictionary<RedisChannel, Subscription> subscriptions = new();
        ...
        internal void OnMessage(RedisChannel subscription, RedisChannel channel, RedisValue payload)
        {
            ...
            if (completable != null)
            {
                UnprocessableCompletionManager.CompleteSyncOrAsync(completable);
            }
        }
        ...
        internal bool SubscriberConnected(RedisChannel channel = default)
        {
            ...
            server = SelectServer(-1, RedisCommand.SUBSCRIBE, CommandFlags.DemandMaster, default);
        ...
        internal long ValidateSubscriptions()
        {
            lock (subscriptions)
            {
                ...
                foreach (var pair in subscriptions)
                {
                    if (pair.Value.Validate(this, pair.Key))
                    {
                        count++;
                    }
                }
        ...
        private sealed class Subscription
        {
            ...
            public Task SubscribeToServer(ConnectionMultiplexer multiplexer, RedisChannel channel, CommandFlags flags, object asyncState, bool internalCall)
            {
                ...
                var selected = multiplexer.SelectServer(-1, cmd, flags, default);
