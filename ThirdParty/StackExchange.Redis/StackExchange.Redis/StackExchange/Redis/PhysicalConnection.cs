namespace StackExchange.Redis
{
    ...
    internal sealed partial class PhysicalConnection : IDisposable, ISocketCallback
    {
        ...
        {
            if (result.CompletedSynchronously || result.AsyncState is not PhysicalConnection physical)
            {
                return;
            }
        ...
        private readonly Queue<Message> outstanding = new();
        ...
        public void RecordConnectionFailed(ConnectionFailureType failureType, ref SocketManager.ManagerState managerState, Exception innerException = null, [CallerMemberName] string origin = null)
        {
            ...
            Bridge.OnDisconnected(failureType, this, out var isCurrent, out var oldState);
            ...
            if (serverEndpoint.RequiresReadMode)
            {
                ...
                if (requiredReadMode != currentReadMode)
                {
                    ...
                    switch (requiredReadMode)
                    {
                        ...
                        return ReusableReadOnlyCommand;
                        case ReadMode.ReadWrite:
        ...
        static LocalCertificateSelectionCallback GetAmbientCertificateCallback()
        {
            try
            {
                ...
                {
                    return delegate
                    { return new X509Certificate2(pfxPath, pfxPassword ?? "", flags ?? X509KeyStorageFlags.DefaultKeySet); };
                }
            }
            catch
            { }
        ...
        bool ISocketCallback.IsDataAvailable
        {
            ...
            {
                try
                { return socketToken.Available > 0; }
                catch { return false; }
            }
        }
        private RawResult ReadArray(byte[] buffer, ref int offset, ref int count)
        {
            ...
            {
                if (!itemCount.TryGetInt64(out var i64))
                {
        ...
        private RawResult ReadBulkString(byte[] buffer, ref int offset, ref int count)
        {
            ...
            if (prefix.HasValue)
            {
                if (!prefix.TryGetInt64(out var i64))
                {
        ...
        RawResult TryParseResult(byte[] buffer, ref int offset, ref int count)
        {
            ...
            {
                // simple string
                '+' => ReadLineTerminatedString(ResultType.SimpleString, buffer, ref offset, ref count),
                // error
                '-' => ReadLineTerminatedString(ResultType.Error, buffer, ref offset, ref count),
                // integer
                ':' => ReadLineTerminatedString(ResultType.Integer, buffer, ref offset, ref count),
                // bulk string
                '$' => ReadBulkString(buffer, ref offset, ref count),
                // array
                '*' => ReadArray(buffer, ref offset, ref count),
                _ => throw new InvalidOperationException("Unexpected response prefix: " + (char)resultType),
            };
        }
