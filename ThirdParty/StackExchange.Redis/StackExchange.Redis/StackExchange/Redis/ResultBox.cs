namespace StackExchange.Redis
{
    ...
    sealed class ResultBox<T> : ResultBox
    {
        ...
        public object AsyncState => stateOrCompletionSource is TaskCompletionSource<T> source
            ? source.Task.AsyncState
        ...
        public static void UnwrapAndRecycle(ResultBox<T> box, bool recycle, out T value, out Exception exception)
        {
            ...
            else
            {
                ...
                if (recycle)
                {
                    for (int i = 0; i < store.Length; i++)
                    {
                        if (Interlocked.CompareExchange(ref store[i], box, null) == null)
                        {
                            return;
                        }
                    }
