namespace StackExchange.Redis
{
    ...
    abstract class ResultProcessor
    {
        ...
        public static readonly TimeSpanProcessor
            TimeSpanFromMilliseconds = new(true),
            TimeSpanFromSeconds = new(false);
        ...
        public virtual bool SetResult(PhysicalConnection connection, Message message, RawResult result)
        {
            ...
            if (result.IsError)
            {
                ...
                if (isMoved || result.AssertStarts(ASK))
                {
                    ...
                    EndPoint endpoint;
                    if (Format.TryParseInt32(parts[1], out var hashSlot) &&
        ...
        public sealed class TimeSpanProcessor : ResultProcessor<TimeSpan?>
        {
            ...
            {
                if (TryParse(result, out var expiry))
                {
        ...
        public sealed class TrackSubscriptionsProcessor : ResultProcessor<bool>
        {
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                if (result.Type == ResultType.MultiBulk)
                {
                    ...
                    if (items.Length >= 3 && items[2].TryGetInt64(out var count))
                    {
        ...
        internal sealed class DemandZeroOrOneProcessor : ResultProcessor<bool>
        {
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                if (TryGet(result, out var value))
                {
        ...
        {
            static readonly Regex sha1 = new("^[0-9a-f]{40}$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                switch (result.Type)
                {
                    ...
                    if (asciiHash == null || asciiHash.Length != 40)
                    {
                        return false;
                    }
        ...
        internal sealed class SortedSetEntryArrayProcessor : ValuePairInterleavedProcessorBase<SortedSetEntry>
        {
            ...
            {
                return new SortedSetEntry(first.AsRedisValue(), second.TryGetDouble(out var val) ? val : double.NaN);
            }
        }
        ...
        {
            static readonly T[] nix = Array.Empty<T>();
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                if (TryParse(result, out var arr))
                {
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                ...
                switch (result.Type)
                {
                    ...
                    if (message != null && message.Command == RedisCommand.CONFIG)
                    {
                        ...
                        {
                            var key = arr[i * 2];
                            if (key.IsEqual(timeout) && arr[(i * 2) + 1].TryGetInt64(out var i64))
                            {
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                ...
                switch (result.Type)
                {
                    ...
                    switch (arr.Length)
                    {
                        ...
                        break;
                        case 2:
                        if (arr[0].TryGetInt64(out unixTime) && arr[1].TryGetInt64(out var micros))
                        {
        ...
        class PubSubNumSubProcessor : Int64Processor
        {
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                ...
                {
                    var arr = result.GetItems();
                    if (arr != null && arr.Length == 2 && arr[1].TryGetInt64(out var val))
                    {
                        SetResult(message, val);
                        return true;
                    }
        ...
        sealed class GeoRadiusResultArrayProcessor : ResultProcessor<GeoRadiusResult[]>
        {
            ...
            static GeoRadiusResultArrayProcessor()
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    instances[i] = new GeoRadiusResultArrayProcessor((GeoRadiusOptions)i);
                }
            }
...
The coordinates as a two items x,y array (longitude,latitude).
                ...
                { distance = (double?)arr[index++].AsRedisValue(); }
                if ((options & GeoRadiusOptions.WithGeoHash) != 0)
                { hash = (long?)arr[index++].AsRedisValue(); }
        ...
        private class TracerProcessor : ResultProcessor<bool>
        {
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                var happy = message.Command switch
                {
                    RedisCommand.ECHO => result.Type == ResultType.BulkString && (!establishConnection || result.IsEqual(connection.Multiplexer.UniqueId)),
                    RedisCommand.PING => result.Type == ResultType.SimpleString && result.IsEqual(RedisLiterals.BytesPONG),
                    RedisCommand.TIME => result.Type == ResultType.MultiBulk && result.GetItems().Length == 2,
                    RedisCommand.EXISTS => result.Type == ResultType.Integer,
                    _ => true,
                };
                if (happy)
                {
...
#pragma warning disable CA1806 // Do not ignore method results
                        innerProcessor.TryParse(rawInnerArray, out var kvpArray);
