namespace StackExchange.Redis
{
    ...
    public sealed class ClientInfo
    {
        ...
        public string Host
        {
            get
            {
                return Format.TryGetHostPort(Address, out var host, out var port) ? host : null;
            }
        }
        ...
        public int Port
        {
            get
            {
                return Format.TryGetHostPort(Address, out var host, out var port) ? port : 0;
            }
        }
        /// <summary>
        ...
        public ClientType ClientType
        {
            get
            {
                if (SubscriptionCount != 0 || PatternSubscriptionCount != 0)
                {
                    return ClientType.PubSub;
                }

                if ((Flags & ClientFlags.Slave) != 0)
                {
                    return ClientType.Slave;
                }
            ...
            using (var reader = new StringReader(input))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    for (int i = 0; i < tokens.Length; i++)
                    {
                        ...
                        {
                            case "addr":
                            client.Address = Format.TryParseEndPoint(value);
                            break;
                            case "age":
                            client.AgeSeconds = Format.ParseInt32(value);
                            break;
                            case "idle":
                            client.IdleSeconds = Format.ParseInt32(value);
                            break;
                            case "db":
                            client.Database = Format.ParseInt32(value);
                            break;
                            case "name":
                            client.Name = value;
                            break;
                            case "sub":
                            client.SubscriptionCount = Format.ParseInt32(value);
                            break;
                            case "psub":
                            client.PatternSubscriptionCount = Format.ParseInt32(value);
                            break;
                            case "multi":
                            client.TransactionCommandLength = Format.ParseInt32(value);
                            break;
                            case "cmd":
                            ...
                            break;
                            case "id":
                            client.Id = Format.ParseInt64(value);
                            break;
                        }
                    }
                    clients.Add(client);
        ...
        {
            if (raw.Contains(token.ToString()))
            {
                value |= toAdd;
            }
        }
        ...
        private class ClientInfoProcessor : ResultProcessor<ClientInfo[]>
        {
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                switch (result.Type)
                {
                    ...
                    {
                        SetResult(message, Parse(result.GetString()));
                        return true;
                    }
                }
                return false;
