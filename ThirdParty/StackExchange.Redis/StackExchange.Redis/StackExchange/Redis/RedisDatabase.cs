namespace StackExchange.Redis
{
    ...
    internal class RedisDatabase : RedisBase, IDatabase
    {
        ...
        public Task<RedisValue[]> HashGetAsync(RedisKey key, RedisValue[] hashFields, CommandFlags flags = CommandFlags.None)
        {
            ...
            if (hashFields.Length == 0)
            {
                return CompletedTask<RedisValue[]>.FromResult(Array.Empty<RedisValue>(), asyncState);
            }
        ...
        public IEnumerable<HashEntry> HashScan(RedisKey key, RedisValue pattern = default, int pageSize = CursorUtils.DefaultPageSize, long cursor = CursorUtils.Origin, int pageOffset = 0, CommandFlags flags = CommandFlags.None)
        {
            ...
            {
                throw ExceptionFactory.NoCursor(RedisCommand.HGETALL);
            }

            if (pattern.IsNull)
            {
                return HashGetAll(key, flags);
            }
        ...
        {
            var msg = GetExpiryMessage(key, flags, expiry, out var server);
        ...
        sealed class KeyMigrateCommandMessage : Message.CommandKeyBase // MIGRATE is atypical
        {
            ...
            readonly RedisValue toHost, toPort;
                ...
                : base(db, flags, RedisCommand.MIGRATE, key)
            {
                ...
                if (!Format.TryGetHostPort(toServer, out var toHost, out var toPort))
                {
                    throw new ArgumentException("toServer");
                }
                ...
                if (toDatabase < 0)
                {
                    throw new ArgumentOutOfRangeException(nameof(toDatabase));
                }
            ...
            internal override void WriteImpl(PhysicalConnection physical)
            {
                ...
                if (isReplace)
                {
                    physical.Write(RedisLiterals.REPLACE);
                }
            }
        }
        ...
        public IEnumerable<RedisValue> SetScan(RedisKey key, RedisValue pattern = default, int pageSize = CursorUtils.DefaultPageSize, long cursor = CursorUtils.Origin, int pageOffset = 0, CommandFlags flags = CommandFlags.None)
        {
            ...
            if (cursor != 0 || pageOffset != 0)
            {
                throw ExceptionFactory.NoCursor(RedisCommand.SMEMBERS);
            }
        ...
        {
            var msg = GetSortedSetAddMessage(default, key, skip, take, order, sortType, by, get, flags);
            return ExecuteSync(msg, ResultProcessor.RedisValueArray);
        }
        ...
        public IEnumerable<SortedSetEntry> SortedSetScan(RedisKey key, RedisValue pattern = default, int pageSize = CursorUtils.DefaultPageSize, long cursor = CursorUtils.Origin, int pageOffset = 0, CommandFlags flags = CommandFlags.None)
        {
            ...
            if (cursor != 0 || pageOffset != 0)
            {
                throw ExceptionFactory.NoCursor(RedisCommand.ZRANGE);
            }

            if (pattern.IsNull)
            {
                return SortedSetRangeByRankWithScores(key, flags: flags);
            }
        ...
        {
            var msg = GetStringGetWithExpiryMessage(key, flags, out var processor, out var server);
        ...
        private RedisValue GetRange(double value, Exclude exclude, bool isStart)
        {
            if (isStart)
            {
                if ((exclude & Exclude.Start) == 0)
                {
                    return value; // inclusive is default
                }
            }
            else
            {
                if ((exclude & Exclude.Stop) == 0)
                {
                    return value; // inclusive is default
                }
            }
        ...
        private Message GetSortedSetAddMessage(RedisKey destination, RedisKey key, long skip, long take, Order order, SortType sortType, RedisValue by, RedisValue[] get, CommandFlags flags)
        {
            ...
            if (destination.IsNull && skip == 0 && take == -1 && by.IsNull && (get == null || get.Length == 0))
            {
                switch (order)
                {
                    ...
                    switch (sortType)
                    {
                        ...
                        return Message.Create(Database, flags, RedisCommand.SORT, key);
                        case SortType.Alphabetic:
                        return Message.Create(Database, flags, RedisCommand.SORT, key, RedisLiterals.ALPHA);
                    }
                    break;
                    case Order.Descending:
                    switch (sortType)
                    {
                        case SortType.Numeric:
                        return Message.Create(Database, flags, RedisCommand.SORT, key, RedisLiterals.DESC);
                        case SortType.Alphabetic:
                        return Message.Create(Database, flags, RedisCommand.SORT, key, RedisLiterals.DESC, RedisLiterals.ALPHA);
                    }
                    break;
                }
            }
            ...
            {
                return Message.Create(Database, flags, RedisCommand.SORT, key, values.ToArray());
            }
        ...
        private Message GetStringSetMessage(KeyValuePair<RedisKey, RedisValue>[] values, When when = When.Always, CommandFlags flags = CommandFlags.None)
        {
            ...
            switch (values.Length)
            {
                ...
                return null;
                case 1:
                return GetStringSetMessage(values[0].Key, values[0].Value, null, when, flags);
                default:
                WhenAlwaysOrNotExists(when);
        ...
        Message GetStringSetMessage(RedisKey key, RedisValue value, TimeSpan? expiry = null, When when = When.Always, CommandFlags flags = CommandFlags.None)
        {
            ...
            { // no expiry
                ...
                {
                    case When.Always:
                    return Message.Create(Database, flags, RedisCommand.SET, key, value);
                    case When.NotExists:
                    return Message.Create(Database, flags, RedisCommand.SETNX, key, value);
                    case When.Exists:
                    return Message.Create(Database, flags, RedisCommand.SET, key, value, RedisLiterals.XX);
                }
            }
            ...
            if ((milliseconds % 1000) == 0)
            {
                ...
                {
                    case When.Always:
                    return Message.Create(Database, flags, RedisCommand.SETEX, key, seconds, value);
                    case When.Exists:
                    return Message.Create(Database, flags, RedisCommand.SET, key, value, RedisLiterals.EX, seconds, RedisLiterals.XX);
                    case When.NotExists:
                    return Message.Create(Database, flags, RedisCommand.SET, key, value, RedisLiterals.EX, seconds, RedisLiterals.NX);
                }
            }
        ...
        private IEnumerable<T> TryScan<T>(RedisKey key, RedisValue pattern, int pageSize, long cursor, int pageOffset, CommandFlags flags, RedisCommand command, ResultProcessor<ScanIterator<T>.ScanResult> processor)
        {
            ...
            if (!multiplexer.CommandMap.IsAvailable(command))
            {
                return null;
            }
        ...
        sealed class HashScanResultProcessor : ScanResultProcessor<HashEntry>
        {
            ...
            {
                if (!HashEntryArray.TryParse(result, out var pairs))
                {
                    pairs = null;
                }
        ...
        private sealed class ScriptEvalMessage : Message, IMultiMessage
        {
                ...
                : base(db, flags, command)
            {
                ...
                if (keys == null)
                {
                    keys = RedisKey.EmptyArray;
                }

                if (values == null)
                {
                    values = RedisValue.EmptyArray;
                }
        ...
        sealed class SortedSetScanResultProcessor : ScanResultProcessor<SortedSetEntry>
        {
            ...
            {
                if (!SortedSetWithScores.TryParse(result, out var pairs))
                {
                    pairs = null;
                }
        ...
        private class StringGetWithExpiryProcessor : ResultProcessor<RedisValueWithExpiry>
        {
            ...
            protected override bool SetResultCore(PhysicalConnection connection, Message message, RawResult result)
            {
                switch (result.Type)
                {
                    ...
                    TimeSpan? expiry;
                    Exception ex;
                    if (message is StringGetWithExpiryMessage sgwem && sgwem.UnwrapValue(out expiry, out ex))
                    {
                        if (ex == null)
                        {
