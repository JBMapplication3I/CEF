namespace StackExchange.Redis
{
    ...
    class ScriptParameterMapper
    {
        ...
        static readonly Regex ParameterExtractor = new(@"@(?<paramName> ([a-z]|_) ([a-z]|_|\d)*)", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
        ...
        static void ConvertToRedisValue(MemberInfo member, ILGenerator il, LocalBuilder needsPrefixBool, ref LocalBuilder redisKeyLoc)
        {
            ...
            var t = member is FieldInfo info ? info.FieldType : ((PropertyInfo)member).PropertyType;
            ...
            {
                redisKeyLoc ??= il.DeclareLocal(typeof(RedisKey));
            ...
            if (t == typeof(int))
            {
                convertOp = RedisValue_FromInt;
            }

            if (t == typeof(int?))
            {
                convertOp = RedisValue_FromNullableInt;
            }

            if (t == typeof(long))
            {
                convertOp = RedisValue_FromLong;
            }

            if (t == typeof(long?))
            {
                convertOp = RedisValue_FromNullableLong;
            }

            if (t == typeof(double))
            {
                convertOp = RedisValue_FromDouble;
            }

            if (t == typeof(double?))
            {
                convertOp = RedisValue_FromNullableDouble;
            }

            if (t == typeof(string))
            {
                convertOp = RedisValue_FromString;
            }

            if (t == typeof(byte[]))
            {
                convertOp = RedisValue_FromByteArray;
            }

            if (t == typeof(bool))
            {
                convertOp = RedisValue_FromBool;
            }

            if (t == typeof(bool?))
            {
                convertOp = RedisValue_FromNullableBool;
            }
        ...
        public static bool IsValidParameterHash(Type t, LuaScript script, out string missingMember, out string badTypeMember)
        {
            for (var i = 0; i < script.Arguments.Length; i++)
            {
                ...
                var memberType = member is FieldInfo info ? info.FieldType : ((PropertyInfo)member).PropertyType;
