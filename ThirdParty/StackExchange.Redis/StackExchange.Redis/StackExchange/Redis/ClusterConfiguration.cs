namespace StackExchange.Redis
{
    ...
    public struct SlotRange : IEquatable<SlotRange>, IComparable<SlotRange>, IComparable
    {
        ...
        public static bool TryParse(string range, out SlotRange value)
        {
            ...
            {
                if (TryParseInt16(range, 0, i++, out from) && TryParseInt16(range, i, range.Length - i, out var to))
                {
        ...
        {
            return obj is SlotRange range ? CompareTo(range) : -1;
        }
    }
    ...
    public sealed class ClusterConfiguration
    {
        ...
        public ClusterNode this[EndPoint endpoint]
        {
            get
            {
                return endpoint == null ? null
                    : nodeLookup.TryGetValue(endpoint, out var result) ? result : null;
            }
        }
        ...
        public ClusterNode GetBySlot(int slot)
        {
            ...
            {
                if (!node.IsSlave && node.ServesSlot(slot))
                {
                    return node;
                }
            }
    ...
    public sealed class ClusterNode : IEquatable<ClusterNode>, IComparable<ClusterNode>, IComparable
    {
        ...
        private static readonly IList<ClusterNode> NoNodes = Array.Empty<ClusterNode>();

        private static readonly IList<SlotRange> NoSlots = Array.Empty<SlotRange>();
        ...
        internal ClusterNode(ClusterConfiguration configuration, string raw, EndPoint origin)
        {
            ...
            {
                if (SlotRange.TryParse(parts[i], out var range))
                {
                    if (slots == null)
                    {
        ...
        public ClusterNode Parent
        {
            get
            {
                if (parent != null)
                {
                    return parent == Dummy ? null : parent;
                }
        ...
        public int CompareTo(ClusterNode other)
        {
            ...
            {
                return IsSlave ? 1 : -1; // masters first
            }
        ...
        public override string ToString()
        {
            ...
            if (IsSlave)
            {
                ...
                if (parent != null)
                {
                    sb.Append(" at ").Append(parent.EndPoint);
                }
            }
            ...
            switch (childCount)
            {
                ...
                break;
                default:
                sb.Append(", ").Append(childCount).Append(" slaves");
                break;
            }
            if (Slots.Count != 0)
            {
