namespace StackExchange.Redis
{
    ...
    public class ConnectionCounters
    {
        ...
        internal void Append(StringBuilder sb)
        {
            ...
            if (Subscriptions != 0)
            {
                sb.Append(", subs=").Append(Subscriptions);
            }

            if (FailedAsynchronously != 0)
            {
                sb.Append(", async-fail=").Append(FailedAsynchronously);
            }

            if (CompletedSynchronously != 0)
            {
                sb.Append(", sync=").Append(CompletedSynchronously);
            }

            if (CompletedAsynchronously != 0)
            {
                sb.Append(", async=").Append(CompletedAsynchronously);
            }

            if (SocketCount != 0)
            {
                sb.Append(", socks=").Append(SocketCount);
            }

            if (NonPreferredEndpointCount != 0)
            {
                sb.Append(", non-pref=").Append(NonPreferredEndpointCount);
            }
        }
    }
}
