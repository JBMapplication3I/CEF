namespace StackExchange.Redis
{
    ...
    internal sealed partial class RedisServer : RedisBase, IServer
    {
        ...
        public IEnumerable<RedisKey> Keys(int database = 0, RedisValue pattern = default, int pageSize = CursorUtils.DefaultPageSize, long cursor = CursorUtils.Origin, int pageOffset = 0, CommandFlags flags = CommandFlags.None)
        {
            if (pageSize <= 0)
            {
        ...
        internal static Message CreateSlaveOfMessage(EndPoint endpoint, CommandFlags flags = CommandFlags.None)
        {
            ...
            {
                if (Format.TryGetHostPort(endpoint, out var hostRaw, out var portRaw))
                {
                    host = hostRaw;
                    port = portRaw;
                }
