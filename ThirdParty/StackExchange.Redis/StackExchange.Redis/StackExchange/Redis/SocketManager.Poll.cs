namespace StackExchange.Redis
{
    ...
    partial class SocketManager
    {
        ...
        readonly Queue<ISocketCallback> readQueue = new(), errorQueue = new();

        private readonly Dictionary<IntPtr, SocketPair> socketLookup = new();
            ...
            while (true)
            {
                ...
                if (callback != null)
                {
                    try
                    {
                        switch (operation)
                        {
                            case CallbackOperation.Read:
                            {
                                callback.Read();
                                break;
                            }
                            case CallbackOperation.Error:
                            {
                                callback.Error();
                                break;
                            }
        ...
        private void ProcessItems(bool setState)
        {
            if (setState)
            {
                managerState = ManagerState.ProcessReadQueue;
            }

            ProcessItems(readQueue, CallbackOperation.Read);
            if (setState)
            {
                managerState = ManagerState.ProcessErrorQueue;
            }
        ...
        private ISocketCallback GetCallback(IntPtr key)
        {
            lock (socketLookup)
            {
                return socketLookup.TryGetValue(key, out var pair) ? pair.Callback : null;
            }
        }
        private void ReadImpl()
        {
            List<IntPtr> dead = null, active = new();
