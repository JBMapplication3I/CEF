    ...
    sealed class ProfileContextTracker
    {
        ...
        struct ProfileContextCell : IEquatable<ProfileContextCell>
        {
            ...
            public bool IsContextLeaked
            {
                get
                {
                    return !TryGetTarget(out var ignored);
                }
            }
            ...
            {
                if (Reference is not WeakReference asWeakRef)
                {
                    target = Reference;
                    return true;
                }
            ...
            public bool Equals(ProfileContextCell other)
            {
                ...
                if (other.TryGetTarget(out var otherObj) != TryGetTarget(out var thisObj))
                {
                    return false;
                }
        ...
        {
            public static readonly ProfileContextCellComparer Singleton = new();
        ...
        {
            var cell = ProfileContextCell.ToLookupBy(ctx);
            if (!profiledCommands.TryRemove(cell, out var storage))
            {
                commands = default;
                return false;
            }
        ...
        public bool TryCleanup()
        {
            ...
            using (var e = profiledCommands.GetEnumerator())
            {
                while (e.MoveNext())
                {
                    ...
                    {
                        if (profiledCommands.TryRemove(pair.Key, out var abandoned))
                        {
