namespace StackExchange.Redis
{
    ...
    public sealed partial class ConnectionMultiplexer : IConnectionMultiplexer, IDisposable
    {
        ...
        public void ExportConfiguration(Stream destination, ExportOptions options = ExportOptions.All)
        {
            ...
            {
                mask |= ExportOptions.Info;
            }

            if (CommandMap.IsAvailable(RedisCommand.CONFIG))
            {
                mask |= ExportOptions.Config;
            }

            if (CommandMap.IsAvailable(RedisCommand.CLIENT))
            {
                mask |= ExportOptions.Client;
            }

            if (CommandMap.IsAvailable(RedisCommand.CLUSTER))
            {
                mask |= ExportOptions.Cluster;
            }
        ...
        internal void MakeMaster(ServerEndPoint server, ReplicationChangeOptions options, TextWriter log)
        {
            ...
            {
                throw ExceptionFactory.AdminModeNotEnabled(IncludeDetailInExceptions, RedisCommand.SLAVEOF, null, server);
            }
            ...
            if (!srv.IsConnected)
            {
                throw ExceptionFactory.NoConnectionAvailable(IncludeDetailInExceptions, IncludePerformanceCountersInExceptions, RedisCommand.SLAVEOF, null, server, GetServerSnapshot());
            }
            ...
            RedisKey tieBreakerKey = default;
            ...
            if ((options & ReplicationChangeOptions.EnslaveSubordinates) != 0)
            {
                foreach (var node in nodes)
                {
                    if (node == server || node.ServerType != ServerType.Standalone)
                    {
                        continue;
                    }
        ...
        internal void LogLocked(TextWriter log, string line, object arg0, object arg1, object arg2)
        {
            ...
            {
                lock (LogSyncLock)
                { log.WriteLine(line, arg0, arg1, arg2); }
            }
        }
        ...
        public EndPoint[] GetEndPoints(bool configuredOnly = false)
        {
            if (configuredOnly)
            {
                return RawConfig.EndPoints.ToArray();
            }
        ...
        private void LogLockedWithThreadPoolStats(TextWriter log, string message, out int busyWorkerCount)
        {
            ...
            if (log != null)
            {
                ...
                sb.Append(message);
                busyWorkerCount = GetThreadPoolStats(out var iocp, out var worker);
        ...
        private async Task<bool> WaitAllIgnoreErrorsAsync(Task[] tasks, int timeoutMilliseconds, TextWriter log)
        {
            ...
            var watch = Stopwatch.StartNew();
#if !CORE_CLR
            LogLockedWithThreadPoolStats(log, "Awaiting task completion", out var busyWorkerCount);
#endif
            try
            {
                // if none error, great
        ...
        static ConnectionMultiplexer CreateMultiplexer(object configuration)
        {
            ...
            if (configuration is string @string)
            {
                config = ConfigurationOptions.Parse(@string);
            }
            else if (configuration is ConfigurationOptions options)
            {
        ...
        private readonly Hashtable servers = new();
        private volatile ServerEndPoint[] serverSnapshot = NilServers;

        private static readonly ServerEndPoint[] NilServers = Array.Empty<ServerEndPoint>();
        ...
        private ConnectionMultiplexer(ConfigurationOptions configuration)
        {
            ...
            this.RawConfig = configuration ?? throw new ArgumentNullException(nameof(configuration));
        ...
        internal CompletionManager UnprocessableCompletionManager { get; private set; }
        ...
        public ISubscriber GetSubscriber(object asyncState = null)
        {
            ...
            {
                throw new NotSupportedException("The pub/sub API is not available via twemproxy");
            }
        ...
        public IServer GetServer(EndPoint endpoint, object asyncState = null)
        {
            ...
            if (RawConfig.Proxy == Proxy.Twemproxy)
            {
                throw new NotSupportedException("The server API is not available via twemproxy");
            }
        ...
        public long OperationCount
        {
            get
            {
                ...
                for (int i = 0; i < snapshot.Length; i++)
                {
                    total += snapshot[i].OperationCount;
                }
        ...
        internal int SyncConnectTimeout(bool forConnect)
        {
            ...
            if (timeout >= int.MaxValue / retryCount)
            {
                return int.MaxValue;
            }

            timeout *= retryCount;
            if (timeout >= int.MaxValue - 500)
            {
                return int.MaxValue;
            }
        ...
        internal async Task<bool> ReconfigureAsync(bool first, bool reconfigureAll, TextWriter log, EndPoint blame, string cause, bool publishReconfigure = false, CommandFlags publishReconfigureFlags = CommandFlags.None)
        {
            ...
            {   // note that "activeReconfigs" starts at one; we don't need to set it the first time
                ...
                if (first)
                {
                    ...
                    lock (servers)
                    {
                        serverSnapshot = new ServerEndPoint[RawConfig.EndPoints.Count];
                ...
                do
                {
                    ...
                    for (int iter = 0; iter < iterCount; ++iter)
                    {
                        ...
                        RedisKey tieBreakerKey = useTieBreakers ? (RedisKey)RawConfig.TieBreaker : default;
...
#pragma warning restore IDE1006 // Naming Styles
                ...
                for (int i = 0; i < tieBreakers.Length; i++)
                {
                    ...
                    switch (status)
                    {
                        ...
                        else
                        {
                            ...
                            if (!uniques.TryGetValue(s, out var count))
                            {
                                count = 0;
                            }
                        ...
                        {
                            if (ex.Message.StartsWith("MOVED ") || ex.Message.StartsWith("ASK "))
                            {
                                continue;
                            }
        ...
        {
            if (servers == null || string.IsNullOrWhiteSpace(endpoint))
            {
                return null;
            }
        ...
        static string DeDotifyHost(string input)
        {
            ...
            {
                return input; // need first char to be alpha for this to work
            }

            int periodPosition = input.IndexOf('.');
            if (periodPosition <= 0)
            {
                return input; // no period or starts with a period? nothing useful to split
            }
        ...
        internal void UpdateClusterRange(ClusterConfiguration configuration)
        {
            ...
            {
                if (node.IsSlave || node.Slots.Count == 0)
                {
                    continue;
                }
        ...
        private bool TryPushMessageToBridge<T>(Message message, ResultProcessor<T> processor, ResultBox<T> resultBox, ref ServerEndPoint server)
        {
            ...
            if (server != null)
            {
                ...
                if (profCtx != null)
                {
                    if (profiledCommands.TryGetValue(profCtx, out var inFlightForCtx))
                    {
        ...
        internal ConfigurationOptions RawConfig { get; private set; }

        internal ServerSelectionStrategy ServerSelectionStrategy { get; private set; }
        ...
        internal T ExecuteSyncImpl<T>(Message message, ResultProcessor<T> processor, ServerEndPoint server)
        {
            ...
            else
            {
                ...
                lock (source)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            int queue = server.GetOutstandingCount(message.Command, out var inst, out var qu, out var qs, out var qc, out var wr, out var wq, out var @in, out var ar);
...
#if !CORE_CLR
                            int busyWorkerCount = GetThreadPoolStats(out var iocp, out var worker);
                            ...
                            if (StormLogThreshold >= 0 && queue >= StormLogThreshold && Interlocked.CompareExchange(ref haveStormLog, 1, 0) == 0)
                            {
                                ...
                                {
                                    Interlocked.Exchange(ref stormLogSnapshot, log);
                                }
                            }
                        }
                ...
                ResultBox<T>.UnwrapAndRecycle(source, true, out var val, out var ex); // now that we aren't locking it...
                if (ex != null)
                {
                    throw ex;
                }
        ...
        {
            GetThreadPoolStats(out var iocp, out var worker);
...
#pragma warning disable CA1416 // Validate platform compatibility
            if (PerfCounterHelper.TryGetSystemCPU(out var systemCPU))
            {
