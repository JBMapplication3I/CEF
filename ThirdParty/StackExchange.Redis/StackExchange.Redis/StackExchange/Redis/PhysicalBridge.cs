namespace StackExchange.Redis
{
    ...
    sealed partial class PhysicalBridge : IDisposable
    {
        ...
        internal void OnDisconnected(ConnectionFailureType failureType, PhysicalConnection connection, out bool isCurrent, out State oldState)
        {
            ...
            var ping = queue.DequeueUnsentPing(out var count);
            if (ping != null)
            {
            ...
            oldState = default; // only defined when isCurrent = true
            ...
            try
            {
                ...
                switch (state)
                {
                    ...
                    if (shouldRetry)
                    {
                        ...
                        OnDisconnected(ConnectionFailureType.UnableToConnect, snapshot, out var isCurrent, out var oldState);
                        using (snapshot)
                        { } // dispose etc
                        TryConnect(null);
                    }
                    ...
                    if (tmp != null)
                    {
                        ...
                        else if (writeEverySeconds > 0 && tmp.LastWriteSecondsAgo >= writeEverySeconds)
                        {
                            ...
                            {
                                OnDisconnected(ConnectionFailureType.SocketFailure, tmp, out var ignore, out var oldState);
                            }
                        }
