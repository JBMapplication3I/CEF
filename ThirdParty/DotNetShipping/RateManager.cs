namespace DotNetShipping
{
    ...
    /// <summary>Responsible for coordinating the retrieval of rates from the specified providers for a specified
    /// shipment.</summary>
    public class RateManager
    {
        ...
        /// <summary>Creates a new RateManager instance.</summary>
        public RateManager()
        {
        ...
        /// <summary>Adds the specified provider to be rated when <see cref="GetRates" /> is called.</summary>
        /// <param name="provider">A provider-specific implementation of <see cref="IShippingProvider" />.</param>
        ...
        /// <summary>Retrieves rates for all of the specified providers using the specified address and package
        /// information.</summary>
        /// <param name="originAddress">     An instance of <see cref="Address" /> specifying the origin of the shipment.</param>
        /// <param name="destinationAddress">An instance of <see cref="Address" /> specifying the destination of the
        ///                                  shipment.</param>
        ...
        public Task<Shipment> GetRates(Address originAddress, Address destinationAddress, Package package)
        {
            return GetRates(originAddress, destinationAddress, new List<Package> { package });
        }

        /// <summary>Retrieves rates for all of the specified providers using the specified address and packages
        /// information.</summary>
        /// <param name="originAddress">     An instance of <see cref="Address" /> specifying the origin of the shipment.</param>
        /// <param name="destinationAddress">An instance of <see cref="Address" /> specifying the destination of the
        ///                                  shipment.</param>
        /// <param name="packages">          An instance of <see cref="PackageCollection" /> specifying the packages to
        ///                                  be rated.</param>
        /// <returns>A <see cref="Shipment" /> instance containing all returned rates.</returns>
        public Task<Shipment> GetRates(Address originAddress, Address destinationAddress, List<Package> packages)
        {
            var shipment = new Shipment(originAddress, destinationAddress, packages) { RateAdjusters = _adjusters };
            return GetRatesAsync(shipment);
        }

        private async Task<Shipment> GetRatesAsync(Shipment shipment)
        {
            await Task.WhenAll(
                _providers
                    .Select(x =>
