namespace DotNetShipping.ShippingProviders
{
    ...
    public class USPSProvider : AbstractShippingProvider
    {
        ...
        /// </summary>
        private readonly Dictionary<string, string> _serviceCodes = new()
        {
        ...
        {
            return GetRatesAsync(false);
        }

        public async Task GetRatesAsync(bool baseRatesOnly)
        {
            ...
            using (var writer = XmlWriter.Create(sb, settings))
            {
                ...
                foreach (var package in Shipment.Packages)
                {
                    ...
                    writer.WriteElementString(
                        "ZipOrigination",
                        Shipment.OriginAddress.CountryCode == "US" && Shipment.OriginAddress.PostalCode.Length > 5
                            ? Shipment.OriginAddress.PostalCode.Substring(0, 5)
                            : Shipment.OriginAddress.PostalCode);
                    writer.WriteElementString(
                        "ZipDestination",
                        Shipment.DestinationAddress.CountryCode == "US"
                        && Shipment.DestinationAddress.PostalCode.Length > 5
                            ? Shipment.DestinationAddress.PostalCode.Substring(0, 5)
        ...
        {
            return Shipment.OriginAddress.IsUnitedStatesAddress()
                && Shipment.DestinationAddress.IsUnitedStatesAddress();
        }
        ...
        private void ParseResult(string response, IList<string> includeSpecialServiceCodes = null)
        {
            ...
            var rates =
                from item in document.Descendants("Postage")
            ...
            foreach (var r in rates)
            {
                ...
                if (includeSpecialServiceCodes != null
                    && includeSpecialServiceCodes.Count > 0
                    && r.SpecialServices != null)
                {
                ...
                if (r.DeliveryDate != null)
                {
                    AddRate(
                        ...
                        r.TotalCharges + additionalCharges,
                        DateTime.Parse(r.DeliveryDate));
                }
                else
                {
                    AddRate(
            ...
            {
                var errors =
                    from item in document.Descendants("Error")
