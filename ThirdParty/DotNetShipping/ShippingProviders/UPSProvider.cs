namespace DotNetShipping.ShippingProviders
{
    ...
    public class UPSProvider : AbstractShippingProvider
    {
        ...
        private readonly Hashtable _serviceCodes = new(12);
        ...
        public UPSProvider(string licenseNumber, string userId, string password) : this(
            licenseNumber,
            userId,
            password,
            DEFAULT_TIMEOUT)
        ...
        private byte[] BuildRatesRequestMessage()
        {
            ...
            writer.WriteElementString(
                "RequestOption",
                string.IsNullOrWhiteSpace(_serviceDescription) ? "Shop" : _serviceDescription);
            ...
            else
            {
                writer.WriteElementString(
                    "Code",
                    string.IsNullOrWhiteSpace(_shipperNumber)
                        ? "01"
                        : "00"); // 00 gets shipper number rates, 01 for daily rates
            }
            ...
            if (UseNegotiatedRates)
            {
                ...
                writer.WriteElementString("NegotiatedRatesIndicator", string.Empty);
        ...
        private void ParseRatesResponseMessage(XDocument xDoc)
        {
            if (xDoc.Root != null)
            {
                ...
                foreach (var rateNode in ratedShipment)
                {
                    ...
                    var totalCharges =
                        Convert.ToDecimal(rateNode.XPathSelectElement("TotalCharges/MonetaryValue").Value);
                    if (UseNegotiatedRates)
                    {
                        var negotiatedRate = rateNode.XPathSelectElement(
                    ...
                    {
                        // no gauranteed delivery date, so use MaxDate to ensure correct sorting
                    ...
                    {
                        // no scheduled delivery time, so use 11:59:00 PM to ensure correct sorting
                        date += " 11:59:00 PM";
                    }
                    else
                    {
