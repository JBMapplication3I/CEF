...
namespace ServiceStack.Support
{
    ...
    public abstract class AdapterBase
    {
        ...
        protected T Execute<T>(Func<T> action)
        {
...
#if !NETFX_CORE && !WP
                Log.Debug($"Action '{action.Method().Name}' executed. Took {timeTaken.TotalMilliseconds} ms.");
#endif
                return result;
            }
            catch (Exception ex)
            {
#if !NETFX_CORE && !WP
                Log.Error($"There was an error executing Action '{action.Method().Name}'. Message: {ex.Message}");
#endif
                throw;
            }
        }
