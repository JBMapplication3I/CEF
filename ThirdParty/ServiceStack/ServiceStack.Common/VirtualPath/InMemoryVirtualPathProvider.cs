...
namespace ServiceStack.VirtualPath
{
    ...
    /// <summary>In Memory Virtual Path Provider.</summary>
    /// <seealso cref="AbstractVirtualPathProviderBase"/>
    /// <seealso cref="IVirtualFiles"/>
    /// <seealso cref="IWriteableVirtualPathProvider"/>
    public class MemoryVirtualFiles : AbstractVirtualPathProviderBase, IVirtualFiles
...
#pragma warning restore 618
    {
        ...
        public void WriteFile(string filePath, string textContents)
        {
            ...
            DeleteFile(filePath);
            files.Add(new(this, CreateDirectory(GetDirPath(filePath)))
            {
                FilePath = filePath,
        ...
        {
            filePath = SanitizePath(filePath);
            files.RemoveAll(x => x.FilePath == filePath);
        }
    ...
    public class InMemoryVirtualFile : AbstractVirtualFileBase
    {
        ...
        public string DirPath => Directory.VirtualPath;
        ...
        {
            var file = VirtualPathProvider.GetFile(VirtualPath) as InMemoryVirtualFile;
            if (file != null)
            {
                FilePath = file.FilePath;
                FileLastModified = file.FileLastModified;
                TextContents = file.TextContents;
                ByteContents = file.ByteContents;
            }
        }
    }
}