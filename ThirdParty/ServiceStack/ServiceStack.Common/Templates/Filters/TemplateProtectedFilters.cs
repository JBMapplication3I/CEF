    ...
    public class TemplateProtectedFilters : TemplateFilter
    {
        ...
        private static string ConvertDataTypeToContentType(string dataType)
        {
            return dataType switch
            {
                ...
                "csv" => MimeTypes.Csv,
                "xml" => MimeTypes.Xml,
                "text" => MimeTypes.PlainText,
                "form" => MimeTypes.FormUrlEncoded,
                _ => throw new NotSupportedException($"Unknown dataType '{dataType}'"),
            };
        }
        ...
        internal IDictionary GetCache(string cacheName)
        {
            ...
            {
                "Cache" => Context.Cache,
                "ExpiringCache" => Context.ExpiringCache,
                "BinderCache" => Context.BinderCache,
                "AssignExpressionCache" => Context.AssignExpressionCache,
        ...
        public object cacheClear(TemplateScopeContext scope, object cacheNames)
        {
            ...
            if (cacheNames is string strName)
            {
                caches = strName.EqualsIgnoreCase("all")
                    ? new() {"Cache", "ExpiringCache", "BinderCache", "AssignExpressionCache", "PathMappings"}
            ...
            {
                throw new NotSupportedException(nameof(cacheClear) + 
