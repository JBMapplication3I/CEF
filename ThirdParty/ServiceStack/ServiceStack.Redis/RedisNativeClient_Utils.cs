...
namespace ServiceStack.Redis
{
    public partial class RedisNativeClient
    {
        ...
        {
            var safeLastCommand = string.IsNullOrEmpty(Password) ? lastCommand : (lastCommand ?? "").Replace(Password, "");
        ...
        {
            currentBufferIndex = 0;
            for (var i = cmdBuffer.Count - 1; i >= 0; i--)
            {
        ...
        public string CalculateSha1(string luaBody)
        {
            ...
            var buffer = Encoding.UTF8.GetBytes(luaBody);
