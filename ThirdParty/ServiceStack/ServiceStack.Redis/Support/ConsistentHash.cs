...
namespace ServiceStack.Redis.Support
{
	public class ConsistentHash<T>
	{
		...
		public T GetTarget(string key)
		{
			...
			var firstNode = ModifiedBinarySearch(_circle.Keys.ToArray(), hash);
		...
		public void AddTarget(T node, int weight)
		{
			...
			var repeat = weight > 0 ? weight * Replicas : Replicas;

			for (var i = 0; i < repeat; i++)
			{
				var identifier = node.GetHashCode().ToString() + "-" + i;
				var hashCode = _hashFunction(identifier);
		...
		public static ulong Md5Hash(string key)
		{
			...
			{
				var data = hash.ComputeHash(Encoding.UTF8.GetBytes(key));
