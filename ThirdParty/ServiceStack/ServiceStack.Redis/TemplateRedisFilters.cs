...
namespace ServiceStack.Redis
{
    ...
    public class TemplateRedisFilters : TemplateFilter
    {
        ...
        List<string> parseCommandString(string cmd)
        {
            ...
            for (var i = 0; i < cmd.Length; i++)
            {
                ...
                if (c == ' ')
                {
                    ...
                    if (cmdArgCounts.TryGetValue(args[0], out var argCount) && args.Count == argCount - 1)
                        break;
                }
            }
        ...
        public string redisToConnectionString(TemplateScopeContext scope, object connectionInfo, object options)
        {
            ...
            {
                var host = (d.TryGetValue("host", out var h) ? h as string : null) ?? "localhost";
                var port = d.TryGetValue("port", out var p) ? DynamicInt.Instance.ConvertFrom(p) : 6379;
                var db = d.TryGetValue("db", out var oDb) ? DynamicInt.Instance.ConvertFrom(oDb) : 0;

                connectionString = $"{host}:{port}?db={db}";

                if (d.TryGetValue("password", out var password))
        ...
        public string redisSearchKeysAsJson(TemplateScopeContext scope, string query, object options)
        {
            ...
            try
            {
                ...
                var limit = args.TryGetValue("limit", out var value)
                    ? value.ConvertTo<int>()
