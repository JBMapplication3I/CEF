...
namespace ServiceStack
{
    public static class HttpUtils
    {
        ...
        public static HttpStatusCode? GetStatus(this Exception ex)
        {
            ...
            {
                null => null,
                WebException webEx => GetStatus(webEx),
                IHasStatusCode hasStatus => (HttpStatusCode) hasStatus.StatusCode,
                _ => null,
            };
        }
            ...
            string method = "POST")
        {
            ...
            PclExport.Instance.InitHttpWebRequest(httpReq, contentLength: contentLength, allowAutoRedirect: false, keepAlive: false);
            if (ResultsFilter != null)
            {
...
namespace ServiceStack
{
    public static class MimeTypes
    {
        ...
        public static string GetExtension(string mimeType)
        {
            ...
            return parts.Length switch
            {
                1 => "." + parts[0],
                2 => "." + parts[1],
                _ => throw new NotSupportedException("Unknown mimeType: " + mimeType),
            };
        }
        ...
        public static void AssertIsValid(string compressionType)
        {
            if (!IsValid(compressionType))
            {
                throw new NotSupportedException(
                    compressionType + " is not a supported compression type. Valid types: gzip, deflate, br.");
            }
        }
    ...
    public static class HttpStatus
    {
        ...
        {
            if (statusCode is < 100 or >= 600)
            {
                return string.Empty;
            }
            var i = statusCode / 100;
            var j = statusCode % 100;
            return j < Descriptions[i].Length ? Descriptions[i][j] : string.Empty;
        }
