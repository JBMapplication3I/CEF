...
namespace ServiceStack.MiniProfiler
{
    ...
    {
        private ConcurrentDictionary<Tuple<object, ExecuteType>, SqlTiming> inProgress = new();
        private ConcurrentDictionary<DbDataReader, SqlTiming> inProgressReaders = new();
        ...
        public void ExecuteFinishImpl(DbCommand command, ExecuteType type, DbDataReader reader = null)
        {
            ...
            inProgress.TryRemove(id, out var ignore);
            if (reader != null)
            {
        ...
        public void ReaderFinishedImpl(DbDataReader reader)
        {
            ...
            if (inProgressReaders.TryGetValue(reader, out var stat))
            {
                stat.ReaderFetchComplete();
                inProgressReaders.TryRemove(reader, out var ignore);
            }
        }
    }
