...
namespace ServiceStack.Auth
{
    ...
    public class YammerAuthProvider : OAuthProvider
    {
        ...
        {
            ClientId = appSettings.GetString("oauth.yammer.ClientId");
            ClientSecret = appSettings.GetString("oauth.yammer.ClientSecret");
            PreAuthUrl = appSettings.GetString("oauth.yammer.PreAuthUrl");
        }
        ...
        public override object Authenticate(IServiceBase authService, IAuthSession session, Authenticate request)
        {
            ...
            {
                var preAuthUrl = $"{PreAuthUrl}?client_id={ClientId}&redirect_uri={RedirectUrl.UrlEncode()}";
            ...
            var accessTokenUrl = $"{AccessTokenUrl}?client_id={ClientId}&client_secret={ClientSecret}&code={code}";
            ...
            try
            {
                ...
                // Pass along
                var response = OnAuthenticated(authService, session, tokens, authInfo.ToDictionary());
                if (response != null)
                {
                    return response;
                }
                ...
                // Has access!
                return authService.Redirect(SuccessRedirectUrlFilter(this, CallbackUrl.SetParam("s", "1")));
            }
            catch (WebException webEx)
            {
                ...
                {
                    return authService.Redirect(FailedRedirectUrlFilter(this, CallbackUrl.SetParam("f", "AccessTokenFailed")));
                }
            }

            // Unknown error, shouldn't get here.
            return authService.Redirect(FailedRedirectUrlFilter(this, CallbackUrl.SetParam("f", "Unknown")));
        }
