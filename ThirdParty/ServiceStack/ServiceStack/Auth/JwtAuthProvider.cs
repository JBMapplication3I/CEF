...
namespace ServiceStack.Auth
{
    ...
    public class JwtAuthProvider : JwtAuthProviderReader, IAuthResponseFilter
    {
        ...
        {
            SetBearerTokenOnAuthenticateResponse = appSettings?.Get("jwt.SetBearerTokenOnAuthenticateResponse", true) != false;
    ...
    public class ConvertSessionToTokenService : Service
    {
        ...
        {
            if (AuthenticateService.GetAuthProvider(JwtAuthProviderReader.Name) is not JwtAuthProvider jwtAuthProvider)
            {
    ...
    public class GetAccessTokenService : Service
    {
        public object Any(GetAccessToken request)
        {
            ...
            if (AuthRepository is not IUserAuthRepository userRepo)
            {
            ...
            if (jwtAuthProvider.ValidateRefreshToken?.Invoke(jwtPayload, Request) == false)
            {
