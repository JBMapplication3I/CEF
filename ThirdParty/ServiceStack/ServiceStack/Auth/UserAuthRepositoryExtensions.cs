    ...
    public static class UserAuthRepositoryExtensions
    {
            ...
            ICollection<string> roles = null, ICollection<string> permissions = null)
        {
            ...
            else
            {
                if (!roles.IsEmpty())
                {
                    foreach (var missingRole in roles.Where(x => userAuth.Roles?.Contains(x) != true))
                    {
                        userAuth.Roles ??= new();
                ...
                {
                    foreach (var missingPermission in permissions.Where(x => userAuth.Permissions?.Contains(x) != true))
                    {
                        userAuth.Permissions ??= new();
        ...
        private static IUserAuthRepository AssertUserAuthRepository(this IAuthRepository repo)
        {
            if (repo is not IUserAuthRepository userRepo)
            {
