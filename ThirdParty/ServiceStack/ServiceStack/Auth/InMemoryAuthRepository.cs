...
namespace ServiceStack.Auth
{
    ...
    {
        public static readonly InMemoryAuthRepository Instance = new();
        ...
        private class TypedData<T> : IClearable
        {
        ...
        private class InMemoryManagerFacade : IRedisClientManagerFacade
        {
        ...
        private class InMemoryClientFacade : IRedisClientFacade
        {
            private readonly InMemoryAuthRepository root;
            ...
            private class InMemoryTypedClientFacade<T> : ITypedRedisClientFacade<T>
            {
                private readonly InMemoryAuthRepository root;
            ...
            public HashSet<string> GetAllItemsFromSet(string setId)
            {
                lock (root.Sets)
                {
                    return root.Sets.TryGetValue(setId, out var set) ? set : new();
                }
            }
            ...
            public void Store<T>(T item)
            {
                ...
                lock (TypedData<T>.Instance.Items)
                {
                    for (var i = 0; i < TypedData<T>.Instance.Items.Count; i++)
                    {
                        ...
                        if (o.ToId().ToString() != item.ToId().ToString())
                        {
                            continue;
                        }
