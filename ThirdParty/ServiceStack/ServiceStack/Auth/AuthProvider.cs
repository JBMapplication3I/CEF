...
namespace ServiceStack.Auth
{
    public abstract class AuthProvider : IAuthProvider
    {
        ...
        private static readonly ConcurrentDictionary<string, long> transientUserIdsMap = new();
        ...
        public virtual void OnFailedAuthentication(IAuthSession session, IRequest httpReq, IResponse httpRes)
        {
            ...
            httpRes.AddHeader(HttpHeaders.WwwAuthenticate, "{0} realm=\"{1}\"".Fmt(Provider, AuthRealm));
            httpRes.EndRequest();
        }
    ...
    public static class AuthExtensions
    {
        ...
        {
            return authProvider?.IsAuthorized(session, tokens) == true;
        }
        ...
        {
            return url?.EndsWith("/auth/logout") != false
                ? null
                : url;
        }
        ...
        public static void SaveSession(this IAuthProvider provider, IServiceBase authService, IAuthSession session, TimeSpan? sessionExpiry = null)
        {
            ...
            var persistSession = authProvider?.PersistSession != false;
            if (persistSession)
            {
