...
namespace ServiceStack.Auth
{
    ...
    public class ApiKeyAuthProvider : AuthProvider, IAuthWithRequest, IAuthPlugin
    {
        ...
        {
            return session?.IsAuthenticated == true && !session.UserAuthName.IsNullOrEmpty();
        }
        ...
        private void PreAuthenticateWithApiKey(IRequest req, IResponse res, ApiKey apiKey)
        {
            ...
            {
                var response = authService.Post(new()
                {
                    provider = Name,
                    UserName = "ApiKey",
                    Password = apiKey.Id,
        ...
        public void Register(IAppHost appHost, AuthFeature feature)
        {
            ...
            if (authRepo is not IManageApiKeys apiRepo)
            {
    ...
    public class GetApiKeysService : Service
    {
        ...
        {
            var apiKeyAuth = Request.AssertValidApiKeyRequest();
            ...
            var apiRepo = (IManageApiKeys)HostContext.AppHost.GetAuthRepository(Request);
            using (apiRepo as IDisposable)
            {
