...
namespace ServiceStack.Auth
{
    public abstract class OAuthProvider : AuthProvider
    {
        ...
        {
            AuthRealm = appSettings.Get("OAuthRealm", authRealm);

            Provider = oAuthProvider;
            RedirectUrl = appSettings.GetString($"oauth.{oAuthProvider}.RedirectUrl")
                ?? FallbackConfig(appSettings.GetString("oauth.RedirectUrl"));
            CallbackUrl = appSettings.GetString($"oauth.{oAuthProvider}.CallbackUrl")
                ?? FallbackConfig(appSettings.GetString("oauth.CallbackUrl"));
            ConsumerKey = appSettings.GetString($"oauth.{oAuthProvider}.{consumerKeyName}");
            ConsumerSecret = appSettings.GetString($"oauth.{oAuthProvider}.{consumerSecretName}");

            RequestTokenUrl = appSettings.Get($"oauth.{oAuthProvider}.RequestTokenUrl", authRealm + "oauth/request_token");
            AuthorizeUrl = appSettings.Get($"oauth.{oAuthProvider}.AuthorizeUrl", authRealm + "oauth/authorize");
            AccessTokenUrl = appSettings.Get($"oauth.{oAuthProvider}.AccessTokenUrl", authRealm + "oauth/access_token");
            SaveExtendedUserInfo = appSettings.Get($"oauth.{oAuthProvider}.SaveExtendedUserInfo", true);

            OAuthUtils = new(this);
            AuthHttpGateway = new AuthHttpGateway();
        }
        ...
        public override bool IsAuthorized(IAuthSession session, IAuthTokens tokens, Authenticate request = null)
        {
            ...
            return session?.IsAuthenticated == true && !string.IsNullOrEmpty(tokens?.AccessTokenSecret);
        }
