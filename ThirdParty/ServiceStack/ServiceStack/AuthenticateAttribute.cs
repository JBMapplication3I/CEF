...
namespace ServiceStack
{
    ...
    public class AuthenticateAttribute : RequestFilterAttribute
    {
        ...
        public AuthenticateAttribute(ApplyTo applyTo)
            : base(applyTo)
        {
            Priority = (int)RequestFilterPriority.Authenticate;
        }
        ...
        public override void Execute(IRequest req, IResponse res, object requestDto)
        {
            ...
            var matchingOAuthConfigs = AuthenticateService.AuthProviders.Where(x =>
                ...
                || x.Provider == Provider).ToList();

            if (matchingOAuthConfigs.Count == 0)
            {
                res.WriteError(req, requestDto, $"No OAuth Configs found matching {Provider ?? "any"} provider");
                res.EndRequest();
                return;
            }
            ...
            {
                if (DoHtmlRedirectIfConfigured(req, res, true))
                {
                    return;
                }
