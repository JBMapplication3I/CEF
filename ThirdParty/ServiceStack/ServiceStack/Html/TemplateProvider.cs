...
namespace ServiceStack.Html
{
    public class TemplateProvider
    {
        ...
        private AutoResetEvent waiter = new(false);
        ...
        private readonly Dictionary<string, IVirtualFile> templatePathsFound = new(StringComparer.OrdinalIgnoreCase);
        private readonly HashSet<string> templatePathsNotFound = new(StringComparer.OrdinalIgnoreCase);

        public string GetTemplatePath(IVirtualDirectory fileDir)
        {
            try
            {
                if (templatePathsNotFound.Contains(fileDir.VirtualPath))
                {
                    return null;
                }
        ...
        private readonly ConcurrentQueue<IViewPage> compilePages = new();
        private readonly ConcurrentQueue<IViewPage> priorityCompilePages = new();
        ...
        public void CompileQueuedPages()
        {
            ...
            if (compileInParallel)
            {
                ...
                if (threadsToRun <= runningThreads)
                {
                    return;
                }
        ...
        private void CompileAllPages()
        {
            try
            {
                ...
                {
                    if (compilePages.TryDequeue(out var viewPage))
                    {
                        viewPage.Compile();
                    }
                }
        ...
        {
            if (compilePages.IsEmpty && priorityCompilePages.IsEmpty)
            {
                return;
            }
