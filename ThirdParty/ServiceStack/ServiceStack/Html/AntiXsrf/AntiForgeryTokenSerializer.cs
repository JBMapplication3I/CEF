...
namespace ServiceStack.Html.AntiXsrf
{
    internal sealed class AntiForgeryTokenSerializer : IAntiForgeryTokenSerializer
    {
        ...
        public AntiForgeryToken Deserialize(string serializedToken)
        {
            try
            {
                using (var stream = MemoryStreamFactory.GetStream(_cryptoSystem.Unprotect(serializedToken)))
                {
        ...
        private static AntiForgeryToken DeserializeImpl(BinaryReader reader)
        {
            ...
            deserializedToken.SecurityToken = new(AntiForgeryToken.SecurityTokenBitLength, securityTokenBytes);
            ...
            if (!deserializedToken.IsSessionToken)
            {
                ...
                if (isClaimsBased)
                {
                    ...
                    deserializedToken.ClaimUid = new(AntiForgeryToken.ClaimUidBitLength, claimUidBytes);
                }
                else
                {
