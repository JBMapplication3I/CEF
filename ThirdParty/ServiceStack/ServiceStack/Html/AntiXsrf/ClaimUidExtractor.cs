...
namespace ServiceStack.Html.AntiXsrf
{
    internal sealed class ClaimUidExtractor : IClaimUidExtractor
    {
        ...
        {
            if (identity?.IsAuthenticated != true || _config.SuppressIdentityHeuristicChecks)
            {
            ...
            var uniqueIdentifierParameters = GetUniqueIdentifierParameters(claimsIdentity, _config.UniqueClaimTypeIdentifier);
            var claimUidBytes = CryptoUtil.ComputeSHA256(uniqueIdentifierParameters);
            return new(256, claimUidBytes);
        }
        ...
        internal static string[] GetUniqueIdentifierParameters(ClaimsIdentity claimsIdentity, string uniqueClaimTypeIdentifier)
        {
            ...
            if (!string.IsNullOrEmpty(uniqueClaimTypeIdentifier))
            {
                ...
                if (string.IsNullOrEmpty(matchingClaim?.Value))
                {
            ...
            if (string.IsNullOrEmpty(nameIdentifierClaim?.Value) 
