...
namespace ServiceStack.Html.AntiXsrf
{
    ...
    {
        private static readonly RNGCryptoServiceProvider _prng = new();
        ...
        public static byte[] ComputeSHA256(IList<string> parameters)
        {
            using (var ms = MemoryStreamFactory.GetStream())
            {
                using (var bw = new BinaryWriter(ms))
                {
                    ...
                    using (var sha256 = _sha256Factory())
                    {
                        var retVal = sha256.ComputeHash(ms.GetBuffer(), 0, checked((int)ms.Length));
                        return retVal;
                    }
                }
            }
