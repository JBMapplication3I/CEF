...
namespace ServiceStack.Html
{
    public class HtmlHelper
    {
        ...
        public static List<Type> HtmlExtensions = new()
        {
        ...
        {
            RenderHtml = true;
            HttpRequest = httpReq ?? (htmlHelper != null ? htmlHelper.HttpRequest : null);
            HttpResponse = httpRes ?? (htmlHelper != null ? htmlHelper.HttpResponse : null);
            ViewEngine = viewEngine;
            ViewData = viewData;
            ViewData.PopulateModelState();
        }
        ...
        {
            var masterModel = viewData;
            try
            {
                viewData = new(model);
                var result = ViewEngine.RenderPartial(viewName, model, RenderHtml, Writer, this);
                return MvcHtmlString.Create(result);
            }
            finally
            {
                viewData = masterModel;
            }
        }
        ...
        {
            get { return viewBag ??= new(() => ViewData); }
        }
        ...
        {
            return MvcHtmlString.Create(AntiXsrf.AntiForgery.GetHtml().ToString());
        }
        ...
        {
            if (HttpRequest == null || HttpRequest.HttpMethod == HttpMethods.Get)
            {
                return null;
            }

            var postedValue = HttpRequest.FormData[key];
            if (postedValue == null)
            {
                return null;
            }
        ...
        public MvcHtmlString HttpMethodOverride(string httpMethod)
        {
            ...
            var tagBuilder = new TagBuilder("input")
            {
                ...
                {
                    ["type"] = "hidden",
                    ["name"] = HttpHeaders.XHttpMethodOverride,
                    ["value"] = httpMethod,
                },
            };
        ...
        {
            var errorStatus = HttpRequest.GetItem(HtmlFormat.ErrorStatusKey);
        ...
        public MvcHtmlString RenderMarkdownToHtml(string markdown)
        {
            ...
            return feature != null
                ...
                : new(new MarkdownSharp.Markdown().Transform(markdown));
        }
    }
