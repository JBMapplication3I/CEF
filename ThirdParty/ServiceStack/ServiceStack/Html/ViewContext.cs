...
//]]>
        ...
        private static readonly object _formContextKey = new();
        private static readonly object _lastFormNumKey = new();
        ...
        private FormContext _defaultFormContext = new();
        ...
        public ViewContext(HttpContextBase httpContext, IView view, ViewDataDictionary viewData, TextWriter writer)
        {
            ...
            HttpContext = httpContext ?? throw new ArgumentException("httpContext");
        }

        public virtual bool ClientValidationEnabled
        {
            get => GetClientValidationEnabled(Scope, HttpContext);
        ...
        public virtual FormContext FormContext
        {
            get =>
                ...
                HttpContext.Items[_formContextKey] as FormContext ?? _defaultFormContext;
        ...
        {
            get => _scopeThunk ?? GlobalScopeThunk;
            set => _scopeThunk = value;
        }

        public virtual bool UnobtrusiveJavaScriptEnabled
        {
            get => GetUnobtrusiveJavaScriptEnabled(Scope, HttpContext);
        ...
        {
            var formNum = IncrementFormCount(HttpContext.Items);
        ...
        {
            var lastFormNum = items[_lastFormNumKey];
            var newFormNum = lastFormNum != null ? (int)lastFormNum + 1 : 0;
        ...
        {
            var formContext = GetFormContextForClientValidation();
            if (formContext == null || UnobtrusiveJavaScriptEnabled)
            {
                return; // do nothing
            }

            var scriptWithCorrectNewLines = ClientValidationScript.Replace("\r\n", Environment.NewLine);
            var validationJson = formContext.GetJsonValidationMetadata();
            var formatted = string.Format(CultureInfo.InvariantCulture, scriptWithCorrectNewLines, validationJson);
        ...
        private sealed class ScopeCache
        {
            ...
            {
                get => _unobtrusiveJavaScriptEnabled;
                set
                {
