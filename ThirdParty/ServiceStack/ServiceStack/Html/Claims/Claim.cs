...
namespace ServiceStack.Html.Claims
{
    ...
    internal sealed class Claim
    {
        ...
        private static class ClaimFactory<TClaim>
        {
            ...
            {
                return new(_claimTypeGetter(claim), _valueGetter(claim));
            }
            ...
            {
                var propInfo = typeof(TClaim).GetProperty(propertyName, BindingFlags.Public | BindingFlags.Instance, null, typeof(string), Type.EmptyTypes, null);
                if (propInfo == null)
                {
                    return null;
                }
