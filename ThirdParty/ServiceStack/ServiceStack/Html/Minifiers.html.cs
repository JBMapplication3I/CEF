...
namespace ServiceStack.Html
{
    ...
    public sealed class HtmlCompressor : ICompressor
    {
        ...
        public static readonly Regex PHP_TAG_PATTERN = new("<\\?php.*?\\?>",
         ...
         * Could be passed inside a list to {@link #setPreservePatterns(List) setPreservePatterns} method.
         */
        public static readonly Regex SERVER_SCRIPT_TAG_PATTERN = new("<%.*?%>", RegexOptions.Singleline);

        /**
         * Predefined pattern that matches <code>&lt;--# ... --></code> tags. 
         * Could be passed inside a list to {@link #setPreservePatterns(List) setPreservePatterns} method.
         */
        public static readonly Regex SERVER_SIDE_INCLUDE_PATTERN = new("<!--\\s*#.*?-->", RegexOptions.Singleline);
        ...
        private static readonly Regex emptyPattern = new("\\s");

        private static readonly Regex skipPattern = new("<!--\\s*\\{\\{\\{\\s*-->(.*?)<!--\\s*\\}\\}\\}\\s*-->",
                                                              RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex condCommentPattern = new("(<!(?:--)?\\[[^\\]]+?]>)(.*?)(<!\\[[^\\]]+]-->)",
                                                                     RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex commentPattern = new("<!---->|<!--[^\\[].*?-->",
                                                                 RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex intertagPattern_TagTag = new(">\\s+<",
                                                                         RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex intertagPattern_TagCustom = new(">\\s+%%%~",
                                                                            RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex intertagPattern_CustomTag = new("~%%%\\s+<",
                                                                            RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex intertagPattern_CustomCustom = new("~%%%\\s+%%%~",
                                                                               RegexOptions.Singleline |
                                                                               RegexOptions.IgnoreCase);

        private static readonly Regex multispacePattern = new("\\s+", RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex tagEndSpacePattern = new("(<(?:[^>]+?))(?:\\s+?)(/?>)",
                                                                     RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex tagLastUnquotedValuePattern = new("=\\s*[a-z0-9-_]+$", RegexOptions.IgnoreCase);

        private static readonly Regex tagQuotePattern = new("\\s*=\\s*([\"'])([a-z0-9-_]+?)\\1(/?)(?=[^<]*?>)",
                                                                  RegexOptions.IgnoreCase);

        private static readonly Regex prePattern = new("(<pre[^>]*?>)(.*?)(</pre>)",
                                                             RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex taPattern = new("(<textarea[^>]*?>)(.*?)(</textarea>)",
                                                            RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex scriptPattern = new("(<script[^>]*?>)(.*?)(</script>)",
                                                                RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex stylePattern = new("(<style[^>]*?>)(.*?)(</style>)",
                                                               RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex tagPropertyPattern = new("(\\s\\w+)\\s*=\\s*(?=[^<]*?>)", RegexOptions.IgnoreCase);

        private static readonly Regex cdataPattern = new("\\s*<!\\[CDATA\\[(.*?)\\]\\]>\\s*",
                                                               RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex doctypePattern = new("<!DOCTYPE[^>]*>",
                                                                 RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex typeAttrPattern = new("type\\s*=\\s*([\\\"']*)(.+?)\\1",
                                                                  RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex jsTypeAttrPattern =
            new("(<script[^>]*)type\\s*=\\s*([\"']*)(?:text|application)/javascript\\2([^>]*>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex jsLangAttrPattern =
            new("(<script[^>]*)language\\s*=\\s*([\"']*)javascript\\2([^>]*>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex styleTypeAttrPattern =
            new("(<style[^>]*)type\\s*=\\s*([\"']*)text/style\\2([^>]*>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex linkTypeAttrPattern =
            new("(<link[^>]*)type\\s*=\\s*([\"']*)text/(?:css|plain)\\2([^>]*>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex linkRelAttrPattern =
            new("<link(?:[^>]*)rel\\s*=\\s*([\"']*)(?:alternate\\s+)?stylesheet\\1(?:[^>]*)>",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex formMethodAttrPattern = new("(<form[^>]*)method\\s*=\\s*([\"']*)get\\2([^>]*>)",
                                                                        RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex inputTypeAttrPattern = new("(<input[^>]*)type\\s*=\\s*([\"']*)text\\2([^>]*>)",
                                                                       RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex booleanAttrPattern =
            new("(<\\w+[^>]*)(checked|selected|disabled|readonly)\\s*=\\s*([\"']*)\\w*\\3([^>]*>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex eventJsProtocolPattern = new("^javascript:\\s*(.+)",
                                                                         RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex httpProtocolPattern =
            new("(<[^>]+?(?:href|src|cite|action)\\s*=\\s*['\"])http:(//[^>]+?>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex httpsProtocolPattern =
            new("(<[^>]+?(?:href|src|cite|action)\\s*=\\s*['\"])https:(//[^>]+?>)",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex relExternalPattern =
            new("<(?:[^>]*)rel\\s*=\\s*([\"']*)(?:alternate\\s+)?external\\1(?:[^>]*)>",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);
        ...
        private static readonly Regex lineBreakPattern = new("(?:[ \t]*(\\r?\\n)[ \t]*)+");

        private static readonly Regex surroundingSpacesMinPattern =
            new("\\s*(</?(?:" + BLOCK_TAGS_MIN.Replace(",", "|") + ")(?:>|[\\s/][^>]*>))\\s*",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex surroundingSpacesMaxPattern =
            new("\\s*(</?(?:" + BLOCK_TAGS_MAX.Replace(",", "|") + ")(?:>|[\\s/][^>]*>))\\s*",
                      RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex surroundingSpacesAllPattern = new("\\s*(<[^>]+>)\\s*",
                                                                              RegexOptions.Singleline |
                                                                              RegexOptions.IgnoreCase);

        //patterns for searching for temporary replacements
        private static readonly Regex tempCondCommentPattern = new("%%%~COMPRESS~COND~(\\d+?)~%%%");
        private static readonly Regex tempPrePattern = new("%%%~COMPRESS~PRE~(\\d+?)~%%%");
        private static readonly Regex tempTextAreaPattern = new("%%%~COMPRESS~TEXTAREA~(\\d+?)~%%%");
        private static readonly Regex tempScriptPattern = new("%%%~COMPRESS~SCRIPT~(\\d+?)~%%%");
        private static readonly Regex tempStylePattern = new("%%%~COMPRESS~STYLE~(\\d+?)~%%%");
        private static readonly Regex tempEventPattern = new("%%%~COMPRESS~EVENT~(\\d+?)~%%%");
        private static readonly Regex tempSkipPattern = new("%%%~COMPRESS~SKIP~(\\d+?)~%%%");
        private static readonly Regex tempLineBreakPattern = new("%%%~COMPRESS~LT~(\\d+?)~%%%");
        ...
        private string removeSurroundingSpaces(string html)
        {
            ...
            if (RemoveSurroundingSpaces != null)
            {
                ...
                {
                    pattern = new(string.Format("\\s*(</?(?:{0})(?:>|[\\s/][^>]*>))\\s*",
    ...
    public sealed class HtmlCompressorStatistics
    {
         ...
         * @see HtmlMetrics
         */
        public HtmlMetrics OriginalMetrics = new();
         ...
         * @see HtmlMetrics
         */
        public HtmlMetrics CompressedMetrics = new();
