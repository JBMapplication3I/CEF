...
namespace MarkdownDeep
{
    ...
    public class Markdown
    {
        ...
        public Markdown()
        {
            ...
            m_StringBuilder = new();
            m_StringScanner = new();
            m_SpanFormatter = new(this);
            m_LinkDefinitions = new(StringComparer.CurrentCultureIgnoreCase);
            m_Footnotes = new();
            m_UsedFootnotes = new();
            m_UsedHeaderIDs = new();
        }
        ...
        {
            return Transform(str, out var defs);
        }
            ...
            else
            {
                ...
                if (m_UsedFootnotes.Count > 0)
                {
                    ...
                    for (var i = 0; i < m_UsedFootnotes.Count; i++)
                    {
                        ...
                        // in the footnote
                        var strReturnLink = string.Format("<a href=\"#fnref:{0}\" rev=\"footnote\">&#8617;</a>", (string)fn.data);
        ...
        {
            if (m_Footnotes.TryGetValue(id, out var footnote))
            {
        ...
        {
            if (m_LinkDefinitions.TryGetValue(id, out var link))
            {
                return link;
            }
            else
        ...
        internal void AddAbbreviation(string abbr, string title)
        {
            ...
            // Store abbreviation
            m_AbbreviationMap.Add(abbr, new(abbr, title));
        ...
        private Stack<Block> m_SpareBlocks = new();
