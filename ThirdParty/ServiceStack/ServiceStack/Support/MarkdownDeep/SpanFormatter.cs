    ...
    internal class SpanFormatter : StringScanner
    {
        ...
        public Token CreateEmphasisMark()
        {
            ...
            if (bof || char.IsWhiteSpace(CharAtOffset(-1)))
            {
                ...
                {
                    return new(TokenType.Html, savepos, position - savepos);
                }
            ...
            while (!eof)
            {
                ...
                if (ch == '>')
                {
                    ...
                    if (Utils.IsEmailAddress(url))
                    {
                        ...
                        li = new(new("auto", url, null), link_text);
                    }
                    else if (Utils.IsWebAddress(url))
                    {
                        li = new(new("auto", url, null), url);
                    }
        ...
        internal Token CreateToken(TokenType type, int startOffset, int length)
        {
            ...
            {
                return new(type, startOffset, length);
            }
        }
        ...
        private Stack<Token> m_SpareTokens = new();

        #endregion

        private Markdown m_Markdown;
        internal bool DisableLinks;
        private List<Token> m_Tokens = new();
    }
}
