...
namespace ServiceStack.Support.Markdown
{
    public static class TemplateExtensions
    {
        ...
        private static readonly char[] LineEndChars = new[] { '\r', '\n' };
        private static readonly char[] WhiteSpaceChars = new[] { ' ', '\t', '\r', '\n' };

        private static readonly char[] WhiteSpaceAndSymbolChars = new[] {
            ' ', '\t', '\r', '\n', '(', ')', '!', '+', '-'
        };

        private static readonly bool[] AlphaNumericFlags = new bool[128];
        private static readonly bool[] MemberExprFlags = new bool[128];
        private static readonly bool[] StatementFlags = new bool[128];
        private static readonly bool[] WhiteSpaceFlags = new bool[128];
        ...
        public static void SkipIfNextIs(this string content, ref int pos, string text)
        {
            ...
            if (content.Length < pos + text.Length)
            {
                return;
            }
        ...
        public static List<TemplateBlock> CreateTemplateBlocks(this string content, List<StatementExprBlock> statementBlocks)
        {
            ...
            while ((pos = content.IndexOf('@', lastPos)) != -1)
            {
                ...
                else if (content[pos] == StatementPlaceholderChar)
                {
                    ...
                    if (int.TryParse(index, out var statementNo))
                    {
        ...
        public static string GetNextAlphaNumericExpr(this string content, ref int fromPos)
        {
            ...
            for (; fromPos < content.Length; fromPos++)
            {
                ...
                if (exprChar >= MemberExprFlags.Length)
                {
                    return null;
                }

                if (!MemberExprFlags[exprChar])
                {
                    break;
                }
            }
        ...
        public static string GetVarName(this string memberExpr)
        {
            ...
            for (var i = 0; i < memberExpr.Length; i++)
            {
                ...
                if (exprChar >= AlphaNumericFlags.Length)
                {
                    return null;
                }

                if (!AlphaNumericFlags[exprChar])
                {
                    return memberExpr.Substring(0, i);
                }
            }
