...
<wsdl:definitions name=""{0}"" 
	...
	{9}
        ...
        public override string ToString()
        {
            ...
            var replyMessages = RepeaterTemplate(ReplyMessagesTemplate, ReplyOperationNames);
            var replyOperations = RepeaterTemplate(ReplyOperationsTemplate, ReplyOperationNames);
            ...
            var replyActions = RepeaterTemplate(ReplyActionsTemplate, wsdlSoapActionNamespace, ReplyOperationNames);
            var replyBindings = string.Format(ReplyBindingContainerTemplate, replyActions, replyServiceName);
            var replyEndpointUri = string.Format(ReplyEndpointUriTemplate, ServiceName, ReplyEndpointUri, replyServiceName);
            ...
            {
                oneWayMessages = RepeaterTemplate(OneWayMessagesTemplate, OneWayOperationNames);
                oneWayOperations = RepeaterTemplate(OneWayOperationsTemplate, OneWayOperationNames);
                ...
                var oneWayActions = RepeaterTemplate(OneWayActionsTemplate, wsdlSoapActionNamespace, OneWayOperationNames);
                oneWayBindings = string.Format(OneWayBindingContainerTemplate, oneWayActions, oneWayServiceName);
                oneWayEndpointUri = string.Format(OneWayEndpointUriTemplate, ServiceName, OneWayEndpointUri, oneWayServiceName);
            }
