...
namespace ServiceStack.NativeTypes
{
    ...
    public class NativeTypesService : Service
    {
        ...
        public object Any(TypesFSharp request)
        {
            ...
            var typesConfig = NativeTypesMetadata.GetConfig(request);
            var metadataTypes = NativeTypesMetadata.GetMetadataTypes(Request, typesConfig);
            var fsharp = new FSharpGenerator(typesConfig).GetCode(metadataTypes, Request);
            return fsharp;
        }
        ...
        public object Any(TypesVbNet request)
        {
            ...
            var typesConfig = NativeTypesMetadata.GetConfig(request);
            var metadataTypes = NativeTypesMetadata.GetMetadataTypes(Request, typesConfig);
            var vbnet = new VbNetGenerator(typesConfig).GetCode(metadataTypes, Request);
            return vbnet;
        }
        ...
        public string GenerateTypeScript(NativeTypesBase request, MetadataTypesConfig typesConfig)
        {
            ...
            {
                typesConfig.IgnoreTypesInNamespaces = new();
            }
            ...
            var typeScript = new TypeScriptGenerator(typesConfig).GetCode(metadataTypes, Request, NativeTypesMetadata);
            return typeScript;
        }
        ...
        public object Any(TypesSwift request)
        {
            ...
            try
            {
                var swift = new SwiftGenerator(typesConfig).GetCode(metadataTypes, Request);
                return swift;
            }
            catch (Exception)
            {
        ...
        public object Any(TypesJava request)
        {
            ...
            var java = new JavaGenerator(typesConfig).GetCode(metadataTypes, Request, NativeTypesMetadata);
            return java;
        }
        ...
        public object Any(TypesKotlin request)
        {
            ...
            var java = new KotlinGenerator(typesConfig).GetCode(metadataTypes, Request, NativeTypesMetadata);
            return java;
        }

        private static void ExportMissingSystemTypes(MetadataTypesConfig typesConfig)
        {
            typesConfig.ExportTypes ??= new();
