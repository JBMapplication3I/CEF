...
namespace ServiceStack.Host
{
    public class ContentTypes : IContentTypes
    {
        ...
        public ContentTypes()
        {
            ContentTypeFormats = new();
        }

        public void ClearCustomFilters()
        {
            ContentTypeFormats = new();
            ContentTypeSerializers = new();
            ContentTypeDeserializers = new();
        }
        ...
        public byte[] SerializeToBytes(IRequest req, object response)
        {
            ...
            if (ContentTypeSerializers.TryGetValue(contentType, out var responseStreamWriter) ||
                ContentTypeSerializers.TryGetValue(ContentFormat.GetRealContentType(contentType), out responseStreamWriter))
            {
            ...
            if (ContentTypeResponseSerializers.TryGetValue(contentType, out var responseWriter) ||
                ContentTypeResponseSerializers.TryGetValue(ContentFormat.GetRealContentType(contentType), out responseWriter))
            {
                using (var ms = MemoryStreamFactory.GetStream())
                {
        ...
        {
            if (ContentTypeSerializers.TryGetValue(contentType, out var responseWriter) ||
                ContentTypeSerializers.TryGetValue(ContentFormat.GetRealContentType(contentType), out responseWriter))
            {
                return responseWriter;
            }
        ...
        public StreamDeserializerDelegate GetStreamDeserializer(string contentType)
        {
            ...
            if (ContentTypeDeserializers.TryGetValue(realContentType, out var streamReader))
            {
                return streamReader;
            }
