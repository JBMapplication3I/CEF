namespace ServiceStack.Host
{
    ...
    public class RestPath : IRestPath
    {
        ...
        // in most cases URL parts are short-lengthy and we can create lookup for
        ...
        /// <summary>The number of segments separated by '/' determinable by path.Split('/').Length
        /// e.g. /path/to/here.ext == 3.</summary>
        /// <value>The number of path components.</value>
        public int PathComponentsCount { get; set; }

        /// <summary>The total number of segments after subparts have been exploded ('.')
        /// e.g. /path/to/here.ext == 4.</summary>
        /// <value>The total number of components count.</value>
        ...
        {
            return new(RequestType, Path, AllowedVerbs, 0);
        }
        ...
        public RestPath(Type requestType, string path, string verbs, string summary = null, string notes = null)
        {
            ...
            {
                if (component.Contains(VariablePrefix) && component.Contains(ComponentSeparator))
                {
            ...
            typeDeserializer = new(RequestType);
        ...
        private void RegisterCaseInsensitivePropertyNameMappings()
        {
            ...
            catch (Exception)
            {
                throw new AmbiguousMatchException(
                    "Property names are case-insensitive: " + RequestType.GetOperationName() + "." + propertyName);
            }
        }

        public bool IsValid { get; set; }

        /// <summary>Provide for quick lookups based on hashes that can be determined from a request url.</summary>
        /// <value>The first match hash key.</value>
        ...
        private readonly Dictionary<string, string> propertyNamesMap = new();
        ...
        /// <summary>For performance withPathInfoParts should already be a lower case string to minimize redundant
        /// matching operations.</summary>
        /// <param name="httpMethod">       The HTTP method.</param>
        /// <param name="withPathInfoParts">The with path information parts.</param>
        /// <returns>True if match, false if not.</returns>
        public bool IsMatch(string httpMethod, string[] withPathInfoParts)
        {
            return IsMatch(httpMethod, withPathInfoParts, out _);
        }
        ...
        /// <param name="httpMethod">        The HTTP method.</param>
        /// <param name="withPathInfoParts"> The with path information parts.</param>
        /// <param name="wildcardMatchCount">Number of wildcard matches.</param>
        /// <returns>True if match, false if not.</returns>
        public bool IsMatch(string httpMethod, string[] withPathInfoParts, out int wildcardMatchCount)
        {
            ...
            if (!AllowsAllVerbs && !AllowedVerbs.Contains(httpMethod.ToUpper()))
            {
                return false;
            }
        ...
        private bool ExplodeComponents(ref string[] withPathInfoParts)
        {
            ...
            for (var i = 0; i < withPathInfoParts.Length; i++)
            {
                ...
                if (PathComponentsCount == TotalComponentsCount || !componentsWithSeparators[i])
                {
        ...
        public object CreateRequest(string pathInfo, Dictionary<string, string> queryStringAndFormData, object fromInstance)
        {
            ...
            {
                var isValidWildCardPath = IsWildCardPath && requestComponents.Length >= TotalComponentsCount - wildcardCount;
                if (!isValidWildCardPath)
                {
                    throw new ArgumentException(
                        $"Path Mismatch: Request Path '{pathInfo}' has invalid number of components compared to: '{Path}'");
                }
            }
            ...
            for (var i = 0; i < TotalComponentsCount; i++)
            {
                ...
                if (!propertyNamesMap.TryGetValue(variableName.ToLower(), out var propertyNameOnRequest))
                {
                    if (!Keywords.Ignore.EqualsIgnoreCase(variableName))
                    {
                        throw new ArgumentException(
                            "Could not find property " + variableName + " on " + RequestType.GetOperationName());
                    }
                    pathIx++;
                    continue;
                }
            ...
            return typeDeserializer.PopulateFromMap(
                fromInstance,
                requestKeyValuesMap,
