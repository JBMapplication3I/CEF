...
namespace ServiceStack
{
    public static class HttpResponseExtensionsInternal
    {
        ...
        public static bool WriteToOutputStream(IResponse response, object result, byte[] bodyPrefix, byte[] bodySuffix)
        {
            ...
            if (HostContext.Config.AllowPartialResponses && partialResult?.IsPartialRequest == true)
            {
        ...
        /// <returns></returns>
        public static async Task<bool> WriteToResponse(this IResponse response, object result, ResponseSerializerDelegate defaultAction, IRequest request, byte[] bodyPrefix, byte[] bodySuffix, CancellationToken token = default(CancellationToken))
        {
                    ...
                    using (resultScope)
                    {
                        using (HostContext.Config.AllowJsConfig ? JsConfig.CreateScope(request.QueryString[Keywords.JsConfig]) : null)
                        {
                            ...
                            if (result is string responseText)
                            {
                                ...
                                var len = (bodyPrefix?.Length).GetValueOrDefault() + strBytes.Length + (bodySuffix?.Length).GetValueOrDefault();
                                response.SetContentLength(len);
                                if (response.ContentType == null || response.ContentType == MimeTypes.Html)
                                {
                                    response.ContentType = defaultContentType;
        ...
        internal static Task<bool> HandleResponseWriteException(this Exception originalEx, IRequest request, IResponse response, string defaultContentType)
        {
            ...
            var errorMessage = $"Error occurred while Processing Request: [{originalEx.GetType().GetOperationName()}] {originalEx.Message}";
            try
            {
        ...
        private static ErrorResponse ToErrorResponse(this Exception ex)
        {
            ...
            if (HostContext.DebugMode)
            {
                ...
                if (compileEx?.Results.Errors.HasErrors == true)
                {
                    errors = new();
                    foreach (var err in compileEx.Results.Errors)
                    {
                        errors.Add(new() { Message = err.ToString() });
                    }
                }
#endif
            }
        ...
        {
            if (!ShouldWriteGlobalHeaders(httpRes))
            {
                return;
            }
