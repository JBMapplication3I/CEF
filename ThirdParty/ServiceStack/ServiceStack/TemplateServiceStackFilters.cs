    ...
    public class TemplateServiceStackFilters : TemplateFilter
    {
        ...
        public object sendToAutoQuery(TemplateScopeContext scope, object dto, string requestName, object options)
        {
            try
            {
                ...
                {
                    if (Context.TemplateFilters.FirstOrDefault(x => x is IAutoQueryDbFilters) is not IAutoQueryDbFilters ssFillter)
                    {
                ...
                if (requestDto is not IQueryData aqDto)
                {
        ...
        [HandleUnknownValue]
        public object ifAuthenticated(TemplateScopeContext scope) => isAuthenticated(scope)
            ? (object)IgnoreResult.Value : StopExecution.Value;

        [HandleUnknownValue]
        public object ifNotAuthenticated(TemplateScopeContext scope) => !isAuthenticated(scope)
            ? (object)IgnoreResult.Value : StopExecution.Value;

        [HandleUnknownValue]
        public object onlyIfAuthenticated(TemplateScopeContext scope, object value) => isAuthenticated(scope)
            ? value : StopExecution.Value;

        [HandleUnknownValue]
        public object endIfAuthenticated(TemplateScopeContext scope, object value) => !isAuthenticated(scope)
            ? value : StopExecution.Value;
    }
}