...
namespace ServiceStack.Formats
{
    ...
    public class MarkdownFormat : IViewEngine, IPlugin
    {
        ...
        public static MarkdownFormat Instance => instance ??= new();

        // ~/View - Dynamic Pages
        public Dictionary<string, MarkdownPage> ViewPages = new(
            StringComparer.CurrentCultureIgnoreCase);

        // ~/View/Shared - Dynamic Shared Pages
        public Dictionary<string, MarkdownPage> ViewSharedPages = new(
            StringComparer.CurrentCultureIgnoreCase);

        //Content Pages outside of ~/View
        public Dictionary<string, MarkdownPage> ContentPages = new(
            StringComparer.CurrentCultureIgnoreCase);

        public Dictionary<string, MarkdownTemplate> MasterPageTemplates = new(
            StringComparer.CurrentCultureIgnoreCase);
        ...
        private readonly TemplateProvider templateProvider = new(DefaultTemplateName);

        public MarkdownFormat()
        {
            markdown = new(); //Note: by default MarkdownDeep is used

            MarkdownBaseType = typeof(MarkdownViewBase);
            MarkdownGlobalHelpers = new();
            FindMarkdownPagesFn = FindMarkdownPages;
            ReplaceTokens = new();
        }

        internal static readonly char[] DirSeps = new[] { '\\', '/' };
        private static HashSet<string> catchAllPathsNotFound = new();
        ...
        public void Register(IAppHost appHost)
        {
            ...
            MarkdownBaseType = appHost.Config.MarkdownBaseType ?? MarkdownBaseType;
            MarkdownGlobalHelpers = appHost.Config.MarkdownGlobalHelpers ?? MarkdownGlobalHelpers;

            ReplaceTokens = appHost.Config.HtmlReplaceTokens ?? new Dictionary<string, string>();
            var webHostUrl = appHost.Config.WebHostUrl;
            if (!webHostUrl.IsNullOrEmpty())
            {
                ReplaceTokens["~/"] = webHostUrl.WithTrailingSlash();
            }
        ...
        {
            if (markdownPage == null || !CheckLastModifiedForChanges)
            {
                return;
            }
            ...
            if (markdownPage.DirectiveTemplate != null
                && MasterPageTemplates.TryGetValue(markdownPage.DirectiveTemplate, out var template))
            {
            ...
            if (markdownPage.Template != null
                && MasterPageTemplates.TryGetValue(markdownPage.Template, out template))
            {
        ...
        public MarkdownPage GetViewPage(string pageName)
        {
            ...
            ViewPages.TryGetValue(pageName, out var markdownPage);
            if (markdownPage != null)
            {
                return markdownPage;
            }
        ...
        {
            ContentPages.TryGetValue(pageFilePath, out var markdownPage);
        ...
        public IEnumerable<MarkdownPage> FindMarkdownPages(string dirPath)
        {
            ...
            foreach (var markDownFile in markDownFiles)
            {
                ...
                yield return new(this, markDownFile.VirtualPath,
        ...
        {
            if (MasterPageTemplates.TryGetValue(templatePath, out var template))
            {
                return template;
            }
            ...
            template = new(templatePath, templateName, templateContents)
            {
        ...
        public string RenderStaticPage(string filePath, bool renderHtml)
        {
            ...
            if (!ContentPages.TryGetValue(filePath, out var markdownPage))
            {
        ...
        private string RenderStaticPage(MarkdownPage markdownPage, bool renderHtml)
        {
            ...
            return RenderDynamicPage(markdownPage, new(), renderHtml, true);
        }
        ...
        private string RenderInTemplateIfAny(MarkdownPage markdownPage, Dictionary<string, object> scopeArgs, string pageHtml, string templatePath = null)
        {
            ...
            if (markdownTemplate == null)
            {
                ...
                if (markdownTemplate == null)
                {
                    ...
                    throw new("No template found for page: " + markdownPage.FilePath);
                }
            }
        ...
        {
            return RenderDynamicPage(GetViewPage(pageName), new(), true, true);
        }
