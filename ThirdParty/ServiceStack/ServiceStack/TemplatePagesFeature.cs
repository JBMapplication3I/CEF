...
namespace ServiceStack
{
    public class TemplatePagesFeature : TemplateContext, IPlugin
    {
        ...
        public bool ExcludeProtectedFilters
        {
            set
            {
                if (value)
                {
                    TemplateFilters.RemoveAll(x => x is TemplateProtectedFilters);
                }
            }
        }
    ...
    public class TemplateHotReloadService : Service
    {
        ...
        public async Task<HotReloadPageResponse> Any(HotReloadPage request)
        {
            ...
            {
                return new() { ETag = lastModified.Ticks.ToString() };
            }

            var shouldReload = lastModified.Ticks > long.Parse(request.ETag);
            return new() { Reload = shouldReload, ETag = lastModified.Ticks.ToString() };
        }
    }
    ...
    public class TemplateApiPagesService : Service
    {
        public async Task<object> Any(ApiPages request)
        {
            ...
            Request.OperationName = pageName + (pathArgs.Length > 0 ? "_" + string.Join("_", pathArgs) : "");
            ...
            if (feature.ApiDefaultContentType != null &&
                !hasPathContentType &&
                !hasPageContentType &&
                Request.QueryString["format"] == null && Request.ResponseContentType == MimeTypes.Html)
            {
                Request.ResponseContentType = feature.ApiDefaultContentType;
            }

            var pagePath = feature.ApiPath.CombineWith(pageName).TrimStart('/');
            var page = Request.GetPage(pagePath);
            if (page == null)
            {
                throw HttpError.NotFound($"No API Page was found at '{pagePath}'");
            }

            var requestArgs = Request.GetTemplateRequestParams();
...
<td><pre>{{ networkIpv4Addresses | select: \n{ it } }}</pre></td><td><pre>{{ networkIpv6Addresses | select: \n{ it } }}</pre><td></tr></pre></td>
    ...
    public class TemplatePageHandler : HttpAsyncTaskHandler
    {
        ...
        {
            RequestName = !string.IsNullOrEmpty(page.VirtualPath) ? page.VirtualPath : nameof(TemplatePageHandler);
            this.page = page;
    ...
    public class MarkdownPageFormat : PageFormat
    {
        ...
        private static readonly MarkdownSharp.Markdown markdown = new();
...
authRepository ??= HostContext.AppHost.GetAuthRepository(Request);
        ...
        public virtual bool IsAuthenticated => GetSession().IsAuthenticated;
