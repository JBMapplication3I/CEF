...
namespace ServiceStack
{
    public class WebSudoFeature : IPlugin, IAuthEvents
    {
        ...
        {
            WebSudoDuration = TimeSpan.FromMinutes(20);
        }
        ...
        {
            if (AuthenticateService.CurrentSessionFactory() is not IWebSudoAuthSession s)
            {
        ...
        {
            if (!request.Items.ContainsKey(SessionCopyRequestItemKey))
            {
                return;
            }

            if (request.Items[SessionCopyRequestItemKey] is not IWebSudoAuthSession copy)
            {
                return;
            }
        ...
        {
            if (!httpReq.Items.ContainsKey(SessionCopyRequestItemKey))
            {
                return;
            }

            if (httpReq.Items[SessionCopyRequestItemKey] is not IWebSudoAuthSession copy)
            {
                return;
            }
        ...
        {
            if (session is not IWebSudoAuthSession webSudoSession)
            {
                return;
            }
            ...
            {
                webSudoSession.AuthenticatedWebSudoUntil = webSudoSession.AuthenticatedAt.Add(WebSudoDuration);
            }
        }
