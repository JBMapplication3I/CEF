...
namespace ServiceStack
{
    ...
    public class ServerEventsHandler : HttpAsyncTaskHandler
    {
        ...
        public override Task ProcessRequestAsync(IRequest req, IResponse res, string operationName)
        {
            ...
            var subscription = new EventSubscription(res)
            {
                ...
                Meta = {
                    { "userId", userId },
                    { "isAuthenticated", session?.IsAuthenticated == true ? "true": "false" },
                    { "displayName", displayName },
            ...
            subscription.ConnectArgs = new(subscription.Meta);
            ...
            subscription.ConnectArgs = new(subscription.ConnectArgs) {
                {"id", subscriptionId },
    ...
    public class ServerEventsUnRegisterService : Service
    {
        ...
        public object Any(UnRegisterEventSubscriber request)
        {
            ...
            var feature = HostContext.GetPlugin<ServerEventsFeature>();
            if (!feature.CanAccessSubscription(Request, subscription))
            {
    ...
    */
    ...
    public class EventSubscription : SubscriptionInfo, IEventSubscription
    {
        ...
        {
            get => new(Interlocked.Read(ref LastPulseAtTicks), DateTimeKind.Utc);
        ...
        public IRequest Request => response.Request;
        ...
        public EventSubscription(IResponse response)
        {
            ...
            Meta = new();
            WriteEvent = HostContext.GetPlugin<ServerEventsFeature>().WriteEvent;
        }

        public void UpdateChannels(string[] channels)
        {
            Channels = channels;
            Meta["channels"] = string.Join(",", channels);
        }
        ...
        public Action<IResponse, string> WriteEvent { get; set; }
        public Action<IEventSubscription, Exception> OnError { get; set; }
        public bool IsClosed => response.IsClosed;
    ...
    public class MemoryServerEvents : IServerEvents
    {
        ...
        {
            Subcriptions = new();
            ChannelSubcriptions = new();
            UserIdSubcriptions = new();
            UserNameSubcriptions = new();
            SessionSubcriptions = new();
        }
        ...
        {
            get => new(Interlocked.Read(ref lastCleanAtTicks), DateTimeKind.Utc);
        ...
        private void RegisterSubscription(IEventSubscription subscription, string key,
            ...
            ConcurrentDictionary<string, ConcurrentDictionary<IEventSubscription, bool>> map)
        {
            ...
            var subs = map.GetOrAdd(key, k => new());
        ...
        private void UnRegisterSubscription(IEventSubscription subscription, string key,
            ConcurrentDictionary<string, ConcurrentDictionary<IEventSubscription, bool>> map)
        {
            if (key == null || subscription == null)
            {
            ...
            try
            {
                ...
                subs.TryRemove(subscription, out var flag);
            }
            catch (Exception ex)
            {
            ...
            ConcurrentDictionary<string, IEventSubscription> map)
        {
            ...
            map.TryRemove(key, out var inMap);
        }
