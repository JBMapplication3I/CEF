...
namespace ServiceStack
{
        ...
        : IHttpResult, IStreamWriterAsync, IPartialWriterAsync, IDisposable
    {
        ...
        public HttpResult(object response, string contentType, HttpStatusCode statusCode)
        {
            Headers = new();
            Cookies = new();
            ResponseFilter = ContentTypes.Instance;
        ...
        {
            FileInfo = fileResponse ?? throw new ArgumentNullException(nameof(fileResponse));
            LastModified = fileResponse.LastWriteTime;
            AllowsPartialResponse = true;
            if (!FileInfo.Exists)
            {
            ...
            : this(null, contentType ?? MimeTypes.GetMimeType(fileResponse.Name), HttpStatusCode.OK)
        {
            ...
            ResponseStream = fileResponse.OpenRead();
        ...
        {
            AllowsPartialResponse = true;
            ResponseStream = MemoryStreamFactory.GetStream(responseBytes);
        }
        ...
        public bool AllowsPartialResponse
        {
            set
            {
                ...
                {
                    Headers.Add(HttpHeaders.AcceptRanges, "bytes");
                }
                else
                {
                    Headers.Remove(HttpHeaders.AcceptRanges);
                }
            }
        ...
        public string Location
        {
            set
            {
                ...
                Headers[HttpHeaders.Location] = value;
            }
        }
        ...
        {
            path = path ?? "/";
            Headers[HttpHeaders.SetCookie] = $"{name}={value};path={path}";
        }
        ...
        public void SetCookie(string name, string value, DateTime expiresAt, string path, bool secure = false, bool httpOnly = false)
        {
            ...
            Headers[HttpHeaders.SetCookie] = cookie;
        }
        ...
        private async Task WriteToAsync(Stream responseStream, int paddingLength, CancellationToken token)
        {
            ...
            if (Response is byte[] bytesResponse)
            {
            ...
            ResponseFilter.SerializeToStream(RequestContext, Response, responseStream);
        }
    ...
    public static class HttpResultExtensions
    {
        ...
        {
            var to = new Cookie(httpCookie.Name, httpCookie.Value, httpCookie.Path)
            {
