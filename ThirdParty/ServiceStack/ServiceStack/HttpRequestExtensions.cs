...
namespace ServiceStack
{
    ...
    public static class HttpRequestExtensions
    {
        ...
        {
            string value;
            if ((value = httpReq.Headers[HttpHeaders.XParamOverridePrefix + name]) != null)
            {
                return value;
            }
            if ((value = httpReq.QueryString[name]) != null)
            {
                return value;
            }
            if ((value = httpReq.FormData[name]) != null)
            {
                return value;
            }
            ...
            if (httpReq.Items.TryGetValue(name, out var oValue))
            {
        ...
        {
            return httpReq is IHasVirtualFiles vfsReq
                ? vfsReq.IsFile
        ...
        public static string GetHttpMethodOverride(this IRequest httpReq)
        {
            ...
            if (httpMethod is not HttpMethods.Post)
            {
                return httpMethod;
            }
            var overrideHttpMethod = httpReq.Headers[HttpHeaders.XHttpMethodOverride].ToNullIfEmpty()
            ...
            return overrideHttpMethod is not null and not HttpMethods.Get and not HttpMethods.Post
                ? overrideHttpMethod
                : httpMethod;
        }
        ...
        public static string GetFormatModifier(this IRequest httpReq)
        {
            ...
            return pos >= 0 ? format.Substring(pos.Value + 1) : null;
        }
        ...
        public static int ToStatusCode(this Exception ex)
        {
            ...
            if (HostContext.AppHost != null && HostContext.Config != null)
            {
                ...
                foreach (var entry in HostContext.Config.MapExceptionToStatusCode.Where(entry => entry.Key.IsAssignableFromType(exType)))
                {
                    return entry.Value;
                }
            }
            return ex switch
            {
                HttpError httpEx => httpEx.Status,
                NotImplementedException => (int)HttpStatusCode.MethodNotAllowed,
                NotSupportedException => (int)HttpStatusCode.MethodNotAllowed,
                FileNotFoundException => (int)HttpStatusCode.NotFound,
                ArgumentException => (int)HttpStatusCode.BadRequest,
                SerializationException => (int)HttpStatusCode.BadRequest,
        ...
        private static readonly string[] VirtualPathPrefixes = System.Web.Hosting.HostingEnvironment.ApplicationVirtualPath is null or "/"
        ...
        public static string GetPathInfo(string fullPath, string mode, string appPath)
        {
            ...
            return IsNullOrEmpty(pathInfo) ? fullPath : pathInfo;
        }
        ...
        public static string ResolvePathInfoFromMappedPath(string fullPath, string mappedPathRoot)
        {
            ...
            for (var fullPathIndex = 0; fullPathIndex < fullPathParts.Length; fullPathIndex++)
            {
                ...
                {
                    pathRootFound = !mappedPathRootParts
                        .Where((t, mappedPathRootIndex) => !string.Equals(
                            fullPathParts[fullPathIndex - fullPathIndexOffset + mappedPathRootIndex],
                            t,
        ...
        /// <summary>Duplicate Params are given a unique key by appending a #1 suffix.</summary>
        ...
        public static Dictionary<string, string> GetRequestParams(this IRequest request)
        {
            ...
            if (request.Verb is not HttpMethods.Post and not HttpMethods.Put || request.FormData == null)
            {
                return map;
            }
        ...
        public static Dictionary<string, string> GetFlattenedRequestParams(this IRequest request)
        {
            ...
            if (request.Verb is not (HttpMethods.Post or HttpMethods.Put) || request.FormData == null)
            {
                return map;
            }
            foreach (var name in request.FormData.AllKeys)
        ...
        public static string GetQueryStringContentType(this IRequest httpReq)
        {
            ...
            if (format == null)
            {
                ...
                if (end - start == 3)
                {
                    if (Compare(pi, start, "xml", 0, "xml".Length, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        return MimeTypes.Xml;
                    }
                    if (Compare(pi, start, "jsv", 0, "jsv".Length, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                ...
                {
                    if (Compare(pi, start, "json", 0, "json".Length, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        return MimeTypes.Json;
                    }
                }
        ...
        /// <summary>Use this to treat Request.Items[] as a cache by returning pre-computed items to save calculating
        /// them multiple times.</summary>
        ...
        {
            if (HostContext.DebugMode && request.QueryString != null) //Mock<IHttpRequest>
            {
        ...
        public static bool IsMultiRequest(this IRequest req)
        {
            ...
            return req.Dto?.GetType().IsArray == true;
        }
