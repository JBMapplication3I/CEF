namespace ServiceStack
{
    ...
    public class HttpHandlerFactory : IHttpHandlerFactory
    {
        ...
        internal static void Init()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(config.DefaultRedirectPath))
                {
                    ...
                    NonRootModeDefaultHttpHandler = new() { RelativeUrl = config.DefaultRedirectPath };
                }
                if (DefaultHttpHandler == null && !string.IsNullOrEmpty(config.MetadataRedirectPath))
                {
                    ...
                    NonRootModeDefaultHttpHandler = new() { RelativeUrl = config.MetadataRedirectPath };
                }
        ...
        public IHttpHandler GetHandler(HttpContext ctx, string requestType, string url, string pathTranslated)
        {
            ...
            return GetHandlerForPathInfo(httpReq, pathTranslated) ?? NotFoundHttpHandler;
        }
#endif
        ...
        public static IHttpHandler GetHandler(IHttpRequest httpReq)
        {
            ...
            return GetHandlerForPathInfo(httpReq, httpReq.GetPhysicalPath()) ?? NotFoundHttpHandler;
        }
        ...
        public static IHttpHandler GetHandlerForPathInfo(IRequest httpReq, string filePath)
        {
            ...
            {
                isDirectory = StaticFileHandler.MonoDirectoryExists(
                    filePath,
                    filePath.Substring(0, filePath.Length - pathInfo.Length));
            }
            ...
            var restPath = RestHandler.FindMatchingRestPath(httpMethod, pathInfo, out var contentType);
            if (restPath != null)
            {
                return new RestHandler
                {
            ...
            if (isFile || isDirectory)
            {
                ...
                if (appHost.Config.RedirectDirectoriesToTrailingSlashes
                    && isDirectory
                    && !httpReq.OriginalPathInfo.EndsWith("/"))
                {
