...
namespace ServiceStack
{
    ...
    public class AutoQueryDataFeature : IPlugin, IPostInitPlugin
    {
        ...
        public void Register(IAppHost appHost)
        {
            ...
            foreach (var entry in ImplicitConventions)
            {
                ...
                if (!ConditionsAliases.TryGetValue(conditioAlias, out var query))
                {
        ...
        private Type GenerateMissingServices(IEnumerable<Type> misingRequestTypes)
        {
    ...
    public class OrderByExpression : FilterExpression
    {
        ...
        private class OrderByComparator<T> : IComparer<T>
        {
            private readonly GetMemberDelegate[] getters;
            private readonly bool[] orderAsc;
    ...
    public class DataQuery<T> : IDataQuery
    {
        ...
        public DataQuery(QueryDataContext context)
        {
            ...
            Dto = context.Dto;
            DynamicParams = context.DynamicParams;
            Conditions = new();
        }
        ...
        {
            OnlyFields = fields == null || fields.Length == 0
                ? null //All Fields
        ...
        private void OrderByFieldsImpl(string[] fieldNames, Func<string, bool> orderFn)
        {
            ...
            if (getters.Count > 0)
            {
                OrderBy = new(fields.ToArray(), getters.ToArray(), orderAscs.ToArray());
            }
        }

        public virtual void OrderByPrimaryKey()
        {
            OrderBy = new(PrimaryKey.Name, TypeProperties<T>.Instance.GetPublicGetter(PrimaryKey));
        }
    ...
    public class AutoQueryData : IAutoQueryData, IAutoQueryDataOptions, IDisposable
    {
        ...
        {
            if (TypedQueries.TryGetValue(requestDtoType, out var defaultValue))
            {
                return defaultValue;
            }
            ...
            do
            {
                ...
                newCache = new(TypedQueries) { [requestDtoType] = defaultValue };
        ...
        {
            response.Meta = new(StringComparer.OrdinalIgnoreCase);
            ...
            if (IncludeTotal || totalRequested)
            {
                if (!totalRequested)
                {
                    commands.Add(new() { Name = "COUNT".ToStringSegment(), Args = { "*".ToStringSegment() } });
                }
        ...
        public IQueryResponse Execute(IQueryData request, IDataQuery q)
        {
            ...
            do
            {
                ...
                newCache = new(genericAutoQueryCache)
                {
...
#pragma warning disable CS0693 // Type parameter has the same name as the type parameter from outer type (inherited need to make it generic on it's own)
...
#pragma warning restore CS0693 // Type parameter has the same name as the type parameter from outer type
        ...
        public virtual object SelectAggregate(IDataQuery q, string name, IEnumerable<string> args)
        {
            ...
            if (firstArg?.StartsWithIgnoreCase("DISTINCT ") == true)
            {
                modifier = "DISTINCT";
    ...
    {
        private static readonly Dictionary<string, GetMemberDelegate> RequestPropertyGetters =
            new();

        private static readonly Dictionary<string, QueryDataField> QueryFieldMap =
            new();
        ...
        public IDataQuery AddToQuery(
            ...
            IAutoQueryDataOptions options = null)
        {
            dynamicParams = new(dynamicParams, StringComparer.OrdinalIgnoreCase);
            ...
            var defaultTerm = dtoAttr?.DefaultTerm == QueryTerm.Or ? QueryTerm.Or : QueryTerm.And;
            ...
            if (options?.EnableUntypedQueries == true)
            {
        ...
        private static void AppendLimits(IDataQuery q, IQueryData dto, IAutoQueryDataOptions options)
        {
            ...
            else if (dto.OrderByDesc != null)
            {
                ...
                q.OrderByFieldsDescending(fieldNames);
                     ...
                     && options?.OrderByPrimaryKeyOnLimitQuery == true)
            {
                q.OrderByPrimaryKey();
            }
        ...
        private static MatchQuery GetQueryMatch(IDataQuery q, string name, IAutoQueryDataOptions options)
        {
            ...
            var match = options.IgnoreProperties?.Contains(name) != true
            ...
            if (match == null)
            {
                foreach (var startsWith in options.StartsWithConventions)
                {
                    ...
                    if (match != null)
                    {
                        return new(match, startsWith.Value);
                    }
                }
            }
            if (match == null)
            {
                foreach (var endsWith in options.EndsWithConventions)
                {
                    ...
                    if (match != null)
                    {
                        return new(match, endsWith.Value);
                    }
                }
            }
