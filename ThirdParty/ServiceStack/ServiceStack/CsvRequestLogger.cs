...
namespace ServiceStack
{
    public class CsvRequestLogger : InMemoryRollingRequestLogger
    {
        ...
        public CsvRequestLogger(IVirtualFiles files = null, string requestLogsPattern = null, string errorLogsPattern = null, TimeSpan? appendEvery = null)
        {
            ...
            appendEverySecs = (int)appendEvery.GetValueOrDefault(TimeSpan.FromSeconds(1)).TotalSeconds;
            ...
            timer = new(OnFlush, null, appendEverySecs, Timeout.Infinite);
        }
        ...
        private RequestLogEntry ReadLastEntry(string logFile)
        {
            try
            {
                ...
                {
                    var file = files.GetFile(logFile);
        ...
        {
            var logFile = files.GetFile(GetLogFilePath(requestLogsPattern, DateTime.UtcNow));
            if (logFile.Exists())
            {
