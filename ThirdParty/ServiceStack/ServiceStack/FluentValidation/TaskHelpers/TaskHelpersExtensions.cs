...
namespace System.Threading.Tasks
{
    ...
    internal static class TaskHelpersExtensions
    {
        ...
        internal static Task Catch(this Task task, Func<CatchInfo, CatchInfo.CatchResult> continuation, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return task.CatchImpl(() => continuation(new(task, cancellationToken)).Task.ToTask<AsyncVoid>(), cancellationToken);
        }
...
#else
            var getAwaiterMethod = typeof(Task).GetTypeInfo().GetDeclaredMethod("GetAwaiter");
            ...
            else
            {
                ...
                try
                {
                    if (AppDomain.CurrentDomain.IsFullyTrusted)
                    {
                        ...
                        func(new()); // make sure the method call succeeds before assigning the 'prepForRemoting' local variable
        ...
        where TTask : Task
    {
        ...
        protected CatchInfoBase(TTask task, CancellationToken cancellationToken)
        {
            ...
            {
                Diagnostics.Debug.Assert(cancellationToken.IsCancellationRequested);
                _exception = new OperationCanceledException(cancellationToken);
            }
        }

        protected TTask Task => _task;
    ...
    {
        private static CatchResult _completed = new() { Task = TaskHelpers.Completed() };
        ...
        public CatchResult Throw()
        {
            ...
            {
                return new() { Task = base.Task };
            }
            else
            {
                // Canceled via CancelationToken
                return new() { Task = TaskHelpers.Canceled() };
            }
        }
        ...
        {
            return new() { Task = TaskHelpers.FromError<object>(ex) };
        }
    }
    ...
    internal class CatchInfo<T> : CatchInfoBase<Task<T>>
    {
        ...
        {
            return new() { Task = TaskHelpers.FromResult(returnValue) };
        }
        ...
        public CatchResult Throw()
        {
            ...
            {
                // Canceled via CancelationToken
                return new() { Task = TaskHelpers.Canceled<T>() };
            }
        }
        ...
        public CatchResult Throw(Exception ex)
        {
            return new() { Task = TaskHelpers.FromError<T>(ex) };
        }
    }
}
