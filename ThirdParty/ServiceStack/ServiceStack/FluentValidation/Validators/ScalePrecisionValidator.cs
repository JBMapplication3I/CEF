        ...
        private void Init(int scale, int precision)
        {
            ...
            if (Scale < 0)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(scale), string.Format("Scale must be a positive integer. [value:{0}].", Scale));
            }

            if (Precision < 0)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(precision), string.Format("Precision must be a positive integer. [value:{0}].", Precision));
            }
