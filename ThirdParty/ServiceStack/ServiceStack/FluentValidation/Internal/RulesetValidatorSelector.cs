namespace ServiceStack.FluentValidation.Internal
{
    ...
    public class RulesetValidatorSelector : IValidatorSelector
    {
        ...
        public bool CanExecute(IValidationRule rule, string propertyPath, ValidationContext context)
        {
            ...
            if (string.IsNullOrEmpty(rule.RuleSet) && rulesetsToExecute.Length > 0 && rulesetsToExecute.Contains("default", StringComparer.OrdinalIgnoreCase))
            {
                return true;
            }

            if (!string.IsNullOrEmpty(rule.RuleSet) && rulesetsToExecute.Length > 0 && rulesetsToExecute.Contains(rule.RuleSet))
            {
                return true;
            }

            if (rulesetsToExecute.Contains("*"))
            {
                return true;
            }
