...
namespace Amib.Threading.Internal
{
    ...
    /// <summary>WorkItemsQueue class.</summary>
    /// <seealso cref="IDisposable"/>
    public class WorkItemsQueue : IDisposable
    {
        #region Member variables
        /// <summary>(Immutable)
        /// Waiters queue (implemented as stack).</summary>
        private readonly WaiterEntry _headWaiterEntry = new();

        /// <summary>Waiters count.</summary>
        private int _waitersCount = 0;

        /// <summary>(Immutable)
        /// Work items queue.</summary>
        private readonly PriorityQueue _workItems = new();

        /// <summary>Indicate that work items are allowed to be queued.</summary>
        private bool _isWorkItemsQueueActive = true;

        [ThreadStatic]
        private static WaiterEntry _waiterEntry;

        /// <summary>Each thread in the thread pool keeps its own waiter entry.</summary>
        /// <value>The current waiter entry.</value>
        ...
        /// <summary>A flag that indicates if the WorkItemsQueue has been disposed.</summary>
        private bool _isDisposed = false;
        #endregion

        #region Public properties
        /// <summary>Returns the current number of work items in the queue.</summary>
        /// <value>The count.</value>
        public int Count => _workItems.Count;

        /// <summary>Returns the current number of waiters.</summary>
        /// <value>The number of waiters.</value>
        public int WaitersCount => _waitersCount;
        #endregion

        #region Public methods
        /// <summary>Enqueue a work item to the queue.</summary>
        ...
        /// <summary>Waits for a work item or exits on timeout or cancel.</summary>
            ...
            WaitHandle cancelEvent)
        {
            ...
            var waitHandles = new[] { waiterEntry.WaitHandle, cancelEvent };
            ...
            var index = STPEventWaitHandle.WaitAny(
                waitHandles,
                millisecondsTimeout,
                true);
            lock (this)
            {
                ...
                if (success)
                {
                    workItem = waiterEntry.WorkItem ?? _workItems.Dequeue() as WorkItem;
                }
            }
        ...
        /// <summary>Cleanup the work items queue, hence no more work items are allowed to be queue.</summary>
        ...
        public object[] GetStates()
        {
            lock (this)
            {
                var states = new object[_workItems.Count];
                var i = 0;
                foreach (WorkItem workItem in _workItems)
                {
        ...
        public void PushWaiter(WaiterEntry newWaiterEntry)
        {
            ...
            else
            {
                ...
                var oldFirstWaiterEntry = _headWaiterEntry._nextWaiterEntry;
        ...
        private void RemoveWaiter(WaiterEntry waiterEntry, bool popDecrement)
        {
            ...
            var prevWaiterEntry = waiterEntry._prevWaiterEntry;

            // Store the next entry in the list
            var nextWaiterEntry = waiterEntry._nextWaiterEntry;
        ...
        public sealed class WaiterEntry : IDisposable
        {
            ...
            /// <summary>
            /// A work item that passed directly to the waiter without going
            /// through the queue
            /// </summary>
