...
namespace Amib.Threading.Internal
{
 ...
 "concurrency must be greater than zero");
        ...
        public override object[] GetStates()
        {
            lock (_lock)
            {
                var states = new object[_workItemsQueue.Count];
                var i = 0;
        ...
        {
            var iwir = (IInternalWorkItemResult)wir;
        ...
        private void FireOnIdleImpl(WorkItemsGroupIdleHandler onIdle)
        {
            ...
            var delegates = onIdle.GetInvocationList();
            ...
            lock (_lock)
            {
                ...
                if (0 == _workItemsQueue.Count)
                {
                    if (0 == _workItemsInStpQueue)
                    {
                        ...
                        {
                            _stp.QueueWorkItem(new(FireOnIdle));
                        }
                    }
                    return;
                }
