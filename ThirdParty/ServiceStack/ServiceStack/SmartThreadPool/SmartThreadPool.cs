...
namespace Amib.Threading
{
    ...
    public partial class SmartThreadPool : WorkItemsGroupBase, IDisposable
    {
        ...
        /// </summary>
        private readonly SynchronizedDictionary<Thread, ThreadEntry> _workerThreads = new();

        /// <summary>
        /// Queue of work items.
        /// </summary>
        private readonly WorkItemsQueue _workItemsQueue = new();
        ...
        /// </summary>
        private readonly SynchronizedDictionary<IWorkItemsGroup, IWorkItemsGroup> _workItemsGroups = new();

        /// <summary>
        /// A common object for all the work items int the STP
        /// so we can mark them to cancel in O(1)
        /// </summary>
        private CanceledWorkItemsGroup _canceledSmartThreadPool = new();
        ...
        {
            var threadsCount = Math.Max(_workItemsQueue.Count, _stpStartInfo.MinWorkerThreads);
        ...
        private void IncrementWorkItemsCount()
        {
            ...
            var count = Interlocked.Increment(ref _currentWorkItemsCount);
        ...
        {
            var count = Interlocked.Decrement(ref _currentWorkItemsCount);
            //Trace.WriteLine("WorkItemsCount = " + _currentWorkItemsCount.ToString());
            if (count == 0)
            {
                IsIdle = true;
            ...
            lock (_workerThreads.SyncRoot)
            {
                ...
                for (var i = 0; i < threadsCount; ++i)
                {
                    ...
                    var workerThread =
                        ...
                        ? new(ProcessQueuedItems, _stpStartInfo.MaxStackSize.Value)
                    ...
                    _workerThreads[workerThread] = new(this);
                    _windowsPCs.SampleThreads(_workerThreads.Count, _inUseWorkerThreads);
                    _localPCs.SampleThreads(_workerThreads.Count, _inUseWorkerThreads);
                }
            }
        ...
        private void ProcessQueuedItems()
        {
            ...
            try
            {
                var bInUseWorkerThreadsWasIncremented = false;
                ...
                while (!_shutdown)
                {
                    ...
                    try
                    {
                        ...
                        var inUseWorkerThreads = Interlocked.Increment(ref _inUseWorkerThreads);
                    ...
                    finally
                    {
                        ...
                        {
                            var inUseWorkerThreads = Interlocked.Decrement(ref _inUseWorkerThreads);
                            _windowsPCs.SampleThreads(_workerThreads.Count, inUseWorkerThreads);
                            _localPCs.SampleThreads(_workerThreads.Count, inUseWorkerThreads);
                        }
        ...
        internal static void ValidateWorkItemsGroupWaitForIdle(IWorkItemsGroup workItemsGroup)
        {
            ...
            var workItem = CurrentThreadEntry.CurrentWorkItem;
        ...
        {
            if (null != workItemsGroup &&
                workItem?.WasQueuedBy(workItemsGroup) == true)
            {
                throw new NotSupportedException("WaitForIdle cannot be called from a thread on its SmartThreadPool, it causes a deadlock");
            }
        }
        ...
        public void Shutdown(bool forceAbort, int millisecondsTimeout)
        {
            ...
            var millisecondsLeft = millisecondsTimeout;
            var stopwatch = Stopwatch.StartNew();
            //DateTime start = DateTime.UtcNow;
            var waitInfinitely = Timeout.Infinite == millisecondsTimeout;
            var timeout = false;
            ...
            foreach (var thread in threads)
            {
                ...
                var success = thread.Join(millisecondsLeft);
                if (!success)
                {
                    timeout = true;
                    break;
        ...
        {
            foreach (var threadEntry in _workerThreads.Values)
            {
                var workItem = threadEntry.CurrentWorkItem;
                if (workItem?.WasQueuedBy(wig) == true &&
                    !workItem.IsCanceled)
                {
        ...
        {
            var states = _workItemsQueue.GetStates();
            return states;
        }
        ...
        {
            add => throw new NotImplementedException("This event is not implemented in the SmartThreadPool class. Please create a WorkItemsGroup in order to use this feature.");//_onIdle += value;
            remove => throw new NotImplementedException("This event is not implemented in the SmartThreadPool class. Please create a WorkItemsGroup in order to use this feature.");//_onIdle -= value;
        }
        ...
        {
            var wigStartInfo = new WIGStartInfo { StartSuspended = true };
            var workItemsGroup = CreateWorkItemsGroup(int.MaxValue, wigStartInfo);
        ...
        public int Choice(IEnumerable<Action> actions)
        {
            ...
            var anActionCompleted = new ManualResetEvent(false);

            var choiceIndex = new ChoiceIndex();
        ...
        {
            var wigStartInfo = new WIGStartInfo { StartSuspended = true };
            var workItemsGroup = CreateWorkItemsGroup(1, wigStartInfo);
            foreach (var action in actions)
            {
                var act = action;
