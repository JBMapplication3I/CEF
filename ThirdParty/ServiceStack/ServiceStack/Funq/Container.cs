...
namespace Funq
{
    ...
    {
        private Dictionary<ServiceKey, ServiceEntry> services = new();
        private Dictionary<ServiceKey, ServiceEntry> servicesReadOnlyCopy;

        public int disposablesCount
        {
            get { lock (disposables)
                {
        ...
        private Stack<WeakReference> disposables = new();
        // We always hold a strong reference to child containers.
        private Stack<Container> childContainers = new();
        private Container parent;

        /// <include file='Container.xdoc' path='docs/doc[@for="Container.ctor"]/*'/>
        public Container()
        {
            services[new(typeof(Func<Container, Container>), null)] =
        ...
        public virtual void Dispose()
        {
            ...
            {
                if (serviceEntry.GetInstance() is IDisposable disposable && disposable is not Container)
                {
                    disposable.Dispose();
                }
            }
        ...
        protected virtual ServiceEntry<TService, TFunc> GetEntry<TService, TFunc>(string serviceName, bool throwIfMissing)
        {
            ...
            if (entry == null)
            {
                ...
                if (genericDef?.Name.StartsWith("Func`") == true) //Lazy Dependencies
                {
