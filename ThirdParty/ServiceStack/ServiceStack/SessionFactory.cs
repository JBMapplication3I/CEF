...
namespace ServiceStack
{
    public class SessionFactory : ISessionFactory
    {
        ...
        public class SessionCacheClient : ISession
        {
            ...
            {
                this.cacheClient = cacheClient;
                prefixNs = "sess:" + sessionId + ":";
            }

            public object this[string key]
            {
                get => cacheClient.Get<object>(prefixNs + key);
                set => JsWriter.WriteDynamic(() =>
                                            cacheClient.Set(prefixNs + key, value));
            }
            ...
            public void Set<T>(string key, T value)
            {
                ...
                if (expiry != null)
                {
                    cacheClient.Set(prefixNs + key, value, expiry.Value);
                }
                else
                {
                    cacheClient.Set(prefixNs + key, value);
            ...
            {
                return cacheClient.Get<T>(prefixNs + key);
            }

            public bool Remove(string key)
            {
                return cacheClient.Remove(prefixNs + key);
            }

            public void RemoveAll()
            {
                cacheClient.RemoveByPattern(prefixNs + "*");
            }
        }
