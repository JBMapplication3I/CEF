namespace ServiceStack
{
    ...
    /// <summary>Donated by Ivan Korneliuk from his post:
    /// http://korneliuk.blogspot.com/2012/08/servicestack-reusing-dtos.html
    /// Modified to only allow using routes matching the supplied HTTP Verb.</summary>
    public static class UrlExtensions
    {
        ...
        /// <summary>Generate a url from a Request DTO. Pretty URL generation require Routes to be defined using
        /// `[Route]` on the Request DTO.</summary>
        /// <param name="requestDto">                     The requestDto to act on.</param>
        ...
        public static string GetOperationName(this Type type)
        {
            ...
            var endIndex = fullname.IndexOf("[[", startIndex, StringComparison.Ordinal); // Generic Fullname
            if (endIndex == -1)
            {
        ...
        public static string ToUrl(
            this object requestDto,
            string httpMethod = "GET",
            string formatFallbackToPredefinedRoute = null)
        {
            ...
            if (matchingRoutes.Count == 0)
            {
                ...
                throw new InvalidOperationException(
                    $"None of the given rest routes matches '{requestType.GetOperationName()}' request:{errors}");
            }
            ...
            if (matchingRoutes.Count > 1)
            {
                ...
                if (matchingRoute == null)
                {
                    ...
                    throw new InvalidOperationException(
                        $"Ambiguous matching routes found for '{requestType.Name}' request:{errors}");
                }
            }
            else
            {
        ...
        public static string AsHttps(this string absoluteUrl)
        {
            return string.IsNullOrEmpty(absoluteUrl)
                ? null
                : absoluteUrl.ReplaceFirst("http://", "https://");
        }
    ...
    public class RestRoute
    {
        ...
        public static Func<object, string> FormatVariable = value =>
        {
            ...
            valueString ??= value.ToJsv().Trim(ArrayBrackets);
        ...
        private readonly IDictionary<string, RouteMember> variablesMap
            = new Dictionary<string, RouteMember>(PclExport.Instance.InvariantComparerIgnoreCase);

        public RestRoute(Type type, string path, string verbs, int priority)
        {
            HttpMethods = (verbs ?? string.Empty).Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
            Type = type;
            Path = path;
            Priority = priority;
            queryProperties = GetQueryProperties(type);
            foreach (var variableName in GetUrlVariables(path))
            {
                var safeVarName = variableName.TrimEnd('*');
                if (!queryProperties.TryGetValue(safeVarName, out var propertyInfo))
                {
                    AppendError($"Variable '{variableName}' does not match any property.");
                    continue;
                }
                queryProperties.Remove(safeVarName);
                variablesMap[variableName] = propertyInfo;
            }
        }
        ...
        public bool IsValid => string.IsNullOrEmpty(ErrorMsg);
        ...
        public ICollection<string> Variables => variablesMap.Keys;

        public List<string> QueryStringVariables => queryProperties
            .Where(x => !x.Value.IgnoreInQueryString)
            .Select(x => x.Key)
            .ToList();
        ...
        public RouteResolutionResult Apply(object request, string httpMethod)
        {
            ...
            foreach (var variable in variablesMap)
            {
            ...
            {
                return RouteResolutionResult.Error(
                    this,
                    "Could not match following variables: " + string.Join(",", unmatchedVariables.ToArray()));
            }
        ...
        {
            return GetQueryString(request, queryProperties);
        }
        ...
        internal static string GetQueryString(object request, IDictionary<string, RouteMember> propertyMap)
        {
            ...
            foreach (var queryProperty in propertyMap)
            {
                ...
                if (ClientConfig.SkipEmptyArrays && value is Array { Length: 0 })
                {
                    continue;
                }
        ...
        internal static IDictionary<string, RouteMember> GetQueryProperties(Type requestType)
        {
            ...
            foreach (var propertyInfo in requestType.GetPublicProperties())
            {
                ...
                if (hasDataContract)
                {
                    if (!propertyInfo.HasAttribute<DataMemberAttribute>())
                    {
                        continue;
                    }
                ...
                {
                    // but allow in PathInfo
                    IgnoreInQueryString = propertyInfo.FirstAttribute<IgnoreDataMemberAttribute>() != null,
                };
            }
            ...
            foreach (var fieldInfo in requestType.GetPublicFields())
            {
                ...
                {
                    // but allow in PathInfo
                    IgnoreInQueryString = fieldInfo.FirstAttribute<IgnoreDataMemberAttribute>() != null,
                };
            }
            return result;
        }
        ...
        private IEnumerable<string> GetUrlVariables(string path)
        {
            ...
            foreach (var component in components)
            {
                ...
                if (component[0] != VariablePrefixChar
                    || component[component.Length - 1] != VariablePostfixChar
                    || variableName.Contains(VariablePostfix))
                {
                    AppendError("Component '{0}' can not be parsed".Fmt(component));
                    continue;
                }
    ...
    public class RouteResolutionResult
    {
        ...
        public bool Matches => string.IsNullOrEmpty(FailReason);

        public static RouteResolutionResult Error(RestRoute route, string errorMsg)
        {
            return new() { Route = route, FailReason = errorMsg };
        }

        public static RouteResolutionResult Success(RestRoute route, string uri)
        {
            return new() { Route = route, Uri = uri, Priority = route.Priority };
        }
