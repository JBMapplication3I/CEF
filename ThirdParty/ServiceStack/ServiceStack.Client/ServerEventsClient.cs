namespace ServiceStack
{
    ...
    public partial class ServerEventsClient : IDisposable
    {
        ...
        public string BaseUri
        {
            get
            {
                var meta = ServiceClient as IServiceClientMeta;
                return meta?.BaseUri;
            }
            set
            {
                eventStreamPath = value.CombineWith("event-stream");
                BuildEventStreamUri();

                if (ServiceClient is IServiceClientMeta meta)
                {
                    meta.BaseUri = value;
                }
            }
        ...
        {
            EventStreamUri = eventStreamPath
        ...
        {
            eventStreamPath = baseUri.CombineWith("event-stream");
            Channels = channels;

            ServiceClient = new JsonServiceClient(baseUri);

            Resolver = new NewInstanceResolver();
            ReceiverTypes = new();
            Handlers = new();
            NamedReceivers = new();
        }
        ...
        public ServerEventsClient AddListener(string eventName, Action<ServerEventMessage> handler)
        {
            lock (listeners)
            {
                ...
                {
                    listeners[eventName] = handlers = new();
                }
