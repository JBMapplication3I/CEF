...
namespace ServiceStack
{
    ...
    public partial class AsyncServiceClient : IHasSessionId, IHasVersion
    {
            ...
            Action<TResponse> onSuccess, Action<object, Exception> onError, Action<WebResponse> onResponseInit = null)
        {
            ...
            if (!DisableTimer)
            {
                requestState.StartTimer(Timeout.GetValueOrDefault(DefaultTimeout));
            }
                        ...
                        || OnAuthenticationRequired != null))
                {
                    try
                    {
                        if (RefreshToken != null)
                        {
                            ...
                            if (CookieContainer.GetTokenCookie(BaseUri) != null)
                            {
                                CookieContainer.SetTokenCookie(accessToken, BaseUri);
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    response = (T)StreamDeserializer(typeof(T), reader);
                                }
                            }
                            finally
                            {
            ...
            if (PclExportClient.Instance.IsWebException(webEx))
            {
                ...
                try
                {
                    ...
                    if (stream.CanSeek)
                    {
                        ...
                        serviceEx.ResponseDto = StreamDeserializer(errorResponseType, stream);
                    }
                    else //Android
                    {
                        using var ms = MemoryStreamFactory.GetStream(bytes);
                        serviceEx.ResponseDto = StreamDeserializer(errorResponseType, ms);
                    }
