...
namespace ServiceStack
{
    ...
    public abstract class ServiceClientBase : IServiceClient, IMessageProducer, IHasCookieContainer, IServiceClientMeta
    {
        ...
        protected ServiceClientBase()
        {
            ...
            CookieContainer = new();
            StoreCookies = true; //leave
            UserAgent = DefaultUserAgent;

            asyncClient.HandleCallbackOnUiThread = HandleCallbackOnUiThread = true;
            asyncClient.ShareCookiesWithBrowser = ShareCookiesWithBrowser = true;
        ...
        protected ServiceClientBase(string syncReplyBaseUri, string asyncOneWayBaseUri)
            : this()
        {
            SyncReplyBaseUri = syncReplyBaseUri;
            AsyncOneWayBaseUri = asyncOneWayBaseUri;
        }
        ...
        {
            BaseUri = baseUri;
            asyncClient.BaseUri = baseUri;
            SyncReplyBaseUri = baseUri.WithTrailingSlash() + Format + "/reply/";
            AsyncOneWayBaseUri = baseUri.WithTrailingSlash() + Format + "/oneway/";
        }
        ...
        {
            get => shareCookiesWithBrowser;
            set => asyncClient.ShareCookiesWithBrowser = shareCookiesWithBrowser = value;
        }
        ...
        public virtual List<TResponse> SendAll<TResponse>(IEnumerable<object> requests)
        {
            ...
            var requestUri = SyncReplyBaseUri.WithTrailingSlash() + elType.Name + "[]";
        ...
        public virtual TResponse Send<TResponse>(object request)
        {
            ...
            var requestUri = ResolveUrl(httpMethod, UrlResolver == null
                ? SyncReplyBaseUri.WithTrailingSlash() + request.GetType().Name
            ...
            try
            {
                    ...
                    || OnAuthenticationRequired != null))
                {
                    if (RefreshToken != null)
                    {
                        ...
                        var uri = RefreshTokenUri ?? BaseUri.CombineWith(refreshRequest.ToPostUrl());
                        ...
                        {
                            refreshClient.AddBearerToken(BearerToken = accessToken);
                        }
        ...
        private void HandleAuthException(Exception ex, WebRequest client)
        {
            ...
            if (webEx?.Response != null)
            {
                ...
                {
                    authInfo = new(doAuthHeader);
                    if (authInfo?.method == "basic" || authInfo?.method == "digest")
                    {
                        client.AddAuthInfo(UserName, Password, authInfo);
                    }
        ...
        {
            if (webEx is not {Response: { }, Status: WebExceptionStatus.ProtocolError})
            {
                return null;
            }
            ...
            {
                // Oh, well, we tried
                return new(errorResponse.StatusDescription, innerEx)
                {
                    StatusCode = (int)errorResponse.StatusCode,
                    StatusDescription = errorResponse.StatusDescription,
        ...
        protected virtual void SerializeRequestToStream(object request, Stream requestStream, bool keepOpen=false)
        {
            switch (request)
            {
                ...
                {
#if !SL5
                    requestStream = RequestCompressionType switch
                    {
                        CompressionTypes.Deflate => new System.IO.Compression.DeflateStream(requestStream, System.IO.Compression.CompressionMode.Compress),
                        CompressionTypes.GZip => new System.IO.Compression.GZipStream(requestStream, System.IO.Compression.CompressionMode.Compress),
#if NET60_OR_GREATER
                        CompressionTypes.Brotli => new System.IO.Compression.BrotliStream(requestStream, System.IO.Compression.CompressionMode.Compress),
#endif
                        _ => requestStream,
                    };
#endif
        ...
        protected WebRequest PrepareWebRequest(string httpMethod, string requestUri, object request, Action<HttpWebRequest> sendRequestAction)
        {
            ...
            var client = PclExport.Instance.CreateWebRequest(requestUri, emulateHttpViaPost: EmulateHttpViaPost);
            try
            {
        ...
        {
            var elType = requests.GetType().GetCollectionType();
            var requestUri = AsyncOneWayBaseUri.WithTrailingSlash() + elType.Name + "[]";
        ...
        {
            if (request is not IVerb)
            {
                return null;
            }
            return request switch
            {
                IGet _ => HttpMethods.Get,
                IPost _ => HttpMethods.Post,
                IPut _ => HttpMethods.Put,
                IDelete _ => HttpMethods.Delete,
                IPatch _ => HttpMethods.Patch,
                _ => null,
            };
        }
        ...
        {
            var requestUri = AsyncOneWayBaseUri.WithTrailingSlash() + request.GetType().Name;
    ...
    public static partial class ServiceClientExtensions
    {
        ...
        public static void PopulateRequestMetadata(this IHasSessionId client, object request)
        {
            if (client.SessionId != null
                && request is IHasSessionId hasSession
                && hasSession.SessionId == null)
            {
                hasSession.SessionId = client.SessionId;
            }
            if (client is IHasVersion clientVersion
                && clientVersion.Version > 0
                && request is IHasVersion hasVersion
        ...
        public static Dictionary<string, string> ToDictionary(this CookieContainer cookies, string baseUri)
        {
            ...
            foreach (Cookie cookie in cookies.GetCookies(new(baseUri)))
            {
        ...
        {
            if (client is not ServiceClientBase c)
            {
