...
namespace ServiceStack.Messaging
{
        ...
        : IMessageQueueClient, IOneWayClient
    {
        ...
        public void Notify(string queueName, IMessage message)
        {
            var messageBytes = message.ToBytes();
            ReadWriteClient.LPush(queueName, messageBytes);
            ReadWriteClient.LTrim(queueName, 0, MaxSuccessQueueSize);
            ReadWriteClient.Publish(QueueNames.TopicOut, queueName.ToUtf8Bytes());
        }

        public IMessage<T> Get<T>(string queueName, TimeSpan? timeOut = null)
        {
            var unblockingKeyAndValue = ReadWriteClient.BRPop(queueName, (int)timeOut.GetValueOrDefault().TotalSeconds);
        ...
        {
            var messageBytes = ReadWriteClient.RPop(queueName);
            return messageBytes.ToMessage<T>();
        }
