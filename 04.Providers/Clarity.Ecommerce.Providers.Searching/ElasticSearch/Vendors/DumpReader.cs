...
namespace Clarity.Ecommerce.Providers.Searching.ElasticSearch
{
    ...
    /// <summary>A vendor dump reader.</summary>
    internal class VendorDumpReader : DumpReaderBase<VendorIndexableModel>
    {
        /// <inheritdoc/>
        public override IEnumerable<VendorIndexableModel> GetRecords(string? contextProfileName, CancellationToken ct)
        {
            ...
            var keys = ElasticSearchingProviderConfig.SearchingVendorIndexFilterByTypeKeys;
            ...
            if (Contract.CheckValidKey(ElasticSearchingProviderConfig.SearchingVendorIndexQueryByAttributeKeys))
            {
                queryableAttributeKeyList = ElasticSearchingProviderConfig.SearchingVendorIndexQueryByAttributeKeys
                    !.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                    .Select(x => x.Trim())
                    .ToArray();
            }
            foreach (var version in context.Vendors
                .FilterByActive(true)
                .FilterByTypeKeys<Vendor, VendorType>(keyList)
                .OrderBy(x => x.ID)
                .Select(x => new
                {
                    // Base Properties
                ...
                .ToList()
                .Select(x => new // VendorIndexableModel
                {
                    // Base Properties
                    x.ID,
            ...
            {
                if (ElasticSearchingProviderConfig.SearchingVendorIndexRequiresABrand && !version.HasBrands
                    // || ElasticSearchingProviderConfig.SearchingVendorIndexRequiresACategory && !version.HasCategories
                    || ElasticSearchingProviderConfig.SearchingVendorIndexRequiresAFranchise && !version.HasFranchises
                    || ElasticSearchingProviderConfig.SearchingVendorIndexRequiresAManufacturer && !version.HasManufacturers
                    || ElasticSearchingProviderConfig.SearchingVendorIndexRequiresAProduct && !version.HasProducts
                    || ElasticSearchingProviderConfig.SearchingVendorIndexRequiresAStore && !version.HasStores
                    /*|| ElasticSearchingProviderConfig.SearchingVendorIndexRequiresAVendor && !version.HasVendors*/)
                {
                    continue;
                }
                VendorIndexableModel indexableModel = new()
                {
                    // Base Properties
                    ID = version.ID,
                ...
                */
                /*
                if (ElasticSearchingProviderConfig.SearchingVendorIndexFiltersIncludeRoles)
                {
        ...
        protected virtual void HandleCustomSuggests(
            VendorIndexableModel indexableModel)
        {
        }
    }
}
