...
namespace Clarity.Ecommerce.Providers.Searching.ElasticSearch
{
    ...
    internal class FranchiseDumpReader : DumpReaderBase<FranchiseIndexableModel>
    {
        /// <inheritdoc/>
        public override IEnumerable<FranchiseIndexableModel> GetRecords(string? contextProfileName, CancellationToken ct)
        {
            ...
            var keys = ElasticSearchingProviderConfig.SearchingFranchiseIndexFilterByTypeKeys;
            ...
            if (Contract.CheckValidKey(ElasticSearchingProviderConfig.SearchingFranchiseIndexQueryByAttributeKeys))
            {
                queryableAttributeKeyList = ElasticSearchingProviderConfig.SearchingFranchiseIndexQueryByAttributeKeys
                    !.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                    .Select(x => x.Trim())
                    .ToArray();
            }
            foreach (var version in context.Franchises
                .FilterByActive(true)
                .FilterByTypeKeys<Franchise, FranchiseType>(keyList)
                .OrderBy(x => x.ID)
                .Select(x => new
                {
                    // Base Properties
                    ...
                    // Associated Objects
                    HasBrands = x.Brands!.Any(y => y.Active && y.Master!.Active),
                    Brands = x.Brands!
                    ...
                    */
                    HasManufacturers = x.Manufacturers!.Any(y => y.Active && y.IsVisibleIn && y.Master!.Active),
                    ...
                    HasProducts = x.Products!.Any(y => y.Active && y.IsVisibleIn && y.Master!.Active),
                    Products = x.Products!
                        .Where(y => y.Active && y.IsVisibleIn && y.Master!.Active)
                        .Select(y => new
                        {
                    ...
                    HasVendors = x.Vendors!.Any(y => y.Active && y.IsVisibleIn && y.Master!.Active),
                    Vendors = x.Vendors!
                        .Where(y => y.Active && y.IsVisibleIn && y.Master!.Active)
                        .Select(y => new
                        {
                ...
                .ToList()
                .Select(x => new // FranchiseIndexableModel
                {
                    // Base Properties
                    x.ID,
            ...
            {
                if (ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresABrand && !version.HasBrands
                    || ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresACategory && !version.HasCategories
                    // || ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresAFranchise && !version.HasFranchises
                    || ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresAManufacturer && !version.HasManufacturers
                    || ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresAProduct && !version.HasProducts
                    || ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresAStore && !version.HasStores
                    || ElasticSearchingProviderConfig.SearchingFranchiseIndexRequiresAVendor && !version.HasVendors)
                {
                    continue;
                }
                FranchiseIndexableModel indexableModel = new()
                {
                    // Base Properties
                    ID = version.ID,
                ...
                /*
                if (ElasticSearchingProviderConfig.SearchingFranchiseIndexFiltersIncludeRoles)
                {
        ...
        protected virtual void HandleCustomSuggests(
            FranchiseIndexableModel indexableModel)
        {
        }
    }
}
