// <copyright file="TargetQuoteSubmitProvider.Analyze.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Providers.Checkouts.TargetQuote
{
    ...
    {
        /// <summary>The true values.</summary>
        protected static readonly string[] TrueValues
            = { "TRUE", "True", "true", "1", "Y", "y", "YES", "Yes", "yes" };

        /// <summary>Dictionary of category split order required.</summary>
        ...
        /// <summary>Dictionary of vendor split order required.</summary>
        ...
        /// <summary>Gets the cart status identifier for New.</summary>
        /// <value>The cart status identifier new.</value>
        protected static int CartStatusIDNew { get; set; }

        /// <summary>Gets the cart state identifier for Work.</summary>
            ...
            string? contextProfileName)
        {
            ...
            var cart = await Workflows.Carts.AdminGetAsync(
                    ...
                    contextProfileName: contextProfileName)
                .ConfigureAwait(false)
                ?? throw new InvalidOperationException("ERROR! Must identify an actual cart");
            return await AnalyzeInnerAsync(
                    checkout: checkout,
            ...
            string? contextProfileName)
        {
            ...
            async Task ReloadOriginalCartAsync(bool validate)
            {
                originalCart = (await TryResolveCartAsync(
                            checkout: checkout,
                            pricingFactoryContext: pricingFactoryContext,
                            SessionCartBySessionAndTypeLookupKey.FromCart(originalCart),
                            taxesProvider: taxesProvider,
                            validate: validate,
                            contextProfileName: contextProfileName)
                        .ConfigureAwait(false))
            ...
            async Task<CEFActionResponse<List<ICartModel>?>> UpdateBillingAsync()
            {
                ...
                await ReloadOriginalCartAsync(false).ConfigureAwait(false);
                ...
                if (!loadedContacts.ContainsKey(target.DestinationContactID))
                {
                    try
                    {
                        loadedContacts[target.DestinationContactID] = context.Contacts
                            ...
                            ?? throw new InvalidOperationException("ERROR! Must identify an actual record");
                    }
                    catch (Exception ex1)
                    {
                        await Logger.LogErrorAsync(
                                name: $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex1.GetType().Name}",
            ...
            Task LoadStoreProductForTargetAsync(IClarityEcommerceEntities context, ISalesItemTargetBaseModel target)
            {
                ...
                ////    {
                ////        await Logger.LogErrorAsync(
                ////                name: $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex2.GetType().Name}",
            ...
            async Task LoadBrandProductForTargetAsync(IClarityEcommerceEntities context, ISalesItemTargetBaseModel target)
            {
                if (!loadedBrandProducts.ContainsKey(Contract.RequiresValidID(target.BrandProductID)))
                {
                    ...
                    {
                        await Logger.LogErrorAsync(
                                $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex2.GetType().Name}",
            ...
            Task LoadVendorProductForTargetAsync(IClarityEcommerceEntities context, ISalesItemTargetBaseModel target)
            {
                ...
                ////    {
                ////        await Logger.LogErrorAsync(
                ////                $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex3.GetType().Name}",
                ////                $"Failed to get vendor product for Target {target.ID} Seeking: '{target.OriginVendorProductID}'",
                ////                ex3,
                ////                contextProfileName)
                ////            .ConfigureAwait(false);
                ////        throw;
            ...
            Task LoadPILSForTargetAsync(IClarityEcommerceEntities context, ISalesItemTargetBaseModel target)
            {
                ...
                ////    {
                ////        await Logger.LogErrorAsync(
                ////                name: $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex4.GetType().Name}",
            ...
            async Task LoadTypeKeyForTargetAsync(IClarityEcommerceEntities context, ISalesItemTargetBaseModel target)
            {
                if (!loadedTargetTypes.ContainsKey(target.TypeID))
                {
                    ...
                    {
                        await Logger.LogErrorAsync(
                                name: $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex5.GetType().Name}",
            ...
            Task ValidateAndFallBackBrandIDAndBrandProductIDAsync(
                IClarityEcommerceEntities context,
                ...
                ISalesItemBaseModel salesItem)
            {
                ...
                if (salesItem.Targets!.Count == 0)
                {
                    ...
                    await Logger.LogErrorAsync(
                            $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.NoTargets",
                            Message,
                            contextProfileName)
                        .ConfigureAwait(false);
            ...
            */
            using (var context = RegistryLoaderWrapper.GetContext(contextProfileName))
            {
                foreach (var salesItem in originalCart.SalesItems!)
                {
                    ...
                    catch (Exception ex3)
                    {
                        await Logger.LogErrorAsync(
                                $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex3.GetType().Name}",
                                $"Failed to get salesItem for {salesItem.ID} (have previous targets)",
                                ex3,
                                contextProfileName)
                            .ConfigureAwait(false);
                        throw;
            ...
            originalCart.Totals ??= RegistryLoaderWrapper.GetInstance<ICartTotals>(contextProfileName);
            ...
            foreach (var targetGrouping in targetGroupings)
            {
                ...
                foreach (var targetSalesItem in targetSalesItems)
                {
                    ...
                    {
                        await Logger.LogErrorAsync(
                                name: $"{nameof(TargetQuoteSubmitProvider)}.{nameof(AnalyzeInnerAsync)}.{ex6.GetType().Name}",
                ...
                foreach (var originalItem in originalItemsForThisGrouping)
                {
                    ...
                    var newItem = RegistryLoaderWrapper.GetInstance<ISalesItemBaseModel<IAppliedCartItemDiscountModel>>();
                    newItem.Active = true;
