// <copyright file="OrbitalPaymentechPaymentsProvider.Subscriptions.cs" company="clarity-ventures.com">
// Copyright (c) 2019-2023 clarity-ventures.com. All rights reserved.
// </copyright>
// <summary>Implements the orbital paymentech payments provider. subscriptions class</summary>
// https://secure.paymentech.com/developercenter/files/file?fid=gTKAbxPnAPY%3D
// Page 37
// * When a participating merchant offers services of an ongoing nature to a cardholder for which the cardholder pays
//   on a recurring basis (for example, insurance premiums, subscriptions, Internet service provider fees, membership
//   fees, tuition, or utility charges), the cardholder payments are considered recurring payments.
// * If the first payment originated as an Electronic Commerce Transaction via the   Internet, it must be submitted
//   with the appropriate Electronic Commerce Indicator (ECI) value, including Verified by Visa or MasterCard Secure
//   Code authentication data (CAVV or AAV respectively), if applicable.
// * All subsequent payments must be submitted as Recurring transactions. The merchant must not store and submit the
//   CAVV with any subsequent transaction.
// Page 67
// * Recurring MIT Transaction signifies a transaction in a series of transactions processed at fixed, regular
//   intervals. These transactions represent an agreement between a cardholder and a merchant to initiate future
//   transactions for the purchase of goods or services provided at regular intervals; for example, a magazine
//   publisher charges the cardholder for a monthly subscription
// Page 71
// * MREC: Merchant Initiated Recurring Transaction
// * Signifies a transaction in a series of transactions processed at fixed, regular intervals. These transactions
//   represent an agreement between a cardholder and a merchant to initiate future transactions for the purchase of
...
// * Card Brand:   Visa,     Discover, Mastercard
// * Stored Cred:  Required, Required, Required
namespace Clarity.Ecommerce.Providers.Payments.OrbitalPaymentech
{
    ...
    public partial class OrbitalPaymentechPaymentsProvider : ISubscriptionProviderBase
    {
        // below added by Brandon
            ...
            string? contextProfileName)
        {
            ...
            Contract.RequiresNotNull(model.Subscription);
            var profileResponse = await CreateCustomerProfileAsync(
                    model.Payment,
                    model.Payment.BillingContact!,
                    contextProfileName)
            ...
            var e = new NewOrderRequestElement
            {
                ...
                mitStoredCredentialInd = OrbitalPaymentechConstants.StoredCredentialIndicators.OnFile,
                mitMsgType = OrbitalPaymentechConstants.InitiatedTransactionMessageCodes.ByMerchant.Recurring,
                profileOrderOverideInd = OrbitalPaymentechConstants.ProfileOverrideIndicators.NoMapping,
                addProfileFromOrder = OrbitalPaymentechConstants.AddProfileFromOrderValues.UseCustomerRefNumField,
                transType = OrbitalPaymentechConstants.TransactionTypes.Authorize,
                customerRefNum = customerRefNum,
                amount = $"{model.Amount:F}".Replace(".", string.Empty),
                mitSubmittedTransactionID = mitReceivedTransactionID,
                recurringInd = OrbitalPaymentechConstants.RecurringIndicators.First,
                mbType = OrbitalPaymentechConstants.ManagedBillingType.Recurring,
                mbOrderIdGenerationMethod = OrbitalPaymentechConstants.ManagedBillingOrderIDGenMethods.UseCustomerRefNum,
                mbRecurringNoEndDateFlag = model.Subscription!.EndsOn == null
                    ? OrbitalPaymentechConstants.RecurringHasNoEndDate.HasNoEndDate
                    : OrbitalPaymentechConstants.RecurringHasNoEndDate.HasEndDate,
                mbRecurringStartDate = model.Subscription.StartsOn.ToString("MMddyyyy"),
                mbRecurringEndDate = model.Subscription.EndsOn?.ToString("MMddyyyy"),
                mbRecurringMaxBillings = model.Subscription.BillingPeriodsTotal.ToString(),
                mbRecurringFrequency = model.Subscription.RepeatTypeName,
            };
            var response = PaymentechGatewayNewOrderRequest(e);
            var result = ToPaymentResponse(response, model.Amount);
             ...
             string? contextProfileName)
        {
            ...
            if (Contract.CheckInvalidID(SubscriptionVariantProductTypeID))
            {
                SubscriptionVariantProductTypeID = await Workflows.ProductTypes.ResolveWithAutoGenerateToIDAsync(
                        byID: null,
                        byKey: "VARIANT-SUBSCRIPTION",
                        byName: "Variant Subscription",
                        byDisplayName: "Variant Subscription",
                        model: null,
                        contextProfileName: contextProfileName)
                    .ConfigureAwait(false);
            }
                ...
                || salesOrderItem.ProductTypeID == SubscriptionVariantProductTypeID)
            {
                return await OnSubscriptionProductSalesOrderItemCreatedAsync(
                        ...
                        fee: salesOrderItem.UnitSoldPrice ?? salesOrderItem.UnitCorePrice,
                        contextProfileName: contextProfileName)
                    .ConfigureAwait(false);
            }
                ...
                || salesOrderItem.ProductTypeID == VariantKitProductTypeID)
            {
                ...
                var components1 = await Workflows.ProductKits.KitComponentBOMDownAsync(
                        id: salesOrderItem.ProductID!.Value,
                        contextProfileName: contextProfileName)
                    .ConfigureAwait(false);
        ...
        protected override async Task<CEFActionResponse> OnSubscriptionProductSalesOrderItemCreatedAsync(
            int productID,
            int? userID,
            int? accountID,
            int? invoiceID,
            ...
            string? contextProfileName)
        {
            ...
            var activeSubscriptionTypes = subscriptionProduct.ProductSubscriptionTypes!
                .Where(y => y.Active
                    && y.Slave!.Active
                    && y.SubscriptionTypeRepeatType!.Active);
            var price = fee
            ...
            if (Contract.CheckEmpty(activeSubscriptionTypes))
            {
