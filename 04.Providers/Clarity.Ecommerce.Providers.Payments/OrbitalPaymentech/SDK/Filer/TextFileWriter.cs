...
{
	/// <summary>
	/// Summary description for TextFileWriter.
	/// </summary>
	public class TextFileWriter : FrameworkBase, IFileWriter
	{
		...
		public TextFileWriter(string fileName, string responseType, IDispatcherFactory factory)
		{
		...
		{
			TestForNull(stream, Error.FileNotOpen, "The file is not open.", true);
		...
		{
			// TODO:  Add TextFileWriter.Write implementation
		}
		...
		{
			this.filename = filename;
			stream = new StreamWrapper(filename, true, true);
		}
		...
		{
			this.filename = filename;
			stream = new StreamWrapper(filename, false, false);
		}
		...
		public bool WriteRecord(byte[] record)
		{
			if (stream == null || !stream.IsOpen)
				CreateFile(this.filename, null, null);
