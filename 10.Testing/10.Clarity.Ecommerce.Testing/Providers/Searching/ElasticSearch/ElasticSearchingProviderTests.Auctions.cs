...
namespace Clarity.Ecommerce.Providers.Searching.ElasticSearch.Testing
{
    ...
    public partial class ElasticSearchingProviderTests
    {
        ...
        public Task LiveIndexRunnerForAuctionss()
        {
        ...
        [Fact]
        public async Task Verify_IndexingThenSuggestionsThenQueryingThenPurging_ForAuctions_CompletesSuccessfully()
        {
            // Arrange
            TestOutputHelper.WriteLine("Loading...");
            CEFConfigDictionary.Load();
            BaseModelMapper.Initialize();
            const string contextProfileName = "ElasticSearchingProviderTests|Verify_IndexingThenSuggestionsThenQueryingThenPurging_ForAuctions_CompletesSuccessfully";
            using (var childContainer = RegistryLoader.RootContainer.CreateChildContainer())
            {
                ...
                // Act/Assert
                await Verify_Indexing_For_Auctions_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Suggestions_For_Auctions_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Querying_For_Auctions_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Purging_For_Auctions_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
            }
            RegistryLoader.RemoveOverrideContainer(contextProfileName);
            TestOutputHelper.WriteLine("...Exiting");
        }

        private async Task Verify_Indexing_For_Auctions_CompletesSuccessfully(
        ...
        private async Task Verify_Suggestions_For_Auctions_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
            TestOutputHelper.WriteLine("Asking for suggestions");
            AuctionCatalogSearchForm form = new()
            {
                // PageSize = 4,
                // Page = 1,
                // PageFormat = "grid",
            ...
            // Act
            var result = await provider.SuggestionsAsync<AuctionCatalogSearchForm, AuctionSuggestResult>(
                    form,
                    ElasticSearchingProviderConfig.SearchingAuctionIndex,
                    contextProfileName)
                .ConfigureAwait(false);
        ...
        private async Task Verify_Querying_For_Auctions_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
            ...
            // Act
            var result = await provider.QueryAsync<AuctionSearchViewModel, AuctionCatalogSearchForm, AuctionIndexableModel>(
                    form,
                    ElasticSearchingProviderConfig.SearchingAuctionIndex,
                    contextProfileName)
                .ConfigureAwait(false);
        ...
        private async Task Verify_Purging_For_Auctions_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
