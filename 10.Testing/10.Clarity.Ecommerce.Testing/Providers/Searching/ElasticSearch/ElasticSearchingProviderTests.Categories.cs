...
namespace Clarity.Ecommerce.Providers.Searching.ElasticSearch.Testing
{
    ...
    public partial class ElasticSearchingProviderTests
    {
        ...
        public Task LiveIndexRunnerForCategories()
        {
        ...
        [Fact]
        public async Task Verify_IndexingThenSuggestionsThenQueryingThenPurging_ForCategories_CompletesSuccessfully()
        {
            // Arrange
            TestOutputHelper.WriteLine("Loading...");
            CEFConfigDictionary.Load();
            BaseModelMapper.Initialize();
            const string contextProfileName = "ElasticSearchingProviderTests|Verify_IndexingThenSuggestionsThenQueryingThenPurging_ForCategories_CompletesSuccessfully";
            using (var childContainer = RegistryLoader.RootContainer.CreateChildContainer())
            {
                ...
                // Act/Assert
                await Verify_Indexing_For_Categories_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Suggestions_For_Categories_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Querying_For_Categories_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Purging_For_Categories_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
            }
            RegistryLoader.RemoveOverrideContainer(contextProfileName);
            TestOutputHelper.WriteLine("...Exiting");
        }

        private async Task Verify_Indexing_For_Categories_CompletesSuccessfully(
        ...
        private async Task Verify_Suggestions_For_Categories_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
            TestOutputHelper.WriteLine("Asking for suggestions");
            CategoryCatalogSearchForm form = new()
            {
                // PageSize = 4,
                // Page = 1,
                // PageFormat = "grid",
            ...
            // Act
            var result = await provider.SuggestionsAsync<CategoryCatalogSearchForm, CategorySuggestResult>(
                    form,
                    ElasticSearchingProviderConfig.SearchingCategoryIndex,
                    contextProfileName)
                .ConfigureAwait(false);
        ...
        private async Task Verify_Querying_For_Categories_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
            ...
            // Act
            var result = await provider.QueryAsync<CategorySearchViewModel, CategoryCatalogSearchForm, CategoryIndexableModel>(
                    form,
                    ElasticSearchingProviderConfig.SearchingCategoryIndex,
                    contextProfileName)
                .ConfigureAwait(false);
            ...
            Assert.True(result.CategoriesTree?.Children?.Any() == null, "Facets (Aggregation Points) of type Categories should have come back with the Query");
            ...
            Assert.True(result.ProductIDs!.Count == 2, "Facets (Aggregation Points) of type Product IDs should have come back with the Query");
            ...
            Assert.True(result.StoreIDs!.Count == 4, "Facets (Aggregation Points) of type Store IDs should have come back with the Query");
            Assert.True(result.VendorIDs!.Count == 0, "Facets (Aggregation Points) of type Vendor IDs should have come back with the Query");
            TestOutputHelper.WriteLine("...Results given");
        }

        private async Task Verify_Purging_For_Categories_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
