...
namespace Clarity.Ecommerce.Providers.Searching.ElasticSearch.Testing
{
    ...
    public partial class ElasticSearchingProviderTests
    {
        ...
        public Task LiveIndexRunnerForVendors()
        {
        ...
        [Fact]
        public async Task Verify_IndexingThenSuggestionsThenQueryingThenPurging_ForVendors_CompletesSuccessfully()
        {
            // Arrange
            TestOutputHelper.WriteLine("Loading...");
            CEFConfigDictionary.Load();
            BaseModelMapper.Initialize();
            const string contextProfileName = "ElasticSearchingProviderTests|Verify_IndexingThenSuggestionsThenQueryingThenPurging_ForVendors_CompletesSuccessfully";
            using (var childContainer = RegistryLoader.RootContainer.CreateChildContainer())
            {
                ...
                // Act/Assert
                await Verify_Indexing_For_Vendors_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Suggestions_For_Vendors_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Querying_For_Vendors_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
                await Verify_Purging_For_Vendors_CompletesSuccessfully(provider, contextProfileName).ConfigureAwait(false);
            }
            RegistryLoader.RemoveOverrideContainer(contextProfileName);
            TestOutputHelper.WriteLine("...Exiting");
        }

        private async Task Verify_Indexing_For_Vendors_CompletesSuccessfully(
        ...
        private async Task Verify_Suggestions_For_Vendors_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
            TestOutputHelper.WriteLine("Asking for suggestions");
            VendorCatalogSearchForm form = new()
            {
                // PageSize = 4,
                // Page = 1,
                // PageFormat = "grid",
            ...
            // Act
            var result = await provider.SuggestionsAsync<VendorCatalogSearchForm, VendorSuggestResult>(
                    form,
                    ElasticSearchingProviderConfig.SearchingVendorIndex,
                    contextProfileName)
                .ConfigureAwait(false);
        ...
        private async Task Verify_Querying_For_Vendors_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
            ...
            // Act
            var result = await provider.QueryAsync<VendorSearchViewModel, VendorCatalogSearchForm, VendorIndexableModel>(
                    form,
                    ElasticSearchingProviderConfig.SearchingVendorIndex,
                    contextProfileName)
                .ConfigureAwait(false);
            ...
            Assert.True(result.ProductIDs!.Count == 4, "Facets (Aggregation Points) of type Product IDs should have come back with the Query");
            ...
            Assert.True(result.StoreIDs!.Count == 1, "Facets (Aggregation Points) of type Vendor IDs should have come back with the Query");
        ...
        private async Task Verify_Purging_For_Vendors_CompletesSuccessfully(
            ISearchingProviderBase provider,
            string contextProfileName)
        {
            // Arrange
