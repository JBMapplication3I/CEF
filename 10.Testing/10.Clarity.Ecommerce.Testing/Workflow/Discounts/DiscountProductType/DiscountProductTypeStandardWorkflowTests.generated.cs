...
namespace Clarity.Ecommerce.Workflow.Testing
{
    ...
    public abstract class Discounts_DiscountProductTypes_WorkflowTestsBase
        : AbstractBase<
            IDiscountProductType,
            DiscountProductType,
            IDiscountProductTypeModel,
            DiscountProductTypeModel,
            IDiscountProductTypeSearchModel,
            DiscountProductTypeSearchModel,
            DiscountProductTypeWorkflow>
    {
        protected Discounts_DiscountProductTypes_WorkflowTestsBase(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }

        protected override MockingSetup GetMockingSetupWithExistingDataForThisTable()
        {
            return new MockingSetup { DoDiscountProductTypeTable = true };
        }
        ...
        protected override MockingSetup GetMockingSetupWithExistingDataForThisTableAndExpandedTables()
        {
            ...
            {
                DoDiscountProductTypeTable = true,
                DoProductTypeTable = true,
            };
        }

        /// <inheritdoc/>
        protected override Mock<DbSet<DiscountProductType>>? GetMockSet(MockingSetup mockingSetup)
        {
            return mockingSetup.DiscountProductTypes;
        }

        /// <inheritdoc/>
        protected override List<Mock<DiscountProductType>>? GetRawSet(MockingSetup mockingSetup)
        {
            return mockingSetup.RawDiscountProductTypes;
        }

        /// <inheritdoc/>
        protected override async Task<DiscountProductTypeModel> GenNewModelAsync(MockingSetup mockingSetup, string contextProfileName)
        {
            return new DiscountProductTypeModel
            {
                Active = true,
                CustomKey = await GetCustomKeyAsync(mockingSetup, contextProfileName).ConfigureAwait(false),
                // Required Related Properties
                SlaveID = mockingSetup.RawProductTypes.First().Object.ID,
                // Optional Related Properties
            };
        }
    }

    [Trait("Category", "Workflows.Discounts.DiscountProductTypes.Standard")]
    public partial class Discounts_DiscountProductTypes_StandardWorkflowTests : Discounts_DiscountProductTypes_WorkflowTestsBase
    {
        public Discounts_DiscountProductTypes_StandardWorkflowTests(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }
