...
namespace Clarity.Ecommerce.Workflow.Testing
{
    ...
    public abstract class Tracking_PageViewEvents_WorkflowTestsBase
        : AbstractBase<
            IPageViewEvent,
            PageViewEvent,
            IPageViewEventModel,
            PageViewEventModel,
            IPageViewEventSearchModel,
            PageViewEventSearchModel,
            PageViewEventWorkflow>
    {
        protected Tracking_PageViewEvents_WorkflowTestsBase(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }

        protected override MockingSetup GetMockingSetupWithExistingDataForThisTable()
        {
            return new MockingSetup { DoPageViewEventTable = true };
        }
        ...
        protected override MockingSetup GetMockingSetupWithExistingDataForThisTableAndExpandedTables()
        {
            ...
            {
                DoEventTable = true,
                DoPageViewTable = true,
                DoPageViewEventTable = true,
            };
        }

        /// <inheritdoc/>
        protected override Mock<DbSet<PageViewEvent>>? GetMockSet(MockingSetup mockingSetup)
        {
            return mockingSetup.PageViewEvents;
        }

        /// <inheritdoc/>
        protected override List<Mock<PageViewEvent>>? GetRawSet(MockingSetup mockingSetup)
        {
            return mockingSetup.RawPageViewEvents;
        }

        /// <inheritdoc/>
        protected override async Task<PageViewEventModel> GenNewModelAsync(MockingSetup mockingSetup, string contextProfileName)
        {
            return new PageViewEventModel
            {
                Active = true,
                CustomKey = await GetCustomKeyAsync(mockingSetup, contextProfileName).ConfigureAwait(false),
                // Required Related Properties
                MasterID = mockingSetup.RawPageViews.First().Object.ID,
                SlaveID = mockingSetup.RawEvents.First().Object.ID,
                // Optional Related Properties
            };
        }
    }

    [Trait("Category", "Workflows.Tracking.PageViewEvents.Standard")]
    public partial class Tracking_PageViewEvents_StandardWorkflowTests : Tracking_PageViewEvents_WorkflowTestsBase
    {
        public Tracking_PageViewEvents_StandardWorkflowTests(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }
