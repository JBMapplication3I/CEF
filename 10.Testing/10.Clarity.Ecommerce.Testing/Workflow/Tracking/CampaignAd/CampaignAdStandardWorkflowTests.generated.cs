...
namespace Clarity.Ecommerce.Workflow.Testing
{
    ...
    public abstract class Tracking_CampaignAds_WorkflowTestsBase
        : AbstractBase<
            ICampaignAd,
            CampaignAd,
            ICampaignAdModel,
            CampaignAdModel,
            ICampaignAdSearchModel,
            CampaignAdSearchModel,
            CampaignAdWorkflow>
    {
        protected Tracking_CampaignAds_WorkflowTestsBase(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }

        protected override MockingSetup GetMockingSetupWithExistingDataForThisTable()
        {
            return new MockingSetup { DoCampaignAdTable = true };
        }
        ...
        protected override MockingSetup GetMockingSetupWithExistingDataForThisTableAndExpandedTables()
        {
            return new MockingSetup
            {
                ...
                DoCampaignAdTable = true,
            };
        }

        /// <inheritdoc/>
        protected override Mock<DbSet<CampaignAd>>? GetMockSet(MockingSetup mockingSetup)
        {
            return mockingSetup.CampaignAds;
        }

        /// <inheritdoc/>
        protected override List<Mock<CampaignAd>>? GetRawSet(MockingSetup mockingSetup)
        {
            return mockingSetup.RawCampaignAds;
        }

        /// <inheritdoc/>
        protected override async Task<CampaignAdModel> GenNewModelAsync(MockingSetup mockingSetup, string contextProfileName)
        {
            return new CampaignAdModel
            {
                Active = true,
                CustomKey = await GetCustomKeyAsync(mockingSetup, contextProfileName).ConfigureAwait(false),
                // Required Related Properties
                MasterID = mockingSetup.RawCampaigns.First().Object.ID,
                SlaveID = mockingSetup.RawAds.First().Object.ID,
                // Optional Related Properties
            };
        }
    }

    [Trait("Category", "Workflows.Tracking.CampaignAds.Standard")]
    public partial class Tracking_CampaignAds_StandardWorkflowTests : Tracking_CampaignAds_WorkflowTestsBase
    {
        public Tracking_CampaignAds_StandardWorkflowTests(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }
