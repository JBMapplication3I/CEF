...
namespace Clarity.Ecommerce.Workflow.Testing
{
    ...
    public abstract class Stores_StoreInventoryLocations_WorkflowTestsBase
        : AbstractBase<
            IStoreInventoryLocation,
            StoreInventoryLocation,
            IStoreInventoryLocationModel,
            StoreInventoryLocationModel,
            IStoreInventoryLocationSearchModel,
            StoreInventoryLocationSearchModel,
            StoreInventoryLocationWorkflow>
    {
        protected Stores_StoreInventoryLocations_WorkflowTestsBase(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }

        protected override MockingSetup GetMockingSetupWithExistingDataForThisTable()
        {
            return new MockingSetup { DoStoreInventoryLocationTable = true };
        }
        ...
        /// <inheritdoc/>
        protected override Mock<DbSet<StoreInventoryLocation>>? GetMockSet(MockingSetup mockingSetup)
        {
            return mockingSetup.StoreInventoryLocations;
        }

        /// <inheritdoc/>
        protected override List<Mock<StoreInventoryLocation>>? GetRawSet(MockingSetup mockingSetup)
        {
            return mockingSetup.RawStoreInventoryLocations;
        }

        /// <inheritdoc/>
        protected override async Task<StoreInventoryLocationModel> GenNewModelAsync(MockingSetup mockingSetup, string contextProfileName)
        {
            return new StoreInventoryLocationModel
            {
                Active = true,
                CustomKey = await GetCustomKeyAsync(mockingSetup, contextProfileName).ConfigureAwait(false),
                // Required Related Properties
                MasterID = mockingSetup.RawStores.First().Object.ID,
                SlaveID = mockingSetup.RawInventoryLocations.First().Object.ID,
                TypeID = mockingSetup.RawStoreInventoryLocationTypes.First().Object.ID,
                // Optional Related Properties
            };
        }
    }

    [Trait("Category", "Workflows.Stores.StoreInventoryLocations.Standard")]
    public partial class Stores_StoreInventoryLocations_StandardWorkflowTests : Stores_StoreInventoryLocations_WorkflowTestsBase
    {
        public Stores_StoreInventoryLocations_StandardWorkflowTests(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }
