...
namespace Clarity.Ecommerce.Workflow.Testing
{
    ...
    public abstract class Messaging_ConversationUsers_WorkflowTestsBase
        : AbstractBase<
            IConversationUser,
            ConversationUser,
            IConversationUserModel,
            ConversationUserModel,
            IConversationUserSearchModel,
            ConversationUserSearchModel,
            ConversationUserWorkflow>
    {
        protected Messaging_ConversationUsers_WorkflowTestsBase(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }

        protected override MockingSetup GetMockingSetupWithExistingDataForThisTable()
        {
            return new MockingSetup { DoConversationUserTable = true };
        }
        ...
        protected override MockingSetup GetMockingSetupWithExistingDataForThisTableAndExpandedTables()
        {
            ...
            {
                DoConversationTable = true,
                DoConversationUserTable = true,
                DoUserTable = true,
            };
        }

        /// <inheritdoc/>
        protected override Mock<DbSet<ConversationUser>>? GetMockSet(MockingSetup mockingSetup)
        {
            return mockingSetup.ConversationUsers;
        }

        /// <inheritdoc/>
        protected override List<Mock<ConversationUser>>? GetRawSet(MockingSetup mockingSetup)
        {
            return mockingSetup.RawConversationUsers;
        }

        /// <inheritdoc/>
        protected override async Task<ConversationUserModel> GenNewModelAsync(MockingSetup mockingSetup, string contextProfileName)
        {
            return new ConversationUserModel
            {
                Active = true,
                CustomKey = await GetCustomKeyAsync(mockingSetup, contextProfileName).ConfigureAwait(false),
                // Required Related Properties
                MasterID = mockingSetup.RawConversations.First().Object.ID,
    ...
    [Trait("Category", "Workflows.Messaging.ConversationUsers.Standard")]
    public partial class Messaging_ConversationUsers_StandardWorkflowTests : Messaging_ConversationUsers_WorkflowTestsBase
    {
        public Messaging_ConversationUsers_StandardWorkflowTests(Xunit.Abstractions.ITestOutputHelper testOutputHelper) : base(testOutputHelper) { }
