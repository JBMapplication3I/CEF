...
namespace Clarity.Ecommerce.Service
{
    ...
    public static class ServiceAppCommonStoreAdmin
    {
        ...
        public static void Configure(ServiceStackHost appHost, Container container)
        {
            ...
            ClarityEcommerceServiceBase.CurrentAPIKind = Enums.APIKind.StoreAdmin;
        ...
        private static Func<IRequest, IResponse, object, Task> StoresRequestFilter()
        {
            return (req, _, dto) => Task.Run(
                async () =>
                {
                    try
                    {
                        if (dto is not IAmFilterableByStoreSearchModel || IsAdminUrl(req.UrlReferrer))
                        {
                            return;
                        }
                        var session = req.GetSession();
                        ...
                        {
                            throw HttpError.Unauthorized("Must be authenticated to access");
                        }
                        ...
                        while (DateExtensions.GenDateTime < timeoutAt)
                        {
                            try
                            {
                                storeID = await StoreWorkflow.GetStoreIDForAdminPortalAsync(hostUrl, session.UserAuthId, null).ConfigureAwait(false);
                                break;
                            }
                            catch
                            {
