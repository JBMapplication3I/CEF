...
namespace Clarity.Ecommerce.Service
{
    ...
    public static class ServiceAppCommonFranchiseAdmin
    {
        ...
        public static void Configure(ServiceStackHost appHost, Container container)
        {
            ...
            ClarityEcommerceServiceBase.CurrentAPIKind = Enums.APIKind.FranchiseAdmin;
                ...
                async () =>
                {
                    try
                    {
                        ...
                        while (DateExtensions.GenDateTime < timeoutAt)
                        {
                            try
                            {
                                franchiseID = await FranchiseWorkflow.CheckExistsByHostUrlAsync(hostUrl, null).ConfigureAwait(false);
                                break;
                            }
                            catch
                            {
                        ...
                        {
                            throw HttpError.NotFound("Could not find a franchise tied to this URL");
                        }
                        req.Headers["CEF-Franchise-ID"] = franchiseID.ToString();
                        switch (dto)
                        {
                            case IAmFilterableByFranchiseSearchModel dto2:
                            {
                                dto2.FranchiseID = franchiseID;
                                break;
                            }
                            case ProductCatalogSearchForm dto3:
                            {
                                dto3.FranchiseID = franchiseID;
                                break;
                            }
                        }
                    }
