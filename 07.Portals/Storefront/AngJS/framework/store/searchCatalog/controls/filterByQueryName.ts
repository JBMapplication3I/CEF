module cef.store.searchCatalog.controls {
    class SearchCatalogFilterByQueryNameController extends core.TemplatedControllerBase {
        ...
        inputKeyPress(event: JQueryKeyEventObject): void {
            ...
            if (!this.cvSearchCatalogService.queryTerm && !this.cvSearchCatalogService.queryName) {
                return;
            }
    ...
    cefApp.directive("cefSearchCatalogFilterByQueryName", ($filter: ng.IFilterService): ng.IDirective => ({
        restrict: "EA",
        scope: {
            kind: "=?"            // 'products', 'stores', etc
        },
        templateUrl: $filter("corsLink")("/framework/store/searchCatalog/controls/filterByQueryName.html", "ui"),
        controller: SearchCatalogFilterByQueryNameController,
        controllerAs: "scfbqnCtrl",
