module cef.store.stores {
    class StoreDirectoryGaf extends core.TemplatedControllerBase {
        // Properties
        ...
        filter = {
            ...
            text: null,
            city: null,
            bySelctedOption: null,
            findSelctedOption: null,
        ...
        filterStores = (): void => {
            ...
            if (currenttime < 100 + this.lastRequestTime) {
                ...
                this.lastRequestTimeoutPromise = this.$timeout(this.filterStores, 1000);
                return;
            }
            if (this.lastRequestTimeoutPromise != null) {
            ...
            switch (this.filter.bySelctedOption) {
                case "Region": {
            ...
            switch (this.filter.findSelctedOption) {
                case "Sellers": {
        ...
        getFilterValues = (): void => {
            ...
            this.cvApi.categories.GetCategories({
                ...
                this.categories = this.categories.length > 0
                    ? this.categories.filter(category => (category.TypeID ? (category.TypeID === 1) : null))
                    : null;
                this.languages = r.data.Results;
                this.languages = this.languages.length > 0
                    ? this.languages.filter(language => (language.TypeID ? (language.TypeID === 2) : null))
                    : null;
            }).then(reason => this.consoleLog(reason));
            this.cvApi.stores.GetStoreTypes({ Active: true, AsListing: true }).then(r => {
        ...
        changeByOption = (): void => {
            var selected = this.filter.bySelctedOption;
    ...
    cefApp.directive("cefStoreDirectoryGaf", ($filter: ng.IFilterService): ng.IDirective => ({
        restrict: "EA",
        templateUrl: $filter("corsLink")("/framework/store/stores/storeDirectoryGaf.html", "ui"),
        controller: StoreDirectoryGaf,
        controllerAs: "storeDirectoryGaf"
    }));
}
