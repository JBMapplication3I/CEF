/**
 * @file framework/store/_services/cvInventoryService.ts
...
module cef.store.services {
    ...
    export class InventoryService {
        ...
        factoryAssignWithUOMs(product: api.ProductModel): api.ProductModel {
            ...
            product.readInventory = () => product["$_rawInventory"];
            // TODO: MemCache the results by product ID so we can avoid repeat calls for same product
            this.cvApi.providers.CalculateInventoryForMultipleUOMs(product.ID).then(r => {
                if (!r || !r.data || !r.data.ActionSucceeded) {
                    console.error(r && r.data);
                    return;
                }
                ...
                let nonUOMInventoryObject: api.CalculatedInventory = r.data.Result.find(x => {
                    if (!x.ProductUOM) return x;
                    else return null;
                })
                // Assign updated values
                /*
                 ...
                 * already.
                 */
                inventory.ProductID = nonUOMInventoryObject.ProductID;
                inventory.IsDiscontinued = nonUOMInventoryObject.IsDiscontinued;
                inventory.IsUnlimitedStock = nonUOMInventoryObject.IsUnlimitedStock;
                inventory.IsOutOfStock = nonUOMInventoryObject.IsOutOfStock;
                inventory.QuantityPresent = nonUOMInventoryObject.QuantityPresent;
                inventory.QuantityAllocated = nonUOMInventoryObject.QuantityAllocated;
                inventory.QuantityOnHand = nonUOMInventoryObject.QuantityOnHand;
                inventory.MaximumPurchaseQuantity = nonUOMInventoryObject.MaximumPurchaseQuantity;
                inventory.MaximumPurchaseQuantityIfPastPurchased = nonUOMInventoryObject.MaximumPurchaseQuantityIfPastPurchased;
                inventory.AllowBackOrder = nonUOMInventoryObject.AllowBackOrder;
                inventory.MaximumBackOrderPurchaseQuantity = nonUOMInventoryObject.MaximumBackOrderPurchaseQuantity;
                inventory.MaximumBackOrderPurchaseQuantityIfPastPurchased = nonUOMInventoryObject.MaximumBackOrderPurchaseQuantityIfPastPurchased;
                inventory.MaximumBackOrderPurchaseQuantityGlobal = nonUOMInventoryObject.MaximumBackOrderPurchaseQuantityGlobal;
                inventory.AllowPreSale = nonUOMInventoryObject.AllowPreSale;
                inventory.PreSellEndDate = nonUOMInventoryObject.PreSellEndDate;
                inventory.QuantityPreSellable = nonUOMInventoryObject.QuantityPreSellable;
                inventory.QuantityPreSold = nonUOMInventoryObject.QuantityPreSold;
                inventory.MaximumPrePurchaseQuantity = nonUOMInventoryObject.MaximumPrePurchaseQuantity;
                inventory.MaximumPrePurchaseQuantityIfPastPurchased = nonUOMInventoryObject.MaximumPrePurchaseQuantityIfPastPurchased;
                inventory.MaximumPrePurchaseQuantityGlobal = nonUOMInventoryObject.MaximumPrePurchaseQuantityGlobal;
                inventory.RelevantLocations = nonUOMInventoryObject.RelevantLocations;
                // Assign calculated values
                /*
                // PILS Stock (TODO: Rework this with variables for store only stock)
                if (r.data.Result.RelevantLocations && r.data.Result.RelevantLocations.length > 0
                ...
                */
                ...
                inventory.loading = false;
                product["$_rawInventory"] = inventory;
                product["$_rawInventoryUOMs"] = r.data.Result;
            });
            return product;
        }
        // Constructor
