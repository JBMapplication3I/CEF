/**
 * @file framework/store/_services/cvLanguageService.ts
...
module cef.store.services {
    ...
    export class LanguageService implements ILanguageService {
        ...
        private setCurrentLanguageByModel(language: api.LanguageModel): void {
            ...
            this.$translate.use(language.CustomKey)
                .finally(() => this.$rootScope.$broadcast(
                    this.cvServiceStrings.events.lang.changeFinished,
                    this.currentLanguage.CustomKey));
        }
        changeToLanguagePromise(key: string): ng.IPromise<boolean> {
            return this.$q<boolean>((resolve, reject) => {
                ...
                this.getLanguageByKey(key).then(r => {
                    ...
                    this.$translateAsyncPartialLoaderStore.addPart(this.currentLanguage.Locale, 1, "lang={lang}&part={part}");
        ...
        getAvailableLanguages(): ng.IHttpPromise<api.LanguagePagedResults> {
            return this.cvApi.globalization.GetLanguages();
        }
        getLanguageByKey(key: string): ng.IHttpPromise<api.LanguageModel> {
            if (!key) { return null; }
        ...
        constructor(
                ...
                private readonly $translateAsyncPartialLoaderStore: ng.translate.ITranslatePartialLoaderService,
            ...
            const construct = () => {
                ...
                // let go: string = null;
                this.getCurrentLanguagePromise(true).then(lang => {
                    if (!this.cefConfig.featureSet.brands.enabled) {
                        return;
                    }
                    this.$q.all([
                        ////this.cvApi.brands.GetBrands({ Active: true, AsListing: true }),
                        this.cvApi.brands.GetBrandLanguages({ Active: true, AsListing: true }),
                        ...
                        const brandLanguages    = rarr[++index].data.Results as api.BrandLanguageModel[];
                        ...
                        for (let i = 0; i < siteDomains.length; i++) {
                            ...
                            if (brandLanguages && brandLanguages.length)
                            {
                                const filtered = brandLanguages.filter(x => x.MasterID === bID);
                                if (!filtered.length) {
                                    continue;
                                }
                                const primaryKey = filtered[0].SlaveKey.toUpperCase();
                                if (lang.CustomKey !== primaryKey) {
                                    this.changeToLanguagePromise(primaryKey).then(() => void 0);
                                    return;
                                }
                            }
                        }
