/**
 * @file framework/store/quotes/steps/ssplitShipping/step.splitShipping.api.ts
...
module cef.store.quotes.steps.splitShipping {
    ...
    export class SubmitQuoteStepSplitShipping extends SubmitQuoteStep {
        // Properties
        get name(): string { return this.cvServiceStrings.submitQuote.steps.splitShipping; }
        ...
        canEnable(cart: api.CartModel): ng.IPromise<boolean> {
            ...
            if (!this.name) {
                this.consoleDebug(`SubmitQuoteStepSplitShipping.canEnable(cart: "${cart && cart.TypeName}") No name yet`);
                return this.$q.reject(`SubmitQuoteStepShipping.canEnable(cart: "${cart && cart.TypeName}") does not have a name yet`);
            }
            if (!this.cefConfig.submitQuote.sections[this.name] ||
                !this.cefConfig.submitQuote.sections[this.name].show) {
                this.consoleDebug(`SubmitQuoteStepSplitShipping.canEnable(cart: "${cart && cart.TypeName}") Section set to not show`);
                return this.$q.resolve(false);
            }
            if (cart.NothingToShip) {
                this.consoleDebug(`SubmitQuoteStepSplitShipping.canEnable(cart: "${cart && cart.TypeName}") Cart does not contain any shippable items`);
                return this.$q.resolve(false);
            }
        ...
        initialize(cart: api.CartModel): ng.IPromise<boolean> {
            ...
            return this.$q((resolve, __) => {
                ...
                this.cvAuthenticationService.preAuth().finally(() => {
                    ...
                    this.cvAddressBookService.refreshContactChecks(false, "submitQuote.steps.splitShipping.service").finally(() => {
        ...
        validate(cart: api.CartModel): ng.IPromise<boolean> {
            this.consoleDebug(`SubmitQuoteStepSplitShipping.validate(cart: "${cart && cart.TypeName}")`);
            if (!this.name) {
                this.invalidByAddress = true;
                return this.$q.reject(`SubmitQuoteStepSplitShipping.validate(cart: "${cart && cart.TypeName}") doesn't have a name yet`);
            }
                ...
                this.$q.all(_.flatten(cart.SalesItems.map(x => x.Targets || [])).map(y => {
                    ...
                    rarr.map(x => x.data).forEach(x => {
                        ...
                        if (x.Address.SerializableAttributes &&
                            _.some(Object.keys(x.Address.SerializableAttributes),
                    ...
                    this.$q.all(_.uniqBy(addressesToCheck, x => x.ID)
                            .map(x => this.cvAddressBookService.validate(x, false)))
        ...
        submit(cartType: string): ng.IPromise<boolean> {
            this.consoleDebug(`SubmitQuoteStepSplitShipping.submit(cartType: "${cartType}")`);
            return this.$q((resolve, reject) => {
                this.showShippingSubtotal = true;
                // NOTE: This is intentionally different from cvPurchaseService
                // as we don't have a payment pane to go to next
        ...
        constructor(
            ...
            this.consoleDebug(`SubmitQuoteStepSplitShipping.ctor()`);
        }
    }
}
