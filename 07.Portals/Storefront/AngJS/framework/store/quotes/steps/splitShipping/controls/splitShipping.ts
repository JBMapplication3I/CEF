module cef.store.quotes.steps.splitShipping.controls {
        ...
        */
        ...
        submit(): void {
            ...
            this.cvCartService.updateCartTargets(this.type, updatedItems).then(() => {
                const dto: api.AnalyzeCurrentQuoteCartToTargetQuoteCartsDto = {
                ...
                this.cvApi.providers.AnalyzeCurrentQuoteCartToTargetQuoteCarts(dto).then(r => {
        ...
        constructor(
            ...
            const unbind1 = $scope.$watch(() => this.cartItems, newValue => this.initializeSalesItems(newValue));
            const unbind2 = $scope.$watch(() => this.contacts, newValue => this.generateAddressOptions(newValue));
    ...
    cefApp.directive("cefSubmitQuoteSplitShipping", ($filter: ng.IFilterService): ng.IDirective => ({
        restrict: "A",
        scope: {
            // Inherited
            type: "=",
        ...
        templateUrl: $filter("corsLink")("/framework/store/quotes/steps/splitShipping/controls/splitShipping.html", "ui"),
