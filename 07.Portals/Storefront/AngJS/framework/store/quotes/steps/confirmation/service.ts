/**
 * @file framework/store/quotes/steps/confirmation/api.ts
 ...
 */
module cef.store.quotes.steps.confirmation {
    // This is part of the Service
    export class SubmitQuoteStepConfirmation extends SubmitQuoteStep {
        // Properties
        get name(): string {
            return this.cvServiceStrings.submitQuote.steps.confirmation;
        }
        ...
        canEnable(cart: api.CartModel): ng.IPromise<boolean> {
            const debug = `SubmitQuoteStepConfirmation.canEnable(cart: "${cart && cart.TypeName}")`;
            ...
            if (cart.Totals.Total <= 0
                ...
                && this.cefConfig.submitQuote.sections[this.name].showConditionally) {
        ...
        initialize(cart: api.CartModel): ng.IPromise<boolean> {
            const debugMsg = `SubmitQuoteStepConfirmation.initialize(cart: "${cart && cart.TypeName}")`;
        ...
        validate(cart: api.CartModel): ng.IPromise<boolean> {
            this.consoleDebug(`SubmitQuoteStepConfirmation.validate(cart: "${cart && cart.TypeName}")`);
            if (!this.name) {
                const msg = `SubmitQuoteStepConfirmation.validate(cart: "${cart && cart.TypeName}") doesn't have a name yet`;
                this.consoleLog(msg);
                this.invalid = true;
        ...
        submit(cartType: string): ng.IPromise<boolean> {
            this.consoleDebug(`SubmitQuoteStepConfirmation.submit(cartType: "${cartType}")`);
            return this.$q.resolve(this.cvSubmitQuoteService.finalize(cartType));
        }
        // Events
        // <None>
        // Constructor
        constructor(
            ...
            this.consoleDebug(`SubmitQuoteStepConfirmation.ctor()`);
        }
    }
}
