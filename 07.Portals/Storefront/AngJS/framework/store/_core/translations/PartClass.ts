module cef.store.core.translations {
    export class Part implements IPart {
        ...
        getTable(
            ...
            let builtPath = "/lib/cef/js/i18n/" + keyStartsWith + lang + ".json";
            if ($window.location && $window.location.host) {
                builtPath += '?brand=' + $window.location.host.split('.')[0];
            }
            const fetchData = (): ng.IPromise<ng.translate.ITranslationTable> =>
                $q((resolve, reject) => $http.get<cefalt.store.Dictionary<string>>(
                    $filter("corsLink")(builtPath, "ui", undefined, undefined, {"cdv": "2"}),
