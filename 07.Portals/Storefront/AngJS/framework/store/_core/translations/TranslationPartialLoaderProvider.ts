function $translateAsyncPartialLoaderStore() {
    'use strict';
    var parts: { [name: string]: cef.store.core.translations.IPart } = {};
    ...
    function getPrioritizedParts(): cef.store.core.translations.IPart[] {
        var prioritizedParts: cef.store.core.translations.IPart[] = [];
    ...
    this.addPart = function (name: string, priority: number = 0, urlTemplate: string | ng.translate.IUrlTemplateFunc = "lang={lang}&part={part}") {
        ...
        if (!hasPart(name)) {
            parts[name] = new cef.store.core.translations.Part(name, priority, urlTemplate);
        }
    ...
    this.setPart = function (lang: string, part: string, table: ng.translate.ITranslationTable) {
        ...
        if (!hasPart(part)) {
            parts[part] = new cef.store.core.translations.Part(part);
    ...
    this.$get = function (
         ...
         */
        const service: any = (options: cef.store.core.translations.IPartialTranslationLoaderOptions): ng.IPromise<ng.translate.ITranslationTable> => {
    ...
    $translateAsyncPartialLoaderStore["displayName"] = "$translateAsyncPartialLoaderStore";
...
angular.module("pascalprecht.translate")
    /**
     * @ngdoc object
     * @name pascalprecht.translate.$translateAsyncPartialLoaderStore
     *
     * @desc
     * By using a $translateAsyncPartialLoaderStore you can configure a list of a needed
     ...
     */
    .provider("$translateAsyncPartialLoaderStore", $translateAsyncPartialLoaderStore as any);
