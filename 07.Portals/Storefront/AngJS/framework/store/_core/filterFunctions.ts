module cef.store.core {
    ...
    export const decodeHtmlFilterFn = () => textString => {
        return window.decodeURIComponent(textString);
    };
    ...
    export const absValueFilterFn = () => (value: number) => {
        return Math.abs(value);
    }

    export const uniqueValueFilterFn = () => (arr: any[]) => {
        return arr.filter((item, i, ar) => ar.indexOf(item) === i);
    }

    export const splitBySpaceValueFilterFn = () => (value: string) => {
        return value.split(" ")[0];
    }

    export const splitByCommaValueFilterFn = () => (value: string) => {
        if (value == null) {
            return [];
        }
        return value.split(",");
    ...
    export const bracketWrapFn = () => (value: string) => {
        if (!value) {
            return '';
        }
        return `[${value}]`;
