/**
 * @file framework/store/_core/grid.ts
...
module cef.store.core {
    ...
    class CVGridController extends core.TemplatedControllerBase {
        ...
        // NOTE: This must remain an arrow function
        deactivate = (id: number): void  => {
            this.cvConfirmModalFactory(this.$translate("ui.storefront.grid.ConfirmDeactivate.Message")).then(result => {
                if (!result) { return; }
        ...
        // NOTE: This must remain an arrow function
        reactivate = (id: number): void => {
            this.cvConfirmModalFactory(this.$translate("ui.storefront.grid.ConfirmReactivate.Message")).then(result => {
                if (!result) { return; }
        ...
        // NOTE: This must remain an arrow function
        delete = (id: number): void => {
            this.cvConfirmModalFactory(this.$translate("ui.storefront.grid.ConfirmDelete.Message")).then(result => {
                if (!result) { return; }
        ...
        onDataBound = (event: kendo.ui.GridDataBoundEvent): boolean => {
            if (!this.autoColumnWidths) {
                // Wait for angular digest cycle to autofit columns (after ng-binding)
                this.$timeout(() => {
                    event.sender.autoFitColumn(this.gridColumns.length - 1);
                }, 250);
                return true;
            }
            // Wait for angular digest cycle to autofit columns (after ng-binding)
    ...
    const cvGridLinkFnInner: ng.IDirectiveLinkFn = (
        ...
        const handleStatusIDColumn = (col: ICVGridColumnDefinition): { hasTemplate: boolean, fullTemplateAppend: string } => {
            ...
            col.template = `<script type="text/kendo-template"><span ng-bind="dataItem.${col.field} | statusIDToText : '${col.preloadStatusesKind}' : ['DisplayName','Translated','Name','CustomKey']"></span></script>\r\n`;
            if (!col.hidden) {
                result.fullTemplateAppend += `<span ng-bind="dataItem.${col.field} | statusIDToText : '${col.preloadStatusesKind}' : ['DisplayName','Translated','Name','CustomKey']"></span>\r\n`;
                result.hasTemplate = true;
            }
            return result;
        };
        ...
        controller.$q.all(promises).finally(() => {
            ...
            if (!controller.autoColumnWidths) {
                const noWidthSpecifiedColumnsCount = _.filter(controller.gridColumns, column => column.width === "[%]").length;
                if (noWidthSpecifiedColumnsCount > 0) {
                    const w = 100 / noWidthSpecifiedColumnsCount;
                    const ws = `${w}%`;
                    controller.gridColumns.forEach(column => {
                        if (column.width === "[%]") {
                            column.width = ws;
                        }
                    });
            ...
            if (controller.onDelete != null
                ...
                if (controller.detailUrl != null) { // Use in Storefront (no angular router)
                    templateButtons.push({
                        ...
                        textKey: "ui.storefront.common.View",
                ...
                commandColumn.template = dataItem => {
                    ...
                    if (controller.detailState != null) { // Use in Storefront's User Dashboard (uses Angular UI Router)
                        templateButtons1.push({
                            ...
                            permission: controller.hasPermissionsBase
                                ...
                                : "",
                            action: `ui-sref="${controller.detailState.split(",")[0]}({ ID: '${id}', Name: '${id}' })"`,
                            show: ""
                        });
                    }
                    angular.forEach(templateButtons, button => {
                        ...
                        tempHtml += `    <span data-translate="${button.textKey}"></span>\r\n`;
                        tempHtml += `</button>\r\n`;
                    });
                ...
                if (controller.detailState != null) { // Use in Storefront's User Dashboard (uses Angular UI Router)
                    const id1 = controller.detailState.replace(`[${idField}]`, `'#= encodeStore(${idField}) #'`).split(",")[1];
                    templateButtons.push({
                        color: "btn-outline-primary",
                        buttonIndex: controller.buttonIndex,
                        textKey: "ui.storefront.common.View",
                        permission: controller.hasPermissionsBase
                            ? `ng-if="$parent.$root.cvSecurityService.hasPermission('${controller.permissionsBase}.View')"`
                            : "",
                        action: `ui-sref="${controller.detailState.split(",")[0]}({ ID: ${id1}, Name: ${id1} })"`,
                        show: "",
                        addedHtml: ""
                    });
                }
                if (templateButtons.length <= 1) {
                    templateButtons.forEach((button, index) => {
                        ...
                        rawTemplateHtml2 += `    ${button.addedHtml}`;
                    ...
                    rawTemplateHtml2 += `   translate-attr="{ title: 'ui.storefront.common.Action.Plural',\r\n`;
                    rawTemplateHtml2 += `                     'aria-label': 'ui.storefront.common.Action.Plural' }"\r\n`;
                    rawTemplateHtml2 += `   ng-click="gridCtrl.openMenu(dataItem.ID, dataItem)">\r\n`;
                    rawTemplateHtml2 += `    <i class="far fa-ellipsis-h"></i>\r\n`;
                    rawTemplateHtml2 += `    <span class="sr-only" data-translate="ui.storefront.common.Action.Plural"></span>\r\n`;
                    rawTemplateHtml2 += `</a>\r\n`;
                    ...
                    templateButtons.forEach(button => {
                        ...
                        rawTemplateHtml2 += `    <a class="p-1 text-regular d-block nowrap"\r\n`;
    ...
    export const cvGridDirectiveFn = ($filter: ng.IFilterService): ng.IDirective => ({
        ...
        templateUrl: $filter("corsLink")("/framework/store/_core/grid.html", "ui"),
        scope: {
            editUrl: "@?",
            editState: "@?",
            detailUrl: "@?",
