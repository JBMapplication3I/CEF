module cef.store.userDashboard.controls.shoppingLists {
    class ShoppingListDetailController extends core.TemplatedControllerBase {
        // Properties
            ...
            return this.$q((resolve, reject) => {
                this.cvAuthenticationService.preAuth().finally(() => {
                    ...
                    this.cvApi.shopping.GetCartTypeForCurrentUser({
                        ...
                        resolve(this.cartPromise = this.cvCartService.loadCart(this.cartName, true, "shoppingListDetail.load"));
        ...
        updateName(): void {
            ...
            this.cartType.DisplayName = this.newCartName;
            this.cvApi.shopping.UpdateCartType(this.cartType).finally(() => {
                this.$state.go("userDashboard.shoppingLists.detail", { Name: this.cartType.Name });
                this.cancelEdit();
            });
        }
        cancelEdit(): void {
            this.newCartName = this.cartType.DisplayName;
            this.toggleEdit();
        }
        toggleEdit(): void {
        ...
        deleteList(): void {
            ...
            .finally(() => (this.cartType.ID
                            ...
                            : this.$q.resolve())
                .finally(() => this.$state.go("userDashboard.shoppingLists.list")));
        }
        ...
        shareWithAccountUsers(): void {
            let messages = [];
            let success = true;
            let productQuantities = { }
            if (this.cartItems) {
                this.cartItems.forEach(x => {
                    productQuantities[x.ProductID] = x.Quantity
                })
            }
            this.setRunning();
            this.cvApi.shopping.ShareShoppingCart(({
                ProductQuantities: productQuantities,
                ShoppingListTypeID: this.cartType.ID,
                IncludedUsers: [ -1 ],
            })).then(r => {
                if (r?.data?.ActionSucceeded) {
                    ...
                    success = false;
                    messages.push(...r?.data?.Messages);
                    this.finishRunning(true, 'An error occurred when attempting to share your shopping list.');
                }
            }).catch(err => {
                success = false;
                messages = [ 'An error occurred when attempting to share your shopping list.', err ]
                this.finishRunning(true, messages[0]);
            }).finally(() => {
                let translationMessage = success
                    ? "This shopping cart has been successfully shared with users on this account."
                    : "An error occurred while attempting to share this shopping list.";
                this.$translate(success
                    ? "ui.storefront.userDashboard2.controls.shoppingListDetail.ShareSuccess"
                    : "ui.storefront.userDashboard2.controls.shoppingListDetail.ShareFail").then(r => {
                    translationMessage = r;
                }).finally(() => this.cvMessageModalFactory(translationMessage));
            });
        }
        ...
        onCartLoaded_GetRelatedProducts = ($event: ng.IAngularEvent, cartType: string): void => {
            this.consoleDebug("shoppingListDetail.onCartLoaded");
    ...
    cefApp.directive("cefShoppingListDetail", ($filter: ng.IFilterService): ng.IDirective => ({
        restrict: "A",
        templateUrl: $filter("corsLink")("/framework/store/userDashboard/controls/shoppingLists/shoppingLists.detail.html", "ui"),
        controller: ShoppingListDetailController,
        controllerAs: "sldCtrl"
    }));
}
