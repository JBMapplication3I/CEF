...
/**
 * @file framework/store/userDashboard/controls/wallet/walletEditorControllerBase.ts
...
module cef.store.userDashboard.controls.wallet {
    export class WalletEditorControllerBase extends core.TemplatedControllerBase {
        ...
        add(): void {
            ...
            this.setRunning();
            this.cvWalletService.addEntry(this.entry)
        ...
        update(): void {
            ...
            this.setRunning();
            this.cvWalletService.updateEntry(this.entry)
                .then(() => this.finishRunning())
                .catch(reason => this.finishRunning(true, reason));
        }
        deactivate(): void {
            ...
            this.cvConfirmModalFactory(this.$translate(this.confirmDeactivateKey)).then(confirmed => {
                ...
                this.setRunning();
                this.cvWalletService.deleteEntry({ id: this.entry.ID }).then(success => {
                    if (!success) {
                        this.finishRunning(
                            true,
