/**
 * @file framework/admin/widgets/associations/_T4/salesReturn.to.SalesReturnPayments.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Return to SalesReturnPayments class
...
module cef.admin.widgets.associators.Returning {
	class AssociatorForSalesReturnToSalesReturnPaymentsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesReturnModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "SalesReturnPayments";
		paging: core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>;
		assigned: core.ServerSidePaging<api.SalesReturnPaymentModel, api.SalesReturnPaymentPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>(
					...
					this.cvApi.payments.GetPayments, 8, `salesReturn-${this.record && this.record.ID}.to.SalesReturnPayments.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesReturn-${this.record && this.record.ID}.to.SalesReturnPayments.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesReturnPaymentModel, api.SalesReturnPaymentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetSalesReturnPayments, 8, `salesReturn-${this.record && this.record.ID}.to.SalesReturnPayments.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesReturn-${this.record && this.record.ID}.to.SalesReturnPayments.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesReturn-${this.record && this.record.ID}.to.SalesReturnPayments.dupe-check`
			};
			this.cvApi.payments.GetSalesReturnPayments(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.payments.CreateSalesReturnPayment(<api.SalesReturnPaymentModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesReturnPaymentModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.payments.DeactivateSalesReturnPaymentByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesReturnToSalesReturnPaymentsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesReturns.to.SalesReturnPayments.html", "ui"),
		controller: AssociatorForSalesReturnToSalesReturnPaymentsController,
		controllerAs: "asCtrl",
