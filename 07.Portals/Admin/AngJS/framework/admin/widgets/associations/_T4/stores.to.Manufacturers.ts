/**
 * @file framework/admin/widgets/associations/_T4/store.to.Manufacturers.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to Manufacturers class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToManufacturersController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreManufacturerModel, api.StoreManufacturerPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>(
					...
					this.cvApi.manufacturers.GetManufacturers, 8, `store-${this.record && this.record.ID}.to.Manufacturers.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.Manufacturers.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreManufacturerModel, api.StoreManufacturerPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreManufacturers, 8, `store-${this.record && this.record.ID}.to.Manufacturers.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.Manufacturers.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.Manufacturers.dupe-check`
			};
			this.cvApi.stores.GetStoreManufacturers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreManufacturer(<api.StoreManufacturerModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreManufacturerModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreManufacturerByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToManufacturersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.Manufacturers.html", "ui"),
		controller: AssociatorForStoreToManufacturersController,
		controllerAs: "asCtrl",
