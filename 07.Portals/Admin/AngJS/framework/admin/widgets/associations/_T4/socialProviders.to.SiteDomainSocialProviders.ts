/**
 * @file framework/admin/widgets/associations/_T4/socialProvider.to.SiteDomainSocialProviders.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Social Provider to SiteDomainSocialProviders class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForSocialProviderToSiteDomainSocialProvidersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SocialProviderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "SiteDomainSocialProviders";
		paging: core.ServerSidePaging<api.SocialProviderModel, api.SocialProviderPagedResults>;
		assigned: core.ServerSidePaging<api.SiteDomainSocialProviderModel, api.SiteDomainSocialProviderPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.SocialProviderModel, api.SocialProviderPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetSocialProviders, 8, `socialProvider-${this.record && this.record.ID}.to.SiteDomainSocialProviders.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `socialProvider-${this.record && this.record.ID}.to.SiteDomainSocialProviders.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SiteDomainSocialProviderModel, api.SiteDomainSocialProviderPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetSiteDomainSocialProviders, 8, `socialProvider-${this.record && this.record.ID}.to.SiteDomainSocialProviders.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `socialProvider-${this.record && this.record.ID}.to.SiteDomainSocialProviders.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `socialProvider-${this.record && this.record.ID}.to.SiteDomainSocialProviders.dupe-check`
			};
			this.cvApi.stores.GetSiteDomainSocialProviders(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateSiteDomainSocialProvider(<api.SiteDomainSocialProviderModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SiteDomainSocialProviderModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateSiteDomainSocialProviderByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSocialProviderToSiteDomainSocialProvidersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/socialProviders.to.SiteDomainSocialProviders.html", "ui"),
		controller: AssociatorForSocialProviderToSiteDomainSocialProvidersController,
		controllerAs: "asCtrl",
