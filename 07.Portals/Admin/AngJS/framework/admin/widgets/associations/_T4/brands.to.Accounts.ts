/**
 * @file framework/admin/widgets/associations/_T4/brand.to.Accounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Brand to Accounts class
...
module cef.admin.widgets.associators.Brands {
	class AssociatorForBrandToAccountsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `brand-${this.record && this.record.ID}.to.Accounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `brand-${this.record && this.record.ID}.to.Accounts.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.BrandAccountModel, api.BrandAccountPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandAccounts, 8, `brand-${this.record && this.record.ID}.to.Accounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `brand-${this.record && this.record.ID}.to.Accounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `brand-${this.record && this.record.ID}.to.Accounts.dupe-check`
			};
	...
	adminApp.directive("cefBrandToAccountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/brands.to.Accounts.html", "ui"),
		controller: AssociatorForBrandToAccountsController,
		controllerAs: "asCtrl",
