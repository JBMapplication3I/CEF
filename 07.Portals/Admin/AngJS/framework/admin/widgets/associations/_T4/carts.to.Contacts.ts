/**
 * @file framework/admin/widgets/associations/_T4/cart.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Cart to Contacts class
...
module cef.admin.widgets.associators.Shopping {
	class AssociatorForCartToContactsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.CartModel;
		...
		assigned: core.ServerSidePaging<api.CartContactModel, api.CartContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `cart-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `cart-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CartContactModel, api.CartContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.shopping.GetCartContacts, 8, `cart-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `cart-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `cart-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.shopping.GetCartContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.shopping.CreateCartContact(<api.CartContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CartContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.shopping.DeactivateCartContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCartToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/carts.to.Contacts.html", "ui"),
		controller: AssociatorForCartToContactsController,
		controllerAs: "asCtrl",
