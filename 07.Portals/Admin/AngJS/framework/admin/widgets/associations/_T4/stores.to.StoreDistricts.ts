/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreDistricts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreDistricts class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreDistrictsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreDistricts";
		paging: core.ServerSidePaging<api.DistrictModel, api.DistrictPagedResults>;
		assigned: core.ServerSidePaging<api.StoreDistrictModel, api.StoreDistrictPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.DistrictModel, api.DistrictPagedResults>(
					...
					this.cvApi.geography.GetDistricts, 8, `store-${this.record && this.record.ID}.to.StoreDistricts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreDistricts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreDistrictModel, api.StoreDistrictPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreDistricts, 8, `store-${this.record && this.record.ID}.to.StoreDistricts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreDistricts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreDistricts.dupe-check`
			};
			this.cvApi.stores.GetStoreDistricts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreDistrict(<api.StoreDistrictModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreDistrictModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreDistrictByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToStoreDistrictsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreDistricts.html", "ui"),
		controller: AssociatorForStoreToStoreDistrictsController,
		controllerAs: "asCtrl",
