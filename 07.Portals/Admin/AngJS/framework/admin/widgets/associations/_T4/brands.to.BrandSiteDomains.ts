/**
 * @file framework/admin/widgets/associations/_T4/brand.to.BrandSiteDomains.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Brand to BrandSiteDomains class
...
module cef.admin.widgets.associators.Brands {
	class AssociatorForBrandToBrandSiteDomainsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.BrandModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "BrandSiteDomains";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SiteDomainModel, api.SiteDomainPagedResults>(
					...
					this.cvApi.stores.GetSiteDomains, 8, `brand-${this.record && this.record.ID}.to.BrandSiteDomains.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `brand-${this.record && this.record.ID}.to.BrandSiteDomains.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.BrandSiteDomainModel, api.BrandSiteDomainPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandSiteDomains, 8, `brand-${this.record && this.record.ID}.to.BrandSiteDomains.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `brand-${this.record && this.record.ID}.to.BrandSiteDomains.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `brand-${this.record && this.record.ID}.to.BrandSiteDomains.dupe-check`
			};
	...
	adminApp.directive("cefBrandToBrandSiteDomainsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/brands.to.BrandSiteDomains.html", "ui"),
		controller: AssociatorForBrandToBrandSiteDomainsController,
		controllerAs: "asCtrl",
