/**
 * @file framework/admin/widgets/associations/_T4/inventoryLocation.to.Regions.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Inventory Location to Regions class
...
module cef.admin.widgets.associators.Inventory {
	class AssociatorForInventoryLocationToRegionsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.InventoryLocationModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Regions";
		paging: core.ServerSidePaging<api.RegionModel, api.RegionPagedResults>;
		assigned: core.ServerSidePaging<api.InventoryLocationRegionModel, api.InventoryLocationRegionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.RegionModel, api.RegionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetRegions, 8, `inventoryLocation-${this.record && this.record.ID}.to.Regions.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Regions.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.InventoryLocationRegionModel, api.InventoryLocationRegionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.inventory.GetInventoryLocationRegions, 8, `inventoryLocation-${this.record && this.record.ID}.to.Regions.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Regions.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `inventoryLocation-${this.record && this.record.ID}.to.Regions.dupe-check`
			};
			this.cvApi.inventory.GetInventoryLocationRegions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.inventory.CreateInventoryLocationRegion(<api.InventoryLocationRegionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.InventoryLocationRegionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.inventory.DeactivateInventoryLocationRegionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefInventoryLocationToRegionsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/inventoryLocations.to.Regions.html", "ui"),
		controller: AssociatorForInventoryLocationToRegionsController,
		controllerAs: "asCtrl",
