/**
 * @file framework/admin/widgets/associations/_T4/account.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to StoredFiles class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AccountFileModel, api.AccountFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `account-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AccountFileModel, api.AccountFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountFiles, 8, `account-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.accounts.GetAccountFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.accounts.CreateAccountFile(<api.AccountFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AccountFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.accounts.DeactivateAccountFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.StoredFiles.html", "ui"),
		controller: AssociatorForAccountToStoredFilesController,
		controllerAs: "asCtrl",
