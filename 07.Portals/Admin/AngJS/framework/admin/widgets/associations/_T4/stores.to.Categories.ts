/**
 * @file framework/admin/widgets/associations/_T4/store.to.Categories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to Categories class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToCategoriesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreCategoryModel, api.StoreCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `store-${this.record && this.record.ID}.to.Categories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.Categories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreCategoryModel, api.StoreCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreCategories, 8, `store-${this.record && this.record.ID}.to.Categories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.Categories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.Categories.dupe-check`
			};
			this.cvApi.stores.GetStoreCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreCategory(<api.StoreCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.Categories.html", "ui"),
		controller: AssociatorForStoreToCategoriesController,
		controllerAs: "asCtrl",
