/**
 * @file framework/admin/widgets/associations/_T4/event.to.PageViewEvents.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Event to PageViewEvents class
...
module cef.admin.widgets.associators.Tracking {
	class AssociatorForEventToPageViewEventsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.EventModel;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.EventModel, api.EventPagedResults>(
					...
					this.cvApi.tracking.GetEvents, 8, `event-${this.record && this.record.ID}.to.PageViewEvents.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `event-${this.record && this.record.ID}.to.PageViewEvents.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.PageViewEventModel, api.PageViewEventPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.tracking.GetPageViewEvents, 8, `event-${this.record && this.record.ID}.to.PageViewEvents.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `event-${this.record && this.record.ID}.to.PageViewEvents.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `event-${this.record && this.record.ID}.to.PageViewEvents.dupe-check`
			};
	...
	adminApp.directive("cefEventToPageViewEventsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/events.to.PageViewEvents.html", "ui"),
		controller: AssociatorForEventToPageViewEventsController,
		controllerAs: "asCtrl",
