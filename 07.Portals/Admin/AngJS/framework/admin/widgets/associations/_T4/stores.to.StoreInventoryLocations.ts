/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreInventoryLocations.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreInventoryLocations class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreInventoryLocationsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreInventoryLocations";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>(
					...
					this.cvApi.inventory.GetInventoryLocations, 8, `store-${this.record && this.record.ID}.to.StoreInventoryLocations.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreInventoryLocations.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.StoreInventoryLocationModel, api.StoreInventoryLocationPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreInventoryLocations, 8, `store-${this.record && this.record.ID}.to.StoreInventoryLocations.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreInventoryLocations.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreInventoryLocations.dupe-check`
			};
	...
	adminApp.directive("cefStoreToStoreInventoryLocationsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreInventoryLocations.html", "ui"),
		controller: AssociatorForStoreToStoreInventoryLocationsController,
		controllerAs: "asCtrl",
