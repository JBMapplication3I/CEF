/**
 * @file framework/admin/widgets/associations/_T4/cart.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Cart to StoredFiles class
...
module cef.admin.widgets.associators.Shopping {
	class AssociatorForCartToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.CartFileModel, api.CartFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `cart-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `cart-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CartFileModel, api.CartFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.shopping.GetCartFiles, 8, `cart-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `cart-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `cart-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.shopping.GetCartFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.shopping.CreateCartFile(<api.CartFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CartFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.shopping.DeactivateCartFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCartToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/carts.to.StoredFiles.html", "ui"),
		controller: AssociatorForCartToStoredFilesController,
		controllerAs: "asCtrl",
