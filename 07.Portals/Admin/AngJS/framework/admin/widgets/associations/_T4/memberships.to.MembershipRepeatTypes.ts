/**
 * @file framework/admin/widgets/associations/_T4/membership.to.MembershipRepeatTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Membership to MembershipRepeatTypes class
...
module cef.admin.widgets.associators.Payments {
	class AssociatorForMembershipToMembershipRepeatTypesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.MembershipModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "MembershipRepeatTypes";
		paging: core.ServerSidePaging<api.RepeatTypeModel, api.RepeatTypePagedResults>;
		assigned: core.ServerSidePaging<api.MembershipRepeatTypeModel, api.MembershipRepeatTypePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.RepeatTypeModel, api.RepeatTypePagedResults>(
					...
					this.cvApi.payments.GetRepeatTypes, 8, `membership-${this.record && this.record.ID}.to.MembershipRepeatTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `membership-${this.record && this.record.ID}.to.MembershipRepeatTypes.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.MembershipRepeatTypeModel, api.MembershipRepeatTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetMembershipRepeatTypes, 8, `membership-${this.record && this.record.ID}.to.MembershipRepeatTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `membership-${this.record && this.record.ID}.to.MembershipRepeatTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `membership-${this.record && this.record.ID}.to.MembershipRepeatTypes.dupe-check`
			};
			this.cvApi.payments.GetMembershipRepeatTypes(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.payments.CreateMembershipRepeatType(<api.MembershipRepeatTypeModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.MembershipRepeatTypeModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.payments.DeactivateMembershipRepeatTypeByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefMembershipToMembershipRepeatTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/memberships.to.MembershipRepeatTypes.html", "ui"),
		controller: AssociatorForMembershipToMembershipRepeatTypesController,
		controllerAs: "asCtrl",
