/**
 * @file framework/admin/widgets/associations/_T4/product.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to StoredFiles class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.ProductFileModel, api.ProductFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `product-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.ProductFileModel, api.ProductFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.products.GetProductFiles, 8, `product-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.products.GetProductFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.products.CreateProductFile(<api.ProductFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.ProductFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.products.DeactivateProductFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefProductToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.StoredFiles.html", "ui"),
		controller: AssociatorForProductToStoredFilesController,
		controllerAs: "asCtrl",
