/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreSubscriptions.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreSubscriptions class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreSubscriptionsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreSubscriptions";
		paging: core.ServerSidePaging<api.SubscriptionModel, api.SubscriptionPagedResults>;
		assigned: core.ServerSidePaging<api.StoreSubscriptionModel, api.StoreSubscriptionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.SubscriptionModel, api.SubscriptionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetSubscriptions, 8, `store-${this.record && this.record.ID}.to.StoreSubscriptions.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreSubscriptions.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreSubscriptionModel, api.StoreSubscriptionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreSubscriptions, 8, `store-${this.record && this.record.ID}.to.StoreSubscriptions.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreSubscriptions.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreSubscriptions.dupe-check`
			};
			this.cvApi.stores.GetStoreSubscriptions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreSubscription(<api.StoreSubscriptionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreSubscriptionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreSubscriptionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToStoreSubscriptionsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreSubscriptions.html", "ui"),
		controller: AssociatorForStoreToStoreSubscriptionsController,
		controllerAs: "asCtrl",
