/**
 * @file framework/admin/widgets/associations/_T4/brand.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Brand to Franchises class
...
module cef.admin.widgets.associators.Brands {
	class AssociatorForBrandToFranchisesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandFranchiseModel, api.BrandFranchisePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.FranchiseModel, api.FranchisePagedResults>(
					...
					this.cvApi.franchises.GetFranchises, 8, `brand-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `brand-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandFranchiseModel, api.BrandFranchisePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandFranchises, 8, `brand-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `brand-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `brand-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.brands.GetBrandFranchises(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandFranchise(<api.BrandFranchiseModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandFranchiseModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandFranchiseByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefBrandToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/brands.to.Franchises.html", "ui"),
		controller: AssociatorForBrandToFranchisesController,
		controllerAs: "asCtrl",
