/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.Users.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to Users class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToUsersController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `franchise-${this.record && this.record.ID}.to.Users.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.Users.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.FranchiseUserModel, api.FranchiseUserPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseUsers, 8, `franchise-${this.record && this.record.ID}.to.Users.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.Users.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.Users.dupe-check`
			};
	...
	adminApp.directive("cefFranchiseToUsersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.Users.html", "ui"),
		controller: AssociatorForFranchiseToUsersController,
		controllerAs: "asCtrl",
