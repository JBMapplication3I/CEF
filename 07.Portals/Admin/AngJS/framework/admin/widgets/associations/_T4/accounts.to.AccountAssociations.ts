/**
 * @file framework/admin/widgets/associations/_T4/account.to.AccountAssociations.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to AccountAssociations class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToAccountAssociationsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AccountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AccountAssociations";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `account-${this.record && this.record.ID}.to.AccountAssociations.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.AccountAssociations.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.AccountAssociationModel, api.AccountAssociationPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountAssociations, 8, `account-${this.record && this.record.ID}.to.AccountAssociations.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.AccountAssociations.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.AccountAssociations.dupe-check`
			};
	...
	adminApp.directive("cefAccountToAccountAssociationsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.AccountAssociations.html", "ui"),
		controller: AssociatorForAccountToAccountAssociationsController,
		controllerAs: "asCtrl",
