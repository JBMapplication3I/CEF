/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreContacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreContacts class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreContactsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreContacts";
		paging: core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>;
		assigned: core.ServerSidePaging<api.StoreContactModel, api.StoreContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `store-${this.record && this.record.ID}.to.StoreContacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreContacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreContactModel, api.StoreContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreContacts, 8, `store-${this.record && this.record.ID}.to.StoreContacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreContacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreContacts.dupe-check`
			};
			this.cvApi.stores.GetStoreContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreContact(<api.StoreContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToStoreContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreContacts.html", "ui"),
		controller: AssociatorForStoreToStoreContactsController,
		controllerAs: "asCtrl",
