/**
 * @file framework/admin/widgets/associations/_T4/discount.to.ProductTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to ProductTypes class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToProductTypesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.DiscountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "ProductTypes";
		paging: core.ServerSidePaging<api.TypeModel, api.ProductTypePagedResults>;
		assigned: core.ServerSidePaging<api.DiscountProductTypeModel, api.DiscountProductTypePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.TypeModel, api.ProductTypePagedResults>(
					...
					this.cvApi.products.GetProductTypes, 8, `discount-${this.record && this.record.ID}.to.ProductTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.ProductTypes.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountProductTypeModel, api.DiscountProductTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountProductTypes, 8, `discount-${this.record && this.record.ID}.to.ProductTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.ProductTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.ProductTypes.dupe-check`
			};
			this.cvApi.discounts.GetDiscountProductTypes(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountProductType(<api.DiscountProductTypeModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountProductTypeModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountProductTypeByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToProductTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.ProductTypes.html", "ui"),
		controller: AssociatorForDiscountToProductTypesController,
		controllerAs: "asCtrl",
