/**
 * @file framework/admin/widgets/associations/_T4/auction.to.Categories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Auction to Categories class
...
module cef.admin.widgets.associators.Auctions {
	class AssociatorForAuctionToCategoriesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AuctionCategoryModel, api.AuctionCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `auction-${this.record && this.record.ID}.to.Categories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `auction-${this.record && this.record.ID}.to.Categories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AuctionCategoryModel, api.AuctionCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.auctions.GetAuctionCategories, 8, `auction-${this.record && this.record.ID}.to.Categories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `auction-${this.record && this.record.ID}.to.Categories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `auction-${this.record && this.record.ID}.to.Categories.dupe-check`
			};
			this.cvApi.auctions.GetAuctionCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.auctions.CreateAuctionCategory(<api.AuctionCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AuctionCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.auctions.DeactivateAuctionCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAuctionToCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/auctions.to.Categories.html", "ui"),
		controller: AssociatorForAuctionToCategoriesController,
		controllerAs: "asCtrl",
