/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.PriceRuleAccountTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to PriceRuleAccountTypes class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToPriceRuleAccountTypesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PriceRuleModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "PriceRuleAccountTypes";
		paging: core.ServerSidePaging<api.TypeModel, api.AccountTypePagedResults>;
		assigned: core.ServerSidePaging<api.PriceRuleAccountTypeModel, api.PriceRuleAccountTypePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.TypeModel, api.AccountTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountTypes, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleAccountTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleAccountTypes.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleAccountTypeModel, api.PriceRuleAccountTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleAccountTypes, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleAccountTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleAccountTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.PriceRuleAccountTypes.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleAccountTypes(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleAccountType(<api.PriceRuleAccountTypeModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleAccountTypeModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleAccountTypeByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToPriceRuleAccountTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.PriceRuleAccountTypes.html", "ui"),
		controller: AssociatorForPriceRuleToPriceRuleAccountTypesController,
		controllerAs: "asCtrl",
