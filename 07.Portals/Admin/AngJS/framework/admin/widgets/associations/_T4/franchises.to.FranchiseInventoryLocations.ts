/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.FranchiseInventoryLocations.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to FranchiseInventoryLocations class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToFranchiseInventoryLocationsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.FranchiseModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "FranchiseInventoryLocations";
		paging: core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseInventoryLocationModel, api.FranchiseInventoryLocationPagedResults>;
		franchiseInventoryLocationTypes: api.TypeModel[];
		...
		load(): void {
			...
			this.$q.all([
				this.cvApi.franchises.GetFranchiseInventoryLocationTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.franchiseInventoryLocationTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>(
					...
					this.cvApi.inventory.GetInventoryLocations, 8, `franchise-${this.record && this.record.ID}.to.FranchiseInventoryLocations.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseInventoryLocations.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseInventoryLocationModel, api.FranchiseInventoryLocationPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseInventoryLocations, 8, `franchise-${this.record && this.record.ID}.to.FranchiseInventoryLocations.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseInventoryLocations.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.FranchiseInventoryLocations.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseInventoryLocations(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseInventoryLocation(<api.FranchiseInventoryLocationModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseInventoryLocationModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseInventoryLocationByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefFranchiseToFranchiseInventoryLocationsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.FranchiseInventoryLocations.html", "ui"),
		controller: AssociatorForFranchiseToFranchiseInventoryLocationsController,
		controllerAs: "asCtrl",
