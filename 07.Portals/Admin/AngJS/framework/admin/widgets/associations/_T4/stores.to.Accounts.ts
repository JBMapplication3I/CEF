/**
 * @file framework/admin/widgets/associations/_T4/store.to.Accounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to Accounts class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToAccountsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `store-${this.record && this.record.ID}.to.Accounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.Accounts.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.StoreAccountModel, api.StoreAccountPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreAccounts, 8, `store-${this.record && this.record.ID}.to.Accounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.Accounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.Accounts.dupe-check`
			};
	...
	adminApp.directive("cefStoreToAccountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.Accounts.html", "ui"),
		controller: AssociatorForStoreToAccountsController,
		controllerAs: "asCtrl",
