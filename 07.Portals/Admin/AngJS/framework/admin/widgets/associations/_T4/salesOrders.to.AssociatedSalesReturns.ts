/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.AssociatedSalesReturns.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to AssociatedSalesReturns class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToAssociatedSalesReturnsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AssociatedSalesReturns";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SalesOrderModel, api.SalesOrderPagedResults>(
					...
					this.cvApi.ordering.GetSalesOrders, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesReturns.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesReturns.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.SalesReturnSalesOrderModel, api.SalesReturnSalesOrderPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.returning.GetSalesReturnSalesOrders, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesReturns.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesReturns.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesReturns.dupe-check`
			};
	...
	adminApp.directive("cefSalesOrderToAssociatedSalesReturnsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.AssociatedSalesReturns.html", "ui"),
		controller: AssociatorForSalesOrderToAssociatedSalesReturnsController,
		controllerAs: "asCtrl",
