/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Vendors.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Vendors class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToVendorsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountVendorModel, api.DiscountVendorPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.VendorModel, api.VendorPagedResults>(
					...
					this.cvApi.vendors.GetVendors, 8, `discount-${this.record && this.record.ID}.to.Vendors.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Vendors.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountVendorModel, api.DiscountVendorPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountVendors, 8, `discount-${this.record && this.record.ID}.to.Vendors.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Vendors.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Vendors.dupe-check`
			};
			this.cvApi.discounts.GetDiscountVendors(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountVendor(<api.DiscountVendorModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountVendorModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountVendorByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToVendorsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Vendors.html", "ui"),
		controller: AssociatorForDiscountToVendorsController,
		controllerAs: "asCtrl",
