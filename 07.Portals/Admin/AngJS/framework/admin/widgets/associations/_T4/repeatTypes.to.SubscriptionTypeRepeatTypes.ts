/**
 * @file framework/admin/widgets/associations/_T4/repeatType.to.SubscriptionTypeRepeatTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Repeat Type to SubscriptionTypeRepeatTypes class
...
module cef.admin.widgets.associators.Payments {
	class AssociatorForRepeatTypeToSubscriptionTypeRepeatTypesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.RepeatTypeModel;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.RepeatTypeModel, api.RepeatTypePagedResults>(
					...
					this.cvApi.payments.GetRepeatTypes, 8, `repeatType-${this.record && this.record.ID}.to.SubscriptionTypeRepeatTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `repeatType-${this.record && this.record.ID}.to.SubscriptionTypeRepeatTypes.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.SubscriptionTypeRepeatTypeModel, api.SubscriptionTypeRepeatTypePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetSubscriptionTypeRepeatTypes, 8, `repeatType-${this.record && this.record.ID}.to.SubscriptionTypeRepeatTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `repeatType-${this.record && this.record.ID}.to.SubscriptionTypeRepeatTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `repeatType-${this.record && this.record.ID}.to.SubscriptionTypeRepeatTypes.dupe-check`
			};
	...
	adminApp.directive("cefRepeatTypeToSubscriptionTypeRepeatTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/repeatTypes.to.SubscriptionTypeRepeatTypes.html", "ui"),
		controller: AssociatorForRepeatTypeToSubscriptionTypeRepeatTypesController,
		controllerAs: "asCtrl",
