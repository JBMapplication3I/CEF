/**
 * @file framework/admin/widgets/associations/_T4/vendor.to.Accounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Vendor to Accounts class
...
module cef.admin.widgets.associators.Vendors {
	class AssociatorForVendorToAccountsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `vendor-${this.record && this.record.ID}.to.Accounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `vendor-${this.record && this.record.ID}.to.Accounts.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.VendorAccountModel, api.VendorAccountPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.vendors.GetVendorAccounts, 8, `vendor-${this.record && this.record.ID}.to.Accounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `vendor-${this.record && this.record.ID}.to.Accounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `vendor-${this.record && this.record.ID}.to.Accounts.dupe-check`
			};
	...
	adminApp.directive("cefVendorToAccountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/vendors.to.Accounts.html", "ui"),
		controller: AssociatorForVendorToAccountsController,
		controllerAs: "asCtrl",
