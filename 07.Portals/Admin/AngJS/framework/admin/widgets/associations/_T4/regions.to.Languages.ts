/**
 * @file framework/admin/widgets/associations/_T4/region.to.Languages.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Region to Languages class
...
module cef.admin.widgets.associators.Geography {
	class AssociatorForRegionToLanguagesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.RegionLanguageModel, api.RegionLanguagePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.LanguageModel, api.LanguagePagedResults>(
					...
					this.cvApi.globalization.GetLanguages, 8, `region-${this.record && this.record.ID}.to.Languages.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `region-${this.record && this.record.ID}.to.Languages.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.RegionLanguageModel, api.RegionLanguagePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetRegionLanguages, 8, `region-${this.record && this.record.ID}.to.Languages.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `region-${this.record && this.record.ID}.to.Languages.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `region-${this.record && this.record.ID}.to.Languages.dupe-check`
			};
			this.cvApi.geography.GetRegionLanguages(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.geography.CreateRegionLanguage(<api.RegionLanguageModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.RegionLanguageModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.geography.DeactivateRegionLanguageByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefRegionToLanguagesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/regions.to.Languages.html", "ui"),
		controller: AssociatorForRegionToLanguagesController,
		controllerAs: "asCtrl",
