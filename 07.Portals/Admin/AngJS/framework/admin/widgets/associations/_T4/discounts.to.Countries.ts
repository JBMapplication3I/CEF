/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Countries.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Countries class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToCountriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.DiscountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Countries";
		paging: core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>;
		assigned: core.ServerSidePaging<api.DiscountCountryModel, api.DiscountCountryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>(
					...
					this.cvApi.geography.GetCountries, 8, `discount-${this.record && this.record.ID}.to.Countries.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Countries.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountCountryModel, api.DiscountCountryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountCountries, 8, `discount-${this.record && this.record.ID}.to.Countries.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Countries.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Countries.dupe-check`
			};
			this.cvApi.discounts.GetDiscountCountries(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountCountry(<api.DiscountCountryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountCountryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountCountryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToCountriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Countries.html", "ui"),
		controller: AssociatorForDiscountToCountriesController,
		controllerAs: "asCtrl",
