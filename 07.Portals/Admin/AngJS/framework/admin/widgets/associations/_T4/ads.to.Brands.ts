/**
 * @file framework/admin/widgets/associations/_T4/ad.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Ad to Brands class
...
module cef.admin.widgets.associators.Advertising {
	class AssociatorForAdToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AdBrandModel, api.AdBrandPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.BrandModel, api.BrandPagedResults>(
					...
					this.cvApi.brands.GetBrands, 8, `ad-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `ad-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AdBrandModel, api.AdBrandPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.advertising.GetAdBrands, 8, `ad-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `ad-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `ad-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.advertising.GetAdBrands(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.advertising.CreateAdBrand(<api.AdBrandModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AdBrandModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.advertising.DeactivateAdBrandByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAdToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/ads.to.Brands.html", "ui"),
		controller: AssociatorForAdToBrandsController,
		controllerAs: "asCtrl",
