/**
 * @file framework/admin/widgets/associations/_T4/zone.to.AdZones.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Zone to AdZones class
...
module cef.admin.widgets.associators.Advertising {
	class AssociatorForZoneToAdZonesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ZoneModel;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ZoneModel, api.ZonePagedResults>(
					...
					this.cvApi.advertising.GetZones, 8, `zone-${this.record && this.record.ID}.to.AdZones.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `zone-${this.record && this.record.ID}.to.AdZones.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.AdZoneModel, api.AdZonePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.advertising.GetAdZones, 8, `zone-${this.record && this.record.ID}.to.AdZones.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `zone-${this.record && this.record.ID}.to.AdZones.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `zone-${this.record && this.record.ID}.to.AdZones.dupe-check`
			};
	...
	adminApp.directive("cefZoneToAdZonesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/zones.to.AdZones.html", "ui"),
		controller: AssociatorForZoneToAdZonesController,
		controllerAs: "asCtrl",
