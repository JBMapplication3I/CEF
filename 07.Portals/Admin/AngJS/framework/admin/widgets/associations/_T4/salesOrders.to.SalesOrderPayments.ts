/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.SalesOrderPayments.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to SalesOrderPayments class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToSalesOrderPaymentsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "SalesOrderPayments";
		paging: core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>;
		assigned: core.ServerSidePaging<api.SalesOrderPaymentModel, api.SalesOrderPaymentPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>(
					...
					this.cvApi.payments.GetPayments, 8, `salesOrder-${this.record && this.record.ID}.to.SalesOrderPayments.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.SalesOrderPayments.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesOrderPaymentModel, api.SalesOrderPaymentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetSalesOrderPayments, 8, `salesOrder-${this.record && this.record.ID}.to.SalesOrderPayments.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.SalesOrderPayments.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.SalesOrderPayments.dupe-check`
			};
			this.cvApi.payments.GetSalesOrderPayments(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.payments.CreateSalesOrderPayment(<api.SalesOrderPaymentModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesOrderPaymentModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.payments.DeactivateSalesOrderPaymentByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesOrderToSalesOrderPaymentsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.SalesOrderPayments.html", "ui"),
		controller: AssociatorForSalesOrderToSalesOrderPaymentsController,
		controllerAs: "asCtrl",
