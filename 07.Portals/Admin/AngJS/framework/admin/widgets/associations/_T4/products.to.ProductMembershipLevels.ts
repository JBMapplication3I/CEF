/**
 * @file framework/admin/widgets/associations/_T4/product.to.ProductMembershipLevels.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to ProductMembershipLevels class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToProductMembershipLevelsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ProductModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "ProductMembershipLevels";
		paging: core.ServerSidePaging<api.MembershipLevelModel, api.MembershipLevelPagedResults>;
		assigned: core.ServerSidePaging<api.ProductMembershipLevelModel, api.ProductMembershipLevelPagedResults>;
		membershipRepeatTypes: api.MembershipRepeatTypeModel[];
		...
		load(): void {
			...
			this.$q.all([
				this.cvApi.payments.GetMembershipRepeatTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.membershipRepeatTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.MembershipLevelModel, api.MembershipLevelPagedResults>(
					...
					this.cvApi.payments.GetMembershipLevels, 8, `product-${this.record && this.record.ID}.to.ProductMembershipLevels.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.ProductMembershipLevels.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.ProductMembershipLevelModel, api.ProductMembershipLevelPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.products.GetProductMembershipLevels, 8, `product-${this.record && this.record.ID}.to.ProductMembershipLevels.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.ProductMembershipLevels.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.ProductMembershipLevels.dupe-check`
			};
			this.cvApi.products.GetProductMembershipLevels(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.products.CreateProductMembershipLevel(<api.ProductMembershipLevelModel>{
					// Base Properties
					ID: null,
					Active: true,
					...
					MembershipRepeatTypeID: 0,
					MembershipRepeatType: null,
					Subscriptions: null,
				}).then(rc => {
					if (!rc || !rc.data) {
		...
		remove(toRemove: api.ProductMembershipLevelModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.products.DeactivateProductMembershipLevelByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefProductToProductMembershipLevelsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.ProductMembershipLevels.html", "ui"),
		controller: AssociatorForProductToProductMembershipLevelsController,
		controllerAs: "asCtrl",
