/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.Manufacturers.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to Manufacturers class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToManufacturersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PriceRuleModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Manufacturers";
		paging: core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>;
		assigned: core.ServerSidePaging<api.PriceRuleManufacturerModel, api.PriceRuleManufacturerPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.manufacturers.GetManufacturers, 8, `priceRule-${this.record && this.record.ID}.to.Manufacturers.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.Manufacturers.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleManufacturerModel, api.PriceRuleManufacturerPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleManufacturers, 8, `priceRule-${this.record && this.record.ID}.to.Manufacturers.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.Manufacturers.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.Manufacturers.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleManufacturers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleManufacturer(<api.PriceRuleManufacturerModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleManufacturerModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleManufacturerByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToManufacturersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.Manufacturers.html", "ui"),
		controller: AssociatorForPriceRuleToManufacturersController,
		controllerAs: "asCtrl",
