/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreCountries.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreCountries class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreCountriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreCountries";
		paging: core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>;
		assigned: core.ServerSidePaging<api.StoreCountryModel, api.StoreCountryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>(
					...
					this.cvApi.geography.GetCountries, 8, `store-${this.record && this.record.ID}.to.StoreCountries.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreCountries.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreCountryModel, api.StoreCountryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreCountries, 8, `store-${this.record && this.record.ID}.to.StoreCountries.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreCountries.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreCountries.dupe-check`
			};
			this.cvApi.stores.GetStoreCountries(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreCountry(<api.StoreCountryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreCountryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreCountryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToStoreCountriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreCountries.html", "ui"),
		controller: AssociatorForStoreToStoreCountriesController,
		controllerAs: "asCtrl",
