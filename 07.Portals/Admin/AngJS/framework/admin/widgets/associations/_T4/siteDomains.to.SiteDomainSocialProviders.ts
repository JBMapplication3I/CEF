/**
 * @file framework/admin/widgets/associations/_T4/siteDomain.to.SiteDomainSocialProviders.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Site Domain to SiteDomainSocialProviders class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForSiteDomainToSiteDomainSocialProvidersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SiteDomainModel;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SocialProviderModel, api.SocialProviderPagedResults>(
					...
					this.cvApi.stores.GetSocialProviders, 8, `siteDomain-${this.record && this.record.ID}.to.SiteDomainSocialProviders.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `siteDomain-${this.record && this.record.ID}.to.SiteDomainSocialProviders.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.SiteDomainSocialProviderModel, api.SiteDomainSocialProviderPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetSiteDomainSocialProviders, 8, `siteDomain-${this.record && this.record.ID}.to.SiteDomainSocialProviders.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `siteDomain-${this.record && this.record.ID}.to.SiteDomainSocialProviders.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `siteDomain-${this.record && this.record.ID}.to.SiteDomainSocialProviders.dupe-check`
			};
	...
	adminApp.directive("cefSiteDomainToSiteDomainSocialProvidersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/siteDomains.to.SiteDomainSocialProviders.html", "ui"),
		controller: AssociatorForSiteDomainToSiteDomainSocialProvidersController,
		controllerAs: "asCtrl",
