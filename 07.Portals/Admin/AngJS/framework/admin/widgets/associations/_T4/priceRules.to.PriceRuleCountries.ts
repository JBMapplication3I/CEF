/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.PriceRuleCountries.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to PriceRuleCountries class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToPriceRuleCountriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PriceRuleModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "PriceRuleCountries";
		paging: core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>;
		assigned: core.ServerSidePaging<api.PriceRuleCountryModel, api.PriceRuleCountryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetCountries, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleCountries.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleCountries.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleCountryModel, api.PriceRuleCountryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleCountries, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleCountries.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleCountries.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.PriceRuleCountries.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleCountries(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleCountry(<api.PriceRuleCountryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleCountryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleCountryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToPriceRuleCountriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.PriceRuleCountries.html", "ui"),
		controller: AssociatorForPriceRuleToPriceRuleCountriesController,
		controllerAs: "asCtrl",
