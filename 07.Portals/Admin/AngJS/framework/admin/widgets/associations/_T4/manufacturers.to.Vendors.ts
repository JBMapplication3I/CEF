/**
 * @file framework/admin/widgets/associations/_T4/manufacturer.to.Vendors.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Manufacturer to Vendors class
...
module cef.admin.widgets.associators.Manufacturers {
	class AssociatorForManufacturerToVendorsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ManufacturerModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Vendors";
		paging: core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>;
		assigned: core.ServerSidePaging<api.VendorManufacturerModel, api.VendorManufacturerPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>(
					...
					this.cvApi.manufacturers.GetManufacturers, 8, `manufacturer-${this.record && this.record.ID}.to.Vendors.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `manufacturer-${this.record && this.record.ID}.to.Vendors.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.VendorManufacturerModel, api.VendorManufacturerPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.vendors.GetVendorManufacturers, 8, `manufacturer-${this.record && this.record.ID}.to.Vendors.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `manufacturer-${this.record && this.record.ID}.to.Vendors.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `manufacturer-${this.record && this.record.ID}.to.Vendors.dupe-check`
			};
			this.cvApi.vendors.GetVendorManufacturers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.vendors.CreateVendorManufacturer(<api.VendorManufacturerModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.VendorManufacturerModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.vendors.DeactivateVendorManufacturerByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefManufacturerToVendorsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/manufacturers.to.Vendors.html", "ui"),
		controller: AssociatorForManufacturerToVendorsController,
		controllerAs: "asCtrl",
