/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to Contacts class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToContactsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesOrderContactModel, api.SalesOrderContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `salesOrder-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesOrderContactModel, api.SalesOrderContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.ordering.GetSalesOrderContacts, 8, `salesOrder-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.ordering.GetSalesOrderContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.ordering.CreateSalesOrderContact(<api.SalesOrderContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesOrderContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.ordering.DeactivateSalesOrderContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesOrderToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.Contacts.html", "ui"),
		controller: AssociatorForSalesOrderToContactsController,
		controllerAs: "asCtrl",
