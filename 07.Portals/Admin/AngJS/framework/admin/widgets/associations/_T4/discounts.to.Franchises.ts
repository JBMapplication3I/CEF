/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Franchises class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToFranchisesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountFranchiseModel, api.DiscountFranchisePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.FranchiseModel, api.FranchisePagedResults>(
					...
					this.cvApi.franchises.GetFranchises, 8, `discount-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountFranchiseModel, api.DiscountFranchisePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountFranchises, 8, `discount-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.discounts.GetDiscountFranchises(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountFranchise(<api.DiscountFranchiseModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountFranchiseModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountFranchiseByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Franchises.html", "ui"),
		controller: AssociatorForDiscountToFranchisesController,
		controllerAs: "asCtrl",
