/**
 * @file framework/admin/widgets/associations/_T4/ad.to.CampaignAds.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Ad to CampaignAds class
...
module cef.admin.widgets.associators.Advertising {
	class AssociatorForAdToCampaignAdsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AdModel, api.AdPagedResults>(
					...
					this.cvApi.advertising.GetAds, 8, `ad-${this.record && this.record.ID}.to.CampaignAds.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `ad-${this.record && this.record.ID}.to.CampaignAds.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.CampaignAdModel, api.CampaignAdPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.tracking.GetCampaignAds, 8, `ad-${this.record && this.record.ID}.to.CampaignAds.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `ad-${this.record && this.record.ID}.to.CampaignAds.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `ad-${this.record && this.record.ID}.to.CampaignAds.dupe-check`
			};
	...
	adminApp.directive("cefAdToCampaignAdsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/ads.to.CampaignAds.html", "ui"),
		controller: AssociatorForAdToCampaignAdsController,
		controllerAs: "asCtrl",
