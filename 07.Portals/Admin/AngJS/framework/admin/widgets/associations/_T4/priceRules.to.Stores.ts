/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to Stores class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToStoresController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.PriceRuleStoreModel, api.PriceRuleStorePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoreModel, api.StorePagedResults>(
					...
					this.cvApi.stores.GetStores, 8, `priceRule-${this.record && this.record.ID}.to.Stores.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.Stores.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleStoreModel, api.PriceRuleStorePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleStores, 8, `priceRule-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleStores(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleStore(<api.PriceRuleStoreModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleStoreModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleStoreByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.Stores.html", "ui"),
		controller: AssociatorForPriceRuleToStoresController,
		controllerAs: "asCtrl",
