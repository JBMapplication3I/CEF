/**
 * @file framework/admin/widgets/associations/_T4/product.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to Stores class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToStoresController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreProductModel, api.StoreProductPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `product-${this.record && this.record.ID}.to.Stores.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.Stores.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreProductModel, api.StoreProductPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreProducts, 8, `product-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
			this.cvApi.stores.GetStoreProducts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreProduct(<api.StoreProductModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreProductModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreProductByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefProductToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.Stores.html", "ui"),
		controller: AssociatorForProductToStoresController,
		controllerAs: "asCtrl",
