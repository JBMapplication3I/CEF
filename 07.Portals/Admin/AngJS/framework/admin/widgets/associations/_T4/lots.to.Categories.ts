/**
 * @file framework/admin/widgets/associations/_T4/lot.to.Categories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Lot to Categories class
...
module cef.admin.widgets.associators.Auctions {
	class AssociatorForLotToCategoriesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.LotCategoryModel, api.LotCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `lot-${this.record && this.record.ID}.to.Categories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `lot-${this.record && this.record.ID}.to.Categories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.LotCategoryModel, api.LotCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.auctions.GetLotCategories, 8, `lot-${this.record && this.record.ID}.to.Categories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `lot-${this.record && this.record.ID}.to.Categories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `lot-${this.record && this.record.ID}.to.Categories.dupe-check`
			};
			this.cvApi.auctions.GetLotCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.auctions.CreateLotCategory(<api.LotCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.LotCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.auctions.DeactivateLotCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefLotToCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/lots.to.Categories.html", "ui"),
		controller: AssociatorForLotToCategoriesController,
		controllerAs: "asCtrl",
