/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Brands class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountBrandModel, api.DiscountBrandPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.BrandModel, api.BrandPagedResults>(
					...
					this.cvApi.brands.GetBrands, 8, `discount-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountBrandModel, api.DiscountBrandPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountBrands, 8, `discount-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.discounts.GetDiscountBrands(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountBrand(<api.DiscountBrandModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountBrandModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountBrandByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Brands.html", "ui"),
		controller: AssociatorForDiscountToBrandsController,
		controllerAs: "asCtrl",
