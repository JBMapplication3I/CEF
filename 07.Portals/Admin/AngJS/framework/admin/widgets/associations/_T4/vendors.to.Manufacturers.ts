/**
 * @file framework/admin/widgets/associations/_T4/vendor.to.Manufacturers.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Vendor to Manufacturers class
...
module cef.admin.widgets.associators.Vendors {
	class AssociatorForVendorToManufacturersController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>(
					...
					this.cvApi.manufacturers.GetManufacturers, 8, `vendor-${this.record && this.record.ID}.to.Manufacturers.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `vendor-${this.record && this.record.ID}.to.Manufacturers.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.VendorManufacturerModel, api.VendorManufacturerPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.vendors.GetVendorManufacturers, 8, `vendor-${this.record && this.record.ID}.to.Manufacturers.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `vendor-${this.record && this.record.ID}.to.Manufacturers.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `vendor-${this.record && this.record.ID}.to.Manufacturers.dupe-check`
			};
	...
	adminApp.directive("cefVendorToManufacturersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/vendors.to.Manufacturers.html", "ui"),
		controller: AssociatorForVendorToManufacturersController,
		controllerAs: "asCtrl",
