/**
 * @file framework/admin/widgets/associations/_T4/category.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Category to Stores class
...
module cef.admin.widgets.associators.Categories {
	class AssociatorForCategoryToStoresController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `category-${this.record && this.record.ID}.to.Stores.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `category-${this.record && this.record.ID}.to.Stores.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.StoreCategoryModel, api.StoreCategoryPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreCategories, 8, `category-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `category-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `category-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
	...
	adminApp.directive("cefCategoryToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/categorys.to.Stores.html", "ui"),
		controller: AssociatorForCategoryToStoresController,
		controllerAs: "asCtrl",
