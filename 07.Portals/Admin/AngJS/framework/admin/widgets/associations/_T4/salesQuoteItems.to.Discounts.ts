/**
 * @file framework/admin/widgets/associations/_T4/salesQuoteItem.to.Discounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Quote Item to Discounts class
...
module cef.admin.widgets.associators.Quoting {
	class AssociatorForSalesQuoteItemToDiscountsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AppliedSalesQuoteItemDiscountModel, api.AppliedSalesQuoteItemDiscountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.DiscountModel, api.DiscountPagedResults>(
					...
					this.cvApi.discounts.GetDiscounts, 8, `salesQuoteItem-${this.record && this.record.ID}.to.Discounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesQuoteItem-${this.record && this.record.ID}.to.Discounts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AppliedSalesQuoteItemDiscountModel, api.AppliedSalesQuoteItemDiscountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetAppliedSalesQuoteItemDiscounts, 8, `salesQuoteItem-${this.record && this.record.ID}.to.Discounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesQuoteItem-${this.record && this.record.ID}.to.Discounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesQuoteItem-${this.record && this.record.ID}.to.Discounts.dupe-check`
			};
			this.cvApi.discounts.GetAppliedSalesQuoteItemDiscounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateAppliedSalesQuoteItemDiscount(<api.AppliedSalesQuoteItemDiscountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AppliedSalesQuoteItemDiscountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateAppliedSalesQuoteItemDiscountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesQuoteItemToDiscountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesQuoteItems.to.Discounts.html", "ui"),
		controller: AssociatorForSalesQuoteItemToDiscountsController,
		controllerAs: "asCtrl",
