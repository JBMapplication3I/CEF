/**
 * @file framework/admin/widgets/associations/_T4/brand.to.BrandInventoryLocations.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Brand to BrandInventoryLocations class
...
module cef.admin.widgets.associators.Brands {
	class AssociatorForBrandToBrandInventoryLocationsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.BrandModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "BrandInventoryLocations";
		paging: core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>;
		assigned: core.ServerSidePaging<api.BrandInventoryLocationModel, api.BrandInventoryLocationPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				this.cvApi.brands.GetBrandInventoryLocationTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.brandInventoryLocationTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>(
					...
					this.cvApi.inventory.GetInventoryLocations, 8, `brand-${this.record && this.record.ID}.to.BrandInventoryLocations.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `brand-${this.record && this.record.ID}.to.BrandInventoryLocations.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandInventoryLocationModel, api.BrandInventoryLocationPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandInventoryLocations, 8, `brand-${this.record && this.record.ID}.to.BrandInventoryLocations.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `brand-${this.record && this.record.ID}.to.BrandInventoryLocations.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `brand-${this.record && this.record.ID}.to.BrandInventoryLocations.dupe-check`
			};
			this.cvApi.brands.GetBrandInventoryLocations(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandInventoryLocation(<api.BrandInventoryLocationModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandInventoryLocationModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandInventoryLocationByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefBrandToBrandInventoryLocationsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/brands.to.BrandInventoryLocations.html", "ui"),
		controller: AssociatorForBrandToBrandInventoryLocationsController,
		controllerAs: "asCtrl",
