/**
 * @file framework/admin/widgets/associations/_T4/campaign.to.CampaignAds.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Campaign to CampaignAds class
...
module cef.admin.widgets.associators.Tracking {
	class AssociatorForCampaignToCampaignAdsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.CampaignModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "CampaignAds";
		paging: core.ServerSidePaging<api.AdModel, api.AdPagedResults>;
		assigned: core.ServerSidePaging<api.CampaignAdModel, api.CampaignAdPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.AdModel, api.AdPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.advertising.GetAds, 8, `campaign-${this.record && this.record.ID}.to.CampaignAds.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `campaign-${this.record && this.record.ID}.to.CampaignAds.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CampaignAdModel, api.CampaignAdPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.tracking.GetCampaignAds, 8, `campaign-${this.record && this.record.ID}.to.CampaignAds.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `campaign-${this.record && this.record.ID}.to.CampaignAds.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `campaign-${this.record && this.record.ID}.to.CampaignAds.dupe-check`
			};
			this.cvApi.tracking.GetCampaignAds(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.tracking.CreateCampaignAd(<api.CampaignAdModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CampaignAdModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.tracking.DeactivateCampaignAdByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCampaignToCampaignAdsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/campaigns.to.CampaignAds.html", "ui"),
		controller: AssociatorForCampaignToCampaignAdsController,
		controllerAs: "asCtrl",
