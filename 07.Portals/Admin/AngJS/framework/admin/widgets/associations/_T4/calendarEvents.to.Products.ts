/**
 * @file framework/admin/widgets/associations/_T4/calendarEvent.to.Products.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Calendar Event to Products class
...
module cef.admin.widgets.associators.CalendarEvents {
	class AssociatorForCalendarEventToProductsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.CalendarEventProductModel, api.CalendarEventProductPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `calendarEvent-${this.record && this.record.ID}.to.Products.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `calendarEvent-${this.record && this.record.ID}.to.Products.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CalendarEventProductModel, api.CalendarEventProductPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.calendarEvents.GetCalendarEventProducts, 8, `calendarEvent-${this.record && this.record.ID}.to.Products.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `calendarEvent-${this.record && this.record.ID}.to.Products.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `calendarEvent-${this.record && this.record.ID}.to.Products.dupe-check`
			};
			this.cvApi.calendarEvents.GetCalendarEventProducts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.calendarEvents.CreateCalendarEventProduct(<api.CalendarEventProductModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CalendarEventProductModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.calendarEvents.DeactivateCalendarEventProductByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCalendarEventToProductsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/calendarEvents.to.Products.html", "ui"),
		controller: AssociatorForCalendarEventToProductsController,
		controllerAs: "asCtrl",
