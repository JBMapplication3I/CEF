/**
 * @file framework/admin/widgets/associations/_T4/salesInvoice.to.AssociatedSalesOrders.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Invoice to AssociatedSalesOrders class
...
module cef.admin.widgets.associators.Invoicing {
	class AssociatorForSalesInvoiceToAssociatedSalesOrdersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesInvoiceModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AssociatedSalesOrders";
		paging: core.ServerSidePaging<api.SalesInvoiceModel, api.SalesInvoicePagedResults>;
		assigned: core.ServerSidePaging<api.SalesOrderSalesInvoiceModel, api.SalesOrderSalesInvoicePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.SalesInvoiceModel, api.SalesInvoicePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.invoicing.GetSalesInvoices, 8, `salesInvoice-${this.record && this.record.ID}.to.AssociatedSalesOrders.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesInvoice-${this.record && this.record.ID}.to.AssociatedSalesOrders.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesOrderSalesInvoiceModel, api.SalesOrderSalesInvoicePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.invoicing.GetSalesOrderSalesInvoices, 8, `salesInvoice-${this.record && this.record.ID}.to.AssociatedSalesOrders.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesInvoice-${this.record && this.record.ID}.to.AssociatedSalesOrders.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesInvoice-${this.record && this.record.ID}.to.AssociatedSalesOrders.dupe-check`
			};
			this.cvApi.invoicing.GetSalesOrderSalesInvoices(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.invoicing.CreateSalesOrderSalesInvoice(<api.SalesOrderSalesInvoiceModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesOrderSalesInvoiceModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.invoicing.DeactivateSalesOrderSalesInvoiceByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesInvoiceToAssociatedSalesOrdersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesInvoices.to.AssociatedSalesOrders.html", "ui"),
		controller: AssociatorForSalesInvoiceToAssociatedSalesOrdersController,
		controllerAs: "asCtrl",
