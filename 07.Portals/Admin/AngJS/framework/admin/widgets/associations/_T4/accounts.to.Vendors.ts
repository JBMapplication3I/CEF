/**
 * @file framework/admin/widgets/associations/_T4/account.to.Vendors.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to Vendors class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToVendorsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.VendorAccountModel, api.VendorAccountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `account-${this.record && this.record.ID}.to.Vendors.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.Vendors.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.VendorAccountModel, api.VendorAccountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.vendors.GetVendorAccounts, 8, `account-${this.record && this.record.ID}.to.Vendors.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.Vendors.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.Vendors.dupe-check`
			};
			this.cvApi.vendors.GetVendorAccounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.vendors.CreateVendorAccount(<api.VendorAccountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.VendorAccountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.vendors.DeactivateVendorAccountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToVendorsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.Vendors.html", "ui"),
		controller: AssociatorForAccountToVendorsController,
		controllerAs: "asCtrl",
