/**
 * @file framework/admin/widgets/associations/_T4/category.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Category to Franchises class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Categories {
	class AssociatorForCategoryToFranchisesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.CategoryModel;
		...
		assigned: core.ServerSidePaging<api.FranchiseCategoryModel, api.FranchiseCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `category-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `category-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseCategoryModel, api.FranchiseCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseCategories, 8, `category-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `category-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `category-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseCategory(<api.FranchiseCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCategoryToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/categorys.to.Franchises.html", "ui"),
		controller: AssociatorForCategoryToFranchisesController,
		controllerAs: "asCtrl",
