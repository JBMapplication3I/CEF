/**
 * @file framework/admin/widgets/associations/_T4/inventoryLocation.to.Users.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Inventory Location to Users class
...
module cef.admin.widgets.associators.Inventory {
	class AssociatorForInventoryLocationToUsersController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.InventoryLocationUserModel, api.InventoryLocationUserPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `inventoryLocation-${this.record && this.record.ID}.to.Users.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Users.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.InventoryLocationUserModel, api.InventoryLocationUserPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.inventory.GetInventoryLocationUsers, 8, `inventoryLocation-${this.record && this.record.ID}.to.Users.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Users.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `inventoryLocation-${this.record && this.record.ID}.to.Users.dupe-check`
			};
			this.cvApi.inventory.GetInventoryLocationUsers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.inventory.CreateInventoryLocationUser(<api.InventoryLocationUserModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.InventoryLocationUserModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.inventory.DeactivateInventoryLocationUserByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefInventoryLocationToUsersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/inventoryLocations.to.Users.html", "ui"),
		controller: AssociatorForInventoryLocationToUsersController,
		controllerAs: "asCtrl",
