/**
 * @file framework/admin/widgets/associations/_T4/salesInvoice.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Invoice to Contacts class
...
module cef.admin.widgets.associators.Invoicing {
	class AssociatorForSalesInvoiceToContactsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesInvoiceContactModel, api.SalesInvoiceContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `salesInvoice-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesInvoice-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesInvoiceContactModel, api.SalesInvoiceContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.invoicing.GetSalesInvoiceContacts, 8, `salesInvoice-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesInvoice-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesInvoice-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.invoicing.GetSalesInvoiceContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.invoicing.CreateSalesInvoiceContact(<api.SalesInvoiceContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesInvoiceContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.invoicing.DeactivateSalesInvoiceContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesInvoiceToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesInvoices.to.Contacts.html", "ui"),
		controller: AssociatorForSalesInvoiceToContactsController,
		controllerAs: "asCtrl",
