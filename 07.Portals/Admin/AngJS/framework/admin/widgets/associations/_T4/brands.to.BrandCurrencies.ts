/**
 * @file framework/admin/widgets/associations/_T4/brand.to.BrandCurrencies.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Brand to BrandCurrencies class
...
module cef.admin.widgets.associators.Brands {
	class AssociatorForBrandToBrandCurrenciesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.BrandModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "BrandCurrencies";
		paging: core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>;
		assigned: core.ServerSidePaging<api.BrandCurrencyModel, api.BrandCurrencyPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>(
					...
					this.cvApi.currencies.GetCurrencies, 8, `brand-${this.record && this.record.ID}.to.BrandCurrencies.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `brand-${this.record && this.record.ID}.to.BrandCurrencies.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandCurrencyModel, api.BrandCurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandCurrencies, 8, `brand-${this.record && this.record.ID}.to.BrandCurrencies.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `brand-${this.record && this.record.ID}.to.BrandCurrencies.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `brand-${this.record && this.record.ID}.to.BrandCurrencies.dupe-check`
			};
			this.cvApi.brands.GetBrandCurrencies(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandCurrency(<api.BrandCurrencyModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandCurrencyModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandCurrencyByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefBrandToBrandCurrenciesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/brands.to.BrandCurrencies.html", "ui"),
		controller: AssociatorForBrandToBrandCurrenciesController,
		controllerAs: "asCtrl",
