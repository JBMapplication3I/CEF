/**
 * @file framework/admin/widgets/associations/_T4/account.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to Stores class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToStoresController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreAccountModel, api.StoreAccountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `account-${this.record && this.record.ID}.to.Stores.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.Stores.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreAccountModel, api.StoreAccountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreAccounts, 8, `account-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
			this.cvApi.stores.GetStoreAccounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreAccount(<api.StoreAccountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreAccountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreAccountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.Stores.html", "ui"),
		controller: AssociatorForAccountToStoresController,
		controllerAs: "asCtrl",
