/**
 * @file framework/admin/widgets/associations/_T4/product.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to Brands class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandProductModel, api.BrandProductPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `product-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandProductModel, api.BrandProductPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandProducts, 8, `product-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.brands.GetBrandProducts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandProduct(<api.BrandProductModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandProductModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandProductByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefProductToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.Brands.html", "ui"),
		controller: AssociatorForProductToBrandsController,
		controllerAs: "asCtrl",
