/**
 * @file framework/admin/widgets/associations/_T4/salesQuote.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Quote to Contacts class
...
module cef.admin.widgets.associators.Quoting {
	class AssociatorForSalesQuoteToContactsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesQuoteContactModel, api.SalesQuoteContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `salesQuote-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesQuote-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesQuoteContactModel, api.SalesQuoteContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.quoting.GetSalesQuoteContacts, 8, `salesQuote-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesQuote-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesQuote-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.quoting.GetSalesQuoteContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.quoting.CreateSalesQuoteContact(<api.SalesQuoteContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesQuoteContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.quoting.DeactivateSalesQuoteContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesQuoteToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesQuotes.to.Contacts.html", "ui"),
		controller: AssociatorForSalesQuoteToContactsController,
		controllerAs: "asCtrl",
