/**
 * @file framework/admin/widgets/associations/_T4/group.to.Users.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Group to Users class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Groups {
	class AssociatorForGroupToUsersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.GroupModel;
		...
		assigned: core.ServerSidePaging<api.GroupUserModel, api.GroupUserPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `group-${this.record && this.record.ID}.to.Users.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `group-${this.record && this.record.ID}.to.Users.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.GroupUserModel, api.GroupUserPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.groups.GetGroupUsers, 8, `group-${this.record && this.record.ID}.to.Users.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `group-${this.record && this.record.ID}.to.Users.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `group-${this.record && this.record.ID}.to.Users.dupe-check`
			};
			this.cvApi.groups.GetGroupUsers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.groups.CreateGroupUser(<api.GroupUserModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.GroupUserModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.groups.DeactivateGroupUserByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefGroupToUsersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/groups.to.Users.html", "ui"),
		controller: AssociatorForGroupToUsersController,
		controllerAs: "asCtrl",
