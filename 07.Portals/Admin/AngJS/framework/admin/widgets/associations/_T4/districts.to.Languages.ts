/**
 * @file framework/admin/widgets/associations/_T4/district.to.Languages.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for District to Languages class
...
module cef.admin.widgets.associators.Geography {
	class AssociatorForDistrictToLanguagesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.DistrictModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Languages";
		paging: core.ServerSidePaging<api.LanguageModel, api.LanguagePagedResults>;
		assigned: core.ServerSidePaging<api.DistrictLanguageModel, api.DistrictLanguagePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.LanguageModel, api.LanguagePagedResults>(
					...
					this.cvApi.globalization.GetLanguages, 8, `district-${this.record && this.record.ID}.to.Languages.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `district-${this.record && this.record.ID}.to.Languages.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DistrictLanguageModel, api.DistrictLanguagePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetDistrictLanguages, 8, `district-${this.record && this.record.ID}.to.Languages.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `district-${this.record && this.record.ID}.to.Languages.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `district-${this.record && this.record.ID}.to.Languages.dupe-check`
			};
			this.cvApi.geography.GetDistrictLanguages(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.geography.CreateDistrictLanguage(<api.DistrictLanguageModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DistrictLanguageModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.geography.DeactivateDistrictLanguageByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDistrictToLanguagesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/districts.to.Languages.html", "ui"),
		controller: AssociatorForDistrictToLanguagesController,
		controllerAs: "asCtrl",
