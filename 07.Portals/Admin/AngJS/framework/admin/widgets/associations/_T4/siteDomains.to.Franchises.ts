/**
 * @file framework/admin/widgets/associations/_T4/siteDomain.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Site Domain to Franchises class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForSiteDomainToFranchisesController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SiteDomainModel, api.SiteDomainPagedResults>(
					...
					this.cvApi.stores.GetSiteDomains, 8, `siteDomain-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `siteDomain-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.FranchiseSiteDomainModel, api.FranchiseSiteDomainPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseSiteDomains, 8, `siteDomain-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `siteDomain-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `siteDomain-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
	...
	adminApp.directive("cefSiteDomainToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/siteDomains.to.Franchises.html", "ui"),
		controller: AssociatorForSiteDomainToFranchisesController,
		controllerAs: "asCtrl",
