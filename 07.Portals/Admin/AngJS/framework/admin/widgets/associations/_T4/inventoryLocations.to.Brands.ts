/**
 * @file framework/admin/widgets/associations/_T4/inventoryLocation.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Inventory Location to Brands class
...
module cef.admin.widgets.associators.Inventory {
	class AssociatorForInventoryLocationToBrandsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>(
					...
					this.cvApi.inventory.GetInventoryLocations, 8, `inventoryLocation-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Brands.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.BrandInventoryLocationModel, api.BrandInventoryLocationPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandInventoryLocations, 8, `inventoryLocation-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `inventoryLocation-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
	...
	adminApp.directive("cefInventoryLocationToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/inventoryLocations.to.Brands.html", "ui"),
		controller: AssociatorForInventoryLocationToBrandsController,
		controllerAs: "asCtrl",
