/**
 * @file framework/admin/widgets/associations/_T4/subscriptionType.to.ProductSubscriptionTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Subscription Type to ProductSubscriptionTypes class
...
module cef.admin.widgets.associators.Payments {
	class AssociatorForSubscriptionTypeToProductSubscriptionTypesController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SubscriptionTypeModel, api.SubscriptionTypePagedResults>(
					...
					this.cvApi.payments.GetSubscriptionTypes, 8, `subscriptionType-${this.record && this.record.ID}.to.ProductSubscriptionTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `subscriptionType-${this.record && this.record.ID}.to.ProductSubscriptionTypes.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.ProductSubscriptionTypeModel, api.ProductSubscriptionTypePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.products.GetProductSubscriptionTypes, 8, `subscriptionType-${this.record && this.record.ID}.to.ProductSubscriptionTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `subscriptionType-${this.record && this.record.ID}.to.ProductSubscriptionTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `subscriptionType-${this.record && this.record.ID}.to.ProductSubscriptionTypes.dupe-check`
			};
	...
	adminApp.directive("cefSubscriptionTypeToProductSubscriptionTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/subscriptionTypes.to.ProductSubscriptionTypes.html", "ui"),
		controller: AssociatorForSubscriptionTypeToProductSubscriptionTypesController,
		controllerAs: "asCtrl",
