/**
 * @file framework/admin/widgets/associations/_T4/auction.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Auction to Franchises class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Auctions {
	class AssociatorForAuctionToFranchisesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AuctionModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Franchises";
		paging: core.ServerSidePaging<api.AuctionModel, api.AuctionPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseAuctionModel, api.FranchiseAuctionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.AuctionModel, api.AuctionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.auctions.GetAuctions, 8, `auction-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `auction-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseAuctionModel, api.FranchiseAuctionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.auctions.GetFranchiseAuctions, 8, `auction-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `auction-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `auction-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.auctions.GetFranchiseAuctions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.auctions.CreateFranchiseAuction(<api.FranchiseAuctionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseAuctionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.auctions.DeactivateFranchiseAuctionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAuctionToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/auctions.to.Franchises.html", "ui"),
		controller: AssociatorForAuctionToFranchisesController,
		controllerAs: "asCtrl",
