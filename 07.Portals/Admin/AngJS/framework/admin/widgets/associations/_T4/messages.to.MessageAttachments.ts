/**
 * @file framework/admin/widgets/associations/_T4/message.to.MessageAttachments.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Message to MessageAttachments class
...
module cef.admin.widgets.associators.Messaging {
	class AssociatorForMessageToMessageAttachmentsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.MessageModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "MessageAttachments";
		paging: core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>;
		assigned: core.ServerSidePaging<api.MessageAttachmentModel, api.MessageAttachmentPagedResults>;
		users: api.UserModel[];
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `message-${this.record && this.record.ID}.to.MessageAttachments.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `message-${this.record && this.record.ID}.to.MessageAttachments.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.MessageAttachmentModel, api.MessageAttachmentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.messaging.GetMessageAttachments, 8, `message-${this.record && this.record.ID}.to.MessageAttachments.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `message-${this.record && this.record.ID}.to.MessageAttachments.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `message-${this.record && this.record.ID}.to.MessageAttachments.dupe-check`
			};
			this.cvApi.messaging.GetMessageAttachments(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.messaging.CreateMessageAttachment(<api.MessageAttachmentModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.MessageAttachmentModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.messaging.DeactivateMessageAttachmentByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefMessageToMessageAttachmentsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/messages.to.MessageAttachments.html", "ui"),
		controller: AssociatorForMessageToMessageAttachmentsController,
		controllerAs: "asCtrl",
