/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.PriceRuleCategories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to PriceRuleCategories class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToPriceRuleCategoriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PriceRuleModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "PriceRuleCategories";
		paging: core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>;
		assigned: core.ServerSidePaging<api.PriceRuleCategoryModel, api.PriceRuleCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleCategories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleCategories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleCategoryModel, api.PriceRuleCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleCategories, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleCategories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleCategories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.PriceRuleCategories.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleCategory(<api.PriceRuleCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToPriceRuleCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.PriceRuleCategories.html", "ui"),
		controller: AssociatorForPriceRuleToPriceRuleCategoriesController,
		controllerAs: "asCtrl",
