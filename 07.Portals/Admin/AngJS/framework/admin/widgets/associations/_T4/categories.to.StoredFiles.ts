/**
 * @file framework/admin/widgets/associations/_T4/category.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Category to StoredFiles class
...
module cef.admin.widgets.associators.Categories {
	class AssociatorForCategoryToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.CategoryFileModel, api.CategoryFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `category-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `category-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CategoryFileModel, api.CategoryFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.categories.GetCategoryFiles, 8, `category-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `category-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `category-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.categories.GetCategoryFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.categories.CreateCategoryFile(<api.CategoryFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CategoryFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.categories.DeactivateCategoryFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCategoryToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/categorys.to.StoredFiles.html", "ui"),
		controller: AssociatorForCategoryToStoredFilesController,
		controllerAs: "asCtrl",
