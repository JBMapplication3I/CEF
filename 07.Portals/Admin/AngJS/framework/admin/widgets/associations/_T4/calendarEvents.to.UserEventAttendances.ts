/**
 * @file framework/admin/widgets/associations/_T4/calendarEvent.to.UserEventAttendances.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Calendar Event to UserEventAttendances class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.CalendarEvents {
	class AssociatorForCalendarEventToUserEventAttendancesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.CalendarEventModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "UserEventAttendances";
		paging: core.ServerSidePaging<api.UserModel, api.UserPagedResults>;
		assigned: core.ServerSidePaging<api.UserEventAttendanceModel, api.UserEventAttendancePagedResults>;
		userEventAttendanceTypes: api.TypeModel[];
		...
		load(): void {
			...
			this.$q.all([
				this.cvApi.calendarEvents.GetUserEventAttendanceTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.userEventAttendanceTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `calendarEvent-${this.record && this.record.ID}.to.UserEventAttendances.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `calendarEvent-${this.record && this.record.ID}.to.UserEventAttendances.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.UserEventAttendanceModel, api.UserEventAttendancePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.calendarEvents.GetUserEventAttendances, 8, `calendarEvent-${this.record && this.record.ID}.to.UserEventAttendances.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `calendarEvent-${this.record && this.record.ID}.to.UserEventAttendances.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `calendarEvent-${this.record && this.record.ID}.to.UserEventAttendances.dupe-check`
			};
			this.cvApi.calendarEvents.GetUserEventAttendances(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.calendarEvents.CreateUserEventAttendance(<api.UserEventAttendanceModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.UserEventAttendanceModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.calendarEvents.DeactivateUserEventAttendanceByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCalendarEventToUserEventAttendancesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/calendarEvents.to.UserEventAttendances.html", "ui"),
		controller: AssociatorForCalendarEventToUserEventAttendancesController,
		controllerAs: "asCtrl",
