/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.FranchiseCurrencies.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to FranchiseCurrencies class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToFranchiseCurrenciesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.FranchiseModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "FranchiseCurrencies";
		paging: core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseCurrencyModel, api.FranchiseCurrencyPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.currencies.GetCurrencies, 8, `franchise-${this.record && this.record.ID}.to.FranchiseCurrencies.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseCurrencies.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseCurrencyModel, api.FranchiseCurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseCurrencies, 8, `franchise-${this.record && this.record.ID}.to.FranchiseCurrencies.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseCurrencies.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.FranchiseCurrencies.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseCurrencies(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseCurrency(<api.FranchiseCurrencyModel>{
					// Base Properties
					ID: null,
					Active: true,
					...
					IsPrimary: false,
					CustomName: null,
					CustomTranslationKey: null,
					OverrideUnicodeSymbolValue: null,
					OverrideHtmlCharacterCode: null,
					OverrideRawCharacter: null,
					OverrideDecimalPlaceAccuracy: null,
					OverrideUseSeparator: null,
					OverrideRawDecimalCharacter: null,
					OverrideHtmlDecimalCharacterCode: null,
					OverrideRawSeparatorCharacter: null,
					OverrideHtmlSeparatorCharacterCode: null,
				}).then(rc => {
					if (!rc || !rc.data) {
		...
		remove(toRemove: api.FranchiseCurrencyModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseCurrencyByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefFranchiseToFranchiseCurrenciesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.FranchiseCurrencies.html", "ui"),
		controller: AssociatorForFranchiseToFranchiseCurrenciesController,
		controllerAs: "asCtrl",
