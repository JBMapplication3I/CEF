/**
 * @file framework/admin/widgets/associations/_T4/salesReturn.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Return to Contacts class
...
module cef.admin.widgets.associators.Returning {
	class AssociatorForSalesReturnToContactsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesReturnContactModel, api.SalesReturnContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `salesReturn-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesReturn-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesReturnContactModel, api.SalesReturnContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.returning.GetSalesReturnContacts, 8, `salesReturn-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesReturn-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesReturn-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.returning.GetSalesReturnContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.returning.CreateSalesReturnContact(<api.SalesReturnContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesReturnContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.returning.DeactivateSalesReturnContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesReturnToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesReturns.to.Contacts.html", "ui"),
		controller: AssociatorForSalesReturnToContactsController,
		controllerAs: "asCtrl",
