/**
 * @file framework/admin/widgets/associations/_T4/sampleRequest.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sample Request to Contacts class
...
module cef.admin.widgets.associators.Sampling {
	class AssociatorForSampleRequestToContactsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SampleRequestModel;
		...
		assigned: core.ServerSidePaging<api.SampleRequestContactModel, api.SampleRequestContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `sampleRequest-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `sampleRequest-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SampleRequestContactModel, api.SampleRequestContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.sampling.GetSampleRequestContacts, 8, `sampleRequest-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `sampleRequest-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `sampleRequest-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.sampling.GetSampleRequestContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.sampling.CreateSampleRequestContact(<api.SampleRequestContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SampleRequestContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.sampling.DeactivateSampleRequestContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSampleRequestToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/sampleRequests.to.Contacts.html", "ui"),
		controller: AssociatorForSampleRequestToContactsController,
		controllerAs: "asCtrl",
