/**
 * @file framework/admin/widgets/associations/_T4/salesQuote.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Quote to StoredFiles class
...
module cef.admin.widgets.associators.Quoting {
	class AssociatorForSalesQuoteToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesQuoteFileModel, api.SalesQuoteFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `salesQuote-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesQuote-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesQuoteFileModel, api.SalesQuoteFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.quoting.GetSalesQuoteFiles, 8, `salesQuote-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesQuote-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesQuote-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.quoting.GetSalesQuoteFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.quoting.CreateSalesQuoteFile(<api.SalesQuoteFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesQuoteFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.quoting.DeactivateSalesQuoteFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesQuoteToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesQuotes.to.StoredFiles.html", "ui"),
		controller: AssociatorForSalesQuoteToStoredFilesController,
		controllerAs: "asCtrl",
