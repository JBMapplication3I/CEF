/**
 * @file framework/admin/widgets/associations/_T4/product.to.ProductAssociations.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to ProductAssociations class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToProductAssociationsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ProductModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "ProductAssociations";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `product-${this.record && this.record.ID}.to.ProductAssociations.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.ProductAssociations.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.ProductAssociationModel, api.ProductAssociationPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.products.GetProductAssociations, 8, `product-${this.record && this.record.ID}.to.ProductAssociations.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.ProductAssociations.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.ProductAssociations.dupe-check`
			};
	...
	adminApp.directive("cefProductToProductAssociationsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.ProductAssociations.html", "ui"),
		controller: AssociatorForProductToProductAssociationsController,
		controllerAs: "asCtrl",
