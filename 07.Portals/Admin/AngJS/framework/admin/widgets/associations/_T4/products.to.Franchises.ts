/**
 * @file framework/admin/widgets/associations/_T4/product.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to Franchises class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToFranchisesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.FranchiseProductModel, api.FranchiseProductPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `product-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseProductModel, api.FranchiseProductPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseProducts, 8, `product-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseProducts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseProduct(<api.FranchiseProductModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseProductModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseProductByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefProductToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.Franchises.html", "ui"),
		controller: AssociatorForProductToFranchisesController,
		controllerAs: "asCtrl",
