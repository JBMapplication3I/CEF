/**
 * @file framework/admin/widgets/associations/_T4/calendarEvent.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Calendar Event to StoredFiles class
...
module cef.admin.widgets.associators.CalendarEvents {
	class AssociatorForCalendarEventToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.CalendarEventFileModel, api.CalendarEventFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `calendarEvent-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `calendarEvent-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CalendarEventFileModel, api.CalendarEventFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.calendarEvents.GetCalendarEventFiles, 8, `calendarEvent-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `calendarEvent-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `calendarEvent-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.calendarEvents.GetCalendarEventFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.calendarEvents.CreateCalendarEventFile(<api.CalendarEventFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CalendarEventFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.calendarEvents.DeactivateCalendarEventFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCalendarEventToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/calendarEvents.to.StoredFiles.html", "ui"),
		controller: AssociatorForCalendarEventToStoredFilesController,
		controllerAs: "asCtrl",
