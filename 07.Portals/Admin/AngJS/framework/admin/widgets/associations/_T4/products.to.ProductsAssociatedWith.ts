/**
 * @file framework/admin/widgets/associations/_T4/product.to.ProductsAssociatedWith.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to ProductsAssociatedWith class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToProductsAssociatedWithController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ProductModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "ProductsAssociatedWith";
		paging: core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>;
		assigned: core.ServerSidePaging<api.ProductAssociationModel, api.ProductAssociationPagedResults>;
		stores: api.StoreModel[];
		brands: api.BrandModel[];
		productAssociationTypes: api.TypeModel[];
		...
		load(): void {
			...
			this.$q.all([
				...
				this.cvApi.products.GetProductAssociationTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.stores = rarr[index++].data.Results;
				this.brands = rarr[index++].data.Results;
				this.productAssociationTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `product-${this.record && this.record.ID}.to.ProductsAssociatedWith.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.ProductsAssociatedWith.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.ProductAssociationModel, api.ProductAssociationPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.products.GetProductAssociations, 8, `product-${this.record && this.record.ID}.to.ProductsAssociatedWith.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.ProductsAssociatedWith.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.ProductsAssociatedWith.dupe-check`
			};
			this.cvApi.products.GetProductAssociations(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.products.CreateProductAssociation(<api.ProductAssociationModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.ProductAssociationModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.products.DeactivateProductAssociationByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefProductToProductsAssociatedWithAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.ProductsAssociatedWith.html", "ui"),
		controller: AssociatorForProductToProductsAssociatedWithController,
		controllerAs: "asCtrl",
