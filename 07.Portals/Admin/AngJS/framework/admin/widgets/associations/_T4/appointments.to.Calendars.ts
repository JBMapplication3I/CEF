/**
 * @file framework/admin/widgets/associations/_T4/appointment.to.Calendars.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Appointment to Calendars class
...
module cef.admin.widgets.associators.Scheduling {
	class AssociatorForAppointmentToCalendarsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AppointmentModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Calendars";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AppointmentModel, api.AppointmentPagedResults>(
					...
					this.cvApi.scheduling.GetAppointments, 8, `appointment-${this.record && this.record.ID}.to.Calendars.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `appointment-${this.record && this.record.ID}.to.Calendars.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.CalendarAppointmentModel, api.CalendarAppointmentPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.scheduling.GetCalendarAppointments, 8, `appointment-${this.record && this.record.ID}.to.Calendars.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `appointment-${this.record && this.record.ID}.to.Calendars.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `appointment-${this.record && this.record.ID}.to.Calendars.dupe-check`
			};
	...
	adminApp.directive("cefAppointmentToCalendarsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/appointments.to.Calendars.html", "ui"),
		controller: AssociatorForAppointmentToCalendarsController,
		controllerAs: "asCtrl",
