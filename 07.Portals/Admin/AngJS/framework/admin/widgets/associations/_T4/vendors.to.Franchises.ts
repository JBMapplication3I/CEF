/**
 * @file framework/admin/widgets/associations/_T4/vendor.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Vendor to Franchises class
...
module cef.admin.widgets.associators.Vendors {
	class AssociatorForVendorToFranchisesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.VendorModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Franchises";
		paging: core.ServerSidePaging<api.VendorModel, api.VendorPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseVendorModel, api.FranchiseVendorPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.VendorModel, api.VendorPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.vendors.GetVendors, 8, `vendor-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `vendor-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseVendorModel, api.FranchiseVendorPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseVendors, 8, `vendor-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `vendor-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `vendor-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseVendors(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseVendor(<api.FranchiseVendorModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseVendorModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseVendorByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefVendorToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/vendors.to.Franchises.html", "ui"),
		controller: AssociatorForVendorToFranchisesController,
		controllerAs: "asCtrl",
