/**
 * @file framework/admin/widgets/associations/_T4/vendor.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Vendor to Brands class
...
module cef.admin.widgets.associators.Vendors {
	class AssociatorForVendorToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandVendorModel, api.BrandVendorPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.VendorModel, api.VendorPagedResults>(
					...
					this.cvApi.vendors.GetVendors, 8, `vendor-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `vendor-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandVendorModel, api.BrandVendorPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandVendors, 8, `vendor-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `vendor-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `vendor-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.brands.GetBrandVendors(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandVendor(<api.BrandVendorModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandVendorModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandVendorByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefVendorToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/vendors.to.Brands.html", "ui"),
		controller: AssociatorForVendorToBrandsController,
		controllerAs: "asCtrl",
