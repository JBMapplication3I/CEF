/**
 * @file framework/admin/widgets/associations/_T4/product.to.Vendors.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to Vendors class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToVendorsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `product-${this.record && this.record.ID}.to.Vendors.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.Vendors.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.VendorProductModel, api.VendorProductPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.vendors.GetVendorProducts, 8, `product-${this.record && this.record.ID}.to.Vendors.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.Vendors.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.Vendors.dupe-check`
			};
	...
	adminApp.directive("cefProductToVendorsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.Vendors.html", "ui"),
		controller: AssociatorForProductToVendorsController,
		controllerAs: "asCtrl",
