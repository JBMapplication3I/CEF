/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to Franchises class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToFranchisesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PriceRuleModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Franchises";
		paging: core.ServerSidePaging<api.FranchiseModel, api.FranchisePagedResults>;
		assigned: core.ServerSidePaging<api.PriceRuleFranchiseModel, api.PriceRuleFranchisePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.FranchiseModel, api.FranchisePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchises, 8, `priceRule-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleFranchiseModel, api.PriceRuleFranchisePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleFranchises, 8, `priceRule-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleFranchises(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleFranchise(<api.PriceRuleFranchiseModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleFranchiseModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleFranchiseByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.Franchises.html", "ui"),
		controller: AssociatorForPriceRuleToFranchisesController,
		controllerAs: "asCtrl",
