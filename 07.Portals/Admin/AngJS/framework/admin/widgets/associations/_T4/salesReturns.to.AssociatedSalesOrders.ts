/**
 * @file framework/admin/widgets/associations/_T4/salesReturn.to.AssociatedSalesOrders.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Return to AssociatedSalesOrders class
...
module cef.admin.widgets.associators.Returning {
	class AssociatorForSalesReturnToAssociatedSalesOrdersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesReturnModel;
		...
		assigned: core.ServerSidePaging<api.SalesReturnSalesOrderModel, api.SalesReturnSalesOrderPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SalesOrderModel, api.SalesOrderPagedResults>(
					...
					this.cvApi.ordering.GetSalesOrders, 8, `salesReturn-${this.record && this.record.ID}.to.AssociatedSalesOrders.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesReturn-${this.record && this.record.ID}.to.AssociatedSalesOrders.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesReturnSalesOrderModel, api.SalesReturnSalesOrderPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.returning.GetSalesReturnSalesOrders, 8, `salesReturn-${this.record && this.record.ID}.to.AssociatedSalesOrders.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesReturn-${this.record && this.record.ID}.to.AssociatedSalesOrders.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesReturn-${this.record && this.record.ID}.to.AssociatedSalesOrders.dupe-check`
			};
			this.cvApi.returning.GetSalesReturnSalesOrders(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.returning.CreateSalesReturnSalesOrder(<api.SalesReturnSalesOrderModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesReturnSalesOrderModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.returning.DeactivateSalesReturnSalesOrderByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesReturnToAssociatedSalesOrdersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesReturns.to.AssociatedSalesOrders.html", "ui"),
		controller: AssociatorForSalesReturnToAssociatedSalesOrdersController,
		controllerAs: "asCtrl",
