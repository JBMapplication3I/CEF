/**
 * @file framework/admin/widgets/associations/_T4/inventoryLocation.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Inventory Location to Franchises class
...
module cef.admin.widgets.associators.Inventory {
	class AssociatorForInventoryLocationToFranchisesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.InventoryLocationModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Franchises";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.InventoryLocationModel, api.InventoryLocationPagedResults>(
					...
					this.cvApi.inventory.GetInventoryLocations, 8, `inventoryLocation-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.FranchiseInventoryLocationModel, api.FranchiseInventoryLocationPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseInventoryLocations, 8, `inventoryLocation-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `inventoryLocation-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `inventoryLocation-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
	...
	adminApp.directive("cefInventoryLocationToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/inventoryLocations.to.Franchises.html", "ui"),
		controller: AssociatorForInventoryLocationToFranchisesController,
		controllerAs: "asCtrl",
