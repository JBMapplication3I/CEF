/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.AssociatedPurchaseOrders.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to AssociatedPurchaseOrders class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToAssociatedPurchaseOrdersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AssociatedPurchaseOrders";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.PurchaseOrderModel, api.PurchaseOrderPagedResults>(
					...
					this.cvApi.purchasing.GetPurchaseOrders, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedPurchaseOrders.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedPurchaseOrders.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.SalesOrderPurchaseOrderModel, api.SalesOrderPurchaseOrderPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.purchasing.GetSalesOrderPurchaseOrders, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedPurchaseOrders.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedPurchaseOrders.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.AssociatedPurchaseOrders.dupe-check`
			};
	...
	adminApp.directive("cefSalesOrderToAssociatedPurchaseOrdersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.AssociatedPurchaseOrders.html", "ui"),
		controller: AssociatorForSalesOrderToAssociatedPurchaseOrdersController,
		controllerAs: "asCtrl",
