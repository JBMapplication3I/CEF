/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.FranchiseSiteDomains.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to FranchiseSiteDomains class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToFranchiseSiteDomainsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.FranchiseModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "FranchiseSiteDomains";
		paging: core.ServerSidePaging<api.SiteDomainModel, api.SiteDomainPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseSiteDomainModel, api.FranchiseSiteDomainPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.SiteDomainModel, api.SiteDomainPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetSiteDomains, 8, `franchise-${this.record && this.record.ID}.to.FranchiseSiteDomains.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseSiteDomains.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseSiteDomainModel, api.FranchiseSiteDomainPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseSiteDomains, 8, `franchise-${this.record && this.record.ID}.to.FranchiseSiteDomains.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseSiteDomains.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.FranchiseSiteDomains.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseSiteDomains(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseSiteDomain(<api.FranchiseSiteDomainModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseSiteDomainModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseSiteDomainByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefFranchiseToFranchiseSiteDomainsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.FranchiseSiteDomains.html", "ui"),
		controller: AssociatorForFranchiseToFranchiseSiteDomainsController,
		controllerAs: "asCtrl",
