/**
 * @file framework/admin/widgets/associations/_T4/user.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for User to Stores class
...
module cef.admin.widgets.associators.Contacts {
	class AssociatorForUserToStoresController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `user-${this.record && this.record.ID}.to.Stores.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `user-${this.record && this.record.ID}.to.Stores.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.StoreUserModel, api.StoreUserPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreUsers, 8, `user-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `user-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `user-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
	...
	adminApp.directive("cefUserToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/users.to.Stores.html", "ui"),
		controller: AssociatorForUserToStoresController,
		controllerAs: "asCtrl",
