/**
 * @file framework/admin/widgets/associations/_T4/calendar.to.Appointments.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Calendar to Appointments class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Scheduling {
	class AssociatorForCalendarToAppointmentsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.CalendarModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Appointments";
		paging: core.ServerSidePaging<api.AppointmentModel, api.AppointmentPagedResults>;
		assigned: core.ServerSidePaging<api.CalendarAppointmentModel, api.CalendarAppointmentPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.AppointmentModel, api.AppointmentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.scheduling.GetAppointments, 8, `calendar-${this.record && this.record.ID}.to.Appointments.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `calendar-${this.record && this.record.ID}.to.Appointments.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CalendarAppointmentModel, api.CalendarAppointmentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.scheduling.GetCalendarAppointments, 8, `calendar-${this.record && this.record.ID}.to.Appointments.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `calendar-${this.record && this.record.ID}.to.Appointments.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `calendar-${this.record && this.record.ID}.to.Appointments.dupe-check`
			};
			this.cvApi.scheduling.GetCalendarAppointments(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.scheduling.CreateCalendarAppointment(<api.CalendarAppointmentModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CalendarAppointmentModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.scheduling.DeactivateCalendarAppointmentByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCalendarToAppointmentsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/calendars.to.Appointments.html", "ui"),
		controller: AssociatorForCalendarToAppointmentsController,
		controllerAs: "asCtrl",
