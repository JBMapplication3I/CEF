/**
 * @file framework/admin/widgets/associations/_T4/auction.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Auction to Brands class
...
module cef.admin.widgets.associators.Auctions {
	class AssociatorForAuctionToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandAuctionModel, api.BrandAuctionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AuctionModel, api.AuctionPagedResults>(
					...
					this.cvApi.auctions.GetAuctions, 8, `auction-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `auction-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandAuctionModel, api.BrandAuctionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.auctions.GetBrandAuctions, 8, `auction-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `auction-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `auction-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.auctions.GetBrandAuctions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.auctions.CreateBrandAuction(<api.BrandAuctionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandAuctionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.auctions.DeactivateBrandAuctionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAuctionToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/auctions.to.Brands.html", "ui"),
		controller: AssociatorForAuctionToBrandsController,
		controllerAs: "asCtrl",
