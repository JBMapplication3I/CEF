/**
 * @file framework/admin/widgets/associations/_T4/account.to.AccountCurrencies.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to AccountCurrencies class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToAccountCurrenciesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AccountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AccountCurrencies";
		paging: core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>;
		assigned: core.ServerSidePaging<api.AccountCurrencyModel, api.AccountCurrencyPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>(
					...
					this.cvApi.currencies.GetCurrencies, 8, `account-${this.record && this.record.ID}.to.AccountCurrencies.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.AccountCurrencies.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AccountCurrencyModel, api.AccountCurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountCurrencies, 8, `account-${this.record && this.record.ID}.to.AccountCurrencies.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.AccountCurrencies.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.AccountCurrencies.dupe-check`
			};
			this.cvApi.accounts.GetAccountCurrencies(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.accounts.CreateAccountCurrency(<api.AccountCurrencyModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AccountCurrencyModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.accounts.DeactivateAccountCurrencyByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToAccountCurrenciesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.AccountCurrencies.html", "ui"),
		controller: AssociatorForAccountToAccountCurrenciesController,
		controllerAs: "asCtrl",
