/**
 * @file framework/admin/widgets/associations/_T4/account.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to Franchises class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToFranchisesController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `account-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.FranchiseAccountModel, api.FranchiseAccountPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseAccounts, 8, `account-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
	...
	adminApp.directive("cefAccountToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.Franchises.html", "ui"),
		controller: AssociatorForAccountToFranchisesController,
		controllerAs: "asCtrl",
