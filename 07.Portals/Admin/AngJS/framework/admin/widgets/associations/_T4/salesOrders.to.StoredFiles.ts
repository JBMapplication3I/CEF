/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to StoredFiles class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesOrderFileModel, api.SalesOrderFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `salesOrder-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesOrderFileModel, api.SalesOrderFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.ordering.GetSalesOrderFiles, 8, `salesOrder-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.ordering.GetSalesOrderFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.ordering.CreateSalesOrderFile(<api.SalesOrderFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesOrderFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.ordering.DeactivateSalesOrderFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesOrderToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.StoredFiles.html", "ui"),
		controller: AssociatorForSalesOrderToStoredFilesController,
		controllerAs: "asCtrl",
