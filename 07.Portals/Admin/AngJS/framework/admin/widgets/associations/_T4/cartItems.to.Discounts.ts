/**
 * @file framework/admin/widgets/associations/_T4/cartItem.to.Discounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Cart Item to Discounts class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Shopping {
	class AssociatorForCartItemToDiscountsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AppliedCartItemDiscountModel, api.AppliedCartItemDiscountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.DiscountModel, api.DiscountPagedResults>(
					...
					this.cvApi.discounts.GetDiscounts, 8, `cartItem-${this.record && this.record.ID}.to.Discounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `cartItem-${this.record && this.record.ID}.to.Discounts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AppliedCartItemDiscountModel, api.AppliedCartItemDiscountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetAppliedCartItemDiscounts, 8, `cartItem-${this.record && this.record.ID}.to.Discounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `cartItem-${this.record && this.record.ID}.to.Discounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `cartItem-${this.record && this.record.ID}.to.Discounts.dupe-check`
			};
			this.cvApi.discounts.GetAppliedCartItemDiscounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateAppliedCartItemDiscount(<api.AppliedCartItemDiscountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AppliedCartItemDiscountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateAppliedCartItemDiscountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCartItemToDiscountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/cartItems.to.Discounts.html", "ui"),
		controller: AssociatorForCartItemToDiscountsController,
		controllerAs: "asCtrl",
