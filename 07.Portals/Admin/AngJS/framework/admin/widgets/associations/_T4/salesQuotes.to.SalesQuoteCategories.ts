/**
 * @file framework/admin/widgets/associations/_T4/salesQuote.to.SalesQuoteCategories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Quote to SalesQuoteCategories class
...
module cef.admin.widgets.associators.Quoting {
	class AssociatorForSalesQuoteToSalesQuoteCategoriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesQuoteModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "SalesQuoteCategories";
		paging: core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>;
		assigned: core.ServerSidePaging<api.SalesQuoteCategoryModel, api.SalesQuoteCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `salesQuote-${this.record && this.record.ID}.to.SalesQuoteCategories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesQuote-${this.record && this.record.ID}.to.SalesQuoteCategories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesQuoteCategoryModel, api.SalesQuoteCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.quoting.GetSalesQuoteCategories, 8, `salesQuote-${this.record && this.record.ID}.to.SalesQuoteCategories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesQuote-${this.record && this.record.ID}.to.SalesQuoteCategories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesQuote-${this.record && this.record.ID}.to.SalesQuoteCategories.dupe-check`
			};
			this.cvApi.quoting.GetSalesQuoteCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.quoting.CreateSalesQuoteCategory(<api.SalesQuoteCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesQuoteCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.quoting.DeactivateSalesQuoteCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesQuoteToSalesQuoteCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesQuotes.to.SalesQuoteCategories.html", "ui"),
		controller: AssociatorForSalesQuoteToSalesQuoteCategoriesController,
		controllerAs: "asCtrl",
