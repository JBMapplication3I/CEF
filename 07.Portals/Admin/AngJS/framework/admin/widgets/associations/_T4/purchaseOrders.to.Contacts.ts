/**
 * @file framework/admin/widgets/associations/_T4/purchaseOrder.to.Contacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Purchase Order to Contacts class
...
module cef.admin.widgets.associators.Purchasing {
	class AssociatorForPurchaseOrderToContactsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PurchaseOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Contacts";
		paging: core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>;
		assigned: core.ServerSidePaging<api.PurchaseOrderContactModel, api.PurchaseOrderContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.contacts.GetContacts, 8, `purchaseOrder-${this.record && this.record.ID}.to.Contacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `purchaseOrder-${this.record && this.record.ID}.to.Contacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PurchaseOrderContactModel, api.PurchaseOrderContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.purchasing.GetPurchaseOrderContacts, 8, `purchaseOrder-${this.record && this.record.ID}.to.Contacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `purchaseOrder-${this.record && this.record.ID}.to.Contacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `purchaseOrder-${this.record && this.record.ID}.to.Contacts.dupe-check`
			};
			this.cvApi.purchasing.GetPurchaseOrderContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.purchasing.CreatePurchaseOrderContact(<api.PurchaseOrderContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PurchaseOrderContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.purchasing.DeactivatePurchaseOrderContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPurchaseOrderToContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/purchaseOrders.to.Contacts.html", "ui"),
		controller: AssociatorForPurchaseOrderToContactsController,
		controllerAs: "asCtrl",
