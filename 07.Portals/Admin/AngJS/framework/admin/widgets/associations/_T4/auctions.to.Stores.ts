/**
 * @file framework/admin/widgets/associations/_T4/auction.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Auction to Stores class
...
module cef.admin.widgets.associators.Auctions {
	class AssociatorForAuctionToStoresController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreAuctionModel, api.StoreAuctionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AuctionModel, api.AuctionPagedResults>(
					...
					this.cvApi.auctions.GetAuctions, 8, `auction-${this.record && this.record.ID}.to.Stores.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `auction-${this.record && this.record.ID}.to.Stores.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreAuctionModel, api.StoreAuctionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.auctions.GetStoreAuctions, 8, `auction-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `auction-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `auction-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
			this.cvApi.auctions.GetStoreAuctions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.auctions.CreateStoreAuction(<api.StoreAuctionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreAuctionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.auctions.DeactivateStoreAuctionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAuctionToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/auctions.to.Stores.html", "ui"),
		controller: AssociatorForAuctionToStoresController,
		controllerAs: "asCtrl",
