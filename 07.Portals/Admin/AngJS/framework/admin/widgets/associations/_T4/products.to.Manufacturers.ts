/**
 * @file framework/admin/widgets/associations/_T4/product.to.Manufacturers.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Product to Manufacturers class
...
module cef.admin.widgets.associators.Products {
	class AssociatorForProductToManufacturersController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					8,
					`product-${this.record && this.record.ID}.to.Manufacturers.available`,
					null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `product-${this.record && this.record.ID}.to.Manufacturers.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.ManufacturerProductModel, api.ManufacturerProductPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.manufacturers.GetManufacturerProducts, 8, `product-${this.record && this.record.ID}.to.Manufacturers.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `product-${this.record && this.record.ID}.to.Manufacturers.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `product-${this.record && this.record.ID}.to.Manufacturers.dupe-check`
			};
	...
	adminApp.directive("cefProductToManufacturersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/products.to.Manufacturers.html", "ui"),
		controller: AssociatorForProductToManufacturersController,
		controllerAs: "asCtrl",
