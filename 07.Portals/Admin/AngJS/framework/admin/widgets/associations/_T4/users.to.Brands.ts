/**
 * @file framework/admin/widgets/associations/_T4/user.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for User to Brands class
...
module cef.admin.widgets.associators.Contacts {
	class AssociatorForUserToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandUserModel, api.BrandUserPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `user-${this.record && this.record.ID}.to.Brands.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `user-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandUserModel, api.BrandUserPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandUsers, 8, `user-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `user-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `user-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.brands.GetBrandUsers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandUser(<api.BrandUserModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandUserModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandUserByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefUserToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/users.to.Brands.html", "ui"),
		controller: AssociatorForUserToBrandsController,
		controllerAs: "asCtrl",
