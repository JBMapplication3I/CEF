/**
 * @file framework/admin/widgets/associations/_T4/ad.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Ad to Franchises class
...
module cef.admin.widgets.associators.Advertising {
	class AssociatorForAdToFranchisesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AdFranchiseModel, api.AdFranchisePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.FranchiseModel, api.FranchisePagedResults>(
					...
					this.cvApi.franchises.GetFranchises, 8, `ad-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `ad-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AdFranchiseModel, api.AdFranchisePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.advertising.GetAdFranchises, 8, `ad-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `ad-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `ad-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.advertising.GetAdFranchises(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.advertising.CreateAdFranchise(<api.AdFranchiseModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AdFranchiseModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.advertising.DeactivateAdFranchiseByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAdToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/ads.to.Franchises.html", "ui"),
		controller: AssociatorForAdToFranchisesController,
		controllerAs: "asCtrl",
