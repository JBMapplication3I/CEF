/**
 * @file framework/admin/widgets/associations/_T4/account.to.AccountsAssociatedWith.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to AccountsAssociatedWith class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToAccountsAssociatedWithController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AccountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AccountsAssociatedWith";
		paging: core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>;
		assigned: core.ServerSidePaging<api.AccountAssociationModel, api.AccountAssociationPagedResults>;
		accountAssociationTypes: api.TypeModel[];
		...
		load(): void {
			...
			this.$q.all([
				this.cvApi.accounts.GetAccountAssociationTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.accountAssociationTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `account-${this.record && this.record.ID}.to.AccountsAssociatedWith.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.AccountsAssociatedWith.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AccountAssociationModel, api.AccountAssociationPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountAssociations, 8, `account-${this.record && this.record.ID}.to.AccountsAssociatedWith.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.AccountsAssociatedWith.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.AccountsAssociatedWith.dupe-check`
			};
			this.cvApi.accounts.GetAccountAssociations(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.accounts.CreateAccountAssociation(<api.AccountAssociationModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AccountAssociationModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.accounts.DeactivateAccountAssociationByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToAccountsAssociatedWithAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.AccountsAssociatedWith.html", "ui"),
		controller: AssociatorForAccountToAccountsAssociatedWithController,
		controllerAs: "asCtrl",
