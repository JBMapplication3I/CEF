/**
 * @file framework/admin/widgets/associations/_T4/purchaseOrder.to.AssociatedSalesOrders.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Purchase Order to AssociatedSalesOrders class
...
module cef.admin.widgets.associators.Purchasing {
	class AssociatorForPurchaseOrderToAssociatedSalesOrdersController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PurchaseOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AssociatedSalesOrders";
		paging: core.ServerSidePaging<api.PurchaseOrderModel, api.PurchaseOrderPagedResults>;
		assigned: core.ServerSidePaging<api.SalesOrderPurchaseOrderModel, api.SalesOrderPurchaseOrderPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.PurchaseOrderModel, api.PurchaseOrderPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.purchasing.GetPurchaseOrders, 8, `purchaseOrder-${this.record && this.record.ID}.to.AssociatedSalesOrders.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `purchaseOrder-${this.record && this.record.ID}.to.AssociatedSalesOrders.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesOrderPurchaseOrderModel, api.SalesOrderPurchaseOrderPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.purchasing.GetSalesOrderPurchaseOrders, 8, `purchaseOrder-${this.record && this.record.ID}.to.AssociatedSalesOrders.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `purchaseOrder-${this.record && this.record.ID}.to.AssociatedSalesOrders.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `purchaseOrder-${this.record && this.record.ID}.to.AssociatedSalesOrders.dupe-check`
			};
			this.cvApi.purchasing.GetSalesOrderPurchaseOrders(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.purchasing.CreateSalesOrderPurchaseOrder(<api.SalesOrderPurchaseOrderModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesOrderPurchaseOrderModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.purchasing.DeactivateSalesOrderPurchaseOrderByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPurchaseOrderToAssociatedSalesOrdersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/purchaseOrders.to.AssociatedSalesOrders.html", "ui"),
		controller: AssociatorForPurchaseOrderToAssociatedSalesOrdersController,
		controllerAs: "asCtrl",
