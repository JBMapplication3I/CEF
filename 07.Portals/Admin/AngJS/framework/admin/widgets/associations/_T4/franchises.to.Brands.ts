/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to Brands class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToBrandsController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.FranchiseModel, api.FranchisePagedResults>(
					...
					this.cvApi.franchises.GetFranchises, 8, `franchise-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.Brands.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.BrandFranchiseModel, api.BrandFranchisePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandFranchises, 8, `franchise-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
	...
	adminApp.directive("cefFranchiseToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.Brands.html", "ui"),
		controller: AssociatorForFranchiseToBrandsController,
		controllerAs: "asCtrl",
