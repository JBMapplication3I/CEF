/**
 * @file framework/admin/widgets/associations/_T4/account.to.AccountPricePoints.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to AccountPricePoints class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToAccountPricePointsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AccountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AccountPricePoints";
		paging: core.ServerSidePaging<api.PricePointModel, api.PricePointPagedResults>;
		assigned: core.ServerSidePaging<api.AccountPricePointModel, api.AccountPricePointPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.PricePointModel, api.PricePointPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPricePoints, 8, `account-${this.record && this.record.ID}.to.AccountPricePoints.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.AccountPricePoints.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AccountPricePointModel, api.AccountPricePointPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountPricePoints, 8, `account-${this.record && this.record.ID}.to.AccountPricePoints.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.AccountPricePoints.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.AccountPricePoints.dupe-check`
			};
			this.cvApi.accounts.GetAccountPricePoints(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.accounts.CreateAccountPricePoint(<api.AccountPricePointModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AccountPricePointModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.accounts.DeactivateAccountPricePointByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToAccountPricePointsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.AccountPricePoints.html", "ui"),
		controller: AssociatorForAccountToAccountPricePointsController,
		controllerAs: "asCtrl",
