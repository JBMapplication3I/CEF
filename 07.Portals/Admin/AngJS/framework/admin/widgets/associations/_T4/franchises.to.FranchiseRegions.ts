/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.FranchiseRegions.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to FranchiseRegions class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToFranchiseRegionsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.FranchiseModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "FranchiseRegions";
		paging: core.ServerSidePaging<api.RegionModel, api.RegionPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseRegionModel, api.FranchiseRegionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.RegionModel, api.RegionPagedResults>(
					...
					this.cvApi.geography.GetRegions, 8, `franchise-${this.record && this.record.ID}.to.FranchiseRegions.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseRegions.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseRegionModel, api.FranchiseRegionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseRegions, 8, `franchise-${this.record && this.record.ID}.to.FranchiseRegions.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseRegions.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.FranchiseRegions.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseRegions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseRegion(<api.FranchiseRegionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseRegionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseRegionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefFranchiseToFranchiseRegionsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.FranchiseRegions.html", "ui"),
		controller: AssociatorForFranchiseToFranchiseRegionsController,
		controllerAs: "asCtrl",
