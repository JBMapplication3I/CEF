/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.AssociatedSalesQuotes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to AssociatedSalesQuotes class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToAssociatedSalesQuotesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AssociatedSalesQuotes";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SalesOrderModel, api.SalesOrderPagedResults>(
					...
					this.cvApi.ordering.GetSalesOrders, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesQuotes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesQuotes.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.SalesQuoteSalesOrderModel, api.SalesQuoteSalesOrderPagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.quoting.GetSalesQuoteSalesOrders, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesQuotes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesQuotes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesQuotes.dupe-check`
			};
	...
	adminApp.directive("cefSalesOrderToAssociatedSalesQuotesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.AssociatedSalesQuotes.html", "ui"),
		controller: AssociatorForSalesOrderToAssociatedSalesQuotesController,
		controllerAs: "asCtrl",
