/**
 * @file framework/admin/widgets/associations/_T4/subscription.to.SubscriptionHistories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Subscription to SubscriptionHistories class
...
module cef.admin.widgets.associators.Payments {
	class AssociatorForSubscriptionToSubscriptionHistoriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SubscriptionModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "SubscriptionHistories";
		paging: core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>;
		assigned: core.ServerSidePaging<api.SubscriptionHistoryModel, api.SubscriptionHistoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetPayments, 8, `subscription-${this.record && this.record.ID}.to.SubscriptionHistories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `subscription-${this.record && this.record.ID}.to.SubscriptionHistories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SubscriptionHistoryModel, api.SubscriptionHistoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetSubscriptionHistories, 8, `subscription-${this.record && this.record.ID}.to.SubscriptionHistories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `subscription-${this.record && this.record.ID}.to.SubscriptionHistories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `subscription-${this.record && this.record.ID}.to.SubscriptionHistories.dupe-check`
			};
			this.cvApi.payments.GetSubscriptionHistories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.payments.CreateSubscriptionHistory(<api.SubscriptionHistoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SubscriptionHistoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.payments.DeactivateSubscriptionHistoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSubscriptionToSubscriptionHistoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/subscriptions.to.SubscriptionHistories.html", "ui"),
		controller: AssociatorForSubscriptionToSubscriptionHistoriesController,
		controllerAs: "asCtrl",
