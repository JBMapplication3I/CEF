/**
 * @file framework/admin/widgets/associations/_T4/salesOrder.to.AssociatedSalesInvoices.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order to AssociatedSalesInvoices class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderToAssociatedSalesInvoicesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesOrderModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AssociatedSalesInvoices";
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SalesInvoiceModel, api.SalesInvoicePagedResults>(
					...
					this.cvApi.invoicing.GetSalesInvoices, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesInvoices.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesInvoices.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.SalesOrderSalesInvoiceModel, api.SalesOrderSalesInvoicePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.invoicing.GetSalesOrderSalesInvoices, 8, `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesInvoices.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesInvoices.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrder-${this.record && this.record.ID}.to.AssociatedSalesInvoices.dupe-check`
			};
	...
	adminApp.directive("cefSalesOrderToAssociatedSalesInvoicesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrders.to.AssociatedSalesInvoices.html", "ui"),
		controller: AssociatorForSalesOrderToAssociatedSalesInvoicesController,
		controllerAs: "asCtrl",
