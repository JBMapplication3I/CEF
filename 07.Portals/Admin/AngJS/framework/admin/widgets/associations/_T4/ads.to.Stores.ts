/**
 * @file framework/admin/widgets/associations/_T4/ad.to.Stores.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Ad to Stores class
...
module cef.admin.widgets.associators.Advertising {
	class AssociatorForAdToStoresController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AdStoreModel, api.AdStorePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoreModel, api.StorePagedResults>(
					...
					this.cvApi.stores.GetStores, 8, `ad-${this.record && this.record.ID}.to.Stores.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `ad-${this.record && this.record.ID}.to.Stores.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AdStoreModel, api.AdStorePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.advertising.GetAdStores, 8, `ad-${this.record && this.record.ID}.to.Stores.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `ad-${this.record && this.record.ID}.to.Stores.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `ad-${this.record && this.record.ID}.to.Stores.dupe-check`
			};
			this.cvApi.advertising.GetAdStores(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.advertising.CreateAdStore(<api.AdStoreModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AdStoreModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.advertising.DeactivateAdStoreByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAdToStoresAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/ads.to.Stores.html", "ui"),
		controller: AssociatorForAdToStoresController,
		controllerAs: "asCtrl",
