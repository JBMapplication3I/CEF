/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Accounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Accounts class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToAccountsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountAccountModel, api.DiscountAccountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `discount-${this.record && this.record.ID}.to.Accounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Accounts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountAccountModel, api.DiscountAccountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountAccounts, 8, `discount-${this.record && this.record.ID}.to.Accounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Accounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Accounts.dupe-check`
			};
			this.cvApi.discounts.GetDiscountAccounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountAccount(<api.DiscountAccountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountAccountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountAccountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToAccountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Accounts.html", "ui"),
		controller: AssociatorForDiscountToAccountsController,
		controllerAs: "asCtrl",
