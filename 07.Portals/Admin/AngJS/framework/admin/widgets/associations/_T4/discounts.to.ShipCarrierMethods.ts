/**
 * @file framework/admin/widgets/associations/_T4/discount.to.ShipCarrierMethods.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to ShipCarrierMethods class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToShipCarrierMethodsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.DiscountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "ShipCarrierMethods";
		paging: core.ServerSidePaging<api.ShipCarrierMethodModel, api.ShipCarrierMethodPagedResults>;
		assigned: core.ServerSidePaging<api.DiscountShipCarrierMethodModel, api.DiscountShipCarrierMethodPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ShipCarrierMethodModel, api.ShipCarrierMethodPagedResults>(
					...
					this.cvApi.shipping.GetShipCarrierMethods, 8, `discount-${this.record && this.record.ID}.to.ShipCarrierMethods.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.ShipCarrierMethods.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountShipCarrierMethodModel, api.DiscountShipCarrierMethodPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountShipCarrierMethods, 8, `discount-${this.record && this.record.ID}.to.ShipCarrierMethods.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.ShipCarrierMethods.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.ShipCarrierMethods.dupe-check`
			};
			this.cvApi.discounts.GetDiscountShipCarrierMethods(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountShipCarrierMethod(<api.DiscountShipCarrierMethodModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountShipCarrierMethodModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountShipCarrierMethodByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToShipCarrierMethodsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.ShipCarrierMethods.html", "ui"),
		controller: AssociatorForDiscountToShipCarrierMethodsController,
		controllerAs: "asCtrl",
