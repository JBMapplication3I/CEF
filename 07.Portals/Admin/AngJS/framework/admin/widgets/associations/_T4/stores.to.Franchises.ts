/**
 * @file framework/admin/widgets/associations/_T4/store.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to Franchises class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToFranchisesController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoreModel, api.StorePagedResults>(
					...
					this.cvApi.stores.GetStores, 8, `store-${this.record && this.record.ID}.to.Franchises.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.FranchiseStoreModel, api.FranchiseStorePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseStores, 8, `store-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
	...
	adminApp.directive("cefStoreToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.Franchises.html", "ui"),
		controller: AssociatorForStoreToFranchisesController,
		controllerAs: "asCtrl",
