/**
 * @file framework/admin/widgets/associations/_T4/store.to.Vendors.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to Vendors class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToVendorsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreVendorModel, api.StoreVendorPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.VendorModel, api.VendorPagedResults>(
					...
					this.cvApi.vendors.GetVendors, 8, `store-${this.record && this.record.ID}.to.Vendors.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.Vendors.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreVendorModel, api.StoreVendorPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreVendors, 8, `store-${this.record && this.record.ID}.to.Vendors.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.Vendors.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.Vendors.dupe-check`
			};
			this.cvApi.stores.GetStoreVendors(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreVendor(<api.StoreVendorModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreVendorModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreVendorByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToVendorsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.Vendors.html", "ui"),
		controller: AssociatorForStoreToVendorsController,
		controllerAs: "asCtrl",
