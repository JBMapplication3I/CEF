/**
 * @file framework/admin/widgets/associations/_T4/salesInvoice.to.SalesInvoicePayments.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Invoice to SalesInvoicePayments class
...
module cef.admin.widgets.associators.Invoicing {
	class AssociatorForSalesInvoiceToSalesInvoicePaymentsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesInvoiceModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "SalesInvoicePayments";
		paging: core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>;
		assigned: core.ServerSidePaging<api.SalesInvoicePaymentModel, api.SalesInvoicePaymentPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.PaymentModel, api.PaymentPagedResults>(
					...
					this.cvApi.payments.GetPayments, 8, `salesInvoice-${this.record && this.record.ID}.to.SalesInvoicePayments.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesInvoice-${this.record && this.record.ID}.to.SalesInvoicePayments.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesInvoicePaymentModel, api.SalesInvoicePaymentPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.payments.GetSalesInvoicePayments, 8, `salesInvoice-${this.record && this.record.ID}.to.SalesInvoicePayments.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesInvoice-${this.record && this.record.ID}.to.SalesInvoicePayments.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesInvoice-${this.record && this.record.ID}.to.SalesInvoicePayments.dupe-check`
			};
			this.cvApi.payments.GetSalesInvoicePayments(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.payments.CreateSalesInvoicePayment(<api.SalesInvoicePaymentModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesInvoicePaymentModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.payments.DeactivateSalesInvoicePaymentByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesInvoiceToSalesInvoicePaymentsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesInvoices.to.SalesInvoicePayments.html", "ui"),
		controller: AssociatorForSalesInvoiceToSalesInvoicePaymentsController,
		controllerAs: "asCtrl",
