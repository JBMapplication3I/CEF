/**
 * @file framework/admin/widgets/associations/_T4/account.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to Brands class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandAccountModel, api.BrandAccountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.AccountModel, api.AccountPagedResults>(
					...
					this.cvApi.accounts.GetAccounts, 8, `account-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandAccountModel, api.BrandAccountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandAccounts, 8, `account-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.brands.GetBrandAccounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandAccount(<api.BrandAccountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandAccountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandAccountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.Brands.html", "ui"),
		controller: AssociatorForAccountToBrandsController,
		controllerAs: "asCtrl",
