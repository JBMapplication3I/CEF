/**
 * @file framework/admin/widgets/associations/_T4/region.to.Currencies.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Region to Currencies class
...
module cef.admin.widgets.associators.Geography {
	class AssociatorForRegionToCurrenciesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.RegionModel;
		...
		assigned: core.ServerSidePaging<api.RegionCurrencyModel, api.RegionCurrencyPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>(
					...
					this.cvApi.currencies.GetCurrencies, 8, `region-${this.record && this.record.ID}.to.Currencies.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `region-${this.record && this.record.ID}.to.Currencies.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.RegionCurrencyModel, api.RegionCurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetRegionCurrencies, 8, `region-${this.record && this.record.ID}.to.Currencies.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `region-${this.record && this.record.ID}.to.Currencies.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `region-${this.record && this.record.ID}.to.Currencies.dupe-check`
			};
			this.cvApi.geography.GetRegionCurrencies(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.geography.CreateRegionCurrency(<api.RegionCurrencyModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.RegionCurrencyModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.geography.DeactivateRegionCurrencyByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefRegionToCurrenciesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/regions.to.Currencies.html", "ui"),
		controller: AssociatorForRegionToCurrenciesController,
		controllerAs: "asCtrl",
