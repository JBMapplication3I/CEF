/**
 * @file framework/admin/widgets/associations/_T4/country.to.Currencies.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Country to Currencies class
...
module cef.admin.widgets.associators.Geography {
	class AssociatorForCountryToCurrenciesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.CountryModel;
		...
		assigned: core.ServerSidePaging<api.CountryCurrencyModel, api.CountryCurrencyPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>(
					...
					this.cvApi.currencies.GetCurrencies, 8, `country-${this.record && this.record.ID}.to.Currencies.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `country-${this.record && this.record.ID}.to.Currencies.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CountryCurrencyModel, api.CountryCurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetCountryCurrencies, 8, `country-${this.record && this.record.ID}.to.Currencies.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `country-${this.record && this.record.ID}.to.Currencies.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `country-${this.record && this.record.ID}.to.Currencies.dupe-check`
			};
			this.cvApi.geography.GetCountryCurrencies(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.geography.CreateCountryCurrency(<api.CountryCurrencyModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CountryCurrencyModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.geography.DeactivateCountryCurrencyByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCountryToCurrenciesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/countrys.to.Currencies.html", "ui"),
		controller: AssociatorForCountryToCurrenciesController,
		controllerAs: "asCtrl",
