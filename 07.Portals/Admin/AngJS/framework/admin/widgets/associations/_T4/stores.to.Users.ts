/**
 * @file framework/admin/widgets/associations/_T4/store.to.Users.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to Users class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToUsersController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.StoreUserModel, api.StoreUserPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `store-${this.record && this.record.ID}.to.Users.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.Users.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreUserModel, api.StoreUserPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreUsers, 8, `store-${this.record && this.record.ID}.to.Users.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.Users.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.Users.dupe-check`
			};
			this.cvApi.stores.GetStoreUsers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreUser(<api.StoreUserModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreUserModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreUserByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToUsersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.Users.html", "ui"),
		controller: AssociatorForStoreToUsersController,
		controllerAs: "asCtrl",
