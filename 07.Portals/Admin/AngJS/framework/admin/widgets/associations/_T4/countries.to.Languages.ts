/**
 * @file framework/admin/widgets/associations/_T4/country.to.Languages.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Country to Languages class
...
module cef.admin.widgets.associators.Geography {
	class AssociatorForCountryToLanguagesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.CountryLanguageModel, api.CountryLanguagePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.LanguageModel, api.LanguagePagedResults>(
					...
					this.cvApi.globalization.GetLanguages, 8, `country-${this.record && this.record.ID}.to.Languages.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `country-${this.record && this.record.ID}.to.Languages.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.CountryLanguageModel, api.CountryLanguagePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetCountryLanguages, 8, `country-${this.record && this.record.ID}.to.Languages.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `country-${this.record && this.record.ID}.to.Languages.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `country-${this.record && this.record.ID}.to.Languages.dupe-check`
			};
			this.cvApi.geography.GetCountryLanguages(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.geography.CreateCountryLanguage(<api.CountryLanguageModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.CountryLanguageModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.geography.DeactivateCountryLanguageByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefCountryToLanguagesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/countrys.to.Languages.html", "ui"),
		controller: AssociatorForCountryToLanguagesController,
		controllerAs: "asCtrl",
