/**
 * @file framework/admin/widgets/associations/_T4/discount.to.AccountTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to AccountTypes class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToAccountTypesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.DiscountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AccountTypes";
		paging: core.ServerSidePaging<api.TypeModel, api.AccountTypePagedResults>;
		assigned: core.ServerSidePaging<api.DiscountAccountTypeModel, api.DiscountAccountTypePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.TypeModel, api.AccountTypePagedResults>(
					...
					this.cvApi.accounts.GetAccountTypes, 8, `discount-${this.record && this.record.ID}.to.AccountTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.AccountTypes.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountAccountTypeModel, api.DiscountAccountTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountAccountTypes, 8, `discount-${this.record && this.record.ID}.to.AccountTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.AccountTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.AccountTypes.dupe-check`
			};
			this.cvApi.discounts.GetDiscountAccountTypes(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountAccountType(<api.DiscountAccountTypeModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountAccountTypeModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountAccountTypeByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToAccountTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.AccountTypes.html", "ui"),
		controller: AssociatorForDiscountToAccountTypesController,
		controllerAs: "asCtrl",
