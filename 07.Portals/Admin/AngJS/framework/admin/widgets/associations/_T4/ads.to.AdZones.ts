/**
 * @file framework/admin/widgets/associations/_T4/ad.to.AdZones.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Ad to AdZones class
...
module cef.admin.widgets.associators.Advertising {
	class AssociatorForAdToAdZonesController extends core.TemplatedControllerBase {
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ZoneModel, api.ZonePagedResults>(
					...
					this.cvApi.advertising.GetZones, 8, `ad-${this.record && this.record.ID}.to.AdZones.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `ad-${this.record && this.record.ID}.to.AdZones.available`
						};
					},
				...
				this.assigned = new core.ServerSidePaging<api.AdZoneModel, api.AdZonePagedResults>(
					...
					this.$q,
					this.cvServiceStrings,
					this.cvApi.advertising.GetAdZones, 8, `ad-${this.record && this.record.ID}.to.AdZones.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `ad-${this.record && this.record.ID}.to.AdZones.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `ad-${this.record && this.record.ID}.to.AdZones.dupe-check`
			};
	...
	adminApp.directive("cefAdToAdZonesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/ads.to.AdZones.html", "ui"),
		controller: AssociatorForAdToAdZonesController,
		controllerAs: "asCtrl",
