/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Products.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Products class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToProductsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountProductModel, api.DiscountProductPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `discount-${this.record && this.record.ID}.to.Products.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Products.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountProductModel, api.DiscountProductPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountProducts, 8, `discount-${this.record && this.record.ID}.to.Products.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Products.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Products.dupe-check`
			};
			this.cvApi.discounts.GetDiscountProducts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountProduct(<api.DiscountProductModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountProductModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountProductByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToProductsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Products.html", "ui"),
		controller: AssociatorForDiscountToProductsController,
		controllerAs: "asCtrl",
