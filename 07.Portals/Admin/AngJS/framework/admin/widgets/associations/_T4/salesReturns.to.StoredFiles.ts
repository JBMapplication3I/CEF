/**
 * @file framework/admin/widgets/associations/_T4/salesReturn.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Return to StoredFiles class
...
module cef.admin.widgets.associators.Returning {
	class AssociatorForSalesReturnToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SalesReturnFileModel, api.SalesReturnFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `salesReturn-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesReturn-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SalesReturnFileModel, api.SalesReturnFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.returning.GetSalesReturnFiles, 8, `salesReturn-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesReturn-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesReturn-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.returning.GetSalesReturnFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.returning.CreateSalesReturnFile(<api.SalesReturnFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SalesReturnFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.returning.DeactivateSalesReturnFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesReturnToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesReturns.to.StoredFiles.html", "ui"),
		controller: AssociatorForSalesReturnToStoredFilesController,
		controllerAs: "asCtrl",
