/**
 * @file framework/admin/widgets/associations/_T4/sampleRequest.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sample Request to StoredFiles class
...
module cef.admin.widgets.associators.Sampling {
	class AssociatorForSampleRequestToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.SampleRequestFileModel, api.SampleRequestFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `sampleRequest-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `sampleRequest-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.SampleRequestFileModel, api.SampleRequestFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.sampling.GetSampleRequestFiles, 8, `sampleRequest-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `sampleRequest-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `sampleRequest-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.sampling.GetSampleRequestFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.sampling.CreateSampleRequestFile(<api.SampleRequestFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.SampleRequestFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.sampling.DeactivateSampleRequestFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSampleRequestToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/sampleRequests.to.StoredFiles.html", "ui"),
		controller: AssociatorForSampleRequestToStoredFilesController,
		controllerAs: "asCtrl",
