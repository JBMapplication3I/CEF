/**
 * @file framework/admin/widgets/associations/_T4/purchaseOrder.to.StoredFiles.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Purchase Order to StoredFiles class
...
module cef.admin.widgets.associators.Purchasing {
	class AssociatorForPurchaseOrderToStoredFilesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.PurchaseOrderFileModel, api.PurchaseOrderFilePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.StoredFileModel, api.StoredFilePagedResults>(
					...
					this.cvApi.media.GetStoredFiles, 8, `purchaseOrder-${this.record && this.record.ID}.to.StoredFiles.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `purchaseOrder-${this.record && this.record.ID}.to.StoredFiles.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PurchaseOrderFileModel, api.PurchaseOrderFilePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.purchasing.GetPurchaseOrderFiles, 8, `purchaseOrder-${this.record && this.record.ID}.to.StoredFiles.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `purchaseOrder-${this.record && this.record.ID}.to.StoredFiles.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `purchaseOrder-${this.record && this.record.ID}.to.StoredFiles.dupe-check`
			};
			this.cvApi.purchasing.GetPurchaseOrderFiles(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.purchasing.CreatePurchaseOrderFile(<api.PurchaseOrderFileModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PurchaseOrderFileModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.purchasing.DeactivatePurchaseOrderFileByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPurchaseOrderToStoredFilesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/purchaseOrders.to.StoredFiles.html", "ui"),
		controller: AssociatorForPurchaseOrderToStoredFilesController,
		controllerAs: "asCtrl",
