/**
 * @file framework/admin/widgets/associations/_T4/user.to.Franchises.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for User to Franchises class
...
module cef.admin.widgets.associators.Contacts {
	class AssociatorForUserToFranchisesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.FranchiseUserModel, api.FranchiseUserPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.UserModel, api.UserPagedResults>(
					...
					this.cvApi.contacts.GetUsers, 8, `user-${this.record && this.record.ID}.to.Franchises.available`, "IDOrUserNameOrCustomKeyOrEmailOrContactName",
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `user-${this.record && this.record.ID}.to.Franchises.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseUserModel, api.FranchiseUserPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseUsers, 8, `user-${this.record && this.record.ID}.to.Franchises.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `user-${this.record && this.record.ID}.to.Franchises.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `user-${this.record && this.record.ID}.to.Franchises.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseUsers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseUser(<api.FranchiseUserModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseUserModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseUserByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefUserToFranchisesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/users.to.Franchises.html", "ui"),
		controller: AssociatorForUserToFranchisesController,
		controllerAs: "asCtrl",
