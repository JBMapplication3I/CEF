/**
 * @file framework/admin/widgets/associations/_T4/priceRule.to.PriceRuleProductTypes.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Price Rule to PriceRuleProductTypes class
...
module cef.admin.widgets.associators.Pricing {
	class AssociatorForPriceRuleToPriceRuleProductTypesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.PriceRuleModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "PriceRuleProductTypes";
		paging: core.ServerSidePaging<api.TypeModel, api.ProductTypePagedResults>;
		assigned: core.ServerSidePaging<api.PriceRuleProductTypeModel, api.PriceRuleProductTypePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.TypeModel, api.ProductTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.products.GetProductTypes, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleProductTypes.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleProductTypes.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.PriceRuleProductTypeModel, api.PriceRuleProductTypePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.pricing.GetPriceRuleProductTypes, 8, `priceRule-${this.record && this.record.ID}.to.PriceRuleProductTypes.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `priceRule-${this.record && this.record.ID}.to.PriceRuleProductTypes.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `priceRule-${this.record && this.record.ID}.to.PriceRuleProductTypes.dupe-check`
			};
			this.cvApi.pricing.GetPriceRuleProductTypes(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.pricing.CreatePriceRuleProductType(<api.PriceRuleProductTypeModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.PriceRuleProductTypeModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.pricing.DeactivatePriceRuleProductTypeByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPriceRuleToPriceRuleProductTypesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/priceRules.to.PriceRuleProductTypes.html", "ui"),
		controller: AssociatorForPriceRuleToPriceRuleProductTypesController,
		controllerAs: "asCtrl",
