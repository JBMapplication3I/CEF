/**
 * @file framework/admin/widgets/associations/_T4/scout.to.Categories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Scout to Categories class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Scouting {
	class AssociatorForScoutToCategoriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ScoutModel;
		...
		assigned: core.ServerSidePaging<api.ScoutCategoryModel, api.ScoutCategoryPagedResults>;
		scoutCategoryTypes: api.TypeModel[];
		...
		load(): void {
			...
			this.$q.all([
				this.cvApi.scouting.GetScoutCategoryTypes(standardDto),
				this.$q.resolve(null)
			]).then((rarr: ng.IHttpPromiseCallbackArg<any>[]) => {
				let index = 0;
				this.scoutCategoryTypes = rarr[index++].data.Results;
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `scout-${this.record && this.record.ID}.to.Categories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `scout-${this.record && this.record.ID}.to.Categories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.ScoutCategoryModel, api.ScoutCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.scouting.GetScoutCategories, 8, `scout-${this.record && this.record.ID}.to.Categories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `scout-${this.record && this.record.ID}.to.Categories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `scout-${this.record && this.record.ID}.to.Categories.dupe-check`
			};
			this.cvApi.scouting.GetScoutCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.scouting.CreateScoutCategory(<api.ScoutCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.ScoutCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.scouting.DeactivateScoutCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefScoutToCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/scouts.to.Categories.html", "ui"),
		controller: AssociatorForScoutToCategoriesController,
		controllerAs: "asCtrl",
