/**
 * @file framework/admin/widgets/associations/_T4/district.to.Currencies.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for District to Currencies class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Geography {
	class AssociatorForDistrictToCurrenciesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.DistrictModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Currencies";
		paging: core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>;
		assigned: core.ServerSidePaging<api.DistrictCurrencyModel, api.DistrictCurrencyPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CurrencyModel, api.CurrencyPagedResults>(
					...
					this.cvApi.currencies.GetCurrencies, 8, `district-${this.record && this.record.ID}.to.Currencies.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `district-${this.record && this.record.ID}.to.Currencies.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DistrictCurrencyModel, api.DistrictCurrencyPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetDistrictCurrencies, 8, `district-${this.record && this.record.ID}.to.Currencies.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `district-${this.record && this.record.ID}.to.Currencies.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `district-${this.record && this.record.ID}.to.Currencies.dupe-check`
			};
			this.cvApi.geography.GetDistrictCurrencies(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.geography.CreateDistrictCurrency(<api.DistrictCurrencyModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DistrictCurrencyModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.geography.DeactivateDistrictCurrencyByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDistrictToCurrenciesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/districts.to.Currencies.html", "ui"),
		controller: AssociatorForDistrictToCurrenciesController,
		controllerAs: "asCtrl",
