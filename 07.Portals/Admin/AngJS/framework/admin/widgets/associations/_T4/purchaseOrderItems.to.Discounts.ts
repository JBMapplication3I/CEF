/**
 * @file framework/admin/widgets/associations/_T4/purchaseOrderItem.to.Discounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Purchase Order Item to Discounts class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Purchasing {
	class AssociatorForPurchaseOrderItemToDiscountsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.SalesItemBaseModel<api.AppliedDiscountBaseModel>;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "Discounts";
		paging: core.ServerSidePaging<api.DiscountModel, api.DiscountPagedResults>;
		assigned: core.ServerSidePaging<api.AppliedPurchaseOrderItemDiscountModel, api.AppliedPurchaseOrderItemDiscountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.DiscountModel, api.DiscountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscounts, 8, `purchaseOrderItem-${this.record && this.record.ID}.to.Discounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `purchaseOrderItem-${this.record && this.record.ID}.to.Discounts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AppliedPurchaseOrderItemDiscountModel, api.AppliedPurchaseOrderItemDiscountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetAppliedPurchaseOrderItemDiscounts, 8, `purchaseOrderItem-${this.record && this.record.ID}.to.Discounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `purchaseOrderItem-${this.record && this.record.ID}.to.Discounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `purchaseOrderItem-${this.record && this.record.ID}.to.Discounts.dupe-check`
			};
			this.cvApi.discounts.GetAppliedPurchaseOrderItemDiscounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateAppliedPurchaseOrderItemDiscount(<api.AppliedPurchaseOrderItemDiscountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AppliedPurchaseOrderItemDiscountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateAppliedPurchaseOrderItemDiscountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefPurchaseOrderItemToDiscountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/purchaseOrderItems.to.Discounts.html", "ui"),
		controller: AssociatorForPurchaseOrderItemToDiscountsController,
		controllerAs: "asCtrl",
