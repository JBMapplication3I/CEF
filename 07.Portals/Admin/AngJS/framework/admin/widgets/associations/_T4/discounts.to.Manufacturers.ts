/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Manufacturers.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Manufacturers class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToManufacturersController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountManufacturerModel, api.DiscountManufacturerPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ManufacturerModel, api.ManufacturerPagedResults>(
					...
					this.cvApi.manufacturers.GetManufacturers, 8, `discount-${this.record && this.record.ID}.to.Manufacturers.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Manufacturers.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountManufacturerModel, api.DiscountManufacturerPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountManufacturers, 8, `discount-${this.record && this.record.ID}.to.Manufacturers.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Manufacturers.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Manufacturers.dupe-check`
			};
			this.cvApi.discounts.GetDiscountManufacturers(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountManufacturer(<api.DiscountManufacturerModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountManufacturerModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountManufacturerByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToManufacturersAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Manufacturers.html", "ui"),
		controller: AssociatorForDiscountToManufacturersController,
		controllerAs: "asCtrl",
