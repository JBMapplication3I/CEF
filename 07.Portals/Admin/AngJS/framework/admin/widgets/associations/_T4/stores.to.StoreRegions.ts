/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreRegions.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreRegions class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreRegionsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreRegions";
		paging: core.ServerSidePaging<api.RegionModel, api.RegionPagedResults>;
		assigned: core.ServerSidePaging<api.StoreRegionModel, api.StoreRegionPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.RegionModel, api.RegionPagedResults>(
					...
					this.cvApi.geography.GetRegions, 8, `store-${this.record && this.record.ID}.to.StoreRegions.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreRegions.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreRegionModel, api.StoreRegionPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreRegions, 8, `store-${this.record && this.record.ID}.to.StoreRegions.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreRegions.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreRegions.dupe-check`
			};
			this.cvApi.stores.GetStoreRegions(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreRegion(<api.StoreRegionModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreRegionModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreRegionByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToStoreRegionsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreRegions.html", "ui"),
		controller: AssociatorForStoreToStoreRegionsController,
		controllerAs: "asCtrl",
