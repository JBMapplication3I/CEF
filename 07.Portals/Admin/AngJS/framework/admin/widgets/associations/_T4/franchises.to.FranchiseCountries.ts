/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.FranchiseCountries.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to FranchiseCountries class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToFranchiseCountriesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.FranchiseModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "FranchiseCountries";
		paging: core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseCountryModel, api.FranchiseCountryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CountryModel, api.CountryPagedResults>(
					...
					this.cvApi.geography.GetCountries, 8, `franchise-${this.record && this.record.ID}.to.FranchiseCountries.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseCountries.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseCountryModel, api.FranchiseCountryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseCountries, 8, `franchise-${this.record && this.record.ID}.to.FranchiseCountries.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseCountries.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.FranchiseCountries.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseCountries(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseCountry(<api.FranchiseCountryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseCountryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseCountryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefFranchiseToFranchiseCountriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.FranchiseCountries.html", "ui"),
		controller: AssociatorForFranchiseToFranchiseCountriesController,
		controllerAs: "asCtrl",
