/**
 * @file framework/admin/widgets/associations/_T4/salesOrderItem.to.Discounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Order Item to Discounts class
...
module cef.admin.widgets.associators.Ordering {
	class AssociatorForSalesOrderItemToDiscountsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AppliedSalesOrderItemDiscountModel, api.AppliedSalesOrderItemDiscountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.DiscountModel, api.DiscountPagedResults>(
					...
					this.cvApi.discounts.GetDiscounts, 8, `salesOrderItem-${this.record && this.record.ID}.to.Discounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesOrderItem-${this.record && this.record.ID}.to.Discounts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AppliedSalesOrderItemDiscountModel, api.AppliedSalesOrderItemDiscountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetAppliedSalesOrderItemDiscounts, 8, `salesOrderItem-${this.record && this.record.ID}.to.Discounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesOrderItem-${this.record && this.record.ID}.to.Discounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesOrderItem-${this.record && this.record.ID}.to.Discounts.dupe-check`
			};
			this.cvApi.discounts.GetAppliedSalesOrderItemDiscounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateAppliedSalesOrderItemDiscount(<api.AppliedSalesOrderItemDiscountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AppliedSalesOrderItemDiscountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateAppliedSalesOrderItemDiscountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesOrderItemToDiscountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesOrderItems.to.Discounts.html", "ui"),
		controller: AssociatorForSalesOrderItemToDiscountsController,
		controllerAs: "asCtrl",
