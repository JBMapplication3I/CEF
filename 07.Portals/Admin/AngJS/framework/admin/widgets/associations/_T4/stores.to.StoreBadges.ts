/**
 * @file framework/admin/widgets/associations/_T4/store.to.StoreBadges.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Store to StoreBadges class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForStoreToStoreBadgesController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.StoreModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "StoreBadges";
		paging: core.ServerSidePaging<api.BadgeModel, api.BadgePagedResults>;
		assigned: core.ServerSidePaging<api.StoreBadgeModel, api.StoreBadgePagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.BadgeModel, api.BadgePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.badges.GetBadges, 8, `store-${this.record && this.record.ID}.to.StoreBadges.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `store-${this.record && this.record.ID}.to.StoreBadges.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.StoreBadgeModel, api.StoreBadgePagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.stores.GetStoreBadges, 8, `store-${this.record && this.record.ID}.to.StoreBadges.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `store-${this.record && this.record.ID}.to.StoreBadges.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `store-${this.record && this.record.ID}.to.StoreBadges.dupe-check`
			};
			this.cvApi.stores.GetStoreBadges(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.stores.CreateStoreBadge(<api.StoreBadgeModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.StoreBadgeModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.stores.DeactivateStoreBadgeByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefStoreToStoreBadgesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/stores.to.StoreBadges.html", "ui"),
		controller: AssociatorForStoreToStoreBadgesController,
		controllerAs: "asCtrl",
