/**
 * @file framework/admin/widgets/associations/_T4/franchise.to.FranchiseDistricts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Franchise to FranchiseDistricts class
...
module cef.admin.widgets.associators.Franchises {
	class AssociatorForFranchiseToFranchiseDistrictsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.FranchiseModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "FranchiseDistricts";
		paging: core.ServerSidePaging<api.DistrictModel, api.DistrictPagedResults>;
		assigned: core.ServerSidePaging<api.FranchiseDistrictModel, api.FranchiseDistrictPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				let index = 0;
				this.paging = new core.ServerSidePaging<api.DistrictModel, api.DistrictPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.geography.GetDistricts, 8, `franchise-${this.record && this.record.ID}.to.FranchiseDistricts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseDistricts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.FranchiseDistrictModel, api.FranchiseDistrictPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.franchises.GetFranchiseDistricts, 8, `franchise-${this.record && this.record.ID}.to.FranchiseDistricts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `franchise-${this.record && this.record.ID}.to.FranchiseDistricts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `franchise-${this.record && this.record.ID}.to.FranchiseDistricts.dupe-check`
			};
			this.cvApi.franchises.GetFranchiseDistricts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.franchises.CreateFranchiseDistrict(<api.FranchiseDistrictModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.FranchiseDistrictModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.franchises.DeactivateFranchiseDistrictByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefFranchiseToFranchiseDistrictsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/franchises.to.FranchiseDistricts.html", "ui"),
		controller: AssociatorForFranchiseToFranchiseDistrictsController,
		controllerAs: "asCtrl",
