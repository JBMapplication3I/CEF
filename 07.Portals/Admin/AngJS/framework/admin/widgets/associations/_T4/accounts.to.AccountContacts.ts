/**
 * @file framework/admin/widgets/associations/_T4/account.to.AccountContacts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Account to AccountContacts class
...
module cef.admin.widgets.associators.Accounts {
	class AssociatorForAccountToAccountContactsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.AccountModel;
		ctrl: core.TemplatedControllerBase;
		// Properties
		propertyName: string = "AccountContacts";
		paging: core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>;
		assigned: core.ServerSidePaging<api.AccountContactModel, api.AccountContactPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ContactModel, api.ContactPagedResults>(
					...
					this.cvApi.contacts.GetContacts, 8, `account-${this.record && this.record.ID}.to.AccountContacts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `account-${this.record && this.record.ID}.to.AccountContacts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AccountContactModel, api.AccountContactPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.accounts.GetAccountContacts, 8, `account-${this.record && this.record.ID}.to.AccountContacts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `account-${this.record && this.record.ID}.to.AccountContacts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `account-${this.record && this.record.ID}.to.AccountContacts.dupe-check`
			};
			this.cvApi.accounts.GetAccountContacts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.accounts.CreateAccountContact(<api.AccountContactModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AccountContactModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.accounts.DeactivateAccountContactByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefAccountToAccountContactsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/accounts.to.AccountContacts.html", "ui"),
		controller: AssociatorForAccountToAccountContactsController,
		controllerAs: "asCtrl",
