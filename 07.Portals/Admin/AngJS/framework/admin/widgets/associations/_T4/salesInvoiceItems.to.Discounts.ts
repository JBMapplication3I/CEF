/**
 * @file framework/admin/widgets/associations/_T4/salesInvoiceItem.to.Discounts.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Sales Invoice Item to Discounts class
...
module cef.admin.widgets.associators.Invoicing {
	class AssociatorForSalesInvoiceItemToDiscountsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.AppliedSalesInvoiceItemDiscountModel, api.AppliedSalesInvoiceItemDiscountPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.DiscountModel, api.DiscountPagedResults>(
					...
					this.cvApi.discounts.GetDiscounts, 8, `salesInvoiceItem-${this.record && this.record.ID}.to.Discounts.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `salesInvoiceItem-${this.record && this.record.ID}.to.Discounts.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.AppliedSalesInvoiceItemDiscountModel, api.AppliedSalesInvoiceItemDiscountPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetAppliedSalesInvoiceItemDiscounts, 8, `salesInvoiceItem-${this.record && this.record.ID}.to.Discounts.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `salesInvoiceItem-${this.record && this.record.ID}.to.Discounts.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `salesInvoiceItem-${this.record && this.record.ID}.to.Discounts.dupe-check`
			};
			this.cvApi.discounts.GetAppliedSalesInvoiceItemDiscounts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateAppliedSalesInvoiceItemDiscount(<api.AppliedSalesInvoiceItemDiscountModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.AppliedSalesInvoiceItemDiscountModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateAppliedSalesInvoiceItemDiscountByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSalesInvoiceItemToDiscountsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/salesInvoiceItems.to.Discounts.html", "ui"),
		controller: AssociatorForSalesInvoiceItemToDiscountsController,
		controllerAs: "asCtrl",
