/**
 * @file framework/admin/widgets/associations/_T4/manufacturer.to.Products.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Manufacturer to Products class
 * @auto-generated This file was auto-generated by the T4 template associators.tt in the UI project
 */
module cef.admin.widgets.associators.Manufacturers {
	class AssociatorForManufacturerToProductsController extends core.TemplatedControllerBase {
		// Bound Scope Properties
		record: api.ManufacturerModel;
		...
		assigned: core.ServerSidePaging<api.ManufacturerProductModel, api.ManufacturerProductPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.ProductModel, api.ProductPagedResults>(
					...
					this.cvApi.products.GetProducts, 8, `manufacturer-${this.record && this.record.ID}.to.Products.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `manufacturer-${this.record && this.record.ID}.to.Products.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.ManufacturerProductModel, api.ManufacturerProductPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.manufacturers.GetManufacturerProducts, 8, `manufacturer-${this.record && this.record.ID}.to.Products.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `manufacturer-${this.record && this.record.ID}.to.Products.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `manufacturer-${this.record && this.record.ID}.to.Products.dupe-check`
			};
			this.cvApi.manufacturers.GetManufacturerProducts(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.manufacturers.CreateManufacturerProduct(<api.ManufacturerProductModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.ManufacturerProductModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.manufacturers.DeactivateManufacturerProductByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefManufacturerToProductsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/manufacturers.to.Products.html", "ui"),
		controller: AssociatorForManufacturerToProductsController,
		controllerAs: "asCtrl",
