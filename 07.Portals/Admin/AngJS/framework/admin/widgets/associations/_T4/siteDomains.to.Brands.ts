/**
 * @file framework/admin/widgets/associations/_T4/siteDomain.to.Brands.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Site Domain to Brands class
...
module cef.admin.widgets.associators.Stores {
	class AssociatorForSiteDomainToBrandsController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.BrandSiteDomainModel, api.BrandSiteDomainPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.SiteDomainModel, api.SiteDomainPagedResults>(
					...
					this.cvApi.stores.GetSiteDomains, 8, `siteDomain-${this.record && this.record.ID}.to.Brands.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `siteDomain-${this.record && this.record.ID}.to.Brands.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.BrandSiteDomainModel, api.BrandSiteDomainPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.brands.GetBrandSiteDomains, 8, `siteDomain-${this.record && this.record.ID}.to.Brands.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `siteDomain-${this.record && this.record.ID}.to.Brands.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `siteDomain-${this.record && this.record.ID}.to.Brands.dupe-check`
			};
			this.cvApi.brands.GetBrandSiteDomains(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.brands.CreateBrandSiteDomain(<api.BrandSiteDomainModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.BrandSiteDomainModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.brands.DeactivateBrandSiteDomainByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefSiteDomainToBrandsAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/siteDomains.to.Brands.html", "ui"),
		controller: AssociatorForSiteDomainToBrandsController,
		controllerAs: "asCtrl",
