/**
 * @file framework/admin/widgets/associations/_T4/discount.to.Categories.ts
 * @author Copyright (c) 2020-2023 clarity-ventures.com. All rights reserved.
 * @desc Associator for Discount to Categories class
...
module cef.admin.widgets.associators.Discounts {
	class AssociatorForDiscountToCategoriesController extends core.TemplatedControllerBase {
		...
		assigned: core.ServerSidePaging<api.DiscountCategoryModel, api.DiscountCategoryPagedResults>;
		...
		load(): void {
			...
			this.$q.all([
				...
				this.paging = new core.ServerSidePaging<api.CategoryModel, api.CategoryPagedResults>(
					...
					this.cvApi.categories.GetCategories, 8, `discount-${this.record && this.record.ID}.to.Categories.available`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							__caller: `discount-${this.record && this.record.ID}.to.Categories.available`
						};
					},
					() => !this.record || !this.record.ID);
				this.assigned = new core.ServerSidePaging<api.DiscountCategoryModel, api.DiscountCategoryPagedResults>(
					this.$rootScope,
					this.$scope,
					this.$filter,
					this.$q,
					this.cvServiceStrings,
					this.cvApi.discounts.GetDiscountCategories, 8, `discount-${this.record && this.record.ID}.to.Categories.assigned`, null,
					() => {
						return {
							Active: true,
							AsListing: true,
							MasterID: this.record && this.record.ID || 0,
							__caller: `discount-${this.record && this.record.ID}.to.Categories.assigned`
						};
					},
					() => !this.record || !this.record.ID);
		...
		add(id: number): void {
			...
			const dupeCheckDto = {
				...
				SlaveID: id,
				"__caller": `discount-${this.record && this.record.ID}.to.Categories.dupe-check`
			};
			this.cvApi.discounts.GetDiscountCategories(dupeCheckDto).then(r => {
				...
				// Add it
				this.cvApi.discounts.CreateDiscountCategory(<api.DiscountCategoryModel>{
					// Base Properties
					ID: null,
					Active: true,
		...
		remove(toRemove: api.DiscountCategoryModel): void {
			this.cvConfirmModalFactory(
				...
				this.cvApi.discounts.DeactivateDiscountCategoryByID(toRemove.ID).then(r => {
					if (!r || !r.data) {
	...
	adminApp.directive("cefDiscountToCategoriesAssociator", ($filter: ng.IFilterService): ng.IDirective => ({
		restrict: "A",
		scope: {
			record: "=",
			ctrl: "="
		},
		templateUrl: $filter("corsLink")("/framework/admin/widgets/associations/_T4/discounts.to.Categories.html", "ui"),
		controller: AssociatorForDiscountToCategoriesController,
		controllerAs: "asCtrl",
