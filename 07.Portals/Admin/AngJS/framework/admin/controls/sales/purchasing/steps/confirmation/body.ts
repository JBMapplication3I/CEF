/**
 * @file framework/admin/purchasing/steps/confirmation/body.ts
...
module cef.admin.purchasing.steps.confirmation {
                ...
                this.cvAddressBookService.getBook(this.lookupKey.AID).then(book => {
                    ...
                    this.cvAddressBookService.refreshContactChecks(this.lookupKey.AID, false).finally(() => {
                        ...
                        const doBill = (): boolean =>  {
                            ...
                            this.currentAccountContact_Bill = this.cvAddressBookService.defaultBilling[this.lookupKey.AID];
                            return true;
                        };
                        const doShip = (): boolean =>  {
                            if (!this.cvAddressBookService.defaultShippingID[this.lookupKey.AID]) {
                                this.consoleDebug(`${debugMsg} no default shipping available to assign`);
                                return false;
                            }
                            this.consoleDebug(`${debugMsg} assigning default shipping`);
                            this.currentAccountContact_Ship = this.cvAddressBookService.defaultShipping[this.lookupKey.AID];
                            return true;
                        };
    ...
    adminApp.directive("cefPurchaseStepConfirmationBody", ($filter: ng.IFilterService): ng.IDirective => ({
        restrict: "A",
        scope: { lookupKey: "=" },
        templateUrl: $filter("corsLink")("/framework/admin/controls/sales/purchasing/steps/confirmation/body.html", "ui"),
