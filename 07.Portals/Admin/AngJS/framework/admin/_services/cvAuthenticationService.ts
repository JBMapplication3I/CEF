/**
 * @file framework/admin/_services/cvAuthenticationService.ts
...
module cef.admin.services {
    ...
    export interface IAuthenticationService {
        ...
        validateCustomKeyIsUnique: (data: api.ValidateCustomKeyIsUniqueDto) => ng.IHttpPromise<api.CEFActionResponse>;
    ...
    export class AuthenticationService implements IAuthenticationService {
            ...
            return this.getPromiseInstance.account = this.$q((resolve, reject) => {
                this.preAuth().finally(() => {
                    if (!this.isAuthenticated()) {
                        ...
                        reject(this.$translate("ui.admin.common.AccountNotFound"));
        ...
        preAuth(): ng.IPromise<void | any> {
            ...
            if (this.preAuthPromise) {
                ...
                ////              result => this.errorCallback(reject, result, false))
                ...
                switch (provider) {
                    ...
                    case this.cvServiceStrings.auth.providers.cobalt: {
                        ...
                        const ganLogin = (): ng.IPromise<void> => {
                            return this.login({ Username: gan, Password: null })
                                .then(() => {
                                    resolve();
                                    this.cvViewStateService.finishRunning(this.viewStateName);
                                }).catch(reason => this.errorCallback(reject, reason, true));
                    ...
                    default: { // "identity"
                        ...
                        this.cvApi.contacts.GetCurrentUserName().then(r => {
                            ...
                            this.preAuthPromise = null;
                            this.cvViewStateService.finishRunning(this.viewStateName);
                        }, result => this.errorCallback(reject, result, true))
                        .catch(reason => this.errorCallback(reject, reason, true));
                        break;
                    }
                }
            ...
            return this.$q((resolve, reject) => {
                ...
                const provider = this.cefConfig.authProvider.indexOf(",") !== -1
                    ...
                    this.getCurrentUserPromise(true).then(u => {
                        ...
                        if (u.RequirePasswordChangeOnNextLogin) {
                            this.$filter("corsLink")("/", "forcedPasswordReset");
                            return;
                        }
                    ...
                    this.consoleDebug("cvAuthenticationService.login.promise.reply.result:");
                    this.consoleDebug(result);
                    if (this.cefConfig.authProvider.indexOf(",") !== -1 && !alt) {
                        resolve(this.login(loginClone, true));
                        return;
                    }
                    this.errorCallback(reject, result, true);
                    this.loginPromise = null;
                }).catch(reason => {
                    this.consoleDebug("cvAuthenticationService.login.promise.reply.reason:");
                    this.consoleDebug(reason);
                    if (this.cefConfig.authProvider.indexOf(",") !== -1 && !alt) {
                        resolve(this.login(loginData, true));
                        return;
                    }
        ...
        validateCustomKeyIsUnique(data: api.ValidateCustomKeyIsUniqueDto): ng.IHttpPromise<api.CEFActionResponse> {
            return this.cvApi.authentication.ValidateCustomKeyIsUnique(data);
        }
        // Constructor
        constructor(
