/**
 * @file framework/admin/_services/cvStatusesService.ts
 * @author Copyright (c) 2019-2023 clarity-ventures.com. All rights reserved.
 * @desc Statuses Service class, stores the status definitions
...
module cef.admin.services {
    export interface IStatusLookupInterface {
        kind: string;
        id?: number;
        key?: string;
        name?: string;
    ...
    export interface IStatusesService {
        search(kind: string, model?: api.StatusSearchModel): ng.IPromise<api.StatusModel[]>;
        get(lookup: IStatusLookupInterface): ng.IPromise<api.StatusModel>;
        getCached(lookup: IStatusLookupInterface): api.StatusModel;
    }

    export class StatusesService implements IStatusesService {
        private statusesCache: { [kind: string]: { [id: number]: api.StatusModel; } } = { };
        ...
        search(kind: string, model: api.StatusSearchModel = { Active: true, AsListing: true }): ng.IPromise<api.StatusModel[]> {
            if (!kind) {
                return this.$q.reject("Must provided a status kind");
            }
            return this.$q((resolve, reject) => {
                if (this.statusesCache[kind]) {
                ...
                this.kindToSearchPromise(kind)({ Active: true, AsListing: true }).then(r => {
                    ...
                    this.statusesCache[kind] = r.data.Results;
                    resolve(this.searchInner(kind, model));
                });
            });
        }
        private searchInner(kind: string, model: api.StatusSearchModel) {
            // TODO: Filter the data
            // if (!model || model === { }) {
                let retVal = [];
                Object.keys(this.statusesCache[kind]).forEach(
                    key => retVal.push(this.statusesCache[kind][key]));
                return retVal;
                // return;
            // }
        }
        getCached(lookup: IStatusLookupInterface): api.StatusModel {
            ...
            if (!this.statusesCache[lookup.kind]) {
                this.statusesCache[lookup.kind] = [];
            }
            return _.find(this.statusesCache[lookup.kind], x => x.ID === lookup.id);
        }
        get(lookup: IStatusLookupInterface): ng.IPromise<api.StatusModel> {
            ...
            return this.$q((resolve, reject) => {
                ...
                if (this.statusesCache[lookup.kind][lookup.id]) {
                    resolve(this.statusesCache[lookup.kind][lookup.id]);
                    return;
                    ...
                    this.byIDPromises[lookup.kind][lookup.id] = this.kindToIDPromise(lookup.kind)(lookup.id).then(r => {
                        if (!r || !r.data) {
                            reject("ERROR! Unable to locate status by ID");
                            return;
                        }
                        // Save it so we don't have to look again and return the good item
                        resolve(this.statusesCache[lookup.kind][lookup.id] = r.data);
                        this.$rootScope.$broadcast(this.cvServiceStrings.events.statuses.loaded, lookup, r.data);
                    });
                }
            });
        }
        private kindToIDPromise(kind: string): (id: number) => ng.IHttpPromise<api.StatusModel> {
            switch (kind) {
                case "SalesOrder": { return this.cvApi.ordering.GetSalesOrderStatusByID; }
                case "SalesInvoice": { return this.cvApi.invoicing.GetSalesInvoiceStatusByID; }
                case "SalesQuote": { return this.cvApi.quoting.GetSalesQuoteStatusByID; }
                case "SalesReturn": { return this.cvApi.returning.GetSalesReturnStatusByID; }
                case "User": { return this.cvApi.contacts.GetUserStatusByID; }
                default: { throw new Error(`Unknown kind: '${kind}'`); }
            }
        }
        private kindToCustomKeyPromise(kind: string): (key: string) => ng.IHttpPromise<number> {
            switch (kind) {
                case "SalesOrder": { return this.cvApi.ordering.CheckSalesOrderStatusExistsByKey; }
                case "SalesInvoice": { return this.cvApi.invoicing.CheckSalesInvoiceStatusExistsByKey; }
                case "SalesQuote": { return this.cvApi.quoting.CheckSalesQuoteStatusExistsByKey; }
                case "SalesReturn": { return this.cvApi.returning.CheckSalesReturnStatusExistsByKey; }
                case "User": { return this.cvApi.contacts.CheckUserStatusExistsByKey; }
                default: { throw new Error(`Unknown kind: '${kind}'`); }
            }
        }
        private kindToNamePromise(kind: string): (dto: { Name: string }) => ng.IHttpPromise<number> {
            switch (kind) {
                case "SalesOrder": { return this.cvApi.ordering.CheckSalesOrderStatusExistsByName; }
                case "SalesInvoice": { return this.cvApi.invoicing.CheckSalesInvoiceStatusExistsByName; }
                case "SalesQuote": { return this.cvApi.quoting.CheckSalesQuoteStatusExistsByName; }
                case "SalesReturn": { return this.cvApi.returning.CheckSalesReturnStatusExistsByName; }
                case "User": { return this.cvApi.contacts.CheckUserStatusExistsByName; }
                default: { throw new Error(`Unknown kind: '${kind}'`); }
            }
        }
        private kindToDisplayNamePromise(kind: string): (dto: { DisplayName: string }) => ng.IHttpPromise<number> {
            switch (kind) {
                case "SalesOrder": { return this.cvApi.ordering.CheckSalesOrderStatusExistsByDisplayName; }
                case "SalesInvoice": { return this.cvApi.invoicing.CheckSalesInvoiceStatusExistsByDisplayName; }
                case "SalesQuote": { return this.cvApi.quoting.CheckSalesQuoteStatusExistsByDisplayName; }
                case "SalesReturn": { return this.cvApi.returning.CheckSalesReturnStatusExistsByDisplayName; }
                case "User": { return this.cvApi.contacts.CheckUserStatusExistsByDisplayName; }
                default: { throw new Error(`Unknown kind: '${kind}'`); }
            }
        }
        private kindToSearchPromise(kind: string): (dto: api.StatusSearchModel) => ng.IHttpPromise<api.PagedResultsBase<api.StatusModel>> {
            switch (kind) {
                case "SalesOrder": { return this.cvApi.ordering.GetSalesOrderStatuses; }
                case "SalesInvoice": { return this.cvApi.invoicing.GetSalesInvoiceStatuses; }
                case "SalesQuote": { return this.cvApi.quoting.GetSalesQuoteStatuses; }
                case "SalesReturn": { return this.cvApi.returning.GetSalesReturnStatuses; }
                case "User": { return this.cvApi.contacts.GetUserStatuses; }
                default: { throw new Error(`Unknown kind: '${kind}'`); }
            }
        }
        // Constructor
