module cef.admin.core {
    ...
    export const corsLinkFn = (
        ...
        (path: string, area: string = "site", whichUrl: string = "primary", noCache: boolean = false, stateParamsBody: { [key: string]: any } = null): string => {
            if (stateParamsBody && typeof(stateParamsBody) === "string") {
                ...
                for (let i = 0; i < 3; i++) {
                    alt = alt.replace(
                        new RegExp("\'category\'\:\s*\'([A-Za-z0-9\s_\%\$\#\@\!\&\*\(\)\[\]-]*)\'([A-Za-z0-9\s_\%\$\#\@\!\&\*\(\)\[\]-]+)\|([A-Za-z0-9\s_\%\$\#\@\!\&\*\(\)\[\]-]*)\'", "g"),
                        "'category': '$1[apos]$2|$3'");
                }
                for (let i = 0; i < 3; i++) {
                    alt = alt.replace(
                ...
                alt = alt.replace(new RegExp("\'", "g"), "\"").replace(new RegExp("\\[apos\\]", "g"), "\'").replace(new RegExp("\\[quot\\]", "g"), "\"");
            ...
            if (path && path.indexOf(":") !== -1) {
                ...
                if (!stateToGoTo) { stateToGoTo = $state.get(`userDashboard.${stateName}`); } // Try again with 'userDashboard.' in front of it
                if (stateToGoTo) {
            ...
            const appendQueryStringParams = (i: boolean, b: string, n: boolean, s, $httpParamSerializer: (any) => string): string => {
                ...
                return swapReturnUrlToken(b + (qs.startsWith("#!?") ? "" : (b.indexOf('images') !== -1 ? "?" : "#!?")) + qs);
            };
            if (!areaToUse.hostIsProvidedByLookup) {
                // We don't need to inject a host URL from lookups
                const base2 = $filter("corsLinkRootInner")(areaToUse) + (path && path.startsWith("/") ? "" : "/") + (path || "");
            ...
            const base3 = root + (!path ? "" : (path.startsWith("/") ? "" : "/") + (path || ""));
            return swapReturnUrlToken(
                appendQueryStringParams(
                    isState,
                    base3,
    ...
    export const corsProductLinkFn = ($filter: ng.IFilterService) =>
        (seoUrl: string, whichUrl: string = "primary", noCache: boolean = false): string =>
            $filter("corsLink")(
    ...
    export const corsImageLinkFn = ($filter: ng.IFilterService, cefConfig: core.CefConfig) =>
        (fileName: string,
         ...
         whichUrl: string = "primary"
            ...
            if (!fileName) {
                return $filter("corsLink")("/framework/Content/placeholder.jpg", "ui");
            }
