(function () {
  ...
  angular.module('ui.toggle', [])
    ...
    .constant('toggleConfig', {
       ...
       * Possible values are btn-default, btn-primary, btn-success, btn-info, btn-warning, btn-danger
       */
      offstyle: 'btn-default btn-secondary',
      /**
       * Type: string
       * Default: ''
    ...
    .controller('ToggleController',
      ['$scope', '$attrs', '$interpolate', '$log', 'toggleConfig', '$toggleSuppressError',
        function ($scope, $attrs, $interpolate, $log, toggleConfig, $toggleSuppressError) {
          ...
          angular.forEach(['on', 'off', 'size', 'truestyle', 'onstyle', 'offstyle', 'disabled', 'style'], function (key, index) {
          ...
          this.toggle = function () {
            ...
            if (this.isOn) {
              $scope.wrapperClass = [self.truestyle || self.onstyle, self.size, self.style];
            } else {
          ...
          angular.forEach(['on', 'off', 'size', 'truestyle', 'onstyle', 'offstyle', 'style'], function (key) {
          ...
          var unbind1 = $scope.$watch(
            function () { return $scope.$parent.$eval($attrs["disabled"]) },
            function (val) {
    ...
    .directive('toggle', function () {
        return {
          ...
          '<span class="btn btn-default btn-secondary" ng-class="handleClass"></span>' +
          '</div>' +
          '</div>',
          scope: {
            bindModel: '=ngModel'
