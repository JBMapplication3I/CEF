// <autogenerated>
// <copyright file="ConversationUserWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for ConversationUser entities.</summary>
    /// <seealso cref="WorkflowBase{IConversationUserModel, IConversationUserSearchModel, IConversationUser, ConversationUser}"/>
    /// <seealso cref="IConversationUserWorkflow"/>
    public partial class ConversationUserWorkflow
        : WorkflowBase<IConversationUserModel, IConversationUserSearchModel, IConversationUser, ConversationUser>
            , IConversationUserWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<ConversationUser?, string?, IConversationUserModel?> MapFromConcreteFull
            => ModelMapperForConversationUser.MapConversationUserModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<ConversationUser>, string?, IEnumerable<IConversationUserModel>> SelectLiteAndMapToModel
            => ModelMapperForConversationUser.SelectLiteConversationUserAndMapToConversationUserModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ConversationUser>, string?, IEnumerable<IConversationUserModel>> SelectListAndMapToModel
            => ModelMapperForConversationUser.SelectListConversationUserAndMapToConversationUserModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ConversationUser>, string?, IConversationUserModel?> SelectFirstFullAndMapToModel
            => ModelMapperForConversationUser.SelectFirstFullConversationUserAndMapToConversationUserModel;

        /// <inheritdoc/>
        protected override Func<IConversationUser, IConversationUserModel, DateTime, DateTime?, IConversationUser> UpdateEntityFromModel
            => ModelMapperForConversationUser.UpdateConversationUserFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<ConversationUser>> FilterQueryByModelExtensionAsync(
            IQueryable<ConversationUser> query,
            IConversationUserSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterConversationUsersBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IConversationUser entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Master!.UpdateConversationFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the ConversationUser entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the ConversationUser entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the ConversationUser entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (User)resolved.Result!.CreateUserEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the ConversationUser entity");
        }
