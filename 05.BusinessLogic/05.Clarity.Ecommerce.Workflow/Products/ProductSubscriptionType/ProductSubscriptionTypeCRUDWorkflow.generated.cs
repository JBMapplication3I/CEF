// <autogenerated>
// <copyright file="ProductSubscriptionTypeWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for ProductSubscriptionType entities.</summary>
    /// <seealso cref="WorkflowBase{IProductSubscriptionTypeModel, IProductSubscriptionTypeSearchModel, IProductSubscriptionType, ProductSubscriptionType}"/>
    /// <seealso cref="IProductSubscriptionTypeWorkflow"/>
    public partial class ProductSubscriptionTypeWorkflow
        : WorkflowBase<IProductSubscriptionTypeModel, IProductSubscriptionTypeSearchModel, IProductSubscriptionType, ProductSubscriptionType>
            , IProductSubscriptionTypeWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<ProductSubscriptionType?, string?, IProductSubscriptionTypeModel?> MapFromConcreteFull
            => ModelMapperForProductSubscriptionType.MapProductSubscriptionTypeModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductSubscriptionType>, string?, IEnumerable<IProductSubscriptionTypeModel>> SelectLiteAndMapToModel
            => ModelMapperForProductSubscriptionType.SelectLiteProductSubscriptionTypeAndMapToProductSubscriptionTypeModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductSubscriptionType>, string?, IEnumerable<IProductSubscriptionTypeModel>> SelectListAndMapToModel
            => ModelMapperForProductSubscriptionType.SelectListProductSubscriptionTypeAndMapToProductSubscriptionTypeModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductSubscriptionType>, string?, IProductSubscriptionTypeModel?> SelectFirstFullAndMapToModel
            => ModelMapperForProductSubscriptionType.SelectFirstFullProductSubscriptionTypeAndMapToProductSubscriptionTypeModel;

        /// <inheritdoc/>
        protected override Func<IProductSubscriptionType, IProductSubscriptionTypeModel, DateTime, DateTime?, IProductSubscriptionType> UpdateEntityFromModel
            => ModelMapperForProductSubscriptionType.UpdateProductSubscriptionTypeFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<ProductSubscriptionType>> FilterQueryByModelExtensionAsync(
            IQueryable<ProductSubscriptionType> query,
            IProductSubscriptionTypeSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterProductSubscriptionTypesBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the ProductSubscriptionType entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the ProductSubscriptionType entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateSubscriptionTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the ProductSubscriptionType entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (SubscriptionType)resolved.Result!.CreateSubscriptionTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the ProductSubscriptionType entity");
        }

        /// <summary>Relate Required SubscriptionTypeRepeatType.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <param name="entity">            The entity that has a Required SubscriptionTypeRepeatType.</param>
        /// <param name="model">             The model that has a Required SubscriptionTypeRepeatType.</param>
        /// <param name="timestamp">         The timestamp Date/Time.</param>
        /// <param name="contextProfileName">Name of the context profile.</param>
        protected virtual async Task RelateRequiredSubscriptionTypeRepeatTypeAsync(
            IProductSubscriptionType entity,
            IProductSubscriptionTypeModel model,
            DateTime timestamp,
            string? contextProfileName)
        {
            using var context = RegistryLoaderWrapper.GetContext(contextProfileName);
            await RelateRequiredSubscriptionTypeRepeatTypeAsync(
                    entity,
                    model,
                    timestamp,
                    context)
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.SubscriptionTypeRepeatTypes.ResolveWithAutoGenerateOptionalAsync(
                    byID: model.SubscriptionTypeRepeatTypeID, // By Other ID
                    byKey: model.SubscriptionTypeRepeatTypeKey, // By Flattened Other Key
                    model: model.SubscriptionTypeRepeatType, // The other property as a model
                    context: context)
                .ConfigureAwait(false);
            // Check for IDs and objects
            var entityIDIsNull = !Contract.CheckValidID(entity.SubscriptionTypeRepeatTypeID);
            var modelIDIsNull = !Contract.CheckValidID(resolved.Result?.ID);
            var entityObjectIsNull = entity.SubscriptionTypeRepeatType == null;
            if (!resolved.ActionSucceeded && model.SubscriptionTypeRepeatType != null)
            {
                resolved.Result = model.SubscriptionTypeRepeatType;
            }
            var modelObjectIsNull = !resolved.ActionSucceeded;
            if (modelIDIsNull && modelObjectIsNull)
            {
                // [Required] Scenario 1: Trying to put bad data in the database
                throw new ArgumentException("Cannot assign a null to a non-nullable SubscriptionTypeRepeatTypeID");
            }
            var entityIDAndModelIDHaveSameID = !entityIDIsNull && !modelIDIsNull && entity.SubscriptionTypeRepeatTypeID == resolved.Result!.ID;
            var entityObjectAndModelObjectHaveSameID = !entityObjectIsNull && !modelObjectIsNull && entity.SubscriptionTypeRepeatTypeID == resolved.Result!.ID;
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                // [Optional/Required] Scenario 2: They match IDs, just update the entity from the model if it is present
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.SubscriptionTypeRepeatType!.UpdateSubscriptionTypeRepeatTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            {
                // [Optional/Required] Scenario 3: We have IDs but they don't match, assign the model's ID to the entity's ID
                entity.SubscriptionTypeRepeatTypeID = resolved.Result!.ID;
                if (!modelObjectIsNull)
                {
                    if (!entityObjectIsNull)
                    {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SubscriptionTypeRepeatTypeID to the ProductSubscriptionType entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SubscriptionTypeRepeatTypeID = 0;
                entity.SubscriptionTypeRepeatType = (SubscriptionTypeRepeatType)resolved.Result!.CreateSubscriptionTypeRepeatTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given SubscriptionTypeRepeatType to the ProductSubscriptionType entity");
        }
...
#pragma warning restore 1998
            ...
            IClarityEcommerceEntities context)
        {
            ...
            await RelateRequiredSubscriptionTypeRepeatTypeAsync(entity, model, timestamp, context).ConfigureAwait(false);
        }
