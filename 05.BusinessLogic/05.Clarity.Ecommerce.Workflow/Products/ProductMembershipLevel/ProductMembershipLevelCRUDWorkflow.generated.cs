// <autogenerated>
// <copyright file="ProductMembershipLevelWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for ProductMembershipLevel entities.</summary>
    /// <seealso cref="WorkflowBase{IProductMembershipLevelModel, IProductMembershipLevelSearchModel, IProductMembershipLevel, ProductMembershipLevel}"/>
    /// <seealso cref="IProductMembershipLevelWorkflow"/>
    public partial class ProductMembershipLevelWorkflow
        : WorkflowBase<IProductMembershipLevelModel, IProductMembershipLevelSearchModel, IProductMembershipLevel, ProductMembershipLevel>
            , IProductMembershipLevelWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<ProductMembershipLevel?, string?, IProductMembershipLevelModel?> MapFromConcreteFull
            => ModelMapperForProductMembershipLevel.MapProductMembershipLevelModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductMembershipLevel>, string?, IEnumerable<IProductMembershipLevelModel>> SelectLiteAndMapToModel
            => ModelMapperForProductMembershipLevel.SelectLiteProductMembershipLevelAndMapToProductMembershipLevelModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductMembershipLevel>, string?, IEnumerable<IProductMembershipLevelModel>> SelectListAndMapToModel
            => ModelMapperForProductMembershipLevel.SelectListProductMembershipLevelAndMapToProductMembershipLevelModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductMembershipLevel>, string?, IProductMembershipLevelModel?> SelectFirstFullAndMapToModel
            => ModelMapperForProductMembershipLevel.SelectFirstFullProductMembershipLevelAndMapToProductMembershipLevelModel;

        /// <inheritdoc/>
        protected override Func<IProductMembershipLevel, IProductMembershipLevelModel, DateTime, DateTime?, IProductMembershipLevel> UpdateEntityFromModel
            => ModelMapperForProductMembershipLevel.UpdateProductMembershipLevelFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<ProductMembershipLevel>> FilterQueryByModelExtensionAsync(
            IQueryable<ProductMembershipLevel> query,
            IProductMembershipLevelSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterProductMembershipLevelsBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IProductMembershipLevel entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (model.Subscriptions != null) { await Workflows.ProductMembershipLevelWithSubscriptionsAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the ProductMembershipLevel entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the ProductMembershipLevel entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the ProductMembershipLevel entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the ProductMembershipLevel entity");
        }

        /// <summary>Relate Required MembershipRepeatType.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <param name="entity">            The entity that has a Required MembershipRepeatType.</param>
        /// <param name="model">             The model that has a Required MembershipRepeatType.</param>
        /// <param name="timestamp">         The timestamp Date/Time.</param>
        /// <param name="contextProfileName">Name of the context profile.</param>
        protected virtual async Task RelateRequiredMembershipRepeatTypeAsync(
            IProductMembershipLevel entity,
            IProductMembershipLevelModel model,
            DateTime timestamp,
            string? contextProfileName)
        {
            using var context = RegistryLoaderWrapper.GetContext(contextProfileName);
            await RelateRequiredMembershipRepeatTypeAsync(
                    entity,
                    model,
                    timestamp,
                    context)
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.MembershipRepeatTypes.ResolveWithAutoGenerateOptionalAsync(
                    byID: model.MembershipRepeatTypeID, // By Other ID
                    byKey: model.MembershipRepeatTypeKey, // By Flattened Other Key
                    model: model.MembershipRepeatType, // The other property as a model
                    context: context)
                .ConfigureAwait(false);
            // Check for IDs and objects
            var entityIDIsNull = !Contract.CheckValidID(entity.MembershipRepeatTypeID);
            var modelIDIsNull = !Contract.CheckValidID(resolved.Result?.ID);
            var entityObjectIsNull = entity.MembershipRepeatType == null;
            if (!resolved.ActionSucceeded && model.MembershipRepeatType != null)
            {
                resolved.Result = model.MembershipRepeatType;
            }
            var modelObjectIsNull = !resolved.ActionSucceeded;
            if (modelIDIsNull && modelObjectIsNull)
            {
                // [Required] Scenario 1: Trying to put bad data in the database
                throw new ArgumentException("Cannot assign a null to a non-nullable MembershipRepeatTypeID");
            }
            var entityIDAndModelIDHaveSameID = !entityIDIsNull && !modelIDIsNull && entity.MembershipRepeatTypeID == resolved.Result!.ID;
            var entityObjectAndModelObjectHaveSameID = !entityObjectIsNull && !modelObjectIsNull && entity.MembershipRepeatTypeID == resolved.Result!.ID;
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                // [Optional/Required] Scenario 2: They match IDs, just update the entity from the model if it is present
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.MembershipRepeatType!.UpdateMembershipRepeatTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            {
                // [Optional/Required] Scenario 3: We have IDs but they don't match, assign the model's ID to the entity's ID
                entity.MembershipRepeatTypeID = resolved.Result!.ID;
                if (!modelObjectIsNull)
                {
                    if (!entityObjectIsNull)
                    {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive MembershipRepeatTypeID to the ProductMembershipLevel entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.MembershipRepeatTypeID = 0;
                entity.MembershipRepeatType = (MembershipRepeatType)resolved.Result!.CreateMembershipRepeatTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given MembershipRepeatType to the ProductMembershipLevel entity");
        }
...
#pragma warning restore 1998
            ...
            IClarityEcommerceEntities context)
        {
            ...
            await RelateRequiredMembershipRepeatTypeAsync(entity, model, timestamp, context).ConfigureAwait(false);
        }
