// <autogenerated>
// <copyright file="ProductImageWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for ProductImage entities.</summary>
    /// <seealso cref="NameableWorkflowBase{IProductImageModel, IProductImageSearchModel, IProductImage, ProductImage}"/>
    /// <seealso cref="IProductImageWorkflow"/>
    public partial class ProductImageWorkflow
        : NameableWorkflowBase<IProductImageModel, IProductImageSearchModel, IProductImage, ProductImage>
            , IProductImageWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<ProductImage?, string?, IProductImageModel?> MapFromConcreteFull
            => ModelMapperForProductImage.MapProductImageModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductImage>, string?, IEnumerable<IProductImageModel>> SelectLiteAndMapToModel
            => ModelMapperForProductImage.SelectLiteProductImageAndMapToProductImageModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductImage>, string?, IEnumerable<IProductImageModel>> SelectListAndMapToModel
            => ModelMapperForProductImage.SelectListProductImageAndMapToProductImageModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductImage>, string?, IProductImageModel?> SelectFirstFullAndMapToModel
            => ModelMapperForProductImage.SelectFirstFullProductImageAndMapToProductImageModel;

        /// <inheritdoc/>
        protected override Func<IProductImage, IProductImageModel, DateTime, DateTime?, IProductImage> UpdateEntityFromModel
            => ModelMapperForProductImage.UpdateProductImageFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<ProductImage>> FilterQueryByModelExtensionAsync(
            IQueryable<ProductImage> query,
            IProductImageSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterProductImagesBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IProductImage entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.ProductImageTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateProductImageTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the ProductImage entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (ProductImageType)resolved.Result!.CreateProductImageTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the ProductImage entity");
        }
