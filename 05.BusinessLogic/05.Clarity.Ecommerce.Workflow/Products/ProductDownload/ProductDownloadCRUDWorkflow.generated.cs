// <autogenerated>
// <copyright file="ProductDownloadWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for ProductDownload entities.</summary>
    /// <seealso cref="NameableWorkflowBase{IProductDownloadModel, IProductDownloadSearchModel, IProductDownload, ProductDownload}"/>
    /// <seealso cref="IProductDownloadWorkflow"/>
    public partial class ProductDownloadWorkflow
        : NameableWorkflowBase<IProductDownloadModel, IProductDownloadSearchModel, IProductDownload, ProductDownload>
            , IProductDownloadWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<ProductDownload?, string?, IProductDownloadModel?> MapFromConcreteFull
            => ModelMapperForProductDownload.MapProductDownloadModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductDownload>, string?, IEnumerable<IProductDownloadModel>> SelectLiteAndMapToModel
            => ModelMapperForProductDownload.SelectLiteProductDownloadAndMapToProductDownloadModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductDownload>, string?, IEnumerable<IProductDownloadModel>> SelectListAndMapToModel
            => ModelMapperForProductDownload.SelectListProductDownloadAndMapToProductDownloadModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<ProductDownload>, string?, IProductDownloadModel?> SelectFirstFullAndMapToModel
            => ModelMapperForProductDownload.SelectFirstFullProductDownloadAndMapToProductDownloadModel;

        /// <inheritdoc/>
        protected override Func<IProductDownload, IProductDownloadModel, DateTime, DateTime?, IProductDownload> UpdateEntityFromModel
            => ModelMapperForProductDownload.UpdateProductDownloadFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<ProductDownload>> FilterQueryByModelExtensionAsync(
            IQueryable<ProductDownload> query,
            IProductDownloadSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterProductDownloadsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.ProductDownloadTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateProductDownloadTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the ProductDownload entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (ProductDownloadType)resolved.Result!.CreateProductDownloadTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the ProductDownload entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive ProductID to the ProductDownload entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Product to the ProductDownload entity");
        }
