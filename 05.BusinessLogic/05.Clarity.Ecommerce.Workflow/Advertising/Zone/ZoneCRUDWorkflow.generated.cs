// <autogenerated>
// <copyright file="ZoneWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for Zone entities.</summary>
    /// <seealso cref="NameableWorkflowBase{IZoneModel, IZoneSearchModel, IZone, Zone}"/>
    /// <seealso cref="IZoneWorkflow"/>
    public partial class ZoneWorkflow
        : NameableWorkflowBase<IZoneModel, IZoneSearchModel, IZone, Zone>
            , IZoneWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<Zone?, string?, IZoneModel?> MapFromConcreteFull
            => ModelMapperForZone.MapZoneModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<Zone>, string?, IEnumerable<IZoneModel>> SelectLiteAndMapToModel
            => ModelMapperForZone.SelectLiteZoneAndMapToZoneModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<Zone>, string?, IEnumerable<IZoneModel>> SelectListAndMapToModel
            => ModelMapperForZone.SelectListZoneAndMapToZoneModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<Zone>, string?, IZoneModel?> SelectFirstFullAndMapToModel
            => ModelMapperForZone.SelectFirstFullZoneAndMapToZoneModel;

        /// <inheritdoc/>
        protected override Func<IZone, IZoneModel, DateTime, DateTime?, IZone> UpdateEntityFromModel
            => ModelMapperForZone.UpdateZoneFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<Zone>> FilterQueryByModelExtensionAsync(
            IQueryable<Zone> query,
            IZoneSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterZonesBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (model.AdZones != null) { await Workflows.ZoneWithAdZonesAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.ZoneTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateZoneTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the Zone entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (ZoneType)resolved.Result!.CreateZoneTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the Zone entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.ZoneStatuses.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Status!.UpdateZoneStatusFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive StatusID to the Zone entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.StatusID = 0;
                entity.Status = (ZoneStatus)resolved.Result!.CreateZoneStatusEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Status to the Zone entity");
        }
