// <autogenerated>
// <copyright file="PageViewEventWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for PageViewEvent entities.</summary>
    /// <seealso cref="WorkflowBase{IPageViewEventModel, IPageViewEventSearchModel, IPageViewEvent, PageViewEvent}"/>
    /// <seealso cref="IPageViewEventWorkflow"/>
    public partial class PageViewEventWorkflow
        : WorkflowBase<IPageViewEventModel, IPageViewEventSearchModel, IPageViewEvent, PageViewEvent>
            , IPageViewEventWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<PageViewEvent?, string?, IPageViewEventModel?> MapFromConcreteFull
            => ModelMapperForPageViewEvent.MapPageViewEventModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<PageViewEvent>, string?, IEnumerable<IPageViewEventModel>> SelectLiteAndMapToModel
            => ModelMapperForPageViewEvent.SelectLitePageViewEventAndMapToPageViewEventModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<PageViewEvent>, string?, IEnumerable<IPageViewEventModel>> SelectListAndMapToModel
            => ModelMapperForPageViewEvent.SelectListPageViewEventAndMapToPageViewEventModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<PageViewEvent>, string?, IPageViewEventModel?> SelectFirstFullAndMapToModel
            => ModelMapperForPageViewEvent.SelectFirstFullPageViewEventAndMapToPageViewEventModel;

        /// <inheritdoc/>
        protected override Func<IPageViewEvent, IPageViewEventModel, DateTime, DateTime?, IPageViewEvent> UpdateEntityFromModel
            => ModelMapperForPageViewEvent.UpdatePageViewEventFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<PageViewEvent>> FilterQueryByModelExtensionAsync(
            IQueryable<PageViewEvent> query,
            IPageViewEventSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterPageViewEventsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.PageViews.ResolveAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Master!.UpdatePageViewFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the PageViewEvent entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the PageViewEvent entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Events.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateEventFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the PageViewEvent entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Event)resolved.Result!.CreateEventEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the PageViewEvent entity");
        }
