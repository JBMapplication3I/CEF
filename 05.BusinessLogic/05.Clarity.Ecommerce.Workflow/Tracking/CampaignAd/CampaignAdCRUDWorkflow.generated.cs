// <autogenerated>
// <copyright file="CampaignAdWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for CampaignAd entities.</summary>
    /// <seealso cref="WorkflowBase{ICampaignAdModel, ICampaignAdSearchModel, ICampaignAd, CampaignAd}"/>
    /// <seealso cref="ICampaignAdWorkflow"/>
    public partial class CampaignAdWorkflow
        : WorkflowBase<ICampaignAdModel, ICampaignAdSearchModel, ICampaignAd, CampaignAd>
            , ICampaignAdWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<CampaignAd?, string?, ICampaignAdModel?> MapFromConcreteFull
            => ModelMapperForCampaignAd.MapCampaignAdModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<CampaignAd>, string?, IEnumerable<ICampaignAdModel>> SelectLiteAndMapToModel
            => ModelMapperForCampaignAd.SelectLiteCampaignAdAndMapToCampaignAdModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<CampaignAd>, string?, IEnumerable<ICampaignAdModel>> SelectListAndMapToModel
            => ModelMapperForCampaignAd.SelectListCampaignAdAndMapToCampaignAdModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<CampaignAd>, string?, ICampaignAdModel?> SelectFirstFullAndMapToModel
            => ModelMapperForCampaignAd.SelectFirstFullCampaignAdAndMapToCampaignAdModel;

        /// <inheritdoc/>
        protected override Func<ICampaignAd, ICampaignAdModel, DateTime, DateTime?, ICampaignAd> UpdateEntityFromModel
            => ModelMapperForCampaignAd.UpdateCampaignAdFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<CampaignAd>> FilterQueryByModelExtensionAsync(
            IQueryable<CampaignAd> query,
            ICampaignAdSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterCampaignAdsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Campaigns.ResolveAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Master!.UpdateCampaignFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the CampaignAd entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the CampaignAd entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Ads.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateAdFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the CampaignAd entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Ad)resolved.Result!.CreateAdEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the CampaignAd entity");
        }
