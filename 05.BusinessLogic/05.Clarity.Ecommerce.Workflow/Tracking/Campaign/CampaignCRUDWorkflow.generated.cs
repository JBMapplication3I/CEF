// <autogenerated>
// <copyright file="CampaignWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for Campaign entities.</summary>
    /// <seealso cref="NameableWorkflowBase{ICampaignModel, ICampaignSearchModel, ICampaign, Campaign}"/>
    /// <seealso cref="ICampaignWorkflow"/>
    public partial class CampaignWorkflow
        : NameableWorkflowBase<ICampaignModel, ICampaignSearchModel, ICampaign, Campaign>
            , ICampaignWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<Campaign?, string?, ICampaignModel?> MapFromConcreteFull
            => ModelMapperForCampaign.MapCampaignModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<Campaign>, string?, IEnumerable<ICampaignModel>> SelectLiteAndMapToModel
            => ModelMapperForCampaign.SelectLiteCampaignAndMapToCampaignModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<Campaign>, string?, IEnumerable<ICampaignModel>> SelectListAndMapToModel
            => ModelMapperForCampaign.SelectListCampaignAndMapToCampaignModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<Campaign>, string?, ICampaignModel?> SelectFirstFullAndMapToModel
            => ModelMapperForCampaign.SelectFirstFullCampaignAndMapToCampaignModel;

        /// <inheritdoc/>
        protected override Func<ICampaign, ICampaignModel, DateTime, DateTime?, ICampaign> UpdateEntityFromModel
            => ModelMapperForCampaign.UpdateCampaignFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<Campaign>> FilterQueryByModelExtensionAsync(
            IQueryable<Campaign> query,
            ICampaignSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterCampaignsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (model.CampaignAds != null) { await Workflows.CampaignWithCampaignAdsAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.CampaignTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateCampaignTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the Campaign entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (CampaignType)resolved.Result!.CreateCampaignTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the Campaign entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.CampaignStatuses.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Status!.UpdateCampaignStatusFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive StatusID to the Campaign entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.StatusID = 0;
                entity.Status = (CampaignStatus)resolved.Result!.CreateCampaignStatusEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Status to the Campaign entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given CreatedByUser to the Campaign entity");
        }
