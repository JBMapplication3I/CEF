// <autogenerated>
// <copyright file="BrandInventoryLocationWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for BrandInventoryLocation entities.</summary>
    /// <seealso cref="WorkflowBase{IBrandInventoryLocationModel, IBrandInventoryLocationSearchModel, IBrandInventoryLocation, BrandInventoryLocation}"/>
    /// <seealso cref="IBrandInventoryLocationWorkflow"/>
    public partial class BrandInventoryLocationWorkflow
        : WorkflowBase<IBrandInventoryLocationModel, IBrandInventoryLocationSearchModel, IBrandInventoryLocation, BrandInventoryLocation>
            , IBrandInventoryLocationWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<BrandInventoryLocation?, string?, IBrandInventoryLocationModel?> MapFromConcreteFull
            => ModelMapperForBrandInventoryLocation.MapBrandInventoryLocationModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<BrandInventoryLocation>, string?, IEnumerable<IBrandInventoryLocationModel>> SelectLiteAndMapToModel
            => ModelMapperForBrandInventoryLocation.SelectLiteBrandInventoryLocationAndMapToBrandInventoryLocationModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<BrandInventoryLocation>, string?, IEnumerable<IBrandInventoryLocationModel>> SelectListAndMapToModel
            => ModelMapperForBrandInventoryLocation.SelectListBrandInventoryLocationAndMapToBrandInventoryLocationModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<BrandInventoryLocation>, string?, IBrandInventoryLocationModel?> SelectFirstFullAndMapToModel
            => ModelMapperForBrandInventoryLocation.SelectFirstFullBrandInventoryLocationAndMapToBrandInventoryLocationModel;

        /// <inheritdoc/>
        protected override Func<IBrandInventoryLocation, IBrandInventoryLocationModel, DateTime, DateTime?, IBrandInventoryLocation> UpdateEntityFromModel
            => ModelMapperForBrandInventoryLocation.UpdateBrandInventoryLocationFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<BrandInventoryLocation>> FilterQueryByModelExtensionAsync(
            IQueryable<BrandInventoryLocation> query,
            IBrandInventoryLocationSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterBrandInventoryLocationsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the BrandInventoryLocation entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the BrandInventoryLocation entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the BrandInventoryLocation entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the BrandInventoryLocation entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.BrandInventoryLocationTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Type!.UpdateBrandInventoryLocationTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the BrandInventoryLocation entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.TypeID = 0;
                entity.Type = (BrandInventoryLocationType)resolved.Result!.CreateBrandInventoryLocationTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the BrandInventoryLocation entity");
        }
