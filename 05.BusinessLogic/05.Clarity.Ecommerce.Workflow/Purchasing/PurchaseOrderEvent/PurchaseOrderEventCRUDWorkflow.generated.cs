// <autogenerated>
// <copyright file="PurchaseOrderEventWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for PurchaseOrderEvent entities.</summary>
    /// <seealso cref="NameableWorkflowBase{IPurchaseOrderEventModel, IPurchaseOrderEventSearchModel, IPurchaseOrderEvent, PurchaseOrderEvent}"/>
    /// <seealso cref="IPurchaseOrderEventWorkflow"/>
    public partial class PurchaseOrderEventWorkflow
        : NameableWorkflowBase<IPurchaseOrderEventModel, IPurchaseOrderEventSearchModel, IPurchaseOrderEvent, PurchaseOrderEvent>
            , IPurchaseOrderEventWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<PurchaseOrderEvent?, string?, IPurchaseOrderEventModel?> MapFromConcreteFull
            => ModelMapperForPurchaseOrderEvent.MapPurchaseOrderEventModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<PurchaseOrderEvent>, string?, IEnumerable<IPurchaseOrderEventModel>> SelectLiteAndMapToModel
            => ModelMapperForPurchaseOrderEvent.SelectLitePurchaseOrderEventAndMapToPurchaseOrderEventModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<PurchaseOrderEvent>, string?, IEnumerable<IPurchaseOrderEventModel>> SelectListAndMapToModel
            => ModelMapperForPurchaseOrderEvent.SelectListPurchaseOrderEventAndMapToPurchaseOrderEventModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<PurchaseOrderEvent>, string?, IPurchaseOrderEventModel?> SelectFirstFullAndMapToModel
            => ModelMapperForPurchaseOrderEvent.SelectFirstFullPurchaseOrderEventAndMapToPurchaseOrderEventModel;

        /// <inheritdoc/>
        protected override Func<IPurchaseOrderEvent, IPurchaseOrderEventModel, DateTime, DateTime?, IPurchaseOrderEvent> UpdateEntityFromModel
            => ModelMapperForPurchaseOrderEvent.UpdatePurchaseOrderEventFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<PurchaseOrderEvent>> FilterQueryByModelExtensionAsync(
            IQueryable<PurchaseOrderEvent> query,
            IPurchaseOrderEventSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterPurchaseOrderEventsBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IPurchaseOrderEvent entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.PurchaseOrderEventTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdatePurchaseOrderEventTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the PurchaseOrderEvent entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (PurchaseOrderEventType)resolved.Result!.CreatePurchaseOrderEventTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the PurchaseOrderEvent entity");
        }
