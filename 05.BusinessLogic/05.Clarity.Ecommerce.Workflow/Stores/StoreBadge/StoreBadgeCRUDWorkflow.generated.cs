// <autogenerated>
// <copyright file="StoreBadgeWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for StoreBadge entities.</summary>
    /// <seealso cref="WorkflowBase{IStoreBadgeModel, IStoreBadgeSearchModel, IStoreBadge, StoreBadge}"/>
    /// <seealso cref="IStoreBadgeWorkflow"/>
    public partial class StoreBadgeWorkflow
        : WorkflowBase<IStoreBadgeModel, IStoreBadgeSearchModel, IStoreBadge, StoreBadge>
            , IStoreBadgeWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<StoreBadge?, string?, IStoreBadgeModel?> MapFromConcreteFull
            => ModelMapperForStoreBadge.MapStoreBadgeModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreBadge>, string?, IEnumerable<IStoreBadgeModel>> SelectLiteAndMapToModel
            => ModelMapperForStoreBadge.SelectLiteStoreBadgeAndMapToStoreBadgeModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreBadge>, string?, IEnumerable<IStoreBadgeModel>> SelectListAndMapToModel
            => ModelMapperForStoreBadge.SelectListStoreBadgeAndMapToStoreBadgeModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreBadge>, string?, IStoreBadgeModel?> SelectFirstFullAndMapToModel
            => ModelMapperForStoreBadge.SelectFirstFullStoreBadgeAndMapToStoreBadgeModel;

        /// <inheritdoc/>
        protected override Func<IStoreBadge, IStoreBadgeModel, DateTime, DateTime?, IStoreBadge> UpdateEntityFromModel
            => ModelMapperForStoreBadge.UpdateStoreBadgeFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<StoreBadge>> FilterQueryByModelExtensionAsync(
            IQueryable<StoreBadge> query,
            IStoreBadgeSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterStoreBadgesBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the StoreBadge entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the StoreBadge entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Badges.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateBadgeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the StoreBadge entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Badge)resolved.Result!.CreateBadgeEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the StoreBadge entity");
        }
