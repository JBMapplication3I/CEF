// <autogenerated>
// <copyright file="StoreSubscriptionWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for StoreSubscription entities.</summary>
    /// <seealso cref="WorkflowBase{IStoreSubscriptionModel, IStoreSubscriptionSearchModel, IStoreSubscription, StoreSubscription}"/>
    /// <seealso cref="IStoreSubscriptionWorkflow"/>
    public partial class StoreSubscriptionWorkflow
        : WorkflowBase<IStoreSubscriptionModel, IStoreSubscriptionSearchModel, IStoreSubscription, StoreSubscription>
            , IStoreSubscriptionWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<StoreSubscription?, string?, IStoreSubscriptionModel?> MapFromConcreteFull
            => ModelMapperForStoreSubscription.MapStoreSubscriptionModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreSubscription>, string?, IEnumerable<IStoreSubscriptionModel>> SelectLiteAndMapToModel
            => ModelMapperForStoreSubscription.SelectLiteStoreSubscriptionAndMapToStoreSubscriptionModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreSubscription>, string?, IEnumerable<IStoreSubscriptionModel>> SelectListAndMapToModel
            => ModelMapperForStoreSubscription.SelectListStoreSubscriptionAndMapToStoreSubscriptionModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreSubscription>, string?, IStoreSubscriptionModel?> SelectFirstFullAndMapToModel
            => ModelMapperForStoreSubscription.SelectFirstFullStoreSubscriptionAndMapToStoreSubscriptionModel;

        /// <inheritdoc/>
        protected override Func<IStoreSubscription, IStoreSubscriptionModel, DateTime, DateTime?, IStoreSubscription> UpdateEntityFromModel
            => ModelMapperForStoreSubscription.UpdateStoreSubscriptionFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<StoreSubscription>> FilterQueryByModelExtensionAsync(
            ...
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterStoreSubscriptionsBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IStoreSubscription entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the StoreSubscription entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the StoreSubscription entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateSubscriptionFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the StoreSubscription entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Subscription)resolved.Result!.CreateSubscriptionEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the StoreSubscription entity");
        }
