// <autogenerated>
// <copyright file="BadgeWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for Badge entities.</summary>
    /// <seealso cref="NameableWorkflowBase{IBadgeModel, IBadgeSearchModel, IBadge, Badge}"/>
    /// <seealso cref="IBadgeWorkflow"/>
    public partial class BadgeWorkflow
        : NameableWorkflowBase<IBadgeModel, IBadgeSearchModel, IBadge, Badge>
            , IBadgeWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<Badge?, string?, IBadgeModel?> MapFromConcreteFull
            => ModelMapperForBadge.MapBadgeModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<Badge>, string?, IEnumerable<IBadgeModel>> SelectLiteAndMapToModel
            => ModelMapperForBadge.SelectLiteBadgeAndMapToBadgeModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<Badge>, string?, IEnumerable<IBadgeModel>> SelectListAndMapToModel
            => ModelMapperForBadge.SelectListBadgeAndMapToBadgeModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<Badge>, string?, IBadgeModel?> SelectFirstFullAndMapToModel
            => ModelMapperForBadge.SelectFirstFullBadgeAndMapToBadgeModel;

        /// <inheritdoc/>
        protected override Func<IBadge, IBadgeModel, DateTime, DateTime?, IBadge> UpdateEntityFromModel
            => ModelMapperForBadge.UpdateBadgeFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<Badge>> FilterQueryByModelExtensionAsync(
            IQueryable<Badge> query,
            IBadgeSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterBadgesBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (model.Images != null) { await Workflows.BadgeWithImagesAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.BadgeTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateBadgeTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the Badge entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (BadgeType)resolved.Result!.CreateBadgeTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the Badge entity");
        }
