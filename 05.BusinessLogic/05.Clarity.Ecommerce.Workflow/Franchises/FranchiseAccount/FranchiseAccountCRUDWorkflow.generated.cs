// <autogenerated>
// <copyright file="FranchiseAccountWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for FranchiseAccount entities.</summary>
    /// <seealso cref="WorkflowBase{IFranchiseAccountModel, IFranchiseAccountSearchModel, IFranchiseAccount, FranchiseAccount}"/>
    /// <seealso cref="IFranchiseAccountWorkflow"/>
    public partial class FranchiseAccountWorkflow
        : WorkflowBase<IFranchiseAccountModel, IFranchiseAccountSearchModel, IFranchiseAccount, FranchiseAccount>
            , IFranchiseAccountWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<FranchiseAccount?, string?, IFranchiseAccountModel?> MapFromConcreteFull
            => ModelMapperForFranchiseAccount.MapFranchiseAccountModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<FranchiseAccount>, string?, IEnumerable<IFranchiseAccountModel>> SelectLiteAndMapToModel
            => ModelMapperForFranchiseAccount.SelectLiteFranchiseAccountAndMapToFranchiseAccountModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<FranchiseAccount>, string?, IEnumerable<IFranchiseAccountModel>> SelectListAndMapToModel
            => ModelMapperForFranchiseAccount.SelectListFranchiseAccountAndMapToFranchiseAccountModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<FranchiseAccount>, string?, IFranchiseAccountModel?> SelectFirstFullAndMapToModel
            => ModelMapperForFranchiseAccount.SelectFirstFullFranchiseAccountAndMapToFranchiseAccountModel;

        /// <inheritdoc/>
        protected override Func<IFranchiseAccount, IFranchiseAccountModel, DateTime, DateTime?, IFranchiseAccount> UpdateEntityFromModel
            => ModelMapperForFranchiseAccount.UpdateFranchiseAccountFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<FranchiseAccount>> FilterQueryByModelExtensionAsync(
            IQueryable<FranchiseAccount> query,
            IFranchiseAccountSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterFranchiseAccountsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the FranchiseAccount entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the FranchiseAccount entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateAccountFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the FranchiseAccount entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Account)resolved.Result!.CreateAccountEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the FranchiseAccount entity");
        }

        /// <summary>Relate Optional PricePoint.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <param name="entity">            The entity that has a Optional PricePoint.</param>
        /// <param name="model">             The model that has a Optional PricePoint.</param>
        /// <param name="timestamp">         The timestamp Date/Time.</param>
        /// <param name="contextProfileName">Name of the context profile.</param>
        protected virtual async Task RelateOptionalPricePointAsync(
            IFranchiseAccount entity,
            IFranchiseAccountModel model,
            DateTime timestamp,
            string? contextProfileName)
        {
            using var context = RegistryLoaderWrapper.GetContext(contextProfileName);
            await RelateOptionalPricePointAsync(
                    entity,
                    model,
                    timestamp,
                    context)
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.PricePoints.ResolveWithAutoGenerateOptionalAsync(
                    byID: model.PricePointID, // By Other ID
                    byKey: model.PricePointKey, // By Flattened Other Key
                    byName: model.PricePointName, // By Flattened Other Name
                    byDisplayName: null, // Skip DisplayName as it's not normally part of the interface
                    model: model.PricePoint, // The other property as a model
                    context: context)
                .ConfigureAwait(false);
            // Check for IDs and objects
            var entityIDIsNull = !Contract.CheckValidID(entity.PricePointID);
            var modelIDIsNull = !Contract.CheckValidID(resolved.Result?.ID);
            var entityObjectIsNull = entity.PricePoint == null;
            if (!resolved.ActionSucceeded && model.PricePoint != null)
            {
                resolved.Result = model.PricePoint;
            }
            var modelObjectIsNull = !resolved.ActionSucceeded;
            ...
            var entityIDAndModelIDHaveSameID = !entityIDIsNull && !modelIDIsNull && entity.PricePointID == resolved.Result!.ID;
            var entityObjectAndModelObjectHaveSameID = !entityObjectIsNull && !modelObjectIsNull && entity.PricePointID == resolved.Result!.ID;
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                // [Optional/Required] Scenario 2: They match IDs, just update the entity from the model if it is present
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.PricePoint!.UpdatePricePointFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            {
                // [Optional/Required] Scenario 3: We have IDs but they don't match, assign the model's ID to the entity's ID
                entity.PricePointID = resolved.Result!.ID;
                if (!modelObjectIsNull)
                {
                    if (!entityObjectIsNull)
                    {
            ...
            if (!modelObjectIsNull && !modelObjectIDIsNull)
            {
                ...
                entity.PricePointID = null;
                entity.PricePoint = null;
                return;
            }
            if (!entityIDIsNull && modelObjectIsActive)
            {
                ...
                entity.PricePointID = null;
                entity.PricePoint = (PricePoint)resolved.Result!.CreatePricePointEntity(timestamp, context.ContextProfileName);
                return;
            }
            if (entityIDIsNull && modelObjectIsActive)
            {
            ...
            //     entity.PricePoint.UpdatePricePointFromModel(resolved.Result, updateTimestamp);
            //     return;
            // }
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given PricePoint to the FranchiseAccount entity");
        }
...
#pragma warning restore 1998
            ...
            IClarityEcommerceEntities context)
        {
            ...
            await RelateOptionalPricePointAsync(entity, model, timestamp, context).ConfigureAwait(false);
        }
