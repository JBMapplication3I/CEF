// <autogenerated>
// <copyright file="CounterLogWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for CounterLog entities.</summary>
    /// <seealso cref="WorkflowBase{ICounterLogModel, ICounterLogSearchModel, ICounterLog, CounterLog}"/>
    /// <seealso cref="ICounterLogWorkflow"/>
    public partial class CounterLogWorkflow
        : WorkflowBase<ICounterLogModel, ICounterLogSearchModel, ICounterLog, CounterLog>
            , ICounterLogWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<CounterLog?, string?, ICounterLogModel?> MapFromConcreteFull
            => ModelMapperForCounterLog.MapCounterLogModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<CounterLog>, string?, IEnumerable<ICounterLogModel>> SelectLiteAndMapToModel
            => ModelMapperForCounterLog.SelectLiteCounterLogAndMapToCounterLogModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<CounterLog>, string?, IEnumerable<ICounterLogModel>> SelectListAndMapToModel
            => ModelMapperForCounterLog.SelectListCounterLogAndMapToCounterLogModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<CounterLog>, string?, ICounterLogModel?> SelectFirstFullAndMapToModel
            => ModelMapperForCounterLog.SelectFirstFullCounterLogAndMapToCounterLogModel;

        /// <inheritdoc/>
        protected override Func<ICounterLog, ICounterLogModel, DateTime, DateTime?, ICounterLog> UpdateEntityFromModel
            => ModelMapperForCounterLog.UpdateCounterLogFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<CounterLog>> FilterQueryByModelExtensionAsync(
            IQueryable<CounterLog> query,
            ICounterLogSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterCounterLogsBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.CounterLogTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateCounterLogTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the CounterLog entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (CounterLogType)resolved.Result!.CreateCounterLogTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the CounterLog entity");
        }

        /// <summary>Relate Required Counter.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <param name="entity">            The entity that has a Required Counter.</param>
        /// <param name="model">             The model that has a Required Counter.</param>
        /// <param name="timestamp">         The timestamp Date/Time.</param>
        /// <param name="contextProfileName">Name of the context profile.</param>
        protected virtual async Task RelateRequiredCounterAsync(
            ICounterLog entity,
            ICounterLogModel model,
            DateTime timestamp,
            string? contextProfileName)
        {
            using var context = RegistryLoaderWrapper.GetContext(contextProfileName);
            await RelateRequiredCounterAsync(
                    entity,
                    model,
                    timestamp,
                    context)
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Counters.ResolveAsync(
                    byID: model.CounterID, // By Other ID
                    byKey: model.CounterKey, // By Flattened Other Key
                    model: model.Counter, // Manual name if not UserProductType and not Discount or Discount and not Master
                    context: context,
                    isInner: false)
                .ConfigureAwait(false);
            // Check for IDs and objects
            var entityIDIsNull = !Contract.CheckValidID(entity.CounterID);
            var modelIDIsNull = !Contract.CheckValidID(resolved.Result?.ID);
            var entityObjectIsNull = entity.Counter == null;
            if (!resolved.ActionSucceeded && model.Counter != null)
            {
                resolved.Result = model.Counter;
            }
            var modelObjectIsNull = !resolved.ActionSucceeded;
            if (modelIDIsNull && modelObjectIsNull)
            {
                // [Required] Scenario 1: Trying to put bad data in the database
                throw new ArgumentException("Cannot assign a null to a non-nullable CounterID");
            }
            var entityIDAndModelIDHaveSameID = !entityIDIsNull && !modelIDIsNull && entity.CounterID == resolved.Result!.ID;
            var entityObjectAndModelObjectHaveSameID = !entityObjectIsNull && !modelObjectIsNull && entity.CounterID == resolved.Result!.ID;
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                // [Optional/Required] Scenario 2: They match IDs, just update the entity from the model if it is present
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Counter!.UpdateCounterFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            {
                // [Optional/Required] Scenario 3: We have IDs but they don't match, assign the model's ID to the entity's ID
                entity.CounterID = resolved.Result!.ID;
                if (!modelObjectIsNull)
                {
                    if (!entityObjectIsNull)
                    {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive CounterID to the CounterLog entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Counter to the CounterLog entity");
        }
...
#pragma warning restore 1998
            ...
            IClarityEcommerceEntities context)
        {
            ...
            await RelateRequiredCounterAsync(entity, model, timestamp, context).ConfigureAwait(false);
        }
