// <autogenerated>
// <copyright file="StoreAuctionWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for StoreAuction entities.</summary>
    /// <seealso cref="WorkflowBase{IStoreAuctionModel, IStoreAuctionSearchModel, IStoreAuction, StoreAuction}"/>
    /// <seealso cref="IStoreAuctionWorkflow"/>
    public partial class StoreAuctionWorkflow
        : WorkflowBase<IStoreAuctionModel, IStoreAuctionSearchModel, IStoreAuction, StoreAuction>
            , IStoreAuctionWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<StoreAuction?, string?, IStoreAuctionModel?> MapFromConcreteFull
            => ModelMapperForStoreAuction.MapStoreAuctionModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreAuction>, string?, IEnumerable<IStoreAuctionModel>> SelectLiteAndMapToModel
            => ModelMapperForStoreAuction.SelectLiteStoreAuctionAndMapToStoreAuctionModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreAuction>, string?, IEnumerable<IStoreAuctionModel>> SelectListAndMapToModel
            => ModelMapperForStoreAuction.SelectListStoreAuctionAndMapToStoreAuctionModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<StoreAuction>, string?, IStoreAuctionModel?> SelectFirstFullAndMapToModel
            => ModelMapperForStoreAuction.SelectFirstFullStoreAuctionAndMapToStoreAuctionModel;

        /// <inheritdoc/>
        protected override Func<IStoreAuction, IStoreAuctionModel, DateTime, DateTime?, IStoreAuction> UpdateEntityFromModel
            => ModelMapperForStoreAuction.UpdateStoreAuctionFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<StoreAuction>> FilterQueryByModelExtensionAsync(
            IQueryable<StoreAuction> query,
            IStoreAuctionSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterStoreAuctionsBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IStoreAuction entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the StoreAuction entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the StoreAuction entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the StoreAuction entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the StoreAuction entity");
        }
