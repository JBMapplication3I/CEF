// <autogenerated>
// <copyright file="LotCategoryWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for LotCategory entities.</summary>
    /// <seealso cref="WorkflowBase{ILotCategoryModel, ILotCategorySearchModel, ILotCategory, LotCategory}"/>
    /// <seealso cref="ILotCategoryWorkflow"/>
    public partial class LotCategoryWorkflow
        : WorkflowBase<ILotCategoryModel, ILotCategorySearchModel, ILotCategory, LotCategory>
            , ILotCategoryWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<LotCategory?, string?, ILotCategoryModel?> MapFromConcreteFull
            => ModelMapperForLotCategory.MapLotCategoryModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<LotCategory>, string?, IEnumerable<ILotCategoryModel>> SelectLiteAndMapToModel
            => ModelMapperForLotCategory.SelectLiteLotCategoryAndMapToLotCategoryModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<LotCategory>, string?, IEnumerable<ILotCategoryModel>> SelectListAndMapToModel
            => ModelMapperForLotCategory.SelectListLotCategoryAndMapToLotCategoryModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<LotCategory>, string?, ILotCategoryModel?> SelectFirstFullAndMapToModel
            => ModelMapperForLotCategory.SelectFirstFullLotCategoryAndMapToLotCategoryModel;

        /// <inheritdoc/>
        protected override Func<ILotCategory, ILotCategoryModel, DateTime, DateTime?, ILotCategory> UpdateEntityFromModel
            => ModelMapperForLotCategory.UpdateLotCategoryFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<LotCategory>> FilterQueryByModelExtensionAsync(
            IQueryable<LotCategory> query,
            ILotCategorySearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterLotCategoriesBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Lots.ResolveAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Master!.UpdateLotFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the LotCategory entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the LotCategory entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the LotCategory entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the LotCategory entity");
        }
