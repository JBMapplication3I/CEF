// <autogenerated>
// <copyright file="FranchiseAuctionWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for FranchiseAuction entities.</summary>
    /// <seealso cref="WorkflowBase{IFranchiseAuctionModel, IFranchiseAuctionSearchModel, IFranchiseAuction, FranchiseAuction}"/>
    /// <seealso cref="IFranchiseAuctionWorkflow"/>
    public partial class FranchiseAuctionWorkflow
        : WorkflowBase<IFranchiseAuctionModel, IFranchiseAuctionSearchModel, IFranchiseAuction, FranchiseAuction>
            , IFranchiseAuctionWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<FranchiseAuction?, string?, IFranchiseAuctionModel?> MapFromConcreteFull
            => ModelMapperForFranchiseAuction.MapFranchiseAuctionModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<FranchiseAuction>, string?, IEnumerable<IFranchiseAuctionModel>> SelectLiteAndMapToModel
            => ModelMapperForFranchiseAuction.SelectLiteFranchiseAuctionAndMapToFranchiseAuctionModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<FranchiseAuction>, string?, IEnumerable<IFranchiseAuctionModel>> SelectListAndMapToModel
            => ModelMapperForFranchiseAuction.SelectListFranchiseAuctionAndMapToFranchiseAuctionModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<FranchiseAuction>, string?, IFranchiseAuctionModel?> SelectFirstFullAndMapToModel
            => ModelMapperForFranchiseAuction.SelectFirstFullFranchiseAuctionAndMapToFranchiseAuctionModel;

        /// <inheritdoc/>
        protected override Func<IFranchiseAuction, IFranchiseAuctionModel, DateTime, DateTime?, IFranchiseAuction> UpdateEntityFromModel
            => ModelMapperForFranchiseAuction.UpdateFranchiseAuctionFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<FranchiseAuction>> FilterQueryByModelExtensionAsync(
            IQueryable<FranchiseAuction> query,
            IFranchiseAuctionSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterFranchiseAuctionsBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IFranchiseAuction entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the FranchiseAuction entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the FranchiseAuction entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateAuctionFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the FranchiseAuction entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Auction)resolved.Result!.CreateAuctionEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the FranchiseAuction entity");
        }
