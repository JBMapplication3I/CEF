// <autogenerated>
// <copyright file="SalesQuoteWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for SalesQuote entities.</summary>
    /// <seealso cref="SalesCollectionWorkflowBase{ISalesQuoteModel, ISalesQuoteSearchModel, ISalesQuote, SalesQuote, SalesQuoteStatus, SalesQuoteType, ITypeModel, ISalesQuoteItem, SalesQuoteItem, AppliedSalesQuoteDiscount, SalesQuoteState, SalesQuoteFile, ISalesQuoteFileModel, SalesQuoteContact, ISalesQuoteContactModel, SalesQuoteEvent, ISalesQuoteEventModel, SalesQuoteEventType, AppliedSalesQuoteItemDiscount, SalesQuoteItemTarget, IAppliedSalesQuoteDiscountModel, IAppliedSalesQuoteItemDiscountModel}"/>
    /// <seealso cref="ISalesQuoteWorkflow"/>
    public partial class SalesQuoteWorkflow
        : SalesCollectionWorkflowBase<ISalesQuoteModel,
            ...
            , ISalesQuoteWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<SalesQuote?, string?, ISalesQuoteModel?> MapFromConcreteFull
            => ModelMapperForSalesQuote.MapSalesQuoteModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<SalesQuote>, string?, IEnumerable<ISalesQuoteModel>> SelectLiteAndMapToModel
            => ModelMapperForSalesQuote.SelectLiteSalesQuoteAndMapToSalesQuoteModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<SalesQuote>, string?, IEnumerable<ISalesQuoteModel>> SelectListAndMapToModel
            => ModelMapperForSalesQuote.SelectListSalesQuoteAndMapToSalesQuoteModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<SalesQuote>, string?, ISalesQuoteModel?> SelectFirstFullAndMapToModel
            => ModelMapperForSalesQuote.SelectFirstFullSalesQuoteAndMapToSalesQuoteModel;

        /// <inheritdoc/>
        protected override Func<ISalesQuote, ISalesQuoteModel, DateTime, DateTime?, ISalesQuote> UpdateEntityFromModel
            => ModelMapperForSalesQuote.UpdateSalesQuoteFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<SalesQuote>> FilterQueryByModelExtensionAsync(
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (model.Notes != null)
            {
                ...
                await Workflows.SalesQuoteWithNotesAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false);
            }
            if (model.AssociatedSalesOrders != null) { await Workflows.SalesQuoteWithAssociatedSalesOrdersAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
            if (model.SalesQuoteCategories != null) { await Workflows.SalesQuoteWithSalesQuoteCategoriesAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
            if (model.SalesItems != null) { await Workflows.SalesQuoteWithSalesItemsAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
            // Skipped: Not supposed to map this property in via this manner: Discounts
            if (model.StoredFiles != null) { await Workflows.SalesQuoteWithStoredFilesAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
            if (model.Contacts != null) { await Workflows.SalesQuoteWithContactsAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
            // Skipped: Not supposed to map this property in via this manner: SalesEvents
            if (model.RateQuotes != null) { await Workflows.SalesQuoteWithRateQuotesAssociation.AssociateObjectsAsync(entity, model, timestamp, context).ConfigureAwait(false); }
        }

        #region Relate Workflows
        /// <summary>Relate Optional ResponseAsVendor.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <param name="entity">            The entity that has a Optional ResponseAsVendor.</param>
        /// <param name="model">             The model that has a Optional ResponseAsVendor.</param>
        /// <param name="timestamp">         The timestamp Date/Time.</param>
        /// <param name="contextProfileName">Name of the context profile.</param>
        protected virtual async Task RelateOptionalResponseAsVendorAsync(
            ISalesQuote entity,
            ISalesQuoteModel model,
            DateTime timestamp,
            string? contextProfileName)
        {
            using var context = RegistryLoaderWrapper.GetContext(contextProfileName);
            await RelateOptionalResponseAsVendorAsync(
                    entity,
                    model,
                    timestamp,
                    context)
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Vendors.ResolveWithAutoGenerateOptionalAsync(
                    byID: model.ResponseAsVendorID, // By Other ID
                    byKey: model.ResponseAsVendorKey, // By Flattened Other Key
                    byName: model.ResponseAsVendorName, // By Flattened Other Name
                    model: model.ResponseAsVendor, // The other property as a model
                    context: context)
                .ConfigureAwait(false);
            // Check for IDs and objects
            var entityIDIsNull = !Contract.CheckValidID(entity.ResponseAsVendorID);
            var modelIDIsNull = !Contract.CheckValidID(resolved.Result?.ID);
            var entityObjectIsNull = entity.ResponseAsVendor == null;
            if (!resolved.ActionSucceeded && model.ResponseAsVendor != null)
            {
                resolved.Result = model.ResponseAsVendor;
            }
            ...
            var entityIDAndModelIDHaveSameID = !entityIDIsNull && !modelIDIsNull && entity.ResponseAsVendorID == resolved.Result!.ID;
            var entityObjectAndModelObjectHaveSameID = !entityObjectIsNull && !modelObjectIsNull && entity.ResponseAsVendorID == resolved.Result!.ID;
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.ResponseAsVendor!.UpdateVendorFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            {
                ...
                entity.ResponseAsVendorID = resolved.Result!.ID;
            ...
            if (!modelObjectIsNull && !modelObjectIDIsNull)
            {
                ...
                entity.ResponseAsVendorID = null;
                entity.ResponseAsVendor = null;
                return;
            }
            if (!entityIDIsNull && modelObjectIsActive)
            {
                ...
                entity.ResponseAsVendorID = null;
                entity.ResponseAsVendor = (Vendor)resolved.Result!.CreateVendorEntity(timestamp, context.ContextProfileName);
                return;
            }
            if (entityIDIsNull && modelObjectIsActive)
            {
            ...
            //     entity.ResponseAsVendor.UpdateVendorFromModel(resolved.Result, updateTimestamp);
            //     return;
            // }
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given ResponseAsVendor to the SalesQuote entity");
        }

        /// <summary>Relate Optional ResponseAsStore.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <param name="entity">            The entity that has a Optional ResponseAsStore.</param>
        /// <param name="model">             The model that has a Optional ResponseAsStore.</param>
        /// <param name="timestamp">         The timestamp Date/Time.</param>
        /// <param name="contextProfileName">Name of the context profile.</param>
        protected virtual async Task RelateOptionalResponseAsStoreAsync(
            ISalesQuote entity,
            ISalesQuoteModel model,
            DateTime timestamp,
            string? contextProfileName)
        {
            using var context = RegistryLoaderWrapper.GetContext(contextProfileName);
            await RelateOptionalResponseAsStoreAsync(
                    entity,
                    model,
                    timestamp,
                    context)
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.Stores.ResolveWithAutoGenerateOptionalAsync(
                    byID: model.ResponseAsStoreID, // By Other ID
                    byKey: model.ResponseAsStoreKey, // By Flattened Other Key
                    byName: model.ResponseAsStoreName, // By Flattened Other Name
                    model: model.ResponseAsStore, // The other property as a model
                    context: context)
                .ConfigureAwait(false);
            // Check for IDs and objects
            var entityIDIsNull = !Contract.CheckValidID(entity.ResponseAsStoreID);
            var modelIDIsNull = !Contract.CheckValidID(resolved.Result?.ID);
            var entityObjectIsNull = entity.ResponseAsStore == null;
            if (!resolved.ActionSucceeded && model.ResponseAsStore != null)
            {
                resolved.Result = model.ResponseAsStore;
            }
            var modelObjectIsNull = !resolved.ActionSucceeded;
            if (entityIDIsNull && modelIDIsNull && entityObjectIsNull && modelObjectIsNull)
            {
            ...
            var entityIDAndModelIDHaveSameID = !entityIDIsNull && !modelIDIsNull && entity.ResponseAsStoreID == resolved.Result!.ID;
            var entityObjectAndModelObjectHaveSameID = !entityObjectIsNull && !modelObjectIsNull && entity.ResponseAsStoreID == resolved.Result!.ID;
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                // [Optional/Required] Scenario 2: They match IDs, just update the entity from the model if it is present
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.ResponseAsStore!.UpdateStoreFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            {
                // [Optional/Required] Scenario 3: We have IDs but they don't match, assign the model's ID to the entity's ID
                entity.ResponseAsStoreID = resolved.Result!.ID;
                if (!modelObjectIsNull)
                {
                    if (!entityObjectIsNull)
                    {
            ...
            if (!modelObjectIsNull && !modelObjectIDIsNull)
            {
                ...
                // [Optional] Scenario 5: We have IDs but they don't match and the model has been deactivated, remove the entity from it's master
                entity.ResponseAsStoreID = null;
                entity.ResponseAsStore = null;
                return;
            }
            if (!entityIDIsNull && modelObjectIsActive)
            {
                // [Optional] Scenario 6: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.ResponseAsStoreID = null;
                entity.ResponseAsStore = (Store)resolved.Result!.CreateStoreEntity(timestamp, context.ContextProfileName);
                return;
            }
            if (entityIDIsNull && modelObjectIsActive)
            {
            ...
            //     // TODO: Determine 'Equals' object between the objects so we only update if different
            //     // [Optional] Scenario 9: We have data on both sides, update the object, assign the values using the Update action
            //     entity.ResponseAsStore.UpdateStoreFromModel(resolved.Result, updateTimestamp);
            //     return;
            // }
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given ResponseAsStore to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Account to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Brand to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Franchise to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Store to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given User to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.SalesQuoteStatuses.ResolveWithAutoGenerateOptionalAsync(
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.SalesQuoteStates.ResolveWithAutoGenerateOptionalAsync(
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // Look up the Object Model
            // Allowed to auto-generate if not found
            var resolved = await Workflows.SalesQuoteTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.TypeID = 0;
                entity.Type = (SalesQuoteType)resolved.Result!.CreateSalesQuoteTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given BillingContact to the SalesQuote entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given ShippingContact to the SalesQuote entity");
        }
...
#pragma warning restore 1998
            ...
            IClarityEcommerceEntities context)
        {
            ...
            await RelateOptionalResponseAsVendorAsync(entity, model, timestamp, context).ConfigureAwait(false);
            await RelateOptionalResponseAsStoreAsync(entity, model, timestamp, context).ConfigureAwait(false);
