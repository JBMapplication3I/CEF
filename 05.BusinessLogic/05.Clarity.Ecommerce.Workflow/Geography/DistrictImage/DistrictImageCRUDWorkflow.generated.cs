// <autogenerated>
// <copyright file="DistrictImageWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for DistrictImage entities.</summary>
    /// <seealso cref="NameableWorkflowBase{IDistrictImageModel, IDistrictImageSearchModel, IDistrictImage, DistrictImage}"/>
    /// <seealso cref="IDistrictImageWorkflow"/>
    public partial class DistrictImageWorkflow
        : NameableWorkflowBase<IDistrictImageModel, IDistrictImageSearchModel, IDistrictImage, DistrictImage>
            , IDistrictImageWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<DistrictImage?, string?, IDistrictImageModel?> MapFromConcreteFull
            => ModelMapperForDistrictImage.MapDistrictImageModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<DistrictImage>, string?, IEnumerable<IDistrictImageModel>> SelectLiteAndMapToModel
            => ModelMapperForDistrictImage.SelectLiteDistrictImageAndMapToDistrictImageModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<DistrictImage>, string?, IEnumerable<IDistrictImageModel>> SelectListAndMapToModel
            => ModelMapperForDistrictImage.SelectListDistrictImageAndMapToDistrictImageModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<DistrictImage>, string?, IDistrictImageModel?> SelectFirstFullAndMapToModel
            => ModelMapperForDistrictImage.SelectFirstFullDistrictImageAndMapToDistrictImageModel;

        /// <inheritdoc/>
        protected override Func<IDistrictImage, IDistrictImageModel, DateTime, DateTime?, IDistrictImage> UpdateEntityFromModel
            => ModelMapperForDistrictImage.UpdateDistrictImageFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<DistrictImage>> FilterQueryByModelExtensionAsync(
            IQueryable<DistrictImage> query,
            IDistrictImageSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterDistrictImagesBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IDistrictImage entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            var resolved = await Workflows.DistrictImageTypes.ResolveWithAutoGenerateOptionalAsync(
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Type!.UpdateDistrictImageTypeFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive TypeID to the DistrictImage entity");
            }
            ...
            if (modelObjectIsActive)
            {
                ...
                entity.TypeID = 0;
                entity.Type = (DistrictImageType)resolved.Result!.CreateDistrictImageTypeEntity(timestamp, context.ContextProfileName);
                return;
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Type to the DistrictImage entity");
        }
