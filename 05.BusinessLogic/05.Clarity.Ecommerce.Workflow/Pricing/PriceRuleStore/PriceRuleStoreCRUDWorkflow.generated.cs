// <autogenerated>
// <copyright file="PriceRuleStoreWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for PriceRuleStore entities.</summary>
    /// <seealso cref="WorkflowBase{IPriceRuleStoreModel, IPriceRuleStoreSearchModel, IPriceRuleStore, PriceRuleStore}"/>
    /// <seealso cref="IPriceRuleStoreWorkflow"/>
    public partial class PriceRuleStoreWorkflow
        : WorkflowBase<IPriceRuleStoreModel, IPriceRuleStoreSearchModel, IPriceRuleStore, PriceRuleStore>
            , IPriceRuleStoreWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<PriceRuleStore?, string?, IPriceRuleStoreModel?> MapFromConcreteFull
            => ModelMapperForPriceRuleStore.MapPriceRuleStoreModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<PriceRuleStore>, string?, IEnumerable<IPriceRuleStoreModel>> SelectLiteAndMapToModel
            => ModelMapperForPriceRuleStore.SelectLitePriceRuleStoreAndMapToPriceRuleStoreModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<PriceRuleStore>, string?, IEnumerable<IPriceRuleStoreModel>> SelectListAndMapToModel
            => ModelMapperForPriceRuleStore.SelectListPriceRuleStoreAndMapToPriceRuleStoreModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<PriceRuleStore>, string?, IPriceRuleStoreModel?> SelectFirstFullAndMapToModel
            => ModelMapperForPriceRuleStore.SelectFirstFullPriceRuleStoreAndMapToPriceRuleStoreModel;

        /// <inheritdoc/>
        protected override Func<IPriceRuleStore, IPriceRuleStoreModel, DateTime, DateTime?, IPriceRuleStore> UpdateEntityFromModel
            => ModelMapperForPriceRuleStore.UpdatePriceRuleStoreFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<PriceRuleStore>> FilterQueryByModelExtensionAsync(
            IQueryable<PriceRuleStore> query,
            IPriceRuleStoreSearchModel search,
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterPriceRuleStoresBySearchModel(search);
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the PriceRuleStore entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the PriceRuleStore entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateStoreFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the PriceRuleStore entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            if (modelObjectIsActive)
            {
                // [Required] Scenario 7: We have an entity id, but a new model, remove the id on the entity and assign the new model
                entity.SlaveID = 0;
                entity.Slave = (Store)resolved.Result!.CreateStoreEntity(timestamp, context.ContextProfileName);
                return;
            }
            // [Required] Scenarios 8,9 are only for Optional relationships, skipped
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the PriceRuleStore entity");
        }
