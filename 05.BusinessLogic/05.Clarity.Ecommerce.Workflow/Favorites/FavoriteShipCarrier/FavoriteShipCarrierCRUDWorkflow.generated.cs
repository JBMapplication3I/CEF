// <autogenerated>
// <copyright file="FavoriteShipCarrierWorkflow.generated.cs" company="clarity-ventures.com">
...
namespace Clarity.Ecommerce.Workflow
{
    ...
    /// <summary>A workflow for FavoriteShipCarrier entities.</summary>
    /// <seealso cref="WorkflowBase{IFavoriteShipCarrierModel, IFavoriteShipCarrierSearchModel, IFavoriteShipCarrier, FavoriteShipCarrier}"/>
    /// <seealso cref="IFavoriteShipCarrierWorkflow"/>
    public partial class FavoriteShipCarrierWorkflow
        : WorkflowBase<IFavoriteShipCarrierModel, IFavoriteShipCarrierSearchModel, IFavoriteShipCarrier, FavoriteShipCarrier>
            , IFavoriteShipCarrierWorkflow
    {
        #region Mappers
        /// <inheritdoc/>
        protected override Func<FavoriteShipCarrier?, string?, IFavoriteShipCarrierModel?> MapFromConcreteFull
            => ModelMapperForFavoriteShipCarrier.MapFavoriteShipCarrierModelFromEntityFull;

        /// <inheritdoc/>
        protected override Func<IQueryable<FavoriteShipCarrier>, string?, IEnumerable<IFavoriteShipCarrierModel>> SelectLiteAndMapToModel
            => ModelMapperForFavoriteShipCarrier.SelectLiteFavoriteShipCarrierAndMapToFavoriteShipCarrierModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<FavoriteShipCarrier>, string?, IEnumerable<IFavoriteShipCarrierModel>> SelectListAndMapToModel
            => ModelMapperForFavoriteShipCarrier.SelectListFavoriteShipCarrierAndMapToFavoriteShipCarrierModel;

        /// <inheritdoc/>
        protected override Func<IQueryable<FavoriteShipCarrier>, string?, IFavoriteShipCarrierModel?> SelectFirstFullAndMapToModel
            => ModelMapperForFavoriteShipCarrier.SelectFirstFullFavoriteShipCarrierAndMapToFavoriteShipCarrierModel;

        /// <inheritdoc/>
        protected override Func<IFavoriteShipCarrier, IFavoriteShipCarrierModel, DateTime, DateTime?, IFavoriteShipCarrier> UpdateEntityFromModel
            => ModelMapperForFavoriteShipCarrier.UpdateFavoriteShipCarrierFromModel;
        #endregion

        /// <inheritdoc/>
        protected override async Task<IQueryable<FavoriteShipCarrier>> FilterQueryByModelExtensionAsync(
            ...
            IClarityEcommerceEntities context)
        {
            return (await base.FilterQueryByModelExtensionAsync(query, search, context).ConfigureAwait(false))
                .FilterFavoriteShipCarriersBySearchModel(search);
        }

        /// <inheritdoc/>
        protected override async Task RunDefaultAssociateWorkflowsAsync(
            IFavoriteShipCarrier entity,
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                {
                    entity.Master!.UpdateUserFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            if (!modelObjectIDIsNull && !modelObjectIsActive)
            {
                ...
                throw new InvalidOperationException("Cannot assign an inactive MasterID to the FavoriteShipCarrier entity");
            }
            ...
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Master to the FavoriteShipCarrier entity");
        }
            ...
            IClarityEcommerceEntities context)
        {
            ...
            if (entityIDAndModelIDHaveSameID || entityObjectAndModelObjectHaveSameID)
            {
                ...
                if (!entityObjectIsNull && !modelObjectIsNull)
                {
                    entity.Slave!.UpdateShipCarrierFromModel(resolved.Result!, timestamp, timestamp);
                }
                return;
            }
            if (!modelIDIsNull)
            ...
            {
                // [Required] Scenario 4: We have an ID on the model, but the object is inactive
                throw new InvalidOperationException("Cannot assign an inactive SlaveID to the FavoriteShipCarrier entity");
            }
            if (!modelObjectIDIsNull)
            {
                // [Required] Scenario 5: We have IDs but they don't match, assign the model's ID to the entity's ID (from the model object)
            ...
            // [Optional/Required] Scenario 10: Could not figure out what to do
            throw new InvalidOperationException(
                "Couldn't figure out how to relate the given Slave to the FavoriteShipCarrier entity");
        }
